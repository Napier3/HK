...
CExBaseObject* _CGbScriptGlobalDefine::SelectGbItem(const char* pszPath)
{
	...
	if (nCount == 0)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE,CXLanguageResourceAtsBase::g_sLangTxt_PathNoItem /*L"路径[%s]没有选择的项目"*/, CString(pszPath));
	}
	else if (nCount > 1)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE,  CXLanguageResourceAtsBase::g_sLangTxt_PathSelItem/*L"路径[%s]选择的项目不唯一"*/, CString(pszPath));
	}
	else
	{
