...
BOOL CDevice::ExprCal(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, const CString &strParentPath, double &dValRet, CString &strValRet)
{
	if (strType == _T("dataset"))
	{
		return ExprCal_dataset(pExprParent, strType, strExpression, strParentPath, dValRet);
	}
	else if (strType == _T("sys-para"))
	{
		return ExprCal_sys_para(pExprParent, strType, strExpression, dValRet);
	}
	else if (strType == _T("global-para"))
	{
		return ExprCal_global_para(pExprParent, strType, strExpression, dValRet);
	}
	else if (strType == _T("report"))
	{
		return ExprCal_report(pExprParent, strType, strExpression, strParentPath, dValRet, strValRet);
	}
	...
	{
		return ExprCal_script(pExprParent, strType, strExpression, dValRet);
	}
	else
	{
		return ExprCal_script(pExprParent, strType, strExpression, dValRet);
...
BOOL CDevice::ExprCal_dataset(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, const CString &strParentPath, double &dValRet)
{
...
BOOL CDevice::ExprCal_sys_para(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, double &dValRet)
{
...
BOOL CDevice::ExprCal_global_para(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, double &dValRet)
{
...
//2023-11-8 新增形参 CString &strValRet。用于返回字符串结果，例如“未动作”
BOOL CDevice::ExprCal_report(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, const CString &strParentPath, double &dValRet, CString &strValRet)
{
	CGbItemBase *pItem = (CGbItemBase*)pExprParent;  //表达式的父对象
	CReport *pReport = NULL;
...
BOOL CDevice::ExprCal_script(CExBaseObject *pExprParent, const CString &strType, const CString &strExpression, double &dValRet)
{
...
BOOL CDevice::ExprCal_dataset_Ain_U(CDataGroup *pAinRatios, double &dValRet)
{
	...
	double dRatio = dPrimary / dSecondary * 1000;  //PT变比是 kV / V
...
BOOL CDevice::ExprCal_dataset_Ain_PQ(CDataGroup *pAinRatios, double &dValRet)
{
	...
	double dRatio = (dCtPrimary / dCtSecondary) * (dPtPrimary / dPtSecondary * 1000);  //PT变比是 kV / V
