...
char* GBS_FilterRptExGetString(CTScriptSingleVM *pXvm, long nIndex, const char* pszDataAttrID)
{
	...
	if (!bFind)
	{
        CLogPrint::LogFormatString(XLOGLEVEL_TRACE,g_sLangTxt_DataUnExist.GetString() /*_T("ReportEx数据(Index=%d  AttrID=%s)不存在")*/, nIndex, pszDataAttrID);
	}
...
char* GBS_FilterRptExGetValString(CTScriptSingleVM *pXvm, long nDataIndex, const char* pszValIndex)
{
	...
	if (!pDvmValue)
	{
        CLogPrint::LogFormatString(XLOGLEVEL_TRACE,g_sLangTxt_DataUnExist.GetString() /*_T("ReportEx数据(Index=%d  AttrID=%s)不存在")*/, nDataIndex, pszValIndex);
	}
	else
	{
