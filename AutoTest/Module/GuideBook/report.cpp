...
BOOL CReport::AppendReportValues(CExBaseList *pListDataObj)
{
	...
	{
		pData = (CDataObj *)pListDataObj->GetNext(pos);
		pValue = (CValue*)m_pValues->FindByID(pData->m_strID);
...
void CReport::CalChildrenSize(BOOL bCalChild, long &nObjCount, long &nTotalSize, CFile *pFile)
{
	...
	{
#ifdef _DEBUG
		m_pValues->m_strName.Format(_T("%s:Values"), m_strName);
		m_pValues->m_strID.Format(_T("%s:Values"), m_strID);
#endif
	...
	{
#ifdef _DEBUG
		m_pRptDvmDataset->m_strName.Format(_T("%s:RptDvmDataset"), m_strName);
		m_pRptDvmDataset->m_strID.Format(_T("%s:RptDvmDataset"), m_strID);
#endif
	...
	{
#ifdef _DEBUG
		m_pLogs->m_strName.Format(_T("%s:Logs"), m_strName);
		m_pLogs->m_strID.Format(_T("%s:Logs"), m_strID);
#endif
