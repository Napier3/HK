...
BOOL CRptTemplate_GenDocTool::GenRpt_Word(const CString &strXmlFile, const CString &strDocRptFile)
{
	...
	if (!OpenDocRptFile(strDocRptFile))
	{
...
BOOL CRptTemplate_GenDocTool::GenRpt_Word_TestMacroUiRpt(CRptTemp_TestMacroUiRpt *pRptRemp_TestMacroUiRpt)
{
	...
	while (pos != NULL)
	{
		CRptTemp_Items *pRptTemp_Items = (CRptTemp_Items *)pRptRemp_TestMacroUiRpt->GetNext(pos);
...
BOOL CRptTemplate_GenDocTool::GenRpt_Word_Grid_RowMode(CRptTemp_Grid *pRptTemp_Grid)
{
	...
	MSWord::Table oTable = InsertGrid(nRows, nCols);   //插入表格
...
//插入一个文本
BOOL CRptTemplate_GenDocTool::InsertText(const CString &strText)
{
	if (m_oRptDoc == NULL)
	{
		return FALSE;
	...
	CString strFont, strOldFontName;
	strFont = _T("黑体");
	COleVariant vFont(strFont);
	MSWord::Selection sel;
	sel = m_WordApp.GetSelection();
	...
	if (nPos >= 0)
	{
		...
		sel.TypeText(_bstr_t(strLeft)); //插入文本
...
//插入一个表格
MSWord::Table CRptTemplate_GenDocTool::InsertGrid(long nRows, long nCols)
{
