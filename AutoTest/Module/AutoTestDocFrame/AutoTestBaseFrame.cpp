...
CAutoTestBaseFrame::~CAutoTestBaseFrame()
{
	...
	{
		m_pTestControl->m_oTCtrlMsgRcv.Remove(this);
	}
...
void CAutoTestBaseFrame::CreateClient(CTestControlBase *pTestControl, CDocTemplate* pNewDocTemplate, CDocument* pCurrentDoc, CRuntimeClass* pNewViewClass)
{
...
BOOL CAutoTestBaseFrame::CanCloseAtsFrame()
{
	if (m_pTestControl != NULL)
	{
		...
		{
			CLogPrint::LogString(XLOGLEVEL_TRACE, g_sLangTxt_TestingClose/*L"正在测试中，不能关闭..."*/);
			return FALSE;
		}
	}
...
//界面交互
long CAutoTestBaseFrame::OnException(long nError,const CString &strError, CExBaseObject *pTestControl)
{
...
long CAutoTestBaseFrame::OnConnectSuccess(CExBaseObject *pTestControl)
{
...
/*//2023-8-29  lijunqing 修改时间调用机制，在SttSmartTest中处理
	//shaolie  20210514
	...
	{
		stt_Return_Ats_Event(m_pTestControl, pItem, TRUE, TRUE);
	}

	//再发送当前项目的项目状态改变
	stt_Return_Ats_Event(m_pTestControl, pItem, SYS_STATE_EVENT_OnItemStateChanged);

	//发送大项的状态改变
	stt_Return_Ats_ParentItem_StateChanged(m_pTestControl, pItem);

	//发送进度
	stt_Return_Ats_Event_Process(m_pTestControl, pItem);
...
long CAutoTestBaseFrame::OnStopTest(long nType, CExBaseObject *pTestControl)
{
...
/*//2023-8-29  lijunqing 修改时间调用机制，在SttSmartTest中处理
	if (nType == 1)
	{
		stt_Return_Ats_Event(m_pTestControl, NULL, SYS_STATE_EVENT_OnTestFinished);
		atsrpc_OnTestFinished(m_pTestControl, nType);
	}
	else
	{
		stt_Return_Ats_Event(m_pTestControl, NULL, SYS_STATE_EVENT_OnTestStoped);

		if (m_pTestControl->m_nTestItemIndexBkForAllInlegible > 0)
		{
			//重测结束
			CLogPrint::LogString(XLOGLEVEL_TRACE, _T("+++++++++++重测结束，返回重测结束事件++++++++++++++"));
			stt_Return_Ats_Event(m_pTestControl, NULL, SYS_STATE_EVENT_OnReTestFinished);
		}
...
long CAutoTestBaseFrame::OnUpdateMeasure(long nData, CExBaseObject *pTestControl)
{

	return 0;
}

BOOL CAutoTestBaseFrame::IsRunning(long nType, CExBaseObject *pTestControl)
{
	return FALSE;
}

long CAutoTestBaseFrame::HideAllTestItemUI(long nData, CExBaseObject *pTestControl)
{
...
long CAutoTestBaseFrame::OnTestBegin(CExBaseObject *pTestControl)
{
/*//2023-8-29  lijunqing 修改时间调用机制，在SttSmartTest中处理
	//2021-8-6  lijunqing  立即调用
	atsrpc_OnTestStarted(m_pTestControl, _T(""));
	stt_Return_Ats_Event(m_pTestControl, NULL, SYS_STATE_EVENT_OnTestStarted);

	//子TestControl没有单独的测试开始事件，因此在此处统一发送TestStarted事件
	if (m_pTestControl->GetTestControlMode() == TESTCONTROLMODE_MDVMGRP)
	{
		...
		POS pos = pMainControl->m_listMdvMgrpTestControl.GetHeadPosition();

		while (pos != NULL)
		{
			CTestControlBase *pChildControl = (CTestControlBase *)pMainControl->m_listMdvMgrpTestControl.GetNext(pos);
			stt_Return_Ats_Event(pChildControl, NULL, SYS_STATE_EVENT_OnTestStarted);
		}	
	}
*/
...
void CAutoTestBaseFrame::InsertItemEx(CGbItemBase *pParentItem, CGbItemBase *pSubItem)
{
	if (m_pTreeView == NULL)
	{
		return;
	...
	m_pTreeView->InsertItemEx(pParentItem, pSubItem);
}