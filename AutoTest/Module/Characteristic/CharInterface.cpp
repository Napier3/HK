...
long CCharInterface::XmlReadOwn(CXmlRWNodeBase &oNode, CXmlRWKeys *pXmlRWKeys)
{
...
long CCharInterface::XmlWriteOwn(CXmlRWDocBase &oXMLDoc, CXmlRWElementBase &oElement, CXmlRWKeys *pXmlRWKeys)
{
	CCharacteristicXmlRWKeys *pXmlKeys = (CCharacteristicXmlRWKeys*)pXmlRWKeys;
...
CBaseObject* CCharInterface::CloneEx(BOOL bCopyOwn/*=TRUE*/, BOOL bCopyChildren/*=FALSE*/)
{
	CCharInterface *p = new CCharInterface();
...
long CCharInterface::SerializeOwn(CBinarySerialBuffer &oBinaryBuffer)
{
...
BOOL CCharInterface::GetVariableValue(const CString &strVariable, CString &strValue)
{
	CCharInterfaceVariable *pVariable = (CCharInterfaceVariable*)FindByID(strVariable);
	...
	return pVariable->GetVariableValue(strValue);
}
...
void CCharInterface::InitVariableValues()
{
	...
	while (pos != NULL)
	{
		pVar = (CCharInterfaceVariable *)GetNext(pos);
		pVar->InitVariableValue();
	}
}
