...
void CCharEditDlg::xlang_InitAndAdjust()
{
	...
	m_wndCharName.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_Name);
	m_wndCharMarking.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_Marking);
	m_xFrameCharSystem.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_CoordinateSystem);
	m_xFrameSearchLineMode.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_SearchLineMode);
	m_btnDecSystem.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_DecSystem);
	m_btnPolarSystem.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_PolarSystem);
	m_btnLogSystem.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_LogSystem);
	m_btnVertical.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_Vertical);
	m_btnVerticalLine.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_VerticalLine);
	m_btnHorizontalLine.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_HorizontalLine);
	m_btnOK.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_OK);
	m_btnCharCancel.SetLanguageID(CXLanguageResourceAts_CharLibDev::g_sLangID_Cancel);
	...
	oWndAdjMngr.Adjust(this, TRUE);
	xlang_InitByLgugStr(CXLanguageResourceAts_CharLibDev::g_sLangID_CharCurve, this);
}
...
void CCharEditDlg::OnOK()
{
	...
	if (pFind != NULL)
	{
		...
		{
			strMsg.Format(CXLanguageResourceAts_CharLibDev::g_sLangTxt_ExistNameCurve/*_T("存在名称为“%s”的曲线")*/, m_strName);
			MessageBox(strMsg, CXLanguageResourceAts_CharLibDev::g_sLangTxt_Message1/*_T("提示")*/, MB_OK | MB_ICONERROR);
			return;
		}
	}
