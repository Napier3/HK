...
{
	AfxMessageBox(CXLanguageResourceAts_CharLibDev::g_sLangTxt_AddMemberFctn/*_T("添加成员函数...")*/);
}
...
{
	MessageBox(CXLanguageResourceAts_CharLibDev::g_sLangTxt_NotReali/*_T("还没有实现")*/);
}
...
void CCharLibPane::OnChangeVisualStyle()
{
	...
	{
		TRACE(CXLanguageResourceAts_CharLibDev::g_sLangTxt_UnAddPic/*_T("无法加载位图: %x\n")*/, uiBmpId);
		ASSERT(FALSE);
		return;
	}
