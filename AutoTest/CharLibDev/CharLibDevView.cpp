...
void CCharLibDevView::OnBnClickedBtnDeleteVariable()
{
	...
	CString strMsg;
	strMsg.Format(CXLanguageResourceAts_CharLibDev::g_sLangTxt_SureDelElement/*_T("是否删除变量“%s”？")*/, pVariable->m_strID);
	UINT nRet = MessageBox(strMsg,CXLanguageResourceAts_CharLibDev::g_sLangTxt_Message1 /*_T("提示")*/, MB_YESNO | MB_ICONQUESTION);
...
void CCharLibDevView::OnBnClickedBtnDelete()
{
	...
	CString strMsg;
	strMsg.Format(CXLanguageResourceAts_CharLibDev::g_sLangTxt_SureDelVarious/*_T("你确实要删除元素“%s”吗？")*/, pSel->m_strName);
	
	if (MessageBox(strMsg, CXLanguageResourceAts_CharLibDev::g_sLangTxt_Message1/*_T("提示")*/, MB_YESNO | MB_ICONQUESTION) != IDYES)
	{
		return;
	}
