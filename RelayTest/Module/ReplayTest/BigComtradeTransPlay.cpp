...
BOOL CBigComtradeTransPlay::CreateSttComtradeSocket()
{
	...
	long nRet = m_oComtradeClientSocket.XSendCmd(&oSysCmd);
...
//		pNew->AttatchSttTestMsgViewInterface(this);
...
#ifdef _PSX_QT_LINUX_
	CString strMask;
    CString strChIP = m_pTestEngineClientData->m_pSttClientSocket->SttXmdChAddr();
    stt_net_get_deviceip( strChIP, strMask);
#else
	m_pTestEngineClientData->m_pSttClientSocket->SttXmdChAddr(strIP);
#endif
	...
	m_pTestEngineClientData->m_oCurrUser.IP_User(m_pTestEngineClientData->m_pSttClientSocket->SttXmdChAddr());
	m_pTestEngineClientData->m_oCurrUser.Name_User(m_pTestEngineClientData->m_pSttClientSocket->SttXmdChAddr());
...
void CBigComtradeTransPlay::SetSystemOutputType(int nHasAnalog, int  nHasDigital, int nHasWeek)
{
	m_oComtradePlayConfig.m_oSttModulesComtradeBind.SetSystemOutputType(nHasAnalog, nHasDigital, nHasWeek);
}
