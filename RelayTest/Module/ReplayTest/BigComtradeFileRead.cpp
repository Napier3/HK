...
void CBigComtradeFileRead::InitSrcAnalogsBeginAng_SpecialSmpRate()
{
	...
	for (nIndex = 0;nIndex<m_nSamAmount;nIndex++)
	{
		...
		m_oSrcComtradeFile.UpdateCoef_SpecialSmpRate();
...
BOOL CBigComtradeFileRead::InsertNormalCyclePoints(double dVolValue, double dCurrentValue, double dTime_Second, int nInsertLocation)
{
	...
	if(!m_oSrcComtradeFile.InsertNormalCyclePoints(dVolValue,dCurrentValue,dTime_Second,nInsertLocation))
	{
		return FALSE;
	}
	m_nCurSrcIntervalPoints = m_oSrcComtradeFile.m_nTotalPoints;
...
// 	{
	...
	else
	{
		...
		else if(pModule->IsWeekTypeModule())
		{
			...
			if (nWeekChType == REPLAY_WEEK_CH_ELETYPE_VOL)
			{
...
BOOL CBigComtradeFileRead::UpdateBinaryChReplayBuf( CSttModuleComtradeBind *pModule )
{
	...
	int* pdData1 = (int *)pReplayBufBinary1->GetBuffer_float();
	int* pdData2 = (int *)pReplayBufBinary2->GetBuffer_float();

	int nTotalPoints = m_pInsertComtradeFile->m_nTotalPoints;

	memset(pdData1, 0, nTotalPoints * sizeof(int));
	memset(pdData2, 0, nTotalPoints * sizeof(int));
...
// 							int nBufferIndex = (pBinaryCh->m_nBinaryCh_Index) /16;
							int nBitValue = pBuffer[nPtIndex] >> pSelectData->m_nDataIndex & 0x01;
		...
		if (pdData1 != NULL)
		{
			pdData1[nPtIndex] = (int)(nBoutData & 0xFFFFFFFF); // 低 32 位
		}
		if (pdData2 != NULL)
		{
			pdData2[nPtIndex] = (int)((nBoutData >> 32) & 0xFFFFFFFF); // 高 32 位
		}
	}
