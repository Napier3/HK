...
void QSttIecSmvWaveWidget::CalIndex_NextPage()
{
	...
	//CRtVariables *pVariables = m_pRecordTest->m_pVariables;    只显示当前CapDevice的通道
	CRtVariable* pVariable =NULL;
	...
	{
		//pVariable = (CRtVariable*)pVariables->GetAtIndex(nIndex);    只显示当前CapDevice的通道
	...
	//if (nIndex >= pVariables->GetCount())   只显示当前CapDevice的通道
...
void QSttIecSmvWaveWidget::CalIndex_PrevPage()
{
	...
	{
		//nIndex = pVariables->GetCount()-1;    只显示当前CapDevice的通道
...
//for debug====  8000 修改显示波形的总长度
    long nDataLen = 8000; //m_pRefBuffer->GetDataLength();   用固定时间长度
	dMax = nDataLen;
