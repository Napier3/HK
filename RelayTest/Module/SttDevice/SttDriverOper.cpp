#include "SttDriverOper.h"
#include <fcntl.h>
#include <sys/ioctl.h>
#include <signal.h>	//SIGIO
#include<sys/syscall.h>
#include <sys/types.h>
#include <sys/mman.h>


//2024-12-15
#include "../XDriverFuncDef.h"


CSttDriverOper *CSttDriverOper::g_pSttDriverOper=NULL;
long CSttDriverOper::g_nSttDriverOperRef = 0;

//global variable definition: start with "g_"
int g_fd;
unsigned int *g_start;
int g_nmaplen;

int m_nAnalogSync = 0;
unsigned long m_nModuleSet = 0;


CSttDriverOper::CSttDriverOper(QObject *parent) : QObject(parent)
{

}

void CSttDriverOper::Create(int nfd)
{
	g_nSttDriverOperRef++;

	if (g_nSttDriverOperRef == 1)
	{
		g_pSttDriverOper=new CSttDriverOper;
		g_pSttDriverOper->InitDriver(nfd);
	}
}

void CSttDriverOper::Release()
{
	g_nSttDriverOperRef--;

	if (g_nSttDriverOperRef == 0)
	{
		delete g_pSttDriverOper;
		g_pSttDriverOper = NULL;
	}
}

void CSttDriverOper::InitDriver(int nfd)
{
	g_fd=nfd;
	g_nmaplen = 7782528;
	g_start = (unsigned int *)mmap(NULL, g_nmaplen, PROT_WRITE, MAP_SHARED, g_fd, 0);
	m_nAnalogSync=0;
	m_nModuleSet=1;
}

void CSttDriverOper::StartIecDetect()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x130, 1);
}

void CSttDriverOper::StopIecDetect()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x130, 0);
}

void CSttDriverOper::Set_BO_ENABLE(bool bEnable)
{
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x138, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x138, 0);
	}
}

void CSttDriverOper::Set_BI_ENABLE(bool bEnable)
{
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x139, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x139, 0);
	}
}

void CSttDriverOper::Set_Goose_BO_ENABLE(bool bEnable)
{
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x140, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x140, 0);
	}
}

void CSttDriverOper::Set_Goose_BI_ENABLE(bool bEnable)
{
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x141, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x141, 0);
	}
}

int CSttDriverOper::readIecDetectResult(char* pBuff)
{
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x260);
}

void CSttDriverOper::StartRecord()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x131, 5);
}

void CSttDriverOper::StopRecord()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x131, 0);
}

void CSttDriverOper::Start(int nValue)
{
	//nValue 手动1，状态序列2，差动3
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x103, nValue);
}

void CSttDriverOper::Stop()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x103, 0);
}

long CSttDriverOper::getTemprature()
{
	unsigned int *buffer;
	unsigned int nRet = 0;
	buffer = (unsigned int *)malloc(sizeof(unsigned int));
	nRet = *buffer;
	free(buffer);
	return 0;
}

void CSttDriverOper::WriteData(char* pBuff,unsigned long nLen)
{
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}
void* CSttDriverOper::getMmapBuffer()
{
	return g_start;
}
bool CSttDriverOper::getLtReplayBufferAFlag()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x504, 0)&0x02;
}
bool CSttDriverOper::getLtReplayBufferBFlag()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x504, 0)&0x04;
}
void CSttDriverOper::setLtReplayBufferAFlag()
{
	unsigned int temp = drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x504, 0);
	temp |= 0x02;
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x134, temp);

}
void CSttDriverOper::setLtReplayBufferBFlag()
{
	unsigned int temp = drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x504, 0);
	temp |= 0x04;
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x134, temp);
}

void CSttDriverOper::setLtReplayBufferLoopFlag()
{
	unsigned int temp = drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x504, 0);
	temp |= 0x01;
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x134, temp);
}

void CSttDriverOper::WriteRegister(unsigned long nAddr,unsigned long data)
{
	unsigned int *buffer;
	buffer = (unsigned int *)malloc(sizeof(unsigned int));
	buffer[0] = data;

	drv_reg_ioctl(__FILE__, __LINE__, g_fd,nAddr,data);
	free(buffer);
}

void CSttDriverOper::WriteAnalogSync()
{
	if(m_nAnalogSync == 0){
		m_nAnalogSync = 1;
	}
	else{
		m_nAnalogSync = 0;
	}
	unsigned int *buffer;
	buffer = (unsigned int *)malloc(sizeof(unsigned long));
	buffer[0] = m_nAnalogSync;

	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x70000018,m_nAnalogSync);
	free(buffer);
}

void CSttDriverOper::WriteFPGAData(unsigned long nAddr,unsigned long data)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,nAddr,data);
}

void CSttDriverOper::WriteFPGAAnalog(char* buff,unsigned long nLen)
{
	unsigned long* p = (unsigned long*)buff;
	memcpy(g_start+4, buff, nLen);
	for(int i=0;i<16;i++)
	{
		drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x70004010+i*4,p[i]);
	}
	WriteAnalogSync();
}

int CSttDriverOper::IoCtrl(unsigned long ncmd,unsigned long nValue)
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd,ncmd,nValue);
}

int CSttDriverOper::readRampResult(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x207/*sizeof(structRampResult)*100*/);
#else
	return 0;
#endif
}

int CSttDriverOper::readStateResult(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x203/*sizeof(structResultItem)*/);
#else
	return 0;
#endif
}

int CSttDriverOper::readRecordResult(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x261);
#else
	return 0;
#endif
}

int CSttDriverOper::readDeviceInnerPara(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x204);
#else
	return 0;
#endif
}

int CSttDriverOper::readDisBinStatePara()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x208,0);
}

int CSttDriverOper::readDisResult(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x207/*sizeof(distripResult)*/);
#else
	return 0;
#endif
}

void CSttDriverOper::SetisStopIfBiChanged(int nValue)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x104,nValue);
}

void CSttDriverOper::SetStopDelayTime(int nTime)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x105,nTime);
}

void CSttDriverOper::SetBinaryOut(int nState)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x107,nState);
}

void CSttDriverOper::SetBinaryOutTurnOnVaule(int nState)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x137,nState);
}

void CSttDriverOper::SetBinaryInLogic(int nLogic)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x10D,nLogic);
}

void CSttDriverOper::setManualAcAnalogPara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x300,0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif

}

void CSttDriverOper::setManualBISetting(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x302,0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setStatePara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x301,0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setStatePara(char* pBuff,int nLen,int nType)
{
#ifdef _PSX_QT_LINUX_
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x301,nType);
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::SetModuleInfo(char* pBuff,int nLen)
{
#ifdef _PSX_QT_LINUX_
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x331, 0);
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setDiffAcAnalogPara(char* pBuff,int nLen,int Statue)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x303,Statue);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setSearchAcAnalogPara(char* pBuff,int nLen,int Statue)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x307,Statue);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setDisAcAnalogPara(char* pBuff, int nLen, int State)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x305,State);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void  CSttDriverOper::setStateTrigerManual()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x400,0);
}

void CSttDriverOper::setDcPowerValue (char *pChar)
{
	/*****
	float fXS = CSttDevice::g_pSttDevice->getUaxuDcXS();
	float fzero = CSttDevice::g_pSttDevice->getUaxuDcZero();
	int fVlot = CSttDevice::g_pSttDevice->getUdcMaxNum();
	float fUDC = atof(pChar);

	fUDC += fzero;
	fUDC = fUDC*fXS;
	float fTmp = fUDC/fVlot*0.75;
	float fValue = qPow(2,31)*fTmp;
	long nValue = fValue;
	nValue = nValue&0xFFFFFFF0;
	unsigned short model = CSttDevice::g_pSttDevice->getModuleNum(VOLT,0); //zhangsen 20180822
	nValue += model;

	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x10f,nValue);
	*****/
}

int CSttDriverOper::setDeviceDaPara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x304,0);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#else
	return 0;
#endif
}

int CSttDriverOper::setFlashData(char* pbuff,int nLen,int nMode)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x306,nMode);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pbuff,nLen);
#else
	return 0;
#endif
}

int CSttDriverOper::readFlashData(char* pbuff,int nMode,int nCmd)
{
#ifdef _PSX_QT_LINUX_
	if(nMode == 0)
	{
		return drv_reg_read(__FILE__, __LINE__, g_fd,pbuff,0x209);
	}
	else
	{
		return drv_reg_read(__FILE__, __LINE__, g_fd,pbuff,nCmd);
	}
#else
	return 0;
#endif
}

int CSttDriverOper::setFlashReadCmd(int module)
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x123, module);
}

int CSttDriverOper::readFlashDataState(char* pbuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd, pbuff, 0x208);
#else
	return 0;
#endif
}

int CSttDriverOper::setRelayData(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x308,1);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#else
	return 0;
#endif
}

int CSttDriverOper::setRelayConfData(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x308,0);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#else
	return 0;
#endif
}
int CSttDriverOper::setLtRelayConfData(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x352,0);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#else
	return 0;
#endif
}
int CSttDriverOper::readReplayIrp()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x501, 0);
}

int CSttDriverOper::setRampPara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x309, 0);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#else
	return 0;
#endif
}

void CSttDriverOper::set_optical_port(char *pbuff,int nLen,int nFlag)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x321, nFlag)	;	//	flag = 0:光口配置	1：SMV发送配置2：GOOSE发送配置3：FT3发送配置
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pbuff, nLen);
#endif
}

void CSttDriverOper::setConfigIECCoderData(char * pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,  0x320, 0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setGooseChangedCoderData(char * pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x30B, 1);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setGooseTestMode(char * pBuff,int nLen)//置检修
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x30D, 1);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setGooseSubData(char * pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x30C, 1);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setFT3SubData(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x310, 1);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setCurrentMerge(unsigned int nMerge)
{
	int nFlag = m_nModuleSet&(~0x1);
	nFlag = nFlag | nMerge;
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x110, nFlag);
}

void CSttDriverOper::setPollingsyncsecond(unsigned int nsync)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x110, nsync);
}

void CSttDriverOper::setGooseBoMapData(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x30E, 1);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

void CSttDriverOper::setSendAtStop(unsigned long nValue)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x121,nValue);
}

int CSttDriverOper::updateFpga(char* pbuff,int nLen,int nModule)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x30F,nModule);
#ifdef _PSX_QT_LINUX_
	return drv_reg_wite(__FILE__, __LINE__, g_fd,pbuff,nLen);
#else
	return 0;
#endif
}

int CSttDriverOper::ReadupdateFpga(char* pbuff,int nType)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pbuff,nType);
#else
	return 0;
#endif
}

int CSttDriverOper::ReadupdateFpgaState()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x502,0);
}

int CSttDriverOper::sendUpdateFpgaOver()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x30F,0);
}

int CSttDriverOper::getUpdateFpgaPrecent()
{
	return drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x505,0);
}

void CSttDriverOper::startDMA()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0xA00,1);
}

void CSttDriverOper::stopDMA()
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0xA00,0);
}

void CSttDriverOper::setFilter(int nFilter)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x402, nFilter);
}

int CSttDriverOper::readDMA(char* buffer)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd, buffer, 0x230);
#else
	return 0;
#endif
}

void CSttDriverOper::setSwingingPara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x311,0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setabnStatePara(char* pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd,0x312,0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::setfiberports(int nfiberport)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x122, nfiberport);
}

void CSttDriverOper::SetCurrent_Multiple(int nType)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x124, nType);
}

void CSttDriverOper::setDcModuleTap(int nTap)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x125, nTap);
}

void CSttDriverOper::SetStopDcOutput(char *pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x330, 0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd,pBuff,nLen);
#endif
}

void CSttDriverOper::SetBinaryModule(char *pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x340, 0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

int CSttDriverOper::ReadBinaryModule(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x240);
#else
	return 0;
#endif
}

void CSttDriverOper::setModuleValied(int nModule)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x126, nModule);
}

void CSttDriverOper::SetSyncPara(char *pBuff,int nLen)
{
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x350, 0);
#ifdef _PSX_QT_LINUX_
	drv_reg_wite(__FILE__, __LINE__, g_fd, pBuff, nLen);
#endif
}

int CSttDriverOper::ReadSyncResult(char* pBuff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,pBuff,0x250);
#else
	return 0;
#endif
}

void CSttDriverOper::setRising(int nRising)
{
#ifdef _PSX_QT_LINUX_
	drv_reg_ioctl(__FILE__, __LINE__, g_fd, 0x132, nRising);
#endif
}

int CSttDriverOper::ReadPPStime(char* buff)
{
#ifdef _PSX_QT_LINUX_
	return drv_reg_read(__FILE__, __LINE__, g_fd,buff,0x503);
#endif
}
