...
int CSttDevicePST200s::InitDevice(BOOL bHasAdjFile)
{
    ...
    InitReplayModule();
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0xA00,1);
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x103,0);
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x402,0);//禁止2M报文上送使能
...
void CSttDevicePST200s::SetTestPara(int nTestType, char *pszBuff)
{
    ...
    case IECDataCfg:
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,  0x320, 0);
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pszBuff,sizeof(Drv_IECConfig));
    }
        break;
    case SvCtrlCfg:
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x321, 1);//1：SMV发送配置
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pszBuff,sizeof(SendConfSV));
    }
        break;
    case GseCtrlCfg:
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x321, 2);//2：GOOSE发送配置3：FT3发送配置
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pszBuff,sizeof(SendConfGSE));
    }
        break;
    case Ft3CtrlCfg:
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x321, 3);//3：FT3发送配置
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pszBuff,sizeof(SendConfFT3));
    }
        break;
    case FiberCtrlCfg:
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x321, 0);//flag = 0:光口配置
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pszBuff,sizeof(FiberConf));
    }
        break;
    }
}
...
void CSttDevicePST200s::InitReplayModule()
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x405,0);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,&oPIA48SZero,sizeof(DrvPIA48SZero));

    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x404,0);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,&m_oModuleDef,sizeof(DrvModuleDef));
}
...
BOOL CSttDevicePST200s::OnTimer(BOOL bExec)
{
    ...
    int nSize = drv_reg_read(__FILE__, __LINE__, m_nCommfd,(char*)&m_oSysUtc,0x205);
...
void CSttDevicePST200s::Load_RcdData()
{
    ...
    int nSize = drv_reg_read(__FILE__, __LINE__, CSttDeviceBase::g_pSttDeviceBase->GetDriverFd(),(char*)&oSysUtc,0x205);
    if(nSize == sizeof(Drv_SysUtc))
    {
...
//#endif
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x402,0);//禁止2M报文上送使能
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x505,0);//CLear 2M PlayData
...
void CSttDevicePST200s::SetTestStart(int nStateIdx)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x308,2);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,(char*)&oReplayTrip,sizeof(DrvReplayTrip));
    //开始实验
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x103,6);
    bTestStarted = TRUE;
    //回放数据全部下载完毕，开始等待GPS触发实验
    CDataGroup oDataGroup;
...
void CSttDevicePST200s::EventProcessEx()
{
    ...
    long nRet = drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x501, 0);
    if(nRet == 1)
    {
        //继续下载后续回放数据
        if(m_nCurModule == 0)
    ...
    else if(nRet == 2)
    {
		...
		//读取开入量变位
		int nRet2 = drv_reg_read(__FILE__, __LINE__, m_nCommfd,m_pDrvResults,0x262);
		if(nRet2 > 0)
		{
    ...
    //读取2M RecordData
    nRet = drv_reg_read(__FILE__, __LINE__, m_nCommfd,m_pDrvResults,0x261);
    if(nRet > 0)
    {
...
void CSttDevicePST200s::SendConfToDevice()
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x308,0);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,(char*)&oReplayPara,sizeof(DrvReplayPara));
}
...
void CSttDevicePST200s::SendDataToDevice_Week()
{
    ...
    if(bWriteData)
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x308,1);
        drv_reg_wite(__FILE__, __LINE__, m_nCommfd,(char*)&oReplayDatas,sizeof(DrvReplayDatas));
    }
    else
    {
		m_nCurModule = 2;
