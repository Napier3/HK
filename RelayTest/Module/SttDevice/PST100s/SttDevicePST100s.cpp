...
int CSttDevicePST100s::InitDevice(BOOL bHasAdjFile)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x409, 0); //设置RTDS采样率,0-不修改
...
{
    drv_reg_read(__FILE__, __LINE__, m_nCommfd, &m_o2MTripLog, 0x263);
...
void CSttDevicePST100s::EventProcessEx()
{
    ...
    int ret = drv_reg_read(__FILE__, __LINE__, m_nCommfd, m_pDrvResults, 0x261);

    //    CLogPrint::LogFormatString(XLOGLEVEL_INFOR,_T("ret = %d"),ret);
    if(ret > 0)
    {
        //判断当前数据类型
        nRecordPkgType = drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x406, 0);

        switch(nRecordPkgType)
        {
            case 1:
            {
//            Drvfile.drv_reg_wite(__FILE__, __LINE__, (char*)m_pDrvResults,ret);
                //RTDS 光口1
    ...
    ret = drv_reg_read(__FILE__, __LINE__, m_nCommfd, m_pDrvResults, 0x262);
        ...
        for(int i = 0; i < ret / nDrvResItemLen; i++)
        {
            ...
            switch(m_oResultItem.ntype)
            {
                case 0:
                {
                    ...
                    m_nReadRTDSPort = 0;
                    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x403, 0); //申请RTDS位置1数据
                }
                break;
                ...
                case 3:
                {
                    ...
                    if(g_pSttSimulationBase->GetTestStarting() && nRecordStartType == 1)
                    {
                        if(!bIsRecording)
                        {
                            drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 0x07);
                            bIsRecording = TRUE;
                        }
                    }
                }
                ...
                case 5:
                {
                    ...
                    if(m_nValidRtdsPorts > 1)
                    {
                        ...
                        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x403, 1); //申请RTDS位置2数据
                    }
                    else
                    {
                        //测试结束
...
//                g_pSttSimulationBase->SetRTDSDataDebug(pRtdsSampleForThread->m_pRcdDataBuffers[0]);
                ...
                case 7:
                    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x403, m_nReadRTDSPort);
                    break;
            }
        }
    }
...
void CSttDevicePST100s::SendStartToDrv()
{
    ...
    //数据都已发送完毕
    //1、开始2M回放
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 5);
    ...
    {
        drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x504, 0); //CLear RTDS RecordSignal
    }
}

void CSttDevicePST100s::SetReplayBout(int nTestType)
{
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x506, 0); //清除开关量回放缓存
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30A, 0);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char *)&m_oReplayBout, sizeof(DrvReplayDataBout));
}
...
void CSttDevicePST100s::SetTestStart(int nTestType)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x409, nPoint); //设置RTDS采样率
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x307, 0);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char *)&m_oPST100Para, sizeof(StPST100Para));
    SendStartToDrv();
}
