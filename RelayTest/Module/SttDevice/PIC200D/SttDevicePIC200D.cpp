...
int CSttDevicePIC200D::InitDevice(BOOL bHasAdjFile)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 1);//Start DMA
    //	SENDAtoFPGA(1,0x103);
...
void CSttDevicePIC200D::SetMeasModuleChanGear(int nModuleType,int nModulePos, int nChannel, int nGearId)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 2);//开DMA中断消息
...
void CSttDevicePIC200D::tmt_to_drv_pic200d(tmt_Pic200dTest *pTmt,BOOL bUseGear)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 2);//开DMA中断消息
    //从pTmt取出上位机下发的参数
    //利用m_nCommfd给驱动下发参数,drv_reg_wite(__FILE__, __LINE__, m_nCommfd,,)
}
...
void CSttDevicePIC200D::EventProcessEx()
{
    ...
    int inter_flag =drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x110,0);
    if(inter_flag==0)
        return;
...
{
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 3);//关DMA中断消息
...
{
    int AD_Count=0;
    int nLen = drv_reg_read(__FILE__, __LINE__, m_nCommfd,DMARes,200000);
...
int CSttDevicePIC200D::SENDAtoFPGA(int VALUE,int ADDR)
{
    int VALUEtemp[1];
    VALUEtemp[0] = VALUE;
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,VALUEtemp,ADDR);
...
int CSttDevicePIC200D::Send_To_Fpga(char* pbuff,int nLen,int nMode)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x201,nMode);
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pbuff,nLen);
}
