...
int CSttDevicetest::InitDevice(BOOL bHasAdjFile)
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, PNS330_RegAddr_DigitalCtrl, m_nRegCtrlDigital);
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x102, 0b0010); //00-BCode 01:FT3 10:PPS
...
BOOL CSttDevicetest::OnTimer(BOOL bExec)
{
    ...
    nSize = drv_reg_read(__FILE__, __LINE__, m_nCommfd, (char *)&m_oSysInfo, 0x204);
...
void CSttDevicetest::SetIEC61850Para()
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x301, 0);//GOOSE发送配置
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char *)&oGooseCfg, sizeof(GooseCfg));
    ...
    if(pSysParasMngr != NULL)
    {
        ...
        if(pFiberParas != NULL)
        {
            ...
            drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x122, g_theSystemConfig->m_nFiberType);
        }
    }
    ...
    drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x302, 0);//GOOSE
    drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char *)&m_oGooseItems, sizeof(GooseItems));
