...
int CSttDevicePNI302::InitDevice(BOOL bHasAdjFile)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x7000003c,1);//1588
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x110, 1);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0xA00,1);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x100, 1);
	Sleep(50);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x131, 5);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 1);
}

void CSttDevicePNI302::SetStep6U6IStop()
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0xA00,0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x131, 0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x100, 0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x110, 0);
}
//----------Step6U6I-------------
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x132, nRising);
}

int CSttDevicePNI302::GetPPS()
{
	return drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x503,0);
}
//-----------MUTest-------------
...
void CSttDevicePNI302::SetIecdetectStart(tmt_RecordTest *pTmt)
{
	...
	if(pTmt->m_oRecordPara.m_mode==1)
	{
		...
		drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pDrv, sizeof(GooseSubData));
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x130, 0);
		pDrv->nPort=0;
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30C, 1);
		drv_reg_wite(__FILE__, __LINE__, m_nCommfd, pDrv, sizeof(GooseSubData));
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x130, 1);
	}
}
...
void CSttDevicePNI302::SetIEDTestStart_B2G()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x103,0xa);
}
...
void CSttDevicePNI302::tmt_to_drv_iedtest_B2G(tmt_IEDTest *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x340, 0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char*)stBinOut, sizeof(Drv_SoePara));
}
...
void CSttDevicePNI302::ProIEDTestDrvResult_B2G(tmt_IEDTest *pTmt,tmt_IEDPara *pPrePara)
{
	...
	int nRet = drv_reg_read(__FILE__, __LINE__, m_nCommfd,pDrvResults,0x240);
	if(nRet < 0)
	{
		return;
	}
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0xA00,1);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x402, 0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x103, 0xc);
}

void CSttDevicePNI302::SetIEDTestStop_G2B()
{
	structGSOutMap *stMaps=m_oCIEC61850CfgMngr.stMaps;
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30E, 1);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char*)stMaps, sizeof(structGSOutMap));
}
...
void CSttDevicePNI302::tmt_to_drv_iedtest_G2B(tmt_IEDTest *pTmt)
{
	...
	case SOEMode:
	{
		if(!pSttTestBase->GetTestStarting())
		{
			...
			drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30E, 1);
			drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char*)pGSOutMap, sizeof(structGSOutMap));
		}
	}
		nMode=2;
		break;
	...
	if(!pSttTestBase->GetTestStarting())
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x133, nMode);
	}
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x341,0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,(char*)stBinOut,sizeof(Drv_SoePara));
}
