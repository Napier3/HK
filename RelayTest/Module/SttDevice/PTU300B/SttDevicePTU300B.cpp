...
int CSttDevicePTU300B::InitDevice()
{
	...
	//	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x114,0xff0000);//含小信号档位
	//	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x104,4);//采集档位固定100
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x105,0x80);//电压电流档位
...
void CSttDevicePTU300B::tmt_to_drv_manu(tmt_ManualTest *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x112,nOverload);
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x107,nBo);

	WriteDApara();

	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x300,0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pszBuf,sizeof(structACPara));
}
...
void CSttDevicePTU300B::tmt_to_drv_StatePara(tmt_StateTest *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x302,0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pBuff,sizeof(structStateParam));
}
