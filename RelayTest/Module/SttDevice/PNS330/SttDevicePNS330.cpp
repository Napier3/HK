...
int CSttDevicePNS330::InitDevice(BOOL bHasAdjFile)
{
    ...
    if(m_nCommfd<=0){
        CLogPrint::LogString(XLOGLEVEL_TRACE,"/dev/SttDriver_PNS330 open fail!");
        QCoreApplication::quit();

    }
    char buf_tes[1024];
    g_nStateCount = drv_reg_read(__FILE__, __LINE__, m_nCommfd, buf_tes, 0x201);//xuhm 20241210
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("open good! m_nCommfd=%x"),m_nCommfd);
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
    g_nStateCount = drv_reg_read(__FILE__, __LINE__, m_nCommfd,NULL,0x201);

    //if(g_nStateCount<=0)    //调试添加，xuhm，状态序列功能的状态数量
        g_nStateCount=20;  //tlw-12-28  g_nStateCount=20;
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("333g_nStateCount=%d"),g_nStateCount);

    nFlag = drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x106, 0);   //dirver=#define GET_BOOT_VERSION 0x106   read reg FPGA-version 0x7000_205C
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

	nFlag = nFlag >> 24;

    if(nFlag == 0x34)  //plugin mode
	{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
    SetRegisterValue(PNS330_RegAddr_DigitalCtrl, m_nRegCtrlDigital);   //set input binary mode
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	Sleep(100);
	m_nRegCtrlDigital &= ~(1 << 8);
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	...
	if(g_theDeviceSystemParameter->m_nModuleCount_UI > 0)
	{
		if(m_nBootMode == 0)
		{
			...
			if(pModule != NULL)
			{
                pModule->m_oModuleAttr.m_nModuleIndex = 1;//16;//bit31模拟信号
                ...
                pChDrvPos->m_nChDrvPos[0] = 8;
                pChDrvPos->m_nChDrvPos[1] = 10;
                pChDrvPos->m_nChDrvPos[2] = 12;
                pChDrvPos->m_nChDrvPos[3] = 11;
                pChDrvPos->m_nChDrvPos[4] = 9;
                pChDrvPos->m_nChDrvPos[5] = 7;
                pChDrvPos->m_nChDrvPos[6] = 6;
                pChDrvPos->m_nChDrvPos[7] = 4;
                pChDrvPos->m_nChDrvPos[8] = 2;
                pChDrvPos->m_nChDrvPos[9] = 5;
                pChDrvPos->m_nChDrvPos[10] = 3;
                pChDrvPos->m_nChDrvPos[11] = 1;
				pModule->init_ch_drv_pos();

                if(pModule->m_oModuleAttr.m_nChannelNum > 12)  //10)
				{
                    pModule->m_oModuleAttr.m_nChannelNum = 12;  //10;
				}
    ...
    m_nTouchfd = open("/dev/exc7200dev", O_RDWR);  //exc7200dev:touch panel driver
    if(m_nTouchfd<=0){
        CLogPrint::LogString(XLOGLEVEL_TRACE,"/dev//dev/exc7200dev open fail!");//xuhuaiming
        qDebug()<<"/dev//dev/exc7200dev open fail!";
      }
    qDebug()<<"/dev//dev/exc7200dev open Succ!  m_nTouchfd= " <<m_nTouchfd;
    ...
    g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
    SetRegisterValue(PNS330_RegAddr_AnalogCtrl, m_nRegCtrlAnalog);//缺指令 xuhm
	//Uart0
    SetRegisterValue(0xe0000018, 62); //4G
    SetRegisterValue(0xe0000034, 6);//缺指令 xuhm
    g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
    ...
    g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);//xuhm 驱动没有，屏蔽20241207
    g_nDrvMainSTMode = drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x506, 0);
    g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDevicePNS330::SetDrvMoudlePara()
{
    ...
    if(m_nBootMode == 0)   //main board mode
	{
		if(g_theDeviceSystemParameter->m_nModuleCount_UI > 0)
		{
			int nIndex = oDrv_ModulePara.nAlgCnt;
            oDrv_ModulePara.nAlgTable[nIndex] = 0x8000;  //0x0001;
		...
		{
			int nIndex = oDrv_ModulePara.nAlgCnt;
            oDrv_ModulePara.nAlgTable[nIndex] = 0x4000; //0x0002;
...
BOOL CSttDevicePNS330::OnTimer(BOOL bExec)
{
	...
	if(!bExec)
	{
		...
		{
     //        qDebug()  << "FALSE: m_nDeviceOnTimerCount < 20" <<__FUNCTION__ <<m_nDeviceOnTimerCount<< __LINE__;
			return FALSE;
		}
            //qDebug()  << "m_nDeviceOnTimerCount: "  <<m_nDeviceOnTimerCount<<__FUNCTION__<< __LINE__;
        m_nDeviceOnTimerCount = 0;
	}

	if(!m_bDevStaIsInit)
	{
         //qDebug()  << "FALSE: !m_bDevStaIsInit" <<__FUNCTION__ << __LINE__;
		return FALSE;
	}
 ...
 //   qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
	...
	if(g_theSystemConfig->m_nHasAnalog == 0)
	{
		...
		g_theSttDeviceStatus->m_nUShort = 0;//电压短路
         //qDebug() <<"if(g_theSystemConfig->m_nHasAnalog == 0)";
	}
...
{
	//将锁移到调用处
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, nAddr, nValue);
}
	...
	if(m_nBootMode == 0)
	{
		...
		if(pModule!=NULL)
		{
			for(int nIndex=0;nIndex<g_theSystemConfig->m_oGearSetCurModules.m_nCurModuleNum;nIndex++)
			{
                ...
                if(oCurModuleGear.m_nModulePos == 1) //16)
				{
...
void CSttDevicePNS330::SetPeripheral()
{
    ...
    if(m_nTouchfd<=0){
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nTouchfd, 0x7000002c, nValue);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
    }
    else{
            qDebug()<<"drv_reg_ioctl(__FILE__, __LINE__, m_nTouchfd, 0x7000002c, nValue) ERR!";
    }
    qDebug()<<"drv_reg_ioctl(__FILE__, __LINE__, m_nTouchfd, 0x7000002c, nValue) succ!";
	...
	SetRegisterValue(PNS330_RegAddr_DigitalCtrl, m_nRegCtrlDigital);
	//主板ST口模式设置，330默认RX2是B码
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x102, m_nMainSTMode); //00:None 01:FT3 10:PPS 11:BCode
