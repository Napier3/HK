...
void CSttDevicePFC6A::tmt_to_drv_manu(tmt_ManualTest *pTmt,BOOL bIsStartTest)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x300,0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pszDrv,sizeof(Drv_ManuData));
}
...
void CSttDevicePFC6A::Manual_Current_1000A(tmt_channel *pTmt_channel,char *pszDrv,int nHarm,int nModuleIIndex,BOOL bIsDc,BOOL bIsModuleOutPut)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x70006014,nRetTap);
...
void CSttDevicePFC6A::tmt_to_drv_systemdefaultoutput(TMT_SYSTEM_DEFAULT_OUTPUT_PARAS *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x353,0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pszDrv,sizeof(Drv_ManuData));
}
...
void CSttDevicePFC6A::tmt_to_drv_StateItem(tmt_StateParas *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x301,1);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd,pBuf,nLen);
}
...
void CSttDevicePFC6A::SetDrvMoudlePara()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x331, 0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char*)&oDrv_ModulePara,sizeof(Drv_ModulePara));

	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x356, 0);
	drv_reg_wite(__FILE__, __LINE__, m_nCommfd, (char*)&m_oModuleMaxVaildCnt,sizeof(Drv_ModuleMaxVaildCnt));
}
