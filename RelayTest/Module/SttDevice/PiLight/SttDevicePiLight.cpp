...
int CSttDevicePiLight::InitDevice(BOOL bHasAdjFile)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x110,0);//关闭秒同步减少报文量
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x120,0);//关闭主板开关量模块防抖
	...
	//开始命令
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,1);
	nAngleData_4Bytes[5] = 0x03;
	nAngleData_4Bytes[6] = 0x01;
	drv_reg_wite(__FILE__, __LINE__, m_nfd,(char*)&nAngleData_4Bytes,8);

	//停止命令
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,2);
	nAngleData_4Bytes[5] = 0x03;
	nAngleData_4Bytes[6] = 0x00;
	drv_reg_wite(__FILE__, __LINE__, m_nfd,(char*)&nAngleData_4Bytes,8);

	//读取命令
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,6);
...
void CSttDevicePiLight::ProLightDrvResult(BOOL &bTestFinished,STT_TIME &oTestFinishedTime)
{
	...
	{
		nret = drv_reg_read(__FILE__, __LINE__, m_nfd,pDrvResults,0x261);
...
void CSttDevicePiLight::tmt_to_drv_lightpara(void *pVoidTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,4);
	drv_reg_wite(__FILE__, __LINE__, m_nfd,(char*)&nAngleData_15Bytes,19);
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,5);
	drv_reg_wite(__FILE__, __LINE__, m_nfd,pszDrv,sizeof(Drv_CurrentData));//驱动开始输出零值
	...
	if(!pSttTestBase->GetTestStarting())
	{
		...
		Sleep(1000);
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x145,1);//开始弧光
	}
}
...
void CSttDevicePiLight::tmt_to_drv_lightangle(void *pVoidTmt,unsigned int nType)
{
	...
	if(nType == 0)
	{
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,nData);
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,3);
		drv_reg_wite(__FILE__, __LINE__, m_nfd,(char*)&nAngleData_6Bytes,10);
	}
	else if(nType == 1)
	{
		nAngleData_4Bytes[5] = 0x03;
		nAngleData_4Bytes[6] = 0x02;
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x358,0);
		drv_reg_wite(__FILE__, __LINE__, m_nfd,(char*)&nAngleData_4Bytes,8);//初始化
		Sleep(1000);
		CSttTestBase *pSttTestBase = CSttServerTestCtrlCntr::GetCurrTest();
		pSttTestBase->SetTestStarting(FALSE);
	...
	else
	{
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x145,2);//读取弧光角度
	}
}
...
{
	g_nTestType = TestType_Null;
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0x145,0);//停止弧光
}
