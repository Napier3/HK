...
{
    qDebug() << ">>>>>>CSttDeviceBase::CSttDeviceBase()";
	for(int i = 0;i < 10;i++)
	{
  ...
  qDebug() << ">>>>>>CSttDeviceBase::CSttDeviceBase()";
	...
	stt_init_stt_device_status();
     qDebug() << ">>>>>>stt_init_stt_device_status() ok";
	int fd = open("/dev/mem", O_RDWR| O_SYNC, 0655);
    qDebug() << ">>>>>open(/dev/mem, O_RDWR| O_SYNC, 0655);";
    qDebug() << "CSttDeviceBase::CSttDeviceBase =" << fd;

 //   CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("CSttDeviceBase::CSttDeviceBase()%d"),fd);
 ...
 //   CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("CSttDeviceBase::CSttDeviceBase m_pDrvUtcBase:0x%p,m_pDrvModuleBase:0x%p"),m_pDrvUtcBase,m_pDrvModuleBase);
    qDebug() << "CSttDeviceBase::CSttDeviceBase m_pDrvUtcBase= " << (unsigned long)m_pDrvUtcBase << " m_pDrvModuleBase ="<< (unsigned long)m_pDrvModuleBase;
}
...
void CSttDeviceBase::InitDASetting()
{
	...
	{
        memcpy(&nMngrVer, m_shm, 4);   //read shared memory manager version info
	}
 ...
 qDebug() << "AAAAAAAAAAAAAAAAAAAA" << __FUNCTION__ << __LINE__<<nCnt;
...
void CSttDeviceBase::SetOverLoadTime_PTU(int nTime_ms)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x313, 0);
	drv_reg_write(__FILE__, __LINE__, m_nCommfd, &oDevOverLoad, sizeof(DeviceOverLoad));
...
void CSttDeviceBase::WriteDApara(DeviceDAPara *pDAPara, BOOL IsSysDefaultOut)
{
	...
	if(IsSysDefaultOut)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x355, 0);
		drv_reg_write(__FILE__, __LINE__, m_nCommfd, (char *)pDAPara, sizeof(DeviceDAPara));
	}
	else
	{
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x304, 0);
		drv_reg_write(__FILE__, __LINE__, m_nCommfd, (char *)pDAPara, sizeof(DeviceDAPara));
	}
#endif
}
...
#ifdef _PSX_QT_LINUX_
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x305, 0);
	drv_reg_write(__FILE__, __LINE__, m_nCommfd, (char *)&m_oDAParaForTestStop, sizeof(DeviceDAPara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
#endif
}
...
float CSttDeviceBase::GetChannelMax(tmt_channel &oChannel, float fBaseHz, int nDirection, int nHarmIndex, float fGradientAmp)
{
 ...
 //qDebug() << "AAAAAAAAAAAAAAAAAAAAAAAAA" << __FUNCTION__ << __LINE__<<((unsigned long)(fW[nIndex] + XWVector[nIndex]));
...
//qDebug() << "AAAAAAAAAAAAAAAAAAAAAA"<< __FUNCTION__ << __LINE__<<((unsigned long)(fW[i][nIndex] + XWVector[i][nIndex]))* (double)1.4142135623731;
...
void CSttDeviceBase::SetIecDetect(BOOL bEnable, long nIecType)
{
	if(bEnable)
	{
		...
		if(m_nIsIecMode == 1 || m_nIsIecMode == 2)
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x131, DMA_TYPE_NULL);
			drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x126, 0);//关闭FPGA DMA缓存
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x126, 1);//开启FPGA DMA缓存
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

		Sleep(10);
		m_oIecRecvCfg.nType = nIecType;//0-All,1-SV,2-GOOSE,3-FT3
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30C, 1);
		drv_reg_write(__FILE__, __LINE__, m_nCommfd, &m_oIecRecvCfg, sizeof(IecRecvCfg));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

		Sleep(10);
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x131, DMA_TYPE_PROBE);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	else
	{
...
void CSttDeviceBase::SetIecFilter(IECRecordItems *pRecordItems)
{
	if(pRecordItems->bEnable)
	{
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x30C, 2);
		drv_reg_write(__FILE__, __LINE__, m_nCommfd, (char *)&m_oIecRecvCfg, sizeof(IecRecvCfg));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		...
		if(pRecordItems->nCount6044 > 0)
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x310, 0);
			drv_reg_write(__FILE__, __LINE__, m_nCommfd, (char *)&oSubDatas, sizeof(Ft3SubDatas));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x131, DMA_TYPE_RECORD);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	else
	{
...
void *IecHeaderCapThread(LPVOID pParam)
{
	...
	while(1)
	{
				...
				|| nCount > 10)
		{
			nCount = 0;
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			ret = drv_reg_read(__FILE__, __LINE__, pSttDeviceBase->m_nCommfd, pDrvResults, 0x260);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void *IecPkgCapThread(LPVOID pParam)
{
	...
	while(1)
	{
				...
				|| nCount > 10)
		{
			nCount = 0;
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			ret = drv_reg_read(__FILE__, __LINE__, pSttDeviceBase->m_nCommfd, pDrvResults, 0x261);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x402, nDMAType);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
/*********DMA**********/
...
//m_nMergeCurTerminal==3只有PN466支持
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x124, nCurrentCtrlMode);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
