...
{
        int m_nModulesCount;//????????????????
        int m_nModulesBiCount;//??????????????????
        int m_nModulesBoCount;//??????????????????
...
typedef struct
{
        float m_fTripValue;//?????
        float m_fTripTime;//???????
        float m_fTripValueChl[MAX_BINARYIN_COUNT];//????????????0
        float m_fTripValueChlEx[MAX_ExBINARY_COUNT];//????????????0
        float m_frTimeBinAct[MAX_BINARYIN_COUNT][BIBO_ACTCOUNT]; //???????????64?¦Á?¦Ë???
        float m_frTimeBinExAct[MAX_ExBINARY_COUNT][BIBO_ACTCOUNT]; //??????????????64?¦Á?¦Ë???
        int	  m_nrBinSwitchCount[MAX_BINARYIN_COUNT]; //??¦Ë????
        int	  m_nrBinExSwitchCount[MAX_ExBINARY_COUNT]; //??¦Ë????
        float m_frTimeBoutAct[MAX_BINARYIN_COUNT][BIBO_ACTCOUNT]; //???????????64?¦Á?¦Ë???
        float m_frTimeBoutExAct[MAX_ExBINARY_COUNT][BIBO_ACTCOUNT]; //??????????????64?¦Á?¦Ë???
        int	  m_nrBoutSwitchCount[MAX_BINARYIN_COUNT]; //??¦Ë????
        int	  m_nrBoutExSwitchCount[MAX_ExBINARY_COUNT]; //??¦Ë????
        ...
        int   m_nRampIndexEx[MAX_ExBINARY_COUNT];//¦Ä?????-1??0??????????????;1???????1????????
...
typedef struct
{
        unsigned int	m_nBin[MAX_BINARYIN_COUNT]; //?????
        unsigned int	m_nBinEx[MAX_ExBINARY_COUNT]; //?????
}BI_STATUS;
...
typedef struct
{
        float m_fTripValueV;//????????
        float m_fTripValueF;//???????
        double m_dTimeBinAct[8][3]; //4???????????3?¦Á?¦Ë???
        int	m_nBinSwitchCount[8]; //??¦Ë????
        int m_nAct;
        double m_dActTime;//????????????
...
typedef struct
{
        FT3DbgChanInfo oFT3ChanInfo[16];//???16??????
} FT3DbgChansInfo;

typedef struct
{
        unsigned int nSample;//??????
        unsigned int nEType[10];//0-??????1-??????
        float fTripVolt[10];//??????§Ø????
        unsigned int nMeasGear[10];//?????¦Ë????
        float fCoef[10][ADJUST_MAX_GEAR_COUNT];//???âì?????§µ????
        float fZero[10][ADJUST_MAX_GEAR_COUNT];//???âì?????§µ????
} Drv_BinarySetting;
...
public:
        ...
        unsigned char m_nVoltPowerRefVal;//?????ðÐ??????
        unsigned char m_nCurrPowerRefVal;//??????ðÐ??????
        int  m_nBinRefState[MAX_BINARYIN_COUNT];//????????????????
        int  m_nBinExRefState[MAX_ExBINARY_COUNT];//????????????????
        State_EventRsts m_oStateEvents;//????????????????
        FT3DbgChansInfo m_oFT3DbgChansInfo;//??????FT3????????
        Drv_BinarySetting m_oBinSetting;
        BI_STATUS m_oBIStatus;
        BOOL m_bTestStarting;//??SttTestBase ???????

        long m_nDeviceOnTimerCount;
        long m_nModuleHotCnt;
        long m_nUseIecCfgCRC;//0-???Record?¡¤???CRC 1-???Iec61850Config?¡¤???CRC
        long m_nIsIecMode;//0-IEC??????,1-???????????2-?????????
        ...
        unsigned int m_nBootMode;//330,0-????????1-?????
        ...
        virtual void CloseDevice()
        {
                ...
                drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd,0xA00,0);//???DMA
...
public:
        ...
        virtual void tmt_to_drv_Gradient_NeedStepsValue(tmt_GradientParas *pParas) {} //?????????????20000???????????
...
public:
...
#endif
        CDataGroup m_oUserSetting;//?›¥??????????????¨°???
        virtual void SetSystemLocalTime(CString strTime_t){}
        long m_nSetSysTmCnt;//??????TEST????????
        DevInputInfo m_oDevInputInfo;
        virtual BOOL GetDevInputInfo(){return FALSE;}
        unsigned int m_nCurBinaryCode[10];//???????????
        unsigned int m_nCurBinaryGear[10];//?????????¦Ë
        unsigned int m_nCurnEType[10];//??????????
        unsigned int m_nReadInCode_I;//??????????????????
        unsigned int m_nReadInCode_V;//?????????????????
        unsigned int m_nSetAmpPowerCnt;//???¨´?????????
        ...
        void Set_BinaryGear(Drv_BinarySetting *pBinSetting)
        {
                ...
                g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
                drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x70004070, nCode);
                g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
                ...
                g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
                drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x70004074, nCode);
                g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
        }
        ...
        void Set_BinaryConfig(Drv_BinarySetting *pBinSetting)
        {
                ...
                drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, 0x7000407c, (200<<10)|nCode);//bit25~bit10 = 200
                ...
                for(int i = 0;i < 10;i++)
                {
                        ...
                        else
                        {
                                ...
                                drv_reg_ioctl(__FILE__, __LINE__, m_nCommfd, nAddr[i/2], nCode);
                        }
                }
...
public:
        ...
        BOOL GetCurDAPara()
        {
                ...
                g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
                ret = drv_reg_read(__FILE__, __LINE__, m_nCommfd, &oCurDAPara, 0x262);
                g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
extern float g_fAuxVolt;//??????????????
extern long g_nLogPrintIsZN;//1-?????0-???
