...
{
     CLogPrint::LogString(XLOGLEVEL_DEBUG, _T("void CFpgaUart::InitUart"));
	if(m_nUartInited)
	{
...
//	m_nFd = open("/dev/SttDriver_Uart", O_RDWR);
    m_nFd = open("/dev/ttyUL0", O_RDWR);
    if(m_nFd<=0){
        qDebug()<<"fail /dev/ttyUL0 fail!!";
    }
     qDebug()<<"succ /dev/ttyUL0!";
    SetUart(pUartSettings);
...
void CFpgaUart::SetUart(UartSettings* pUartSettings)
{
	...
	if(m_nUartType)
	{
		...
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x322, 0);
        drv_reg_write(__FILE__, __LINE__, nFd, (char *)&m_oUartSet, sizeof(Drv_UartData));
	...
	if(m_nUartType)
	{
		...
		if(m_oUartSendBuf[0].bValid)
		{
			if(g_oDrvMutex.trylock(__FILE__,__FUNCTION__,__LINE__))
			{
				...
				drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x322, 0);
                drv_reg_write(__FILE__, __LINE__, nFd, (char *)&m_oUartDownload, sizeof(Drv_UartData));
...
void *RecvUartThread(void *pData)
{
    ...
    while(nRunFlag)
    {
		for(int i = 0;i < nUartCnt;i++)
		{
			if(nUartType == 0)
			{
				numrcv = drv_reg_read(__FILE__, __LINE__, nFd, buffer, nModulePos[i]);
			}
			else
			{
				numrcv = drv_reg_read(__FILE__, __LINE__, nFd, buffer, 0);
			}
			...
			if(numrcv > 0)
			{
				...
				{
                    drv_reg_write(__FILE__, __LINE__, pFpgaUart->m_nSocketClient[i], buffer + 4, nSendLen);
				}
			}
		}
