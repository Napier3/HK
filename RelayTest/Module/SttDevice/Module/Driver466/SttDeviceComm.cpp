...
{
   // CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("22g_nStateCount=%d"),g_nStateCount);
    CLogPrint::LogString(XLOGLEVEL_ERROR, "InitBuff");
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("g_nStateCount=%d,"),g_nStateCount);
	m_nDrvResItemLen = sizeof(Drv_ResultItem);
	m_bRdDegRtData = FALSE;
	m_nSmvAbnDelayOffset = 0;
	m_pDrv_StateItem = new Drv_StateItem;
    CLogPrint::LogString(XLOGLEVEL_ERROR, "111InitBuff");
	m_pCurBins = new tmt_BinaryIns[g_nStateCount];
    CLogPrint::LogString(XLOGLEVEL_ERROR, "222InitBuff");
	m_pStateGoosePub = new tmt_GoosePubs[g_nStateCount];
    CLogPrint::LogString(XLOGLEVEL_ERROR, "333InitBuff");
	m_pStateFt3Pub = new tmt_Ft3Pubs[g_nStateCount];
    CLogPrint::LogString(XLOGLEVEL_ERROR, "InitBuff Ok!!!");
}
								  ...
								  unsigned int nhour, unsigned int nmin, unsigned int nsecond)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x351, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)data, 8);
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 0); //停DMA
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

	stt_test_event_set_lock(1);
	stt_clear_test_event_cycle_array();
	stt_test_event_set_lock(0);

#ifdef _MainBoard_PI_
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x143, 0); //停用模块内系数补偿功能,由于各模块上电顺序不统一,故每次开始实验都发一次该命令
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
#else
	if(CSttDeviceBase::g_pSttDeviceBase->m_nBootMode == 1)
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x143, 0); //停用模块内系数补偿功能,由于各模块上电顺序不统一,故每次开始实验都发一次该命令
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	if(CSttDeviceBase::g_pSttDeviceBase->m_nUseIecCfgCRC)
	{
		if(g_theSystemConfig->m_nHasDigital)
		{
			...
			if(m_oCIEC61850CfgMngr.m_nUseSmv92 == SMVTYPE_6044
					|| m_oCIEC61850CfgMngr.m_nUseSmv92 == SMVTYPE_6044_COMMON)
			{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x126, 0);//停DMA缓存
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		else
		{
			CSttDeviceBase::g_pSttDeviceBase->SetDMAType(DMA_TYPE_NULL);
		}
	}
	else
	{
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x100, 1);//打开端口继电器
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	Sleep(100);
	...
	switch(nTestType)
	{
		case GradientTest:
		{
			if(CSttDeviceBase::g_pSttDeviceBase->m_oGradientParas.m_nStepCnt > 0)
			{
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 7);
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			}
			else
			{
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0);
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		...
		case ManualTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 1);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case MUTest:
		{
			if(CSttDeviceBase::g_pSttDeviceBase->m_nUseIecCfgCRC)
			{
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x126, 1);//开DMA缓存
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			}
		...
		case StateTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 2);//启动状态序列测试
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case BinaryStateTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x11);//启动状态序列测试
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case BinaryStateExTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x14);//启动状态序列测试
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case BinaryManuTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x13);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case AbnSvTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 9);//启动异常模拟
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case AbnGseTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x17);//启动异常模拟
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case ShakeTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 8);//启动振荡测试
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case LtReplayTest:
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x0d);//启动故障回放
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case SystemDefaultOutput:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x136, 1);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
			break;
		...
		case SOE:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0xA);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
			break;
		...
		case CboperTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x0F);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		case SynTest:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 1); //开DMA
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0x0B);//同期测试
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
	}
...
void CSttDeviceComm::SetTestStop(BOOL bIsSysDefaultOutput)
{
	...
	if(bIsSysDefaultOutput)
	{
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x136, 0);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	else
	{
...
void CSttDeviceComm::SetTestPara(int nTestType, char *pszBuff)
{
	switch(nTestType)
	{
		case IECDataCfg:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd,  0x320, pIECConfig->mdCnt);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			...
			for(int i = 0;i < pIECConfig->mdCnt;i++)
			{
				...
				{
					g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
					drv_reg_ioctl(__FILE__, __LINE__, m_nfd,  0x323, i);
					drv_reg_write(__FILE__, __LINE__, m_nfd, (char*)&pIECConfig->item[i], sizeof(DatConfigItems));
					g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
					Sleep(10);
				}
			}
		...
		case Ft3DataCfg:
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd,  0x325, pIECConfigFt3->mdCnt);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			...
			for(int i = 0;i < pIECConfigFt3->mdCnt;i++)
			{
				...
				{
					g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
					drv_reg_ioctl(__FILE__, __LINE__, m_nfd,  0x324, i);
					drv_reg_write(__FILE__, __LINE__, m_nfd, (char*)&pIECConfigFt3->item[i], sizeof(DatConfigItemsFt3));
					g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x319, 0);//2M
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(Drv_IECConfig_2M));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x321, 1);//1：SMV发送配置
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(SendConfSV));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x321, 2);//2：GOOSE发送配置
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(SendConfGSE));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x321, 3);//3：FT3发送配置
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(SendConfFT3));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x321, 4);//4：清除SMV发送配置
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x321, 0);//flag = 0:光口配置
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(FiberConf));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x322, 0);
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(Drv_ComData));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
		...
		{
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x30C, 1);
			drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(IecRecvCfg));
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		}
		break;
	}
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x302, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, nLen);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x400, 0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::SetRising(int nRising)
{
	...
	if(nRising)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x132, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x132, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
{
	if(CSttDeviceBase::g_pSttDeviceBase->m_nIsIecMode == 1)
	{
		if(bIecDetectRun)
		{
			bIecDetectRun = FALSE;
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x131, DMA_TYPE_NULL);    //停止探测模式
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x100, 1);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	Sleep(100);
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 3);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::SetADMUChannel(tmt_ManualTest *pTmt)
{
	...
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
		{
			...
			for(int i = 0; i < nChannelNum;i++)
			{
				...
				if(pTmt_channel[m_nTmtPos].Harm[1].fAmp > 0.001 && pTmt_channel[m_nTmtPos].Harm[1].fFreq > 20.0)
				{
	...
	if(nCmd > 0)
	{
		nCmd |= 1<<31;
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x148, nCmd);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
}

void CSttDeviceComm::SetADMUChannel(tmt_HarmTest *pTmt)
{
	...
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		...
		{
			pModule = g_theDeviceSystemParameter->m_oModules_U[nIndex];
			pTmt_channel = pTmt->m_oHarmParas.m_uiVOL;
			if(pModule == NULL || nCmd > 0)
			{
				continue;
			}
		...
		{
			pModule = g_theDeviceSystemParameter->m_oModules_I[nIndex];
			pTmt_channel = pTmt->m_oHarmParas.m_uiCUR;
			if(pModule == NULL || nCmd > 0)
			{
				continue;
			}
...
int CSttDeviceComm::SetModuleFlash(PSTT_MODULE pModule, BYTE *pszBuf, int nLen)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x306, nModulePos);
	nRet = drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)pszBuf, nLen);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
BOOL CSttDeviceComm::GetModuleFlash(PSTT_MODULE pModule, BYTE *pszBuf, int &nOffset)
{
	long nModulePos = ModuleIndexTransToDrv(pModule->m_oModuleAttr.m_nModuleIndex);
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x10C, nModulePos);
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x123, nModulePos);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
L:
	...
	int nsize = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nsize = drv_reg_read(__FILE__, __LINE__, m_nfd, (char *)pszBuf, 0x210);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::GetModulesFPGAInfo()
{
	...
	if((m_oBoardinfo.version[13]>>16) > 2)
	{
        ...
        CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("5555DriverInfo[%08x]    Ver=%s"),
                                   m_oBoardinfo.version[13], TransVersion(m_oBoardinfo.version[13]).GetString());
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("222DriverInfo[%08x]    Ver=%s"),
                               m_oBoardinfo.version[13], TransVersion(m_oBoardinfo.version[13]).GetString());
	strcpy(g_theDeviceSystemParameter->m_oDeviceAttrs.m_strAppName, g_strAppName.GetString());
	strcpy(g_theDeviceSystemParameter->m_oDeviceAttrs.m_strDriverName, g_strDrvName.GetString());
	long nModuleType;
	CString strBoardType;
    CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("333DriverInfo[%08x]    Ver=%s"),
                               m_oBoardinfo.version[13], TransVersion(m_oBoardinfo.version[13]).GetString());

	for(int i = 1; i < 13; i++)
	{
        CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("111DriverInfw    Ver=%d"),
                                   i);
        CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("m_oBoardinfo.version[%d] >> %d"),i,m_oBoardinfo.version[i] >> 24);
                                //   i);
		switch(m_oBoardinfo.version[i] >> 24)
		{
    //    CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("m_oBoardinfo.version[%d]    =%d"),i,
      //                             m_oBoardinfo.version[i]);
			case 0xc0:
			...
			default:
                ...
                CLogPrint::LogFormatString(XLOGLEVEL_INFOR, _T("default-DriverInfw    Ver=%d"),
                                           i);
				strBoardType = "unknow       ";
				nModuleType = -1;
				break;
...
void GetVolReplayCoef(int nChannel, int nModuleUIndex, Drv_ReplayCoefItem *item)
{
	...
	PSTT_CHANNEL_ADJUSTS pChsAdjust = pModule->m_oModuleAdjust.m_oTempAdjust[0].m_pChsAdjust;
	if(pChsAdjust == NULL)
	{
		return;
	}

	PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust[nChannel].m_pChWaveAdj;//统一取基波

	if(pChWaveAdj != NULL)
	{
		long nGearCount = pChWaveAdj[1].m_nGearCount;
		if(nGearCount > 0)
		{
			PSTT_ADJUST_ITEM pAdjItem = &pChWaveAdj[1].m_oChGearAdj[nGearCount - 1].m_oAdjItem;
...
void  GetCurReplayCoef(int nChannel, int nModuleIIndex, Drv_ReplayCoefItem *item)
{
	...
	long nDefChannelNum = pModule->m_oModuleAttr.m_nDefChannelNum;
	if(nChannelNum == 6)
	{
		if(nChannelNum == nDefChannelNum)
		{
			...
			if(nMergeCurTerminal == STT_LARGE_CURRENT_MERGE_ONE_CH)
			{
				item->fTmtCoef = 1/6;
			}
			else if(nMergeCurTerminal == STT_LARGE_CURRENT_MERGE_THREE_CH)
			{
				item->fTmtCoef = 1/2;
			}
		}
		...
		{
			item->fTmtCoef = nDefChannelNum/nChannelNum;
		}
	}
...
void  GetWeekReplayCoef(int nChannel, int nModuleIndex, Drv_ReplayCoefItem *item)
{
	...
	else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
	{
		if(nChannel < 4)
		{
			//Volt
...
void  GetADMUReplayCoef(int nChannel, int nModuleIndex, Drv_ReplayCoefItem *item)
{
...
void CSttDeviceComm::ProLtReplayDAPara(DeviceDAPara *pDAPara, tmt_ReplayParas *pTmt)
{
	...
	for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
	{
		...
		{
			CSttDeviceBase::g_pSttDeviceBase->SetVoltDApara(pModule->m_oModuleAttr.m_fDefChMax, g_theSystemConfig->m_fAuxDC_Vol, nIndex, FALSE);
...
const long nLtReplaySVMap[12] = {1,2,3,14,15,16,5,6,7,17,18,19};
const long nLtReplayFT3Map[12] = {8,9,10,26,27,28,11,12,13,29,30,31};
const QString strLtReplayMap[12]= {"Ua1","Ub1","Uc1","Ia1","Ib1","Ic1","Ua2","Ub2","Uc2","Ia2","Ib2","Ic2"};
void CSttDeviceComm::tmt_to_drv_LtReplay(tmt_ReplayParas *pTmt, char *pszReplayCoef)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x352, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oDrvPara, sizeof(Drv_LtReplayParam));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	int nModuleADMUIndex = 0;

	Drv_LtReplayModuleChMaps oModuleChMaps;
	memset(&oModuleChMaps, 0, sizeof(Drv_LtReplayModuleChMaps));

	Drv_LtReplayModuleSamples oModuleSamples;
	memset(&oModuleSamples, 0, sizeof(Drv_LtReplayModuleSamples));

	for(int i = 0; i < MAX_MODULE_COUNT; i++)
	{
		if(pTmt->m_oModule[i].m_nSel == 0)
		{
			continue;
		}
		...
		{
			PSTT_MODULE pModule = &g_theDeviceSystemParameter->m_oModules[j];
			int nChanSize = pModule->m_oModuleAttr.m_nChannelNum;

			if(pModule->m_oModuleAttr.m_nModuleIndex == (nModulePosIndex + 1))
			{
				long nSampleIndex = oModuleSamples.Count;
				oModuleSamples.oModuleSample[nSampleIndex].module =
						ModuleIndexTransToDrv(pTmt->m_oModule[i].m_nIndex);
				...
				if(pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_VOLT) // 电压
				{
					...
					//20Khz 2500 播放频率快了容易中断，来不及给FPGA传后续数据
					oModuleSamples.oModuleSample[nSampleIndex].nSample = 2500;
					oModuleSamples.Count++;
				}
				...
				else if(pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_DIGITAL_0G8M
						...
						|| pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_DIGITAL_4G4M)
				{
					...
					{
						long nChMapIndex = oModuleChMaps.ChMapCount;
						oModuleChMaps.oModuleChMap[nChMapIndex].module =
								ModuleIndexTransToDrv(pTmt->m_oModule[i].m_nIndex);

						float fTmtCoef[4];
						fTmtCoef[0] = g_theSystemConfig->PkgModePTRate(0) / g_theSystemConfig->m_fVRate[0];
						fTmtCoef[1] = g_theSystemConfig->PkgModeCTRate(0) / g_theSystemConfig->m_fIRate[0];
						fTmtCoef[2] = g_theSystemConfig->PkgModePTRate(1) / g_theSystemConfig->m_fVRate[1];
						fTmtCoef[3] = g_theSystemConfig->PkgModeCTRate(1) / g_theSystemConfig->m_fIRate[1];

						//3U3I、3U3I
						for(int k = 0;k < 12;k++)
						{
							Coef->Items[nModulePosIndex].Item[k].fTmtCoef = fTmtCoef[k/3];
							Coef->Items[nModulePosIndex].Item[k].fMaxValue = 1;
							Coef->Items[nModulePosIndex].Item[k].fReverse = 1;
							Coef->Items[nModulePosIndex].Item[k].fCoef = 1;
							Coef->Items[nModulePosIndex].Item[k].fZero = 0;

							oModuleChMaps.oModuleChMap[nChMapIndex].ChMap[k/4] |=
									nLtReplaySVMap[k]<<(24 - k%4*8);
							Coef->Items[nModulePosIndex].Item[k].nCh = k;
						}

						Coef->Items[nModulePosIndex].nChCnt = 12;
						oModuleChMaps.ChMapCount++;

						//10Khz 5000 数字插件通道比较多数据量大
						oModuleSamples.oModuleSample[nSampleIndex].nSample = 5000;
						oModuleSamples.Count++;
					}
					else if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8)
					{
						...
						for(int k = 0;k < 12;k++)
						{
							...
							FT3MapPointer map = m_oCIEC61850CfgMngr.m_p6044MngrBase->m_Ft3ChannelType.value(strLtReplayMap[k]);
							Coef->Items[nModulePosIndex].Item[k].fTmtCoef = map.fRatio;
							Coef->Items[nModulePosIndex].Item[k].nSampleLen = map.nSampleLen;
					...
					else if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
					{
						//柔直
						long nChMapIndex = oModuleChMaps.ChMapCount;
						oModuleChMaps.oModuleChMap[nChMapIndex].module =
				...
				else if(pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_FT3)
				{
					Coef->Items[nModulePosIndex].nCode = 32;
				...
				else if(pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_TYPE_ADMU)
				{
					nModuleADMUIndex = GetModuleIndex(g_theDeviceSystemParameter->m_oModules_ADMU,
													  g_theDeviceSystemParameter->m_nModuleCount_ADMU,
													  pModule->m_oModuleAttr.m_nModuleIndex);
					Coef->Items[nModulePosIndex].nChCnt = 8;
					Coef->Items[nModulePosIndex].nCode = 20;//ADMU

					for(int k = 0; k < nChanSize; k++)
					{
						Coef->Items[nModulePosIndex].Item[k].nCh = pModule->m_nChDrvPos[k];
						GetADMUReplayCoef(k, nModuleADMUIndex, &Coef->Items[nModulePosIndex].Item[k]);
					}
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x354, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oModuleSamples, sizeof(Drv_LtReplayModuleSamples));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

	if(oModuleChMaps.ChMapCount > 0)
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x354, 1);
		drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oModuleChMaps, sizeof(Drv_LtReplayModuleChMaps));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	...
	if(g_theSystemConfig->m_nHasWeek)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
		{
			...
			{
				g_nWeekTap = GetTapCode_L336D_ECTEVT(fMaxOut, TRUE, NULL, WeekTap[nIndex]);
				bSetTap = TRUE;
			}
			...
			if(bSetTap)
			{
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x125, m_nDCTap); //实验过程中档位不变
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			}
		}
...
BOOL CSttDeviceComm::TranslateDigitalVoltCurrentBuffer(tmt_channel *pVChannel, tmt_channel *pIChannel, long *pBuff, PSTT_MODULE pModule,
		int nHarm, BOOL bIsFT3, int nMode)
{
	...
	for(int i = 0; i < nChannelCnt; i++)
	{
		...
		if(bIsFT3 && g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8)
		{
			//配网FT3，固定映射关系
			if(i < 12)
			{
				nEeType = STT_CH_EETYPE_VOLTAGE;
				nTmtPos = i;
			}
			else if(i < 36)
			{
				nEeType = STT_MODULE_TYPE_CURRENT;
				if(i < 24)
				{
					nTmtPos = i - 12;//映射至Ia1~Ia4
				}
				else
				{
					nTmtPos = i - 24;//重复映射至Ia1~Ia4
				}
			}
			else
			{
		...
		if(nTmtPos >= 0)
		{
			if(nMode == 0)
			{
				...
				{
					//叠加直流
					fAmp = pChannel[nTmtPos].Harm[0].fAmp;
					fPh = pChannel[nTmtPos].Harm[0].fAngle;
					fFre = 0;
				}
				else if(nHarm == 1)
				{
	...
	if(bIsFT3)
	{
		...
		for(int j = 0; j < nCurrDrvCnt; j++)
		{
			...
			pBuff[nDrvPos * 3] = DrvData[(j + nCurrMapBegin) * 3];
			pBuff[nDrvPos * 3 + 1] = DrvData[(j + nCurrMapBegin) * 3 + 1];
			pBuff[nDrvPos * 3 + 2] = DrvData[(j + nCurrMapBegin) * 3 + 2];
		}
	}
	else
	{
...
BOOL CSttDeviceComm::TranslateDigitalVoltCurrentBuffer_Async(tmt_AsyncModule *pAnalogModule, long *pBuff, PSTT_MODULE pModule,
		int nHarm, BOOL bIsFT3)
{
	tmt_AsyncChannel *pChannel;
	int nType;
	int nChannelCnt = 37;//U1-U19,I1-I18
	long DrvData[nChannelCnt * 3];
	memset(DrvData, 0, nChannelCnt * 3 * sizeof(long));
	...
	for(int i = 0; i < nChannelCnt; i++)
	{
		...
		{
			nType = STT_MODULE_TYPE_VOLT;
			pChannel = &pAnalogModule->m_pVolChannel[nFT3VoltChsCnt];
			nFT3VoltChsCnt++;
		}
		else
		{
			nType = STT_MODULE_TYPE_CURRENT;
			pChannel = &pAnalogModule->m_pCurrChannel[nFT3CurrChsCnt];
			nFT3CurrChsCnt++;
		}
		...
		{
			//BASE
			fAmp = pChannel->m_oHarm.fAmp;
			fPh = pChannel->m_oHarm.fAngle;
			fFre = pChannel->m_oHarm.fFreq;
		}
		...
		else
		{
			...
			if(bIsFT3)
			{
				if(nType == STT_MODULE_TYPE_VOLT)
				{
					...
					if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC && nTmtPos < 6)
					{
				...
				else
				{
					...
					{
						nDrvPos = m_oCIEC61850CfgMngr.m_p6044MngrBase->m_nCurrDrvPos[i - 12] - 1;
					}
				}
			...
			if(pChannel[nTmtPos].Ramp[0].nIndex > 0)
			{
				...
				if(fStep > EPSINON)
				{
					...
					{						
						if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
						{
							nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, bIsFT3, nTmtPos);
						}
						else
						{
							if(nType == STT_MODULE_TYPE_VOLT)
							...
							{
								nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, bIsFT3, i - 12);
							}
						}
					}
					else
					{
						nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, bIsFT3, i);
					}
					pDrvStateDeg->changeVal[3 * nDrvPos + 2 + nDrvOffSet] = nCode;
					if(bIsFT3)
					{
						if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
						{
							nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, bIsFT3, nTmtPos);
						}
						else
						{
							if(nType == STT_MODULE_TYPE_VOLT)
							...
							{
								nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, bIsFT3, i - 12);
							}
						}
					}
					else
					{
						nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, bIsFT3, i);
					}
					pDrvStateDeg->lastVal[3 * nDrvPos + 2 + nDrvOffSet] = nCode;
		...
		int nType, PSTT_MODULE pModule, BOOL bIsFT3, int nFT3ChannelTypePos)
{
	...
	if(bIsFT3)
	{
		float nDT = 1000000.0 / m_oCIEC61850CfgMngr.m_p6044MngrBase->GetSample();
		float nDelay = m_oCIEC61850CfgMngr.m_p6044MngrBase->GetFT3DelayTime(nGroupIndex);
		fGroupAdd = nDelay;
		fGroupAdd = fGroupAdd/nDT;
		fGroupAdd = 360.0/(m_oCIEC61850CfgMngr.m_o6044Mngr.GetSample()/50) * fGroupAdd;
	}
	else
	{
		...
		nDelay += m_nSmvAbnDelayOffset;
		nDelay = nDelay%nDT;
		fGroupAdd = nDelay * 4.5;
		fGroupAdd = fGroupAdd / nDT;
	}
...
long CSttDeviceComm::TransDigitalChannelAmp(float fValue, int nGroup, bool bDC, int nType, BOOL bIsFT3, int nFT3ChannelTypePos)
{
	...
	if(bIsFT3)
	{
		...
		nOffset = (4 - nSampleLen) * 8;
		nValue = dValue * pow(2, nOffset);
	...
	else
	{
		...
		if(nType == STT_MODULE_TYPE_VOLT)
		{
			...
			{
				//界面一次值
				if(g_theSystemConfig->m_nPkgOutMode == 1)
				{
					//输出二次值
					dValue = dValue*g_theSystemConfig->GetPTRate(nGroup);
				}
			}
			else
			{
				//界面二次值
				if(g_theSystemConfig->m_nPkgOutMode == 0)
				{
					//输出一次值
					dValue = dValue/g_theSystemConfig->GetPTRate(nGroup);
				}
			...
			nValue = dValue / g_theSystemConfig->m_fVRate[nGroup];
		}
		else
		{
			if(g_nTmtParaMode == 0)
			{
				...
				{
					//输出二次值
					dValue = dValue*g_theSystemConfig->GetCTRate(nGroup);
				}
			}
			else
			{
			...
			nValue = dValue / g_theSystemConfig->m_fIRate[nGroup];
		}
		...
		int nModuleUIndex, long *pnDrvData)
{
    ...
    qDebug() << "pnDrvData[1]: " << pnDrvData[1] << ",pnDrvData[2]: " << pnDrvData[2];
}
...
void CSttDeviceComm::TransAnalogDCCurToDrv(float fAmp, int nChannel, int nModuleIIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	{
		pDrvData[0] = 0;
		pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[nChannel].m_fTotalCurAmp = 0;
		return;
	}
	...
	float fHCoef = 1.0f;
	long nDefChannelNum = pModule->m_oModuleAttr.m_nDefChannelNum;
	if(nDefChannelNum == pModule->m_oModuleAttr.m_nChannelNum)
	{
		long nPcbChanNum = pModule->m_oModuleAttr.m_nDAChanNum;
		if(nPcbChanNum != pModule->m_oModuleAttr.m_nChannelNum)
		{
			fHCoef = pModule->m_oModuleAttr.m_nChannelNum/nPcbChanNum;
		}
	}
	...
	fAmp += fZero;
	/*************fAmp值补偿************/
	pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[nChannel].m_fTotalCurAmp = fAmp;
	/*************fAmp值转FPGA参数************/
...
void CSttDeviceComm::TransAnalogDCUIToDrv_Volt(float fAmp, int nChannel, int nModuleIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust->m_pChWaveAdj;
		if(pChWaveAdj != NULL)
		{
			long nGearCount = pChWaveAdj[0].m_nGearCount;

			if(fabs(fAmp) >= pChWaveAdj[0].m_oChGearAdj[nGearCount - 1].m_fGearValue)
			{
				fCoef = pChWaveAdj[0].m_oChGearAdj[nGearCount - 1].m_oAdjItem.m_fCoef;
			}
			else
			{
				//档位
				for(long nIndex = 0; nIndex < nGearCount; nIndex++)
				{
					if(fabs(fAmp) <= pChWaveAdj[0].m_oChGearAdj[nIndex].m_fGearValue)
					{
						fCoef = pChWaveAdj[0].m_oChGearAdj[nIndex].m_oAdjItem.m_fCoef;
						break;
					}
				}
			}
...
void CSttDeviceComm::TransAnalogDCUIToDrv_Current(float fAmp, int nChannel, int nModuleIndex, long *pDrvData, BOOL bUseDcZero)
{
	...
	{
		pDrvData[0] = 0;
		pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[nChannel].m_fTotalCurAmp = 0.0;
		return;
	}
...
long CSttDeviceComm::TranslateACUIAmp_Volt(float fAmp, int nChannel, int nModuleIndex, int nHarm)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		if(pChWaveAdj != NULL)
		{
			...
			{
				long nGearCount = pChWaveAdj[nHarm].m_nGearCount;

				if(fabs(fAmp) >= pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_fGearValue)
				{
					fCoef = pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_oAdjItem.m_fCoef;
					fZero = pChWaveAdj[nHarm].m_oChGearAdj[nGearCount - 1].m_oAdjItem.m_fZero;
				}
				else
				{
					//档位
					for(long nIndex = 0; nIndex < nGearCount; nIndex++)
					{
						if(fabs(fAmp) <= pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_fGearValue)
						{
							fCoef = pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_oAdjItem.m_fCoef;
							fZero = pChWaveAdj[nHarm].m_oChGearAdj[nIndex].m_oAdjItem.m_fZero;
							break;
						}
					}
				}
		...
		int nModuleIndex, long *pnDrvData)
{
	...
	if(pChsAdjust != NULL)
	{
		...
		{
			if(pChsAdjust[nChannel].nHarmCount > 1)
			{
				PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust[nChannel].m_pChWaveAdj;
				if(pChWaveAdj != NULL)
				{
					...
					if(nGearCount > 1)
					{
						for(int nGear = 0; nGear < nGearCount; nGear++)
						{
							if(fAmp < pChWaveAdj[1].m_oChGearAdj[nGear].m_fGearValue)
							{
								faddPh = pChWaveAdj[1].m_oChGearAdj[nGear].m_oAdjItem.m_fAngle;
								break;
							}
						}
					}
					...
					{
						faddPh = pChWaveAdj[1].m_oChGearAdj[0].m_oAdjItem.m_fAngle;
					}
				}
			}
		}
    ...
    //pnDrvData[1] = TranslateACFre(fFre * fFreCoef);
    pnDrvData[1] = TranslateACFre(fFre); //2025-03-23 tlw
		...
		int nModuleIndex, long *pnDrvData)
{
    ...
    //pnDrvData[1] = TranslateACFre(fFre * fFreCoef);  //2025-03-23 tlw
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x30E, 1);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(Drv_GSOutMap));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x310, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(Ft3SubDatas));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}

void CSttDeviceComm::SetFT3SubCRC(char *pszBuff)
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x312, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszBuff, sizeof(Ft3SubCrcs));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x137, nValue);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::SetBinaryOut(BOOL bOpen)
{
	...
	if(bOpen)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x107, 0xff);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x107, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::Set_BO_ENABLE(bool bEnable)
{
	...
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x138, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x138, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::Set_BI_ENABLE(bool bEnable)
{
	...
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x139, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x139, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::Set_Goose_BO_ENABLE(bool bEnable)
{
	...
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x140, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x140, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::Set_Goose_BI_ENABLE(bool bEnable)
{
	...
	if(bEnable)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x141, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x141, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}

int CSttDeviceComm::getLtReplayBufferFlag()
{
	int nRet = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nRet = drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x504, 0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x357, ntype);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, size);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::SetAuxDcVolt(float fUdc)
{
	...
	DeviceDAPara *pAuxDcDAPara;
	if(g_theSystemDefaultOutput.m_bUseSysOutput)
	{
		pAuxDcDAPara = &CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForSysput;
	}
	else
	{
		pAuxDcDAPara = &CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForTestStop;
	}
	...
	for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
	{
		...
		if(pModule != NULL)
		{
			...
			if(fUdcMax > EPSINON && nModuleIndex <= ADJUST_MAX_MODULE_COUNT)
			{
				...
				if(pPowerCtrlBase != NULL && bAuxDcOutput == FALSE)
				{
					pPowerCtrlBase->SetVoltDA_State();
					pPowerCtrlBase->SetVoltDA_DC_300V(fUdc);
					unsigned long nDa = pPowerCtrlBase->GetModuleDA();
					pAuxDcDAPara->nvolt[nIndex] &= 0xFFFFFF00;
					pAuxDcDAPara->nvolt[nIndex] |= (nDa & 0xFF);
					if(g_theSystemDefaultOutput.m_bUseSysOutput)
					{
					...
					{
						CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForSysput.nvolt[nIndex] &= 0xFFFFFF00;
						CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForSysput.nvolt[nIndex] |= (nDa & 0xFF);
					}
					bAuxDcOutput = TRUE;
				}
			}
	...
	if(bAuxDcOutput)
	{
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		CSttDeviceBase::g_pSttDeviceBase->WriteDApara(pAuxDcDAPara);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x10f, nValue);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

		CSttDeviceBase::g_pSttDeviceBase->WriteDAparaForTestStop();
		CSttDeviceBase::g_pSttDeviceBase->WriteDApara(&CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForSysput, TRUE);
	}
}
...
void CSttDeviceComm::SetSystemConfig()
{
	...
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x110, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x110, 0);
	}

	if(g_theSystemConfig->m_bKeepSendIecPkt)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x121, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x121, 0);
	}

	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x122, g_theSystemConfig->m_nFiberType);//千兆接收不支持自适应必须手动指定
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x120, nTime); //主板开关量模块防抖
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x142, nTime); //扩展开关量模块防抖

	if(g_theSystemConfig->m_n588Syn)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x101, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x101, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x104, 1);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x104, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	CDateTime::SetDateZone(g_theSystemConfig->m_nTimeZone);

	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	if(g_theSystemConfig->m_bGtoB_UseUserTz)
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x105, g_theSystemConfig->m_nTimeZone);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x105, 0);
	}
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::InitDApara(int fd, BOOL bHasAdjFile)
{
...
#ifdef _MainBoard_PI_
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x143, 0); //停用模块内系数补偿功能
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
#else
	if(CSttDeviceBase::g_pSttDeviceBase->m_nBootMode == 1)
	{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x70005064, nBiRegData);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);	
	Sleep(10);
#endif
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x10D, ~nBiRegData);//驱动配置主板有效开入位，用于开入寄存器值软过滤
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x137, nBoutDefValue);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x107, 0xff);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	//使用UTC区域存储时间,防止linux系统时区影响时间转换
	QDateTime oDate = QDateTime::currentDateTimeUtc();
	nYear = oDate.date().year();
	nMon = oDate.date().month();
	nDay = oDate.date().day();
	nHour = oDate.time().hour();
	nMin = oDate.time().minute();
	nSec = oDate.time().second();
	if(nYear == 1970)
	{
		nYear = 2024;
		nMon = 1;
	...
	SetDrvMoudlePara();
	CSttDeviceBase::g_pSttDeviceBase->ReadSystemConfig();
	CDataGroup *pDataGroup = &CSttDeviceBase::g_pSttDeviceBase->m_oUserSetting;
	CDvmData *pDvmData = (CDvmData*)pDataGroup->FindByID("TimeZone");
	if(pDvmData != NULL)
	{
		g_theSystemConfig->m_nTimeZone = pDvmData->m_strValue.toInt();
	}
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x120, nTime);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x142, 0); //扩展开关量模块防抖
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::FormatFlashData(PSTT_MODULE pModule, BYTE *pszBuf, long &nLen)
{
	...
	else
	{
		...
		for(int nChan = 0; nChan < pModule->m_oModuleAttr.m_nChannelNum; nChan++)
		{
			...
			if(pChsAdjust != NULL)
			{
				PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust[nChan].m_pChWaveAdj;
				if(pChWaveAdj != NULL)
				{
					nChDrvPos = pModule->m_nChDrvPos[nChan];
					oFlashAjdData.nZerofloat[nChDrvPos] = pChsAdjust[nChan].m_fDcZero[0] / fMax * 0x7FFFFFFF;
					PSTT_ADJUST_ITEM pAdjItem = &pChWaveAdj[1].m_oChGearAdj[0].m_oAdjItem;
...
void CSttDeviceComm::SetDrvMoudlePara()
{
	...
	if(g_pFpgaUart != NULL)
	{
		memset(&g_pFpgaUart->m_oUartSettings, 0, sizeof(UartSettings));
	}
		...
		if(pModule_ADMU != NULL)
		{
			...
			else
			{
				...
				if(g_pFpgaUart != NULL)
				{
					...
					if(pUartSettings->nUartCnt < UartTotal)
					{
						UartSetting *pUartSetting = &pUartSettings->oUartSetting[pUartSettings->nUartCnt];
						pUartSetting->nModulePos = nModuleIndex;
						pUartSetting->nBand = 115200;
						pUartSetting->nCheck = 0;
						pUartSetting->nStopBit = 0;
						pUartSetting->nDatalen = 8;
						pUartSetting->nOutput = 0;
						pUartSettings->nUartCnt++;
					}
				}
			}
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x331, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oDrv_ModulePara, sizeof(Drv_ModulePara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

	Sleep(10);
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x356, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&m_oModuleMaxVaildCnt, sizeof(Drv_ModuleMaxVaildCnt));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::InitWeekOutputDAData()
{
	...
	for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
	{
		...
		if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PIA12DS
				...
				|| pModule->m_oModuleAttr.m_nModuleType == STT_MODULE_WEEK_L336D_ECTEVT
		  )
		{
	...
	for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_ADMU; nIndex++)
	{
		PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_ADMU[nIndex];
		oWeekInitDA.nModule[nModuleIndex] = ModuleIndexTransToDrv(pModule->m_oModuleAttr.m_nModuleIndex);
		oWeekInitDA.maxVaildCnt[nModuleIndex] = 9 + 8 * 3 * 31;
		nModuleIndex++;
		oWeekInitDA.nCnt++;
		bUsed = TRUE;
	}
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x359, 0);
		drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oWeekInitDA, sizeof(Drv_WeekInitDA));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
}
...
void CSttDeviceComm::tmt_to_drv_systemdefaultoutput(TMT_SYSTEM_DEFAULT_OUTPUT_PARAS *pTmt)
{
	...
	if(g_theSystemConfig->m_nHasAnalog)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
		{
			...
			if(nDefChannelNum == m_nTmtChanNum)
			{
				int nPcbChanNum = pModule->m_oModuleAttr.m_nDAChanNum;
				if(nPcbChanNum != m_nTmtChanNum)
				{
					fIMax = fIMax*m_nTmtChanNum/nPcbChanNum;
				}
			}
		...
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x14C, 0);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		memcpy(&CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForSysput, pDAPara, sizeof(DeviceDAPara));
	}
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x353, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)&oDrv_ManuData, sizeof(Drv_ManuData));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
{
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x144, 0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::ProCommDrvResult(BOOL &bTestFinished, STT_TIME &oTestFinishedTime)
{
	...
	int nret = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nret = drv_reg_read(__FILE__, __LINE__, m_nfd, pDrvResults, 0x203);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	for(int i = 0; i < nret / m_nDrvResItemLen; i++)
	{
		...
		switch(m_oDrvResItem.nType)
		{
			...
			case DrvResult_LtReplay_IRQ_TIMEOUT:
			{
				if(pSttTestBase != NULL)
				{
					pSttTestBase->SetTestStarting(FALSE);
					bTestFinished = TRUE;
					oTestFinishedTime.dTime = 0.0001;
									 ...
									 int nCurStateIndex, int nStep, double dTime1, double dTime2, BOOL bCalcBinAct)
{
	...
	else
	{
		...
		fUIStepValue += oGradientParas.m_nStepDirection * oGradientParas.m_fStep * fPTRate * nStep;
	}
...
{	
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x309, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pDrvPara, sizeof(DrvDegressPara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
		...
		int nModuleUIndex, BOOL bIsDc, int nStatus)
{
	...
	if(bIsDc)
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			{
				//界面一次值
				fAmp = fAmp*1000*g_theSystemConfig->GetPTRate(m_nTmtPos/3);
			}
		...
		int nModuleIIndex, BOOL bIsDc, int nStatus)
{
	...
	if(bIsDc)
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			{
				//界面一次值
				fAmp = fAmp*g_theSystemConfig->GetCTRate(m_nTmtPos/3);
			}

			TransAnalogDCCurToDrv(fAmp * fChTmtCoef, k, nModuleIIndex, dat + nChPos);
		}
	...
	else
	{
		...
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			//BASE
			{
				...
				TransAnalogCurToDrv(fAmp * fChTmtCoef, fAngle,
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
		{
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_D; nIndex++)
			{
				...
				if(nHarm > 1)
				{
					...
					for(; m < MAX_HARM_COUNT; m++)
					{
						for(int i = 0; i < DIGITAL_CHANNEL_MAX; i++)
						{
							long nTmtPos = pModule->m_nChTmtPos[i];
							if(nTmtPos < 0)
							{
								continue;
							}
							...
							for(int nHarmIndex = 2; nHarmIndex < MAX_HARM_COUNT; nHarmIndex++)
							{
								if(m == pChannel[nTmtPos].Harm[nHarmIndex].nSelectIndex)
								{
									pChannel[nTmtPos].Harm[nHarmIndex].nSelectIndex = -1;
									break;
								}
							}
						}
					...
					TranslateDigitalVoltCurrentBuffer(puData, piData, pBuf + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, 0, bIsFT3);
					puData->Harm[0].nSelectIndex = nHarmTotal + 1;
					piData->Harm[0].nSelectIndex = nHarmTotal + 1;
					nHarmTotal++;
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_FT3 > 0)
		{
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_FT3; nIndex++)
			{
				...
				if(nHarm > 1)
				{
					...
					TranslateDigitalVoltCurrentBuffer(puData, piData, pBuf + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, 0, TRUE);
					puData->Harm[0].nSelectIndex = nHarmTotal + 1;
					piData->Harm[0].nSelectIndex = nHarmTotal + 1;
					nHarmTotal++;
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_ADMU; nIndex++)
			{
				...
				if(nHarm > 1)
				{
					...
					for(; m < MAX_HARM_COUNT; m++)
					{
						for(int i = 0; i < DIGITAL_CHANNEL_MAX; i++)
						{
							long nTmtPos = pModule->m_nChTmtPosEx[i];
							if(nTmtPos < 0)
							{
								continue;
							}

							long nEeType = pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[i].m_nEeTypeEx;
							tmt_channel *pChannel;
							if(nEeType == STT_CH_EETYPE_VOLTAGE)
							{
								pChannel = puData;
	...
	if(GradientType == GRADIENT_AMP)
	{
		...
		if(fGearMax > 0 && fChMax > 0)
		{
			...
			if(pChsAdjust != NULL)
			{
				if(nChannel > -1 && nChannel < pModule->m_oModuleAttr.m_nChannelNum)
				{
					if(nHarmIndex < pChsAdjust[nChannel].nHarmCount)
					{
						PSTT_CHANNEL_WAVE_ADJUST pChWaveAdj = pChsAdjust[nChannel].m_pChWaveAdj;
						if(pChWaveAdj != NULL)
						{
							long nGearCount = pChWaveAdj[nHarmIndex].m_nGearCount;
							if(nGearCount <= 1)
							{
								fCoef = pChWaveAdj[nHarmIndex].m_oChGearAdj[0].m_oAdjItem.m_fCoef;
							}
							else
							{
								for(int nGearIndex = 0; nGearIndex < nGearCount; nGearIndex++)
								{
									if(fabs(fEnd) <= pChWaveAdj[nHarmIndex].m_oChGearAdj[nGearIndex].m_fGearValue
											&& fabs(fStart) <= pChWaveAdj[nHarmIndex].m_oChGearAdj[nGearIndex].m_fGearValue)
									{
										fCoef = pChWaveAdj[nHarmIndex].m_oChGearAdj[nGearIndex].m_oAdjItem.m_fCoef;
										break;
									}
								}
							}
				...
				for(int j = 0; j < m_nTmtChanNum; j++)
				{
					...
					if(m_nTmtPos >= 0)
					{
						...
						if(uiVOL.Ramp[0].nIndex == 0)
						{
							...
							{
								//界面一次值
								fStep = fStep*g_theSystemConfig->GetPTRate(m_nTmtPos/3);
								fStart = fStart*1000*g_theSystemConfig->GetPTRate(m_nTmtPos/3);
								fEnd = fEnd*1000*g_theSystemConfig->GetPTRate(m_nTmtPos/3);
							}
							//幅值递变
						...
						if(uiVOL.Ramp[0].nIndex > 0)
						{
							...
							if(bRet)
							{
								...
								if(nHarmIndex == 1 && fFreq < EPSINON)
								{
									long *pDcData = &pDrvData->AcPara.para[nIndex].nFaultLastVal[nTempPos];
									TransAnalogDCVolToDrv(fEnd, j, nIndex, pDcData, FALSE);
								}
								else
								{
									if(fFreq <= m_nEDFre)
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
		{
			...
			if(nMode == 0)
			{
				...
				for(int j = 0; j < m_nTmtChanNum; j++)
				{
					nChPos = pModule->m_nChDrvPos[j];
					m_nTmtPos = pModule->m_nChTmtPos[j];
					fChTmtCoef = pModule->m_fChTmtCoef[j];
					...
					if(m_nTmtPos >= 0)
					{
						...
						if(uiCUR.Ramp[0].nIndex == 0)
						{
							...
							{
								//界面一次值
								fStep = fStep*g_theSystemConfig->GetCTRate(m_nTmtPos/3);
								fStart = fStart*g_theSystemConfig->GetCTRate(m_nTmtPos/3);
								fEnd = fEnd*g_theSystemConfig->GetCTRate(m_nTmtPos/3);
							}
						...
						if(uiCUR.Ramp[0].nIndex > 0)
						{
							...
							if(bRet)
							{
								...
								long nH = fStep *fChTmtCoef * 65536 / fChMax;
								double fH = fStep *fChTmtCoef * 65536 / fChMax;
								long nL = (fH - nH) * 65536;
								pDrvData->AcPara.para[nIndex + nModuleOffset].nFaultChangVal[nTempPos]
									= nSign * (((nH << 16) & 0xFFFF0000) + nL);
								long nHarm;
								...
								if(nHarmIndex == 1 && fFreq < EPSINON)
								{
									long *pDcData = &pDrvData->AcPara.para[nIndex + nModuleOffset].nFaultLastVal[nTempPos];
									TransAnalogDCCurToDrv(fEnd * fChTmtCoef, j, nIndex, pDcData, FALSE);
								}
								else
								{
									if(fFreq <= m_nEDFre)
									...
									pDrvData->AcPara.para[nIndex + nModuleOffset].nFaultLastVal[nTempPos]
										= TranslateACCurrentAmp(fEnd * fChTmtCoef, j, nIndex, nHarm + 1);
								}
							}
						}
			...
			else
			{
				for(int i = 0; i < nCnt; i++)
				{
					...
					for(int j = 0; j < m_nTmtChanNum; j++)
					{
						...
						m_nTmtPos = pModule->m_nChTmtPos[j];
						float fChTmtCoef = pModule->m_fChTmtCoef[j];
						...
						memset(buff, 0, sizeof(long) * 3);
						TransAnalogCurToDrv(fAmp * fChTmtCoef, fAng, fFre, j, nIndex, buff);
						...
						if(Tmtui.Ramp[0].nIndex > 0)
						{
							...
							if(bRet)
							{
								...
								if(nHarmIndex == 1 && fFreq < EPSINON)
								{
									...
									if(bChanIsVolt)
									{
										TransAnalogDCUIToDrv_Volt(fEnd, j, nIndex, pDcData, FALSE);
									}
									else
									{
										TransAnalogDCUIToDrv_Current(fEnd, j, nIndex, pDcData, FALSE);
									}
								}
								else
								{
									...
									if(bChanIsVolt)
									{
										nFaultLastVal = TranslateACUIAmp_Volt(fEnd, j, nIndex, nHarm + 1);
									}
									else
									{
										nFaultLastVal = TranslateACUIAmp_Current(fEnd, j, nIndex, nHarm + 1);
									}
				...
				for(int i = 0; i < DIGITAL_CHANNEL_MAX; i++)
				{
					...
					if(bIsFT3 && g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8)
					{
						...
						else if(i < 36)
						{
							...
							if(i < 24)
							{
								m_nTmtPos = i - 12;//映射至Ia1~Ia4
							}
							else
							{
								m_nTmtPos = i - 24;//重复映射至Ia1~Ia4
							}
						}
						else
						{
					...
					if(i == 18 && (!bIsFT3))
					{
						...
						if(pChannel->Ramp[0].nIndex > 0)
						{
							...
							if(pDrvData->cnt == 0)
							{
								if(!pChannel->Ramp[0].bSecondChan)
								{
									pDrvData->cnt = nStepCount;
								}
							}
						...
						if(pChannel->Ramp[1].nIndex > 0 && (!bDC))
						{
							...
							if(pDrvData->cnt == 0)
							{
								if(!pChannel->Ramp[1].bSecondChan)
								{
									pDrvData->cnt = nStepCount;
								}
							}
						...
						if(pChannel->Ramp[2].nIndex == 1 && (!bDC))
						{
							...
							if(pDrvData->cnt == 0)
							{
								if(!pChannel->Ramp[2].bSecondChan)
								{
									pDrvData->cnt = nStepCount;
								}
							}
					...
					if(bIsFT3)
					{
						...
						else
						{
							...
							{
								nTempPos = m_oCIEC61850CfgMngr.m_p6044MngrBase->m_nCurrDrvPos[i - 12] - 1;
							}
						}
					}
					else
					...
					if(pChannel->Ramp[0].nIndex == 0 && pChannel->Harm[0].nSelectIndex > 0)
					{
						nTempPos2 = nTempPos;
						nTempPos2 += pChannel->Harm[0].nSelectIndex * DIGITAL_CHANNEL_MAX * 3 + 2;
						//幅值递变
						...
						if(bIsFT3)
						{
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, TRUE, nType, bIsFT3, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, TRUE, nType, bIsFT3, i - 12);
								}
							}
						}
						else
						...
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2 + 2] = nCode;

						if(bIsFT3)
						{
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, TRUE, nType, bIsFT3, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, TRUE, nType, bIsFT3, i - 12);
								}
							}
						}
						else
						...
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2 + 2] = nCode;
					}
					...
					if(pChannel->Ramp[0].nIndex > 0)
					{
						...
						{
							nTempPos2 += pChannel->Harm[nHarmIndex].nSelectIndex * DIGITAL_CHANNEL_MAX * 3 + 2;
							if(bIsFT3)
							{
								if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
								{
									nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, FALSE, nType, bIsFT3, m_nTmtPos);
								}
								else
								{
									if(nType == STT_MODULE_TYPE_VOLT)
									...
									{
										nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, FALSE, nType, bIsFT3, i - 12);
									}
								}
							}
							else
							...
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2] = nCode;

							if(bIsFT3)
							{
								if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
								{
									nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, FALSE, nType, bIsFT3, m_nTmtPos);
								}
								else
								{
									if(nType == STT_MODULE_TYPE_VOLT)
									...
									{
										nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, FALSE, nType, bIsFT3, i - 12);
									}
								}
							}
							else
							...
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2] = nCode;
						}
						else if(nHarmIndex == 1) //ManualTest
						{
							...
							if(bIsFT3)
							{
								if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
								{
									nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, bIsFT3, m_nTmtPos);
								}
								else
								{
									if(nType == STT_MODULE_TYPE_VOLT)
							...
							if(bIsFT3)
							{
								if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
								{
									nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, bIsFT3, m_nTmtPos);
								}
								else
								{
									if(nType == STT_MODULE_TYPE_VOLT)
					...
					if(pChannel->Ramp[1].nIndex > 0)
					{
						...
						nTempPos2 += pChannel->Harm[nHarmIndex].nSelectIndex * DIGITAL_CHANNEL_MAX * 3;
						long  nPh;
						nPh = fStep * 65536 / 360;
						nPh = nPh << 16;
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2] = nSign * nPh;
						nPh = fEnd * 65536 / 360;
						nPh = nPh << 16;
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2] = nPh;
					}

					if(pChannel->Ramp[2].nIndex == 1)
					{
						//单波形输出频率递变
						fStart = fabs(pChannel->Harm[1].fFreq);
						fEnd = fabs(pChannel->Ramp[2].fEnd);
			...
			if(nMode == 0)
			{
				...
				for(int i = 0; i < DIGITAL_CHANNEL_MAX; i++)
				{
					...
					if(pChannel->Ramp[0].nIndex == 0 && pChannel->Harm[0].nSelectIndex > 0)
					{
						...
						if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
						{
							nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, TRUE, nType, TRUE, m_nTmtPos);
						}
						else
						{
							if(nType == STT_MODULE_TYPE_VOLT)
							...
							{
								nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, TRUE, nType, TRUE, i - 12);
							}
						}
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2] = nCode;
						...
						if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
						{
							nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, TRUE, nType, TRUE, m_nTmtPos);
						}
						else
						{
							if(nType == STT_MODULE_TYPE_VOLT)
							...
							{
								nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, TRUE, nType, TRUE, i - 12);
							}
						}
						pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2] = nCode;
					}
					...
					if(pChannel->Ramp[0].nIndex > 0)
					{
						...
						if(nHarmIndex > 1) //HarmTest
						{
							...
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, FALSE, nType, TRUE, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, FALSE, nType, TRUE, i - 12);
								}
							}
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2] = nCode;
							...
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, FALSE, nType, TRUE, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, FALSE, nType, TRUE, i - 12);
								}
							}
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2] = nCode;
						}
						else if(nHarmIndex == 1) //ManualTest
						{
							...
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, TRUE, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = nSign * TransDigitalChannelAmp(fStep, nGroupIndex, bDC, nType, TRUE, i - 12);
								}
							}
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultChangVal[nTempPos2 + 2] = nCode;
							...
							if(g_theSystemConfig->m_nIecFormat == STT_IEC_FORMAT_60044_8DC)
							{
								nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, TRUE, m_nTmtPos);
							}
							else
							{
								if(nType == STT_MODULE_TYPE_VOLT)
								...
								{
									nCode = TransDigitalChannelAmp(fEnd, nGroupIndex, bDC, nType, TRUE, i - 12);
								}
							}
							pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos2 + 2] = nCode;
						}
			...
			if(nMode == 0)
			{
				...
				for(int i = 0; i < DIGITAL_CHANNEL_MAX; i++)
				{
					...
					if(pChannel->Ramp[0].nIndex == 0 && pChannel->Harm[0].nSelectIndex > 0)
					{
						...
						pDrvData->AcPara.ADMUPara.para[nIndex].nFaultChangVal[nTempPos2] = nCode;
						...
						pDrvData->AcPara.ADMUPara.para[nIndex].nFaultLastVal[nTempPos2] = nCode;
					}
					...
					if(pChannel->Ramp[0].nIndex > 0)
					{
						...
						else if(nHarmIndex == 1) //ManualTest
						{
							...
							pDrvData->AcPara.ADMUPara.para[nIndex].nFaultChangVal[nTempPos2 + 2] = nCode;
							...
							pDrvData->AcPara.ADMUPara.para[nIndex].nFaultLastVal[nTempPos2 + 2] = nCode;
						}
					}
					...
					if(pChannel->Ramp[1].nIndex > 0)
					{
						...
						nPh = fStep * 65536 / 360;
						nPh = nPh << 16;
						pDrvData->AcPara.ADMUPara.para[nIndex].nFaultChangVal[nTempPos2] = nSign * nPh;
						nPh = fEnd * 65536 / 360;
						nPh = nPh << 16;
						pDrvData->AcPara.ADMUPara.para[nIndex].nFaultLastVal[nTempPos2] = nPh;
					}
...
//								= (((nH << 16) & 0xFFFF0000) + nL);
						...
						if(Tmtui.Ramp[0].nIndex > 0)
						{
							...
							if(nHarmIndex == 1 && fFreq < EPSINON)
							{
								long *pDcData = &pDrvData->AcPara.para[nModuleOffset + nIndex].nFaultLastVal[nTempPos];

								if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PNS331)
								{
									TransAnalogDCToDrv_Week_PNS331(fEnd, j, nIndex, pDcData, FALSE);
								}
								else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
								{
									TransAnalogDCToDrv_Week_L336D(fEnd, j, nIndex, pDcData, FALSE);
								}
								else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PN_MAIN)
								{
									TransAnalogDCToDrv_Week_PIA48S(fEnd, j, nIndex, pDcData, FALSE);
								}
							}
							else
							{
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x110, 0);//递变频率时，自动关闭秒同步
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
...
void CSttDeviceComm::Manual_volt_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm,
								 int nModuleUIndex, BOOL bIsDc)
{
	PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_U[nModuleUIndex];
	...
	if(bIsDc)
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			fAmp = pAnalogModule->m_pVolChannel[k].m_oHarm.fAmp;
	...
	else
	{
		...
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			{
				fAmp = pAnalogModule->m_pVolChannel[k].m_oHarm.fAmp;
				fAngle = pAnalogModule->m_pVolChannel[k].m_oHarm.fAngle;
				fFreq = pAnalogModule->m_pVolChannel[k].m_oHarm.fFreq;
				TransAnalogVolToDrv(fAmp, fAngle,
									fFreq, k, nModuleUIndex, dat + CHANNEL_MAX + nChPos * 3);
			}
		}
...
void CSttDeviceComm::Manual_current_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm,
									int nModuleIIndex, BOOL bIsDc)
{
	int nModuleOffset = nModuleIIndex;
	nModuleOffset += g_theDeviceSystemParameter->m_nModuleCount_U;
	PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_I[nModuleIIndex];
	...
	if(bIsDc)
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			fAmp = pAnalogModule->m_pCurrChannel[k].m_oHarm.fAmp;
	...
	else
	{
		...
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			{
				fAmp = pAnalogModule->m_pCurrChannel[k].m_oHarm.fAmp;
				fAngle = pAnalogModule->m_pCurrChannel[k].m_oHarm.fAngle;
				fFreq = pAnalogModule->m_pCurrChannel[k].m_oHarm.fFreq;
				TransAnalogCurToDrv(fAmp * fChTmtCoef, fAngle,
									fFreq, k, nModuleIIndex, dat + CHANNEL_MAX + nChPos * 3);
			}
							   ...
							   int nIndex, BOOL bIsDc)
{
	...
	else
	{
        ...
        qDebug() << "m_nTmtChanNum: " << m_nTmtChanNum;
        //for(int k = 0; k < m_nTmtChanNum; k++)
        for(k = 0; k < m_nTmtChanNum; k++)
		{
			if(pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[k].m_nEeType == STT_CH_EETYPE_VOLTAGE) //'U'
			...
			//DC
			{
                ...
                qDebug() << "dat(dc) pos = " << nChPos  << "     " << dat [nChPos];
			}
			...
			//BASE
			{
                ...
                qDebug() << "dat(base) pos = " << nChanSize + nChPos * 3 << "     " << dat[nChanSize + nChPos * 3];
			}
			...
			if(nHarm > 1)
			{
				...
				for(int j = 2; j < nHarmCount; j++)
				{
					...
					if(nHarmIndex > 0 && nHarmIndex < 31)
					{
                        ...
                        qDebug() << "dat(harm) pos = " << nChanSize + nChPos * 3 + nHarmIndex * 3 * nChanSize
                                 << "     " << dat [nChPos * 3 + nHarmIndex * 3 * nChanSize];
						nHarmTotal++;
					}
				}
				...
				if(nHarmTotal < 30)
				{
                    ...
                    qDebug() << "dat(harm) pos = " << nChanSize + nChPos * 3 + (nHarmTotal + 1) * 3 * nChanSize;
				}
			}
		}
...
void CSttDeviceComm::Manual_UI_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm,
							   int nIndex, BOOL bIsDc)
{
	int nModuleOffset = nIndex;
	nModuleOffset += g_theDeviceSystemParameter->m_nModuleCount_U;
	...
	if(bIsDc)
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			if(pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[k].m_nEeType == STT_CH_EETYPE_VOLTAGE)
			{
				pTmt_channel = &pAnalogModule->m_pVolChannel[nVoltChanCnt];
				bChanIsVolt = TRUE;
				nVoltChanCnt++;
			}
			else
			{
				pTmt_channel = &pAnalogModule->m_pCurrChannel[nCurrChanCnt];
				bChanIsVolt = FALSE;
				nCurrChanCnt++;
			}

			nChPos = pModule->m_nChDrvPos[k];
			fAmp = pTmt_channel->m_oHarm.fAmp;
	...
	else
	{
		...
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			{
				fAmp = pTmt_channel->m_oHarm.fAmp;
				fAngle = pTmt_channel->m_oHarm.fAngle;
				fFreq = pTmt_channel->m_oHarm.fFreq;
		...
		pDrv->oDrv_HarmParam[nModuleOffset].nDWcnt = nChanSize + 3 * nChanSize;
	}
}
/************公共处理函数部分*************/
...
void CSttDeviceComm::tmt_to_drv_manu(tmt_ManualTest *pTmt, BOOL bIsStartTest)
{
		...
		if(g_theSystemConfig->m_nHasWeek != 0)
		{
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
			{
				...
				if(bSetTap)
				{
					...
					m_nDCTap |= ModuleIndexTransToDrv(pModule->m_oModuleAttr.m_nModuleIndex) << 16;
					g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
					drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x125, m_nDCTap);
					g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
				}
			}
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x300, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, sizeof(Drv_ManuData));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::tmt_to_drv_AsyncManu(tmt_AsyncManualTest *pTmt, BOOL bIsStartTest)
{

	char *pszDrv = stt_test_get_drvparas_buff();

	if(pTmt->m_oParas.m_bAuto)
	{
		// CSttDeviceBase::g_pSttDeviceBase->m_bCurTestIsGradient = TRUE;
		// tmt_GradientParas &oGradientParas = CSttDeviceBase::g_pSttDeviceBase->m_oGradientParas;
		// oGradientParas.m_fPrepareTime = EPSINON;
		...
		// 			m_nDCTap = g_nWeekTap;
		// 			m_nDCTap |= ModuleIndexTransToDrv(pModule->m_oModuleAttr.m_nModuleIndex) << 16;
		// 			if(g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__))
		// 			drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x125, m_nDCTap);
		// 			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		// 		}
		// 	}
	...
	if(pTmt->m_oParas.m_bBinStop)
	{
		oBiSetting.nBinIsStop = TRUE;//开入停止
	}

	SetTestCommBi(oBiSetting.oTrip, pTmt->m_oParas.m_nBinLogic,
				  pTmt->m_oParas.m_binIn, pTmt->m_oParas.m_binInEx);
	SetManualBI((char *)&oBiSetting, sizeof(Drv_ManuBin));
	BOOL  bSetAux = FALSE;
	float fAuxMax = g_theSystemConfig->m_fAuxDC_Vol;
	...
	{
		bSetAux = TRUE;//系统参数辅助直流等于0,使能输出
		fAuxMax = pTmt->m_oParas.m_fVolAux;
	}
	...
	tmt_AsyncModules *pAsyncModules = &pTmt->m_oParas.m_oAsyncModuels;
	tmt_AsyncModule *pAsyncModule = NULL;
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		//VOLT
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
		{
			...
			for(int i = 0;i < pAsyncModules->m_nAnalogCount && i < MAX_MODULE_COUNT;i++)
			{
				if(pAsyncModules->m_oAnalogModules[i].m_nModulePos
						== pModule->m_oModuleAttr.m_nModuleIndex)
				{
					pAsyncModule = &pAsyncModules->m_oAnalogModules[i];
					break;
				}
			}

			if(pAsyncModule != NULL)
				Manual_volt_Async(pAsyncModule, pszDrv, 1, nIndex, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG]);
		}
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
		{
			...
			if(pAsyncModule != NULL)
				Manual_current_Async(pAsyncModule, pszDrv, 1, nIndex, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG]);
		}
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_UI; nIndex++)
		{
			...
			if(pAsyncModule != NULL)
				Manual_UI_Async(pAsyncModule, pszDrv, 1, nIndex, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG]);
		}
	}

	if(g_theSystemConfig->m_nHasDigital != 0)
	{
		tmt_AsyncModule  *pDigitalModule = &pAsyncModules->m_oDigitalModules;
		if(pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_DIGITAL])
		{
			for(int i = 0; i < MAX_VOLTAGE_COUNT; i++)
			{
				if(i < pDigitalModule->m_nVolChCount)
					pDigitalModule->m_pVolChannel[i].m_oHarm.fFreq = 0.0f;
			}
			for(int i = 0; i < MAX_CURRENT_COUNT; i++)
			{
				if(i < pDigitalModule->m_nCurrChCount)
					pDigitalModule->m_pCurrChannel[i].m_oHarm.fFreq = 0.0f;
			}
		}
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
		{
			SetGooseValue_ManuTest(&pTmt->m_oParas.m_oGoosePub[0], bIsStartTest);
			nModuleCnt = g_theDeviceSystemParameter->m_nModuleCount_D;
			BOOL bIsFt3;
			...
			for(int i = 0; i < nModuleCnt; i++)
			{
				...
				//不同数字板的硬件相位补偿不同
				TranslateDigitalVoltCurrentBuffer_Async(pDigitalModule, pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat, pModule, 1, bIsFt3);
				pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].nDWcnt = DIGITAL_CHANNEL_MAX * 3;
			}
		}
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_FT3 > 0)
		{
			...
			for(int i = 0; i < nModuleCnt; i++)
			{
				PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_FT3[i];
				//不同数字板的硬件相位补偿不同
				TranslateDigitalVoltCurrentBuffer_Async(pDigitalModule, pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat, pModule, 1, TRUE);
				pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].nDWcnt = DIGITAL_CHANNEL_MAX * 3;
			}
		}

		SetFt3Value_ManuTest(&pTmt->m_oParas.m_oFt3Pub[0], bIsStartTest);
	}
	...
	if(g_theSystemConfig->m_nHasWeek != 0)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
		{
			PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_WEEK[nIndex];
			pAsyncModule = NULL;
			for(int i = 0;i < pAsyncModules->m_nWeekCount && i < MAX_MODULE_COUNT;i++)
			{
				if(pAsyncModules->m_oWeekModules[i].m_nModulePos
						== pModule->m_oModuleAttr.m_nModuleIndex)
				{
					pAsyncModule = &pAsyncModules->m_oWeekModules[i];
					break;
				}
			}
			if(pAsyncModule == NULL)
			...
			if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PN_MAIN)
			{
				Manual_Week_PN_Main_Async(pAsyncModule, pszDrv, 1, nIndex,
										  pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_WEEK]);
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PIA48S)
			{
				Manual_Week_PIA48S_Async(pAsyncModule, pszDrv,
								   1, nIndex, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_WEEK]);
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PIA12DS)
			{
				Manual_Week_PIA12DS_Async(pAsyncModule, pszDrv, nIndex);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_EVTECT)
			{
				Manual_Week_EVTECT_Async(pAsyncModule, pszDrv,
								   1, nIndex, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_WEEK]);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_EVTECT_Ex1)
			{
				Manual_Week_EVTECT_Ex1_Async(pAsyncModule, pszDrv, nIndex);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_DC6U10V)
			{
				Manual_Week_DC6U10V_Async(pAsyncModule, pszDrv, nIndex);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_DC6I20mA)
			{
				Manual_Week_DC6I20mA_Async(pAsyncModule, pszDrv, nIndex);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PN_DC6U6I)
			{
				Manual_Week_PN_DC6U6I_Async(pAsyncModule, pszDrv, nIndex);
				bSetTap = TRUE;
			}
	...
	SetTestCommBo(pDrv->oDrvComBoSta, pTmt->m_oParas.m_binOut, pTmt->m_oParas.m_binOutEx);
	...
	if(g_theSystemConfig->m_nHasAnalog != 0 || bSetAux)
	{
		CSttDeviceBase::g_pSttDeviceBase->DAValueCalc_CommAsync(pAsyncModules, pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG], 0, 0, fAuxMax);
	}
}
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
			{
				...
				if(oModule_I != NULL)
				{
					if(oModule_I->m_oModuleAttr.m_nModuleIndex == pTmt->m_nModuleIndex)
					{
						...
						{
							long nChannelNum = oModule_I->m_oModuleAttr.m_nChannelNum;
							long nDefChannelNum = oModule_I->m_oModuleAttr.m_nDefChannelNum;
							if((nChannelNum == 6) && (nDefChannelNum != nChannelNum))
							{
								if(nDefChannelNum == 3)
								{
									fIMax = fIMax/2.0;
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_UI; nIndex++)
			{
				...
				if(pModule_UI != NULL)
				{
					if(pModule_UI->m_oModuleAttr.m_nModuleIndex == pTmt->m_nModuleIndex
							...
							|| pModule_UI->m_oModuleAttr.m_nModulePower == STT_MODULE_UI_PNS330_6)
					{
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
			{
				...
				if(pModule_Week != NULL)
				{
					if(pModule_Week->m_oModuleAttr.m_nModuleIndex == pTmt->m_nModuleIndex
							...
							|| pModule_Week->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
					{
						pTmt->m_nModuleIndex = pModule_Week->m_oModuleAttr.m_nModuleIndex;
						...
						else if(pModule_Week->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PNS331)
						{
							...
							g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
							drv_reg_ioctl(__FILE__, __LINE__, m_nfd, PNS330_RegAddr_DigitalCtrl, nRegCtrlDigital);
							g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
							...
							g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
							drv_reg_ioctl(__FILE__, __LINE__, m_nfd, PNS330_RegAddr_AnalogCtrl, nRegCtrlAnalog);
							g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
						...
						else if(pModule_Week->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
						{
	...
	if(g_theSystemConfig->m_nHasDigital != 0)
	{
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
		{
			...
			for(int i = 0; i < nModuleCnt; i++)
			{
				...
				//叠加直流
				TranslateDigitalVoltCurrentBuffer(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
												  pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, 0, bIsFt3);
				nHarmTotal++;
				...
				pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].nDWcnt = DIGITAL_CHANNEL_MAX * 3 * nHarmTotal;
			}
		}
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_FT3 > 0)
		{
			...
			for(int i = 0; i < nModuleCnt; i++)
			{
				...
				//叠加直流
				TranslateDigitalVoltCurrentBuffer(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
												  pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, 0, TRUE);
				nHarmTotal++;
		...
		if(g_theDeviceSystemParameter->m_nModuleCount_ADMU > 0)
		{
			...
			for(int i = 0; i < nModuleCnt; i++)
			{
				...
				{
					bHarmValid = TranslateDigitalVoltCurrentBuffer(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
								 pDrv->oDrv_ADMUHarm.oHarmParam[i].dat + 1 + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, m + 1, TRUE);
				...
				TranslateDigitalVoltCurrentBuffer(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
												  pDrv->oDrv_ADMUHarm.oHarmParam[i].dat + 1 + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, 0, TRUE);
				nHarmTotal++;

				if(nHarmTotal < 30)
				{
					TranslateDigitalVoltCurrentBuffer(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
													  pDrv->oDrv_ADMUHarm.oHarmParam[i].dat + 1 + DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, -1, TRUE);
					nHarmTotal++;
				}

				nHarmTotal++;//合并基波

				pDrv->oDrv_ADMUHarm.oHarmParam[i].nDWcnt = 1 + DIGITAL_CHANNEL_MAX * 3 * nHarmTotal;
			}
		}
	}
...
void CSttDeviceComm::tmt_to_drv_StatePara(tmt_StateParas *pTmt)
{
	...
	pDrv->bSearchNext = pTmt->m_bSearchNext;
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x149, 0);//只有开出保持，无触发
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x301, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, sizeof(Drv_StatePara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
BOOL CSttDeviceComm::tmt_to_drv_SmvAbn(Drv_AbnPara *pAbnPara, tmt_StateAbnormalSMV oTmtAbnPara, tmt_sv_92 *pPreTMtSmvAbn, BOOL bIsDefault)
{
	if(oTmtAbnPara.m_nGroupIndex >= m_oCIEC61850CfgMngr.m_o92Mngr.data_92.size()
			|| oTmtAbnPara.m_nGroupIndex < 0)
	{
		pAbnPara->abnType = 0;
		return FALSE;
	}

	tmt_sv_92 oTMtSmvAbn = m_oCIEC61850CfgMngr.m_o92Mngr.data_92.at(oTmtAbnPara.m_nGroupIndex);
	...
	if(oTmtAbnPara.m_oSmvMsg.m_nUseOrigPkgSet > 0)
	{
		...
		pPreTMtSmvAbn->m_VLanID = oTMtSmvAbn.m_VLanID;
		pPreTMtSmvAbn->m_VLanPri = oTMtSmvAbn.m_VLanPri;
		pPreTMtSmvAbn->m_SyncMode = oTMtSmvAbn.m_SyncMode;
		pPreTMtSmvAbn->m_NetType = oTMtSmvAbn.m_NetType;
		pPreTMtSmvAbn->m_ConfRev = oTMtSmvAbn.m_ConfRev;
		pPreTMtSmvAbn->m_AppID = oTMtSmvAbn.m_AppID;
		pPreTMtSmvAbn->m_MacDesc = oTMtSmvAbn.m_MacDesc;
		pPreTMtSmvAbn->m_MacSrc = oTMtSmvAbn.m_MacSrc;
		pPreTMtSmvAbn->m_SVID = oTMtSmvAbn.m_SVID;
		pPreTMtSmvAbn->m_Datset = oTMtSmvAbn.m_Datset;		
	}
	else
	{
		oTMtSmvAbn.m_VLanID = pPreTMtSmvAbn->m_VLanID;
		oTMtSmvAbn.m_VLanPri = pPreTMtSmvAbn->m_VLanPri;
		oTMtSmvAbn.m_SyncMode = pPreTMtSmvAbn->m_SyncMode;
		oTMtSmvAbn.m_NetType = pPreTMtSmvAbn->m_NetType;
		oTMtSmvAbn.m_ConfRev = pPreTMtSmvAbn->m_ConfRev;
		oTMtSmvAbn.m_AppID = pPreTMtSmvAbn->m_AppID;
		oTMtSmvAbn.m_MacDesc = pPreTMtSmvAbn->m_MacDesc;
		oTMtSmvAbn.m_MacSrc = pPreTMtSmvAbn->m_MacSrc;
		oTMtSmvAbn.m_SVID = pPreTMtSmvAbn->m_SVID;
		oTMtSmvAbn.m_Datset = pPreTMtSmvAbn->m_Datset;
	}
...
void CSttDeviceComm::ProAsyncStateDAPara(DeviceDAPara *pDAPara, float fAuxMax, tmt_AsyncStatePara  &oTmt_FirstState)
{
	DeviceDAPara *pDevBaseDAPara = &CSttDeviceBase::g_pSttDeviceBase->m_oDAPara;
	DeviceDAPara *pDAParaForState2 = &CSttDeviceBase::g_pSttDeviceBase->m_oDAParaForState2;
	int nModuleIndex = 0;
...
void CSttDeviceComm::tmt_to_drv_StateItem(tmt_StateParas *pTmt, long nAbnType)
{
	...
	tmt_sv_92 oPreTMtSmvAbn;
	if(nAbnType == ABN_TYPE_SMV)
	{
		long nGroupIndex = pTmt->m_paraState[0].m_oAbnormalSMV.m_nGroupIndex;
		if(nGroupIndex >= 0 && nGroupIndex < m_oCIEC61850CfgMngr.m_o92Mngr.data_92.size())
		{
			tmt_sv_92 oIecConfigSmv = m_oCIEC61850CfgMngr.m_o92Mngr.data_92.at(nGroupIndex);
			oPreTMtSmvAbn.m_VLanID = oIecConfigSmv.m_VLanID;
			oPreTMtSmvAbn.m_VLanPri = oIecConfigSmv.m_VLanPri;
			oPreTMtSmvAbn.m_SyncMode = oIecConfigSmv.m_SyncMode;
			oPreTMtSmvAbn.m_NetType = oIecConfigSmv.m_NetType;
			oPreTMtSmvAbn.m_ConfRev = oIecConfigSmv.m_ConfRev;
			oPreTMtSmvAbn.m_AppID = oIecConfigSmv.m_AppID;
			oPreTMtSmvAbn.m_MacDesc = oIecConfigSmv.m_MacDesc;
			oPreTMtSmvAbn.m_MacSrc = oIecConfigSmv.m_MacSrc;
			oPreTMtSmvAbn.m_SVID = oIecConfigSmv.m_SVID;
			oPreTMtSmvAbn.m_Datset = oIecConfigSmv.m_Datset;
		}
	}
	...
	for(nStateNum = 0; nStateNum < g_nStateCount; nStateNum++)
	{
		...
		if(g_theSystemConfig->m_nHasDigital != 0)
		{
			...
			if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
			{
				...
				if(nAbnType == ABN_TYPE_SMV)
				{
					if(oTmt_paraState.m_oAbnormalSMV.m_nAbnormalType > SMV_ABNORMAL_TYPE_NULL)
					{
						//本状态有异常
						bRet = tmt_to_drv_SmvAbn(&m_pDrv_StateItem->oAbnPara, oTmt_paraState.m_oAbnormalSMV, &oPreTMtSmvAbn, FALSE);
					}
					else
					{
						//本状态无异常,状态循环时第一态也需恢复正常
						bRet = tmt_to_drv_SmvAbn(&m_pDrv_StateItem->oAbnPara, oTmt_paraState.m_oAbnormalSMV, &oPreTMtSmvAbn, TRUE);
					}
				...
				for(int i = 0; i < nModuleCnt; i++)
				{
					...
					TranslateDigitalVoltCurrentBuffer(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
													  m_pDrv_StateItem->oAcParam.item[nAnalogModuleCnt + i].nVal + DIGITAL_CHANNEL_MAX * 3, pModule, 0, bIsFt3, 1);
			...
			if(g_theDeviceSystemParameter->m_nModuleCount_FT3 > 0)
			{
				...
				for(int i = 0; i < nModuleCnt; i++)
				{
					...
					TranslateDigitalVoltCurrentBuffer(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
													  m_pDrv_StateItem->oAcParam.item[nAnalogModuleCnt + i].nVal + DIGITAL_CHANNEL_MAX * 3, pModule, 0, TRUE, 1);
					TranslateDigitalVoltCurrentBuffer_Gradient(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
			...
			if(g_theDeviceSystemParameter->m_nModuleCount_ADMU > 0)
			{
				...
				for(int i = 0; i < nModuleCnt; i++)
				{
					...
					m_pDrv_StateItem->oADMUPara.oADMUACPara[i].nPhase = 2<<30;
					...
					TranslateDigitalVoltCurrentBuffer(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
													  m_pDrv_StateItem->oADMUPara.oADMUACPara[i].nVal + 1, pModule, 1, TRUE, 0);
					TranslateDigitalVoltCurrentBuffer(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
													  m_pDrv_StateItem->oADMUPara.oADMUACPara[i].nVal + 1 + DIGITAL_CHANNEL_MAX * 3, pModule, 0, TRUE, 1);
					TranslateDigitalVoltCurrentBuffer_Gradient(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x110, 0);//有一个状态包含频率递变，就关闭秒同步
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}

	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x301, 1);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pBuf, pTmt->m_nStateNumbers * sizeof(Drv_StateItem));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::setTrigerAsync(tmt_AsyncStatePara  oTmt_AsyncStatePara, Drv_StateItem *oDrv_StateItem, int nIndex)
{
	switch(oTmt_AsyncStatePara.m_nTriggerCondition)
	{
		case TMT_MDSTATE_MEARSURETRIGGER://手动触发
		{
			oDrv_StateItem->nTirpType = State_Trig_Manual;
		...
		{
			oDrv_StateItem->nTirpType = State_Trig_TimeDelay;
			float fTm = oTmt_AsyncStatePara.m_fTimeState * 1000;
			oDrv_StateItem->oTm.oTime.nMsec = fTm;
			oDrv_StateItem->oTm.oTime.nUsec = (fTm - (int)fTm) * 1000;
		}
		break;
		...
		case TMT_MDSTATE_BINTRIGGER:
		{
			...
			if(oTmt_AsyncStatePara.m_nBinLogic == TMT_BINLOGIC_AND)
			{
				oDrv_StateItem->obiTrip.nBiLogic = 1;
			}

			oDrv_StateItem->obiTrip.nBiRef  = 0;
			oDrv_StateItem->oTm.oTime.nDelay = oTmt_AsyncStatePara.m_fTimeAfterTrigger * 1000;
			SetTestCommBi(oDrv_StateItem->obiTrip, oDrv_StateItem->obiTrip.nBiLogic,
						  oTmt_AsyncStatePara.m_binIn, oTmt_AsyncStatePara.m_binInEx, nIndex);
		}
		break;

		case TMT_MDSTATE_GPSBTRIGGER://GPS触发,注意传参的时区+8/-8问题
		{
			oDrv_StateItem->oTm.oGps.nNsec = oTmt_AsyncStatePara.m_tGps.nMilliSeconds * 1000000; //纳秒值转换
			oDrv_StateItem->nTirpType = State_Trig_GPS;

			short nYear, nMon, nDay, nHour, nMin, nSec;
			TransTimer_t(g_theSttDeviceStatus->m_nUser_s, nYear, nMon, nDay, nHour, nMin, nSec);
			nHour = oTmt_AsyncStatePara.m_tGps.nHour;
			nMin = oTmt_AsyncStatePara.m_tGps.nMinutes;
			nSec = oTmt_AsyncStatePara.m_tGps.nSeconds;
			unsigned int ntime_t;
			TransTimer_To_t(ntime_t, nYear, nMon, nDay, nHour, nMin, nSec);
			ntime_t -= g_theSttDeviceStatus->m_nTimeZone * 3600;
			oDrv_StateItem->oTm.oGps.nSec = ntime_t;
...
void CSttDeviceComm::State_volt_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules,
								int nModuleUIndex)
{
	PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_U[nModuleUIndex];
	...
	if(pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG])
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			fAmp = pAsyncModule->m_pVolChannel[k].m_oHarm.fAmp;
	...
	else
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			fAmp = pAsyncModule->m_pVolChannel[k].m_oHarm.fAmp;
			fAngle = pAsyncModule->m_pVolChannel[k].m_oHarm.fAngle;
			fFreq = pAsyncModule->m_pVolChannel[k].m_oHarm.fFreq;
...
void CSttDeviceComm::State_current_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules,
								   int nModuleIIndex)
{
	int nModuleOffset = nModuleIIndex;
	nModuleOffset += g_theDeviceSystemParameter->m_nModuleCount_U;
	...
	if(pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG])
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			fAmp = pAsyncModule->m_pCurrChannel[k].m_oHarm.fAmp;
	...
	else
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			...
			fAmp = pAsyncModule->m_pCurrChannel[k].m_oHarm.fAmp;
			fAngle = pAsyncModule->m_pCurrChannel[k].m_oHarm.fAngle;
			fFreq = pAsyncModule->m_pCurrChannel[k].m_oHarm.fFreq;
		...
		if(m_nTmtPos >= 0)
		{
			...
			if(uiCUR.Ramp[0].nIndex > 0)
			{
				...
				if(bRet && fStep > StateDegressMinStep)
				{
					...
					oDrvStateDeg.lastVal[CHANNEL_MAX + nChPos * 3 + 2] =
						TranslateACCurrentAmp(fEnd*fChTmtCoef, k, nModuleIIndex, nHarm + 1);
					oDrvStateDeg.nIsChange++;
				}
			}
...
void CSttDeviceComm::State_UI_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules,
							  int nModuleIndex)
{
	int nModuleOffset = nModuleIndex;
	nModuleOffset += g_theDeviceSystemParameter->m_nModuleCount_U;
	...
	if(pAsyncModules->m_nDc[ASYNC_MODULE_TYPE_ANALOG])
	{
		for(int k = 0; k < m_nTmtChanNum; k++)
		{
			if(pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[k].m_nEeType == STT_CH_EETYPE_VOLTAGE)
			{
				pTmt_channel = &pAsyncModule->m_pVolChannel[nVoltChanCnt];
				bChanIsVolt = TRUE;
				nVoltChanCnt++;
			}
			else
			{
				pTmt_channel = &pAsyncModule->m_pCurrChannel[nCurChanCnt];
				bChanIsVolt = FALSE;
				nCurChanCnt++;
			}
	...
	for(int i = 0; i < nret / m_nDrvResItemLen; i++)
	{
		...
		switch(m_oDrvResItem.nType)
		{
			...
			case DrvResult_TestFinish:
			{
				...
				{
					pSttTestBase->SetTestStarting(FALSE);
					pSttTestBase->SyncSingleStateTmtResult();//阻抗类高级实验同步结果
					bTestFinished = TRUE;
					oTestFinishedTime.dTime = dTime;
					oTestFinishedTime.dRealTime = dTime - pSttTestBase->m_dTestStartTime;
				}
			...
			case DrvResult_State_StateChg:
			{
				...
				if(m_oDrvResItem.nStep == 0)
				{
					...
					{
						g_bRtDataLock = TRUE;
						pSttTestServerBase->ReturnSysStatesToChildren(2);//TestRtDatas
						g_bRtDataLock = FALSE;
					}
				...
				else
				{
					...
					{
						pSttTestBase->m_nCurStateIndex = m_oDrvResItem.nStep;
						pSttTestBase->SyncSingleStateTmtResult();//触发状态序列最后一态结果计算
					}
				}
			}
			break;
...
//                CString strTime = oDate.toString("yyyy-MM-dd HH:mm:ss");
			...
			case DrvResult_State_Search_Next:
			{
				if(pSttTestBase != NULL)
				{
					pSttTestBase->SyncSingleStateTmtResult();
					pSttTestBase->ReturnTestStateEvent_SearchPoint(dTime, 0, TRUE);
				}
			}
			break;
		}
...
/************异步状态序列部分*************/
void CSttDeviceComm::tmt_to_drv_AsyncStatePara(tmt_AsyncStateParas *pTmt)
{
...
void CSttDeviceComm::tmt_to_drv_AsyncStateItem(tmt_AsyncStateParas *pTmt)
{
	if(pTmt->m_nStateNumbers == 0)
	{
		return;
	...
	char *pBuf = stt_test_get_drvparas_buff();
	long nStateNum;
	tmt_AsyncStatePara  oTmt_AsyncState, oTmt_FirstState;
	CSttDeviceBase::g_pSttDeviceBase->m_nCurStateCount = pTmt->m_nStateNumbers;
	DeviceDAPara oDAPara;
	float fAuxMax = g_theSystemConfig->m_fAuxDC_Vol;
	BOOL  bSetAux = FALSE;
	...
	if(fabs(g_theSystemConfig->m_fAuxDC_Vol) < 0.001)
	{
		...
		for(nStateNum = 0; nStateNum < g_nStateCount; nStateNum++)
		{
			oTmt_AsyncState = pTmt->m_paraState[nStateNum];

			if(!oTmt_AsyncState.m_bSelected)
			{
				continue;
			}

			if(oTmt_AsyncState.m_fVolAux > fAuxMax)
			{
				fAuxMax = oTmt_AsyncState.m_fVolAux;
			}
	...
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		CSttDeviceBase::g_pSttDeviceBase->DAPowerCtrl();
		CSttDeviceBase::g_pSttDeviceBase->m_bUseDAParaForState2 = TRUE;
		ProAsyncStateDAPara(&oDAPara, fAuxMax, oTmt_FirstState);
	}
	else
	{
		CSttDeviceBase::g_pSttDeviceBase->DAPowerCtrl();
	...
	for(nStateNum = 0; nStateNum < g_nStateCount; nStateNum++)
	{
		...
		if(g_theSystemConfig->m_nHasAnalog != 0)
		{
			...
			{
				//如果含时间触发，小于2秒那么不更新电源
				if(oTmt_AsyncState.m_nTriggerCondition == TMT_MDSTATE_TIMETRIGGER
						|| oTmt_AsyncState.m_nTriggerCondition == TMT_MDSTATE_TIMEBINTRIGGER)
				{
					if(oTmt_AsyncState.m_fTimeState < 2.0)
					{
						CSttDeviceBase::g_pSttDeviceBase->m_bUseDAParaForState2 = FALSE;
					}
				}
		...
		m_pDrv_StateItem->nChangemSec = oTmt_AsyncState.m_nRampTimeGrad;
		m_pDrv_StateItem->oStBo.nTripMode = oTmt_AsyncState.m_binOut[0].nMode;
		m_pDrv_StateItem->oStBo.nPreTripTime = oTmt_AsyncState.m_binOut[0].fTimeTrig * 1000;
		m_pDrv_StateItem->oStBo.nOutPutTime = oTmt_AsyncState.m_binOut[0].fHoldTime * 1000;
		SetTestCommBo(m_pDrv_StateItem->oStBo.boValue, oTmt_AsyncState.m_binOut, oTmt_AsyncState.m_binOutEx);
		...
		if(m_pDrv_StateItem->oStBo.nTripMode == 1)
		{
			...
			{
				m_pDrv_StateItem->oStBo.boValue.item[0].nVal1 |=
					oTmt_AsyncState.m_binOut[i].nState << i;
			}
		}

		if(bSetAux)
		{
			SetManuOrStateAuxVolt(m_pDrv_StateItem->nAuxVolt, oTmt_AsyncState.m_fVolAux);
		}
		...
		if(g_theSystemConfig->m_nHasAnalog != 0)
		{
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
			{
				...
				State_volt_Async(m_pDrv_StateItem, &oTmt_AsyncState.m_oStateAsyncModule, nIndex);
				// State_volt_Gradient(m_pDrv_StateItem, oTmt_paraState, nIndex);
			}
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
			{
				...
				State_current_Async(m_pDrv_StateItem, &oTmt_AsyncState.m_oStateAsyncModule, nIndex);
				// State_current_Gradient(m_pDrv_StateItem, oTmt_paraState, nIndex);
			}
			...
			for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_UI; nIndex++)
			{
				...
				State_UI_Async(m_pDrv_StateItem, &oTmt_AsyncState.m_oStateAsyncModule, nIndex);
				// State_UI_Gradient(m_pDrv_StateItem, oTmt_paraState, nIndex);
			}
		}
		...
		if(g_theSystemConfig->m_nHasDigital != 0)
		{
			tmt_AsyncModule  *pDigitalModule = &oTmt_AsyncState.m_oStateAsyncModule.m_oDigitalModules;
			if(oTmt_AsyncState.m_oStateAsyncModule.m_nDc[ASYNC_MODULE_TYPE_DIGITAL])
			{
				for(int i = 0; i < MAX_VOLTAGE_COUNT; i++)
				{
					pDigitalModule->m_pVolChannel[i].m_oHarm.fFreq = 0.0f;
			...
			if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
			{
				...
				for(int i = 0; i < nModuleCnt; i++)
				{
					...
					m_pDrv_StateItem->oAcParam.item[nAnalogModuleCnt + i].DWCount = DIGITAL_CHANNEL_MAX * 3 * 2;
					TranslateDigitalVoltCurrentBuffer_Async(pDigitalModule,
															m_pDrv_StateItem->oAcParam.item[nAnalogModuleCnt + i].nVal, pModule, 1, bIsFt3);
					// TranslateDigitalVoltCurrentBuffer_Gradient(oTmt_paraState.m_uiVOL, oTmt_paraState.m_uiCUR,
					// 		&m_pDrv_StateItem->oStChange[nAnalogModuleCnt + i], pModule, bIsFt3);
				}

				SetGooseValue_AsyncStateTest(pTmt, nStateIndex, m_pDrv_StateItem);
			}
			...
			SetFt3Value_AsyncStateTest(pTmt, nStateIndex, m_pDrv_StateItem);
		}
		...
		setTrigerAsync(oTmt_AsyncState, m_pDrv_StateItem, nStateIndex);
		...
		{
			//按状态叠加
			if(oTmt_AsyncState.m_bPlusDC)
			{
				bPlusDC = TRUE;
				fTao = oTmt_AsyncState.m_fTao;
			}
		}
			break;
		}
...
void CSttDeviceComm::tmt_to_drv_BinaryManu(tmt_ManualTest *pTmt, BOOL bIsStartTest)
{
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x147, 0);//通知驱动记录时间
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
	{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x303, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, sizeof(Drv_ManuData));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::tmt_to_drv_CboperPara(tmt_ImpBaseParas *pTmt)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x314, 0); //整组实验参数
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, sizeof(Drv_CboperPara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
/************整组实验*************/
...
void CSttDeviceComm::SetSwingPara(char *pszDrv)
{
	...
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_U; nIndex++)
		{
			...
			CSttDeviceBase::g_pSttDeviceBase->SetVoltDApara(oModule_U->m_oModuleAttr.m_fDefChMax,
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x311, 0); //振荡实验参数
	drv_reg_write(__FILE__, __LINE__, m_nfd, pszDrv, sizeof(Drv_Swing));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::tmt_to_drv_SynPara(tmt_SynParas *pTmt)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x350, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, pDrv, sizeof(Drv_SyncParam));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
}
...
void CSttDeviceComm::tmt_to_drv_soe(tmt_SoeTest *pTmt)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x340, 0);
	drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)stBinOut, sizeof(Drv_SoeExPara));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	if(pTmt->m_oSoeParas.m_nSelect == 0)
	{
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x340, 1);
		drv_reg_write(__FILE__, __LINE__, m_nfd, (char *)stBOut, sizeof(Drv_SoePara));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
}
...
void CSttDeviceComm::tmt_to_drv_MUTest(tmt_ManualTest *pTmt, BOOL bIsStartTest)
{
	...
	for(int nHarmIndex = 2;nHarmIndex < MAX_HARM_COUNT;nHarmIndex++)
	{
		for(int i = 0; i < MAX_VOLTAGE_COUNT; i++)
		{
			pTmt->m_oManuParas.m_uiVOL[i].Harm[nHarmIndex].nSelectIndex = -1;
		}

		for(int i = 0; i < MAX_CURRENT_COUNT; i++)
		{
			pTmt->m_oManuParas.m_uiCUR[i].Harm[nHarmIndex].nSelectIndex = -1;
		}
	}
	...
	{
		nHarm = 31;
		long nSelectIndex = pTmt->m_oManuParas.m_nHarmIndex;

		if(nSelectIndex > 1 && nSelectIndex < MAX_HARM_COUNT)
		{
			for(int i = 0; i < MAX_VOLTAGE_COUNT; i++)
			{
				pTmt->m_oManuParas.m_uiVOL[i].Harm[nSelectIndex].nSelectIndex = nSelectIndex - 1;
			}

			for(int i = 0; i < MAX_CURRENT_COUNT; i++)
			{
				pTmt->m_oManuParas.m_uiCUR[i].Harm[nSelectIndex].nSelectIndex = nSelectIndex - 1;
			}
		}
	}
	...
	if(g_theSystemConfig->m_nHasDigital != 0)
	{
		if(g_theDeviceSystemParameter->m_nModuleCount_D > 0)
		{
			...
			for(int i = 0; i < g_theDeviceSystemParameter->m_nModuleCount_D; i++)
			{
				...
				//BASE
				TranslateDigitalVoltCurrentBuffer(pTmt->m_oManuParas.m_uiVOL, pTmt->m_oManuParas.m_uiCUR,
												  pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat, pModule, 1, FALSE);

				for(int m = 1; m < 31; m++)
				{
					bHarmValid = TranslateDigitalVoltCurrentBuffer(pTmt->m_oManuParas.m_uiVOL, pTmt->m_oManuParas.m_uiCUR,
													  pDrv->oDrv_HarmParam[nAnalogModuleCnt + i].dat +
							DIGITAL_CHANNEL_MAX * 3 * (nHarmTotal + 1), pModule, m + 1, FALSE);
					if(bHarmValid)
					{
						nHarmTotal++;
					}
	...
	if(g_theSystemConfig->m_nHasWeek != 0)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
		{
			...
			{
				Manual_Week_PNS331(pTmt->m_oManuParas.m_uiVOL, pTmt->m_oManuParas.m_uiCUR, pszDrv,
								   nHarm, nIndex, pTmt->m_oManuParas.m_bDC, FALSE);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
			{
...
void CSttDeviceComm::tmt_to_drv_MUHarmTest(tmt_HarmTest *pTmt, BOOL bIsStartTest)
{
	if(bIsStartTest && g_theDeviceSystemParameter->m_nModuleCount_D > 0)
	{
		//强制发B码
	...
	char *pszDrv = stt_test_get_drvparas_buff();
	ManuRtData oRtData;
	memcpy(oRtData.uiVOL, pTmt->m_oHarmParas.m_uiVOL, sizeof(tmt_channel)*g_nRtDataVoltCnt);
	memcpy(oRtData.uiCUR, pTmt->m_oHarmParas.m_uiCUR, sizeof(tmt_channel)*g_nRtDataCurrCnt);
	CSttDeviceBase::g_pSttDeviceBase->m_oManuRtList.append(oRtData);
	Drv_ManuData *pDrv = (Drv_ManuData *)pszDrv;
	memset(pDrv, 0, sizeof(Drv_ManuData));
	...
	if(g_theSystemConfig->m_nHasAnalog != 0)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_I; nIndex++)
		{
			Manual_current(pTmt->m_oHarmParas.m_uiCUR, pszDrv, 31, nIndex);
		}

		//UI
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_UI; nIndex++)
		{
			Manual_UI(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR, pszDrv, 31, nIndex);
		}
	}
	...
	if(g_theSystemConfig->m_nHasWeek != 0)
	{
		...
		for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_WEEK; nIndex++)
		{
			...
			if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PN_MAIN)
			{
				Manual_Week_PN_Main(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR, pszDrv,
									31, nIndex, FALSE, FALSE);
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_PNS331)
			{
				Manual_Week_PNS331(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR, pszDrv,
								   31, nIndex, FALSE, FALSE);
				bSetTap = TRUE;
			}
			else if(pModule->m_oModuleAttr.m_nModulePower == STT_MODULE_WEEK_L336D_ECTEVT)
			{
				Manual_Week_L336D(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR, pszDrv,
								  31, nIndex, FALSE, FALSE);
				bSetTap = TRUE;
			}
		...
		{
			PSTT_MODULE pModule = g_theDeviceSystemParameter->m_oModules_ADMU[nIndex];
			Manual_Week_ADMU(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR, pszDrv,
								31, nIndex, FALSE, FALSE);
			m_nDCTap = g_nWeekTap;
			m_nDCTap |= ModuleIndexTransToDrv(pModule->m_oModuleAttr.m_nModuleIndex) << 16;
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	...
	{
		CSttDeviceBase::g_pSttDeviceBase->DAValueCalc_Comm(pTmt->m_oHarmParas.m_uiVOL, pTmt->m_oHarmParas.m_uiCUR,
				31, FALSE, 0, 0, 0, g_theSystemConfig->m_fAuxDC_Vol);
	}
}
...
void CSttDeviceComm::SetIEC61850Para()
{
	...
	//下载IEC配置的处理入口
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x121, 0);//先关闭数字保持
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0xA00, 0); //关DMA
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x103, 0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttDeviceComm::SetGooseValue_AsyncStateTest(tmt_AsyncStateParas *pAsyncStateParas, int nIndex, Drv_StateItem *pDrvState)
{
	CSttTestBase *pSttTestBase = CSttServerTestCtrlCntr::GetCurrTest();
	long nStateNum = pSttTestBase->GetStateNum(nIndex);
	...
	for(int index = 0; index < nGseOutCnt && index < MAX_MODULES_GOOSEPUB_COUNT; index++)
	{
		...
		if(!pAsyncStateParas->m_paraState[nStateNum].m_oGoosePub[index].m_bUseFlag)
		{
			continue;    //无需变位
		}

		bTestValid = pAsyncStateParas->m_paraState[nStateNum].m_oGoosePub[index].m_bHasTest;
		nCurTestMode = pAsyncStateParas->m_paraState[nStateNum].m_oGoosePub[index].m_nTest;
		pDrvState->GooseTest.nCount = pDrvState->oGseChgParas.nModulesCnt;
		...
		else
		{
			...
			if(bTestValid)
			{
				...
				nPreTestMode = pAsyncStateParas->m_paraState[nPreStateNum].m_oGoosePub[index].m_nTest;
		...
		else
		{
			...
			pPreGoosePub = &pAsyncStateParas->m_paraState[nPreStateNum].m_oGoosePub[index];
		}

		bSetGooseValue = SetGooseValue_Com(&pAsyncStateParas->m_paraState[nStateNum].m_oGoosePub[index],
										   &otmt_goose, &pDrvState->oGseChgParas, pPreGoosePub, bIsCompare);
		...
		{
			m_pStateGoosePub[nIndex].m_oGoosePub[index].m_strChannel[j] =
				pAsyncStateParas->m_paraState[nStateNum].m_oGoosePub[index].m_strChannel[j];
		}
	}
...
void CSttDeviceComm::SetGooseValue_ManuTest(tmt_GoosePub *pCurGoosePub, BOOL bIsStartTest)
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x30D, 1);
	drv_reg_write(__FILE__, __LINE__, m_nfd, &oGSTest, sizeof(Drv_GseTest));
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);

	if(bRet)
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x30B, 1);
		drv_reg_write(__FILE__, __LINE__, m_nfd, &oGseChgParas, sizeof(DrvGseChgParas));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
}
									 ...
									 DrvFT3ChgParas *pFt3ChgParas, tmt_Ft3Pub *pPreFt3Pub, BOOL bIsCompare)
{
	...
	for(int chanid = 0; chanid < ptmt_ft3_6044->m_oChanList.size() && chanid < MAX_FT3_CHANNEL_COUNT; chanid++)
	{
		if(chanid >= MAX_FT3_CHANNEL_COUNT)
		{
			return FALSE;
	...
	if((nBoard >= 0) && (nBoard < ALG_MODULE_MAX))
	{
		QByteArray ovalue;
...
void CSttDeviceComm::SetFt3Value_AsyncStateTest(tmt_AsyncStateParas *pAsyncStateParas, int nIndex, Drv_StateItem *pDrvState)
{
	CSttTestBase *pSttTestBase = CSttServerTestCtrlCntr::GetCurrTest();
	long nStateNum = pSttTestBase->GetStateNum(nIndex);
	...
	for(int index = 0; index < nFt3OutCnt && index < MAX_MODULES_FT3PUB_COUNT; index++)
	{
		...
		if(!pAsyncStateParas->m_paraState[nStateNum].m_oFt3Pub[index].m_bUseFlag)
		{
			continue;    //无需变位
		}
		...
		else
		{
			...
			pPreFt3Pub = &pAsyncStateParas->m_paraState[nPreStateNum].m_oFt3Pub[index];
		}

		bSetFt3Value = SetFt3Value_Com(&pAsyncStateParas->m_paraState[nStateNum].m_oFt3Pub[index],
									   p_ft3_6044, &pDrvState->oFt3ChgParas, pPreFt3Pub, bIsCompare);
		...
		{
			m_pStateFt3Pub[nIndex].m_oFt3Pub[index].m_strChannel[j] =
				pAsyncStateParas->m_paraState[nStateNum].m_oFt3Pub[index].m_strChannel[j];
		}
	}
...
void CSttDeviceComm::SetFt3Value_ManuTest(tmt_Ft3Pub *pCurFt3Pub, BOOL bIsStartTest)
{
	...
	for(int index = 0; index < nFT3OutCnt && index < MAX_MODULES_FT3PUB_COUNT; index++)
	{
		...
		for(int i = 0; i < MAX_FT3_CHANNEL_COUNT; i++)
		{
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x30A, 1);
		drv_reg_write(__FILE__, __LINE__, m_nfd, &oFT3ChgParas, sizeof(DrvFT3ChgParas));
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
}
...
void CSttDeviceComm::Acyclic_Digital(Drv_StateItem *pDrv_StateItem, tmt_StatePara  oTmt_paraState, int nT)
{
	...
	for(int nIndex = 0; nIndex < g_theDeviceSystemParameter->m_nModuleCount_D; nIndex++)
	{
		...
		for(int i = 0; i < 6; i++)
		{
			...
			float fAngle = (fCloseAngle - fZAngle) * 3.141593 / 180;
			fout = -sin(fAngle) * 1.414214 * fValue;
			nValue = fout * g_theSystemConfig->PkgModeCTRate(i / 3) / g_theSystemConfig->m_fIRate[i / 3];
...
{
	if(CSttDeviceBase::g_pSttDeviceBase->m_pMMapBuf == NULL)
	{
		return;
	}
	...
	Drv_DegressMsg *pDegressMsg = &CSttDeviceBase::g_pSttDeviceBase->m_pMMapBuf->oDegressMsg;
	int nWriteCount = pDegressMsg->nWriteCount;
	int nReadCount = pDegressMsg->nReadCount;
	...
	if(nReadCount < nWriteCount)
	{
		for(int i = nReadCount; i < nWriteCount; i++)
		{
			...
			memcpy(&oData, pDegressMsg->oData + i, sizeof(Drv_DegressData));
	...
	ProVoltCurRtData();
	pDegressMsg->nReadCount = nWriteCount;
}
...
void CSttDeviceComm::ProVoltCurRtData()
{
	...
	for(int i = 0; i < m_oRtDataList.count(); i++)
	{
		...
		pCurrTest->ProRtDatasEvent(dTime, dTime - pCurrTest->m_dTestStartTime, oDegData.nCurStep, oDegData.nCurState, oDegData.nType);

		if(oDegData.nType == 2)
		{
			if(pCurrTest->m_nCurStateIndex == -1)
			{
				ProCommBoutEvent(m_oGradientBo_Fault, nRealTimeEventCount, -1,
								 oDegData.nCurStep, dTime, dTime - pCurrTest->m_dTestStartTime, FALSE);
...
BOOL CSttDeviceComm::GetDevInputInfo()
{
	//qputenv("QWS_KEYBOARD",strKEYBOARD.toUtf8());
	QList<QString> m_okbdList;//键盘输入设备链表
	std::ifstream file("/proc/bus/input/devices");
	if(!file.is_open())
	{
        qDebug()<<"std::ifstream file(/proc/bus/input/devices) fail!!!";
		return FALSE;
	}
//qDebug()<<"std::ifstream file(/proc/bus/input/devices) OK!!!";
	std::string line;
	QString strData;
	bool bHasMouse = false;
	while(getline(file,line))
	{
		strData = QString::fromStdString(line);
		if(strData.contains("Handlers="))
		{
			if(strData.contains("mouse"))
			{
				bHasMouse = true;
			}
			else if(strData.contains("kbd"))
			{
				long nIndex = strData.indexOf("event");
				QString strEvent = strData.mid(nIndex,6);
				m_okbdList.append(strEvent);
			}
		}
	}

	QString strKEYBOARD = "";
	for(int i = 0;i < m_okbdList.count();i++)
	{
		strKEYBOARD += "linuxinput:/dev/input/";
		strKEYBOARD += m_okbdList.at(i);
		strKEYBOARD += " ";
	}

	DevInputInfo *pDevInputInfo = &CSttDeviceBase::g_pSttDeviceBase->m_oDevInputInfo;
	if((strKEYBOARD != pDevInputInfo->m_strKeyBoard) || (bHasMouse != pDevInputInfo->m_bMouse))
	{
		pDevInputInfo->m_strKeyBoard = strKEYBOARD;
		pDevInputInfo->m_bMouse = bHasMouse;
		pDevInputInfo->m_nVerIdx++;
	}
