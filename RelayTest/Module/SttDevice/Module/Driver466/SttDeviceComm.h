...
#define DrvResult_State_Search_Next	0x207
//递变消息
...
#define	DrvResult_LtReplay_IRQ_TIMEOUT	0x802
//Pcap回放
...
typedef struct
{
	...
	unsigned int nAlgType[ALG_MODULE_MAX];//0-模拟，1-数字
...
typedef struct
{
	...
	unsigned int    bSearchNext;   //0-单次搜索，1-有后续搜索需要不停止输出
} Drv_CboperPara;
...
typedef struct
{
	...
	Drv_BIStatus oGooseInit;			//goose开入初始值
	unsigned int bSearchNext;   //0-单次实验，1-有后续搜索需要不停止输出
} Drv_StatePara;
...
typedef struct
{
	...
	unsigned int ChMap[12];//回放通道数据对应关系，数字量回放
} Drv_LtReplayModuleChMap;

typedef struct
{
	unsigned int ChMapCount;
	Drv_LtReplayModuleChMap oModuleChMap[12];
} Drv_LtReplayModuleChMaps;
...
{
	unsigned int module;	//模块
	unsigned int nSample;	//播放频率
} Drv_LtReplayModuleSample;

typedef struct
{
	unsigned int Count;
	Drv_LtReplayModuleSample oModuleSample[12];
} Drv_LtReplayModuleSamples;
...
public:
	...
	void SetFT3SubCRC(char *pszBuff);
	...
	BOOL TranslateDigitalVoltCurrentBuffer(tmt_channel *pVChannel, tmt_channel *pIChannel, long *pBuff, PSTT_MODULE pModule, int nHarm, BOOL bIsFT3, int nMode = 0);
	BOOL TranslateDigitalVoltCurrentBuffer_Async(tmt_AsyncModule *pAnalogModule, long *pBuff, PSTT_MODULE pModule, int nHarm, BOOL bIsFT3 = FALSE);
	...
	void tmt_to_drv_AsyncManu(tmt_AsyncManualTest *pTmt, BOOL bIsStartTest);
	...
	void tmt_to_drv_MUHarmTest(tmt_HarmTest *pTmt, BOOL bIsStartTest);

	void SetStateTriger();
	void tmt_to_drv_StatePara(tmt_StateParas *pTmt);
	void DealSmvAbn(structSVabnPara  *pSVabn, tmt_StateAbnormalSMV oTmtAbnPara);
	BOOL tmt_to_drv_SmvAbn(Drv_AbnPara *pAbnPara, tmt_StateAbnormalSMV oTmtAbnPara, tmt_sv_92 *pPreTMtSmvAbn, BOOL bIsDefault);
	...
	void tmt_to_drv_AsyncStatePara(tmt_AsyncStateParas *pTmt);
	void tmt_to_drv_AsyncStateItem(tmt_AsyncStateParas *pTmt);
	...
	void SetADMUChannel(tmt_HarmTest *pTmt);
	//IEC61850
public:
	void *GetIec61850CfgMngr()
	{
		return &m_oCIEC61850CfgMngr;
	}
	...
	void SetGooseValue_AsyncStateTest(tmt_AsyncStateParas *pAsyncStateParas, int nIndex, Drv_StateItem *pDrvState);
...
public:
	...
	void SetFt3Value_AsyncStateTest(tmt_AsyncStateParas *pStateParas, int nIndex, Drv_StateItem *pDrvState);
...
public:
	...
	void Manual_volt_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex, BOOL bIsDc = FALSE);
	void Manual_current_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex, BOOL bIsDc = FALSE);
	void Manual_UI_Async(tmt_AsyncModule *pAnalogModule, char *pszDrv, int nHarm, int nIndex, BOOL bIsDc = FALSE);
	void State_volt(Drv_StateItem *oDrv_StateItem, tmt_StatePara  oTmt_paraState, int nModuleUIndex);
	void State_volt_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules, int nModuleUIndex);
	...
	void State_current_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules, int nModuleIIndex);
	...
	void State_UI_Async(Drv_StateItem *oDrv_StateItem, tmt_AsyncModules  *pAsyncModules, int nModuleIndex);
	...
	void setTrigerAsync(tmt_AsyncStatePara  oTmt_AsyncStatePara, Drv_StateItem *oDrv_StateItem, int nIndex);
