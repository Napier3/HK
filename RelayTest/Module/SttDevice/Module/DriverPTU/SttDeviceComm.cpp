...
void CSttDeviceComm::tmt_to_drv_moduleoutput(TMT_ADJ_MODULE_OUTPUT *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x107,nBo);
	...
	drv_reg_wite(__FILE__, __LINE__, m_nfd,pszBuf,sizeof(DrvManuPara));
}
...
void CSttDeviceComm::tmt_to_drv_manu(tmt_ManualTest *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x112,nOverload);
...
void CSttDeviceComm::tmt_to_drv_StatePara(tmt_StateTest *pTmt)
{
	...
	drv_reg_wite(__FILE__, __LINE__, m_nfd,pBuff,sizeof(DrvStatePara));
}
...
void CSttDeviceComm::tmt_to_drv_systemdefaultoutput(TMT_SYSTEM_DEFAULT_OUTPUT_PARAS *pTmt)
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x308,0);
	drv_reg_wite(__FILE__, __LINE__, m_nfd,pszBuf,sizeof(DrvManuPara));
}
...
void CSttDeviceComm::ProManuDrvResult()
{
	...
	int nret = drv_reg_read(__FILE__, __LINE__, m_nfd,pBuf,0x01);
...
//				if(nTap!=m_nTestTap)
//				{
//					drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x105,nTap);
//					Sleep(50);
//					m_nTestTap=nTap;
//				}

//				drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x136, 1);//开始供电
//			}
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x302,0);
	drv_reg_wite(__FILE__, __LINE__, m_nfd,pszBuff,nLen);
}
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x136, 0);//停止供电
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x105,nTestTap);
	Sleep(50);
	m_nTestTap=nTestTap;
}
...
{
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0xA00,1);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x402, 0);
	...
	case SystemDefaultOutput:
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x113, 1);
		break;
	}
}
...
void CSttDeviceComm::SetTestStop(BOOL bIsSysDefaultOutput)
{
	...
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x113, 0);
	}
	else
	{
		drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0xA00,0);
...
void CSttDeviceComm::SetSystemConfig()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x111,nTime);
}
...
void CSttDeviceComm::InitDApara(int fd)
{
	...
	Sleep(100);//等待驱动初始化完成
	m_nfd=fd;
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd, 0x110,5);//电流模块1的通道调整
	m_bSysOutPutRunning=FALSE;
	m_nTestTap=-1;
	SetSystemConfig();

	//开入模式
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x108,0);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x109,0xffffffff);
	drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x10a,0xff);
