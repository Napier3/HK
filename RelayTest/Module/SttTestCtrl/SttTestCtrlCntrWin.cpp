...
void CSttTestCtrlCntrWin::NewTest(CDataGroup *pDevice, CDataGroup *pTestApp, CDataGroup *pCommConfig)
{
	...
	long nRet = m_oSttAtsClient.Ats_GenerateItems_Items(pGenParas);

	if (nRet == STT_CMD_ExecStatus_SUCCESS)
	{
        //stt_Frame_AddUpdateReport_QT("NULL", TRUE);

        stt_Frame_ClearAllReports_QT(); //ftt 2024.10.23 add 清空报告
        //ftt 2024.10.23 使用项目路径更新报告，报告头路径值固定为"ReportHead"
        QString strItemsPath = "ReportHead";
        stt_Frame_AddUpdateReport_QT(strItemsPath, true);
	}
}
...
long CSttTestCtrlCntrWin::Ats_ImportDvmFile(CDvmDevice *pDvmDevice,long nIsDvmMap)
{
	CString strFilePath;
	strFilePath = ChangeFilePostfix(pDvmDevice->m_strID, "xml");

	if ((strFilePath.Find('/') < 0) && (strFilePath.Find('\\') < 0))
	{
		CString strTmpPath;
		
		strTmpPath = _P_GetInstallPath();
		strTmpPath += _T("e-Protocol\\Library\\");
		CreateAllDirectories(strTmpPath);
		strTmpPath += strFilePath;
		strFilePath = strTmpPath;
	}

	if (pDvmDevice->SaveXmlFile(strFilePath, CDataMngrXmlRWKeys::g_pXmlKeys))
	{
		g_theGbSmartGenWzd->m_strDvmFile = strFilePath;
		return Ats_ImportDvmFile(strFilePath);
	}
...
long CSttTestCtrlCntrWin::Ats_AddOneMacroItem(CDataGroup *pMacroParas,const CString &strItemName,const CString &strItemID,const CString &strItemsPath,
	long nAddPos, const CString &strMacroID, const CString &strTestMacroUI_ID)
 {
	 ...
	 pGenParas->AddNewData(STT_CMD_PARA_ItemsPath,strItemsPath);
	 pGenParas->AddNewData(STT_CMD_PARA_AddPos,nAddPos);
	 pGenParas->AddNewData(STT_CMD_PARA_MacroID,strMacroID);
	 pGenParas->AddNewData(STT_CMD_PARA_TestMacroUI_ID, strTestMacroUI_ID);
	 pGenParas->AddNewChild(pMacroParas);
	 return m_oSttAtsClient.Ats_GenerateItems_Items(pGenParas);
 }

// long CSttTestCtrlCntrWin::Ats_UpdateOneMacroItem(CDataGroup *pMacroParas,long nSelect,const CString &strItemsPath,const CString &strMacroID)
// {
// 	if (!m_oSttAtsClient.ConnectAtsTestServer())
// 	{
// 		return 0;
...
// 	pGenParas->m_strID = STT_CMD_PARA_generate_items;
// 	pGenParas->m_strDataType = STT_CMD_PARA_generate_items;
// 	pGenParas->m_strValue = STT_CMD_PARA_generate_items_macro_para_edit;
// 	pGenParas->AddNewData(STT_CMD_PARA_ItemsPath,strItemsPath);
// 	pGenParas->AddNewData(STT_CMD_PARA_MacroID,strMacroID);
// 	 pGenParas->AddNewData(STT_CMD_PARA_Select,nSelect);
// 	pGenParas->AddNewChild(pMacroParas);
// 	return m_oSttAtsClient.Ats_GenerateItems_Items(pGenParas);
// }


long CSttTestCtrlCntrWin::Ats_UpdateOneMacroItem(CSttMacroTest *pSttMacroTest)
{
	if (!m_oSttAtsClient.ConnectAtsTestServer())
	{
		return 0;
	...
	CTestMacro *pSingleMacroRef = NULL;

	if (m_pTestMacros != NULL)
	{
		pSingleMacroRef = (CTestMacro*)m_pTestMacros->FindByID(strMacroID);
	}

	CDataGroup *pMacroParasGroup = new CDataGroup;
	pMacroParasGroup->m_strID = STT_CMD_PARA_TestMacroParas;
	CSttMacroTestParaDatas *pSttTestParaDatas = pSttMacroTest->GetSttMacroTestParaDatas(TRUE,strMacroID);
	CSttMacroTestParaData *pSttMacroTestParaData = NULL;
	CDvmData *pNewData = NULL;
	CShortData *pCurrShortData = NULL;
	...
	while(pos)
	{
		...
		pNewData = pMacroParasGroup->AddNewData(pSttMacroTestParaData->m_strID, pSttMacroTestParaData->m_strValue);

		if (pSttMacroTestParaData->m_strValue.Find("$") != -1)//确认是否为定值关联参数
		{
			pNewData->m_strFormat = pNewData->m_strValue;
		}

		if (pSingleMacroRef != NULL)
		{
			pCurrShortData = pSingleMacroRef->FindDataByID(pSttMacroTestParaData->m_strID);

			if (pCurrShortData != NULL)
			{
				pNewData->m_strDataType = pCurrShortData->m_strDataTypeID;
			}
		}
	}
	
	strItemPath = pSttMacroTest->GetIDPathEx(STTGBXMLCLASSID_CSTTDEVICE, FALSE);
	CDataGroup *pGenParas = new CDataGroup();
	pGenParas->m_strName = STT_CMD_PARA_generate_items;
	pGenParas->m_strID = STT_CMD_PARA_generate_items;
	pGenParas->m_strDataType = STT_CMD_PARA_generate_items;
	pGenParas->m_strValue = STT_CMD_PARA_generate_items_macro_para_edit;
	pGenParas->AddNewData(STT_CMD_PARA_ItemsPath,strItemPath);
	pGenParas->AddNewData(STT_CMD_PARA_MacroID,strMacroID);
	pGenParas->AddNewData(STT_CMD_PARA_Select,pSttMacroTest->m_nSelect);
	pGenParas->AddNewChild(pMacroParasGroup);
	return m_oSttAtsClient.Ats_GenerateItems_Items(pGenParas);
}

long CSttTestCtrlCntrWin::Ats_DeleteOneItem(const CString &strItemID,const CString &strParentItemsPath)
{
	if (!m_oSttAtsClient.ConnectAtsTestServer())
	{
		return 0;
	...
	pGenParas->m_strID = STT_CMD_PARA_generate_items;
	pGenParas->m_strDataType = STT_CMD_PARA_generate_items;
	pGenParas->m_strValue = STT_CMD_PARA_generate_items_delete;
	pGenParas->AddNewData(STT_CMD_PARA_ItemsID,strItemID);
	pGenParas->AddNewData(STT_CMD_PARA_ParentItemsPath,strParentItemsPath);
	return m_oSttAtsClient.Ats_GenerateItems_Items(pGenParas);
}
	...
	*ppGenParas = pNew;
	...
	if (pItems != NULL)
	{
		...
		if (pItems->GetClassID() == STTGBXMLCLASSID_CSTTITEMS)
		{
			...
			//zhouhj 2024.8.7 增加功能ID
			if (!pSttItems->m_strstxmlFile.IsEmpty())
			{
				pItemsParas->AddNewData(STT_CMD_PARA_TestMacroUI_ID, pSttItems->m_strstxmlFile);
			}
		}
	}
}
