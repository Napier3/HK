...
CSttTestCtrlCntrNative::~CSttTestCtrlCntrNative()
{
	...
	g_theTestCntrFrame->SetTestCtrlCntrBase(NULL);
}
...
{
	CString strParentItemPath, strExeStateValue;
	pParas->GetDataValue(STT_CMD_PARA_ParentItemsPath, strParentItemPath);
	pParas->GetDataValue(STT_SYS_STATE_ID_ExecStatus, strExeStateValue);
	...
	if (pItems != NULL)
	{
		...
		{
			CSttItemBase *pParent = stt_gb_find_itembase(m_pSttGuideBook, strParentItemPath);

			if (pParent != NULL)
			{
				CSttItems *pRootItems = (CSttItems *)Stt_GetFirstParentItems(pParent, GBITEMS_NODETYPE_ROOTNODE, TRUE);

				if (pRootItems != NULL)
				{
					pTestMacroUI = CSttMacroTestUI_TestMacroUIDB::FindTestMacroUI(pRootItems->m_strstxmlFile);
	...
	else if (pSttGuideBook != NULL)
	{
		CSttItems *pFirstRootItems = pSttGuideBook->FindFirstItems(GBITEMS_NODETYPE_ROOTNODE);

		if (pFirstRootItems != NULL)
		{
			CSttMacroTestUI_TestMacroUI *pTestMacroUI = CSttMacroTestUI_TestMacroUIDB::FindTestMacroUI(pFirstRootItems->m_strstxmlFile);
	...
	if (strAtsCmd == STT_CMD_TYPE_ATS_GenerateTemplate)
	{
		...
		{
//			CSttItemBase *pBeforeTestItems = stt_gb_find_itembase(pSttGuideBook, STT_ITEMS_ID_BEFORETEST);
			CSttItemBase *pBeforeTestItems = m_pSttGuideBook->UpdateBeforeTestItems_InternationalMode(pSttGuideBook);
			m_pTestCtrlCntrMsg->OnAtsGenerateItems(pBeforeTestItems, TRUE, bIsInternationalMode);
		} 
		else
		{
			if (pSttGuideBook != NULL)
			{
				...
				m_pSttGuideBook->InitAfterRead();//2024-9-9 wuxinyi 修改ClearGuideBook之后重新赋值变量
	...
	else if (strAtsCmd == STT_CMD_TYPE_ATS_GenerateItems)
	{
		if (pItems != NULL)
		{
			...
			if (bIsInternationalMode)
			{
				pItems = (CSttItems*)m_pSttGuideBook->InsertItems_InternationalMode(strParentItemPath, pItems, bUpdateParent);
			} 
			else
			{
			...
			m_pTestCtrlCntrMsg->OnAtsGenerateItems(pItems, bUpdateParent,bIsInternationalMode);
...
void CSttTestCtrlCntrNative::On_Ats_Generate_cmd_add_grp(const CString &strParentItemPath, CSttItems *pItems)
{
	...
	m_pTestCtrlCntrMsg->OnAtsGenerateItems(pParentItem, bUpdateParent,FALSE);
	m_bCmdReplyProcessFinished = TRUE;
}
...
void CSttTestCtrlCntrNative::ModifyReport_StateTest(CDvmValues *pValues)
{
	...
	CDataGroup *pTestMacroUI_Paras = stt_Frame_GetTestMacroUI_Paras();
...
long CSttTestCtrlCntrNative::Ats_GenerateTemplate()
{
    ...
    if (pTestMacroUI->m_strCharlibFile.GetLength() > 0 && stt_Frame_Characteristics() != NULL)
 	{
        ...
        stt_Frame_Characteristics()->GetXml(CCharacteristicXmlRWKeys::g_pXmlKeys, pContents->m_strText);
 	}

	if (g_nLogDebugInfor == 1)	{		CLogPrint::LogString(XLOGLEVEL_TRACE, "***** benin Ats_GenerateTemplate");	}
	if (strDvmFile == ""&& g_theGbSmartGenWzd != NULL)//2024-10-11 zhouyangyong 数据设备模型为空时打开缺省文件
	{
...
//模板生成相关的接口
//单个功能的模板生成
long CSttTestCtrlCntrNative::Ats_CreateTemplate()
{
	BOOL bRet = m_oSttAtsClient.ConnectAtsTestServer();
	...
	{
		nSucc = m_oSttAtsClient.Ats_CreateTemplate(pTestMacroUI->m_strID, "InternationalDevice", "InternationalDevice", g_theGbSmartGenWzd->m_strDvmFile, g_oSystemParas.HasDigitalInOrOut(),
			TRUE, &oRetData);
	} 
	else
	{
		nSucc = m_oSttAtsClient.Ats_CreateTemplate(pTestMacroUI->m_strID, "InternationalDevice", "InternationalDevice", "", g_oSystemParas.HasDigitalInOrOut(),
			TRUE, &oRetData);
	}
	
// 	CSttParas *pSttParas = oRetData.GetSttParas();//zhouhj 2024.7.29 此两处不需要再考虑 
// 	CExBaseList *pGlobalDatas = (CExBaseList*)pSttParas->FindByID(STT_CMD_PARA_GlobalDatas);
// 
// 	if (pGlobalDatas == NULL)
// 	{
// 		return 0;
// 	}
// 
// 	CExBaseList *pSysParaObj = (CExBaseList*)pSttParas->FindByID(STT_CMD_PARA_SysPara);
// 
// 	if (pSysParaObj == NULL)
// 	{
// 		return 0;
...
long CSttTestCtrlCntrNative::Ats_UpdateReportHead()
{
	BOOL bRet = m_oSttAtsClient.ConnectAtsTestServer();
	...
	CSttCmdData oRetData;
	long nSucc = m_oSttAtsClient.Ats_UpdateReportHead(&m_oTestReportHead, TRUE, &oRetData);

	return nSucc;
}

long CSttTestCtrlCntrNative::Ats_UpdateCharacteristic(CExBaseList *pParent)
{
	BOOL bRet = m_oSttAtsClient.ConnectAtsTestServer();
	...
	CSttItems *pRootItems = (CSttItems *)pParent;
	CString strParentItem;
	strParentItem = pParent->GetIDPathEx(STTGBXMLCLASSID_CSTTDEVICE, FALSE);
	CSttContents *pContents = (CSttContents*)pRootItems->FindByClassID(STTGBXMLCLASSID_CSTTCONTENTS);
	CSttCmdData oRetData;

	if (pContents == NULL)
	{
		pContents = new CSttContents;
		pRootItems->AddNewChild(pContents);
	}
	pContents->m_strID = STT_CMD_PARA_Characteristic;
	stt_Frame_Characteristics()->GetXml(CCharacteristicXmlRWKeys::g_pXmlKeys, pContents->m_strText);
	long nSucc = m_oSttAtsClient.Ats_UpdateCharacteristic(pContents, strParentItem, TRUE, &oRetData);
											   ...
											   , long nRptTitle, long nTitleLevel, long nSynMode)
{
    ...
    if (strCurrItemPath == stt_Frame_CurrSelRootNodePath())
	{
		CSttMacroTestUI_TestMacroUI *pTestMacroUI  = stt_Frame_TestMacroUI();/*stt_Frame_TestMacroUI()*/;
		CSttContents *pContents = NULL;//特性曲线定义
	...
	else
	{
		...
		BOOL bInternationalMode = pTestMacroUI->IsInternationalMode();//2024.7.29 zhangyq 判断是否为国际版
		...
		if (pTestMacroUI->m_strCharlibFile.GetLength() > 0)
		{
			...
			if (pSttItemBase != NULL)
			{
				...
				//2024.7.29 zhangyq 如果是国际版（电流反时限），则特性曲线直接从xml获取
				else if (bInternationalMode)
				{
					stt_Frame_Characteristics()->GetXml(CCharacteristicXmlRWKeys::g_pXmlKeys, pContents->m_strText);
				}
			}
...
//			g_theTestCntrFrame->ShowCharEditTab();
		}
// 		else
// 		{
// 			g_theTestCntrFrame->HideCharEditTab();
// 		}

		g_theTestCntrFrame->ShowHideCharactUI(pTestMacroUI->IsInternationalMode(), pTestMacroUI->HasCharLib());
...
// 	}
	...
	if (pTestMacroUI != NULL)
	{
		...
		else if (g_theGbSmartGenWzd)//2024-10-11 zhouyangyong 数据设备模型为空时打开缺省文件
		{
			strDvmFile = g_theGbSmartGenWzd->m_strDvmFile;
		}
	}
	...
	if (pCurrentUIParas != NULL)
	{
			...
			(g_theTestCntrFrame->HasCharChanged()))
		{
			...
			long nRet = m_oSttAtsClient.Ats_StartTest(pTestMacroUI->GetTestMacroUIDataGroup(),
				pCurrentUIParas, pCommCmd, pContents,TRUE,NULL,strDvmFile, nTestMode);
			return Stt_Is_ExecStatus_Success(nRet);
		}
	}
	
	//增加判断执行结果逻辑 zhouhj 2024.3.25
	long nRet = m_oSttAtsClient.Ats_StartTest(NULL, NULL, NULL, NULL,TRUE,NULL,strDvmFile, nTestMode);
	return Stt_Is_ExecStatus_Success(nRet);
}
...
void CSttTestCtrlCntrNative::Ats_IecDetect(long bEnable, long nIECType)
{
	m_oSttAtsClient.Ats_IecDetect(bEnable, nIECType);
}
...
//2024-08-14 zhouyangyong
void CSttTestCtrlCntrNative::Ats_BinRecord(CDataGroup *pBinRecordParas)
{
	m_oSttAtsClient.Ats_BinRecord(pBinRecordParas);
}
