...
long CVmSttMacroTest::Test(const CString & strMacroID, const CString & strParameter)
{
	...
	if (m_pTestMacro->m_strID == STT_CMD_ATS_MACRO_ID_ReplayTest)
	{
		...
		nRet = Test_StartTest(STT_CMD_ATS_MACRO_ID_ReplayTest, &m_oFaultParas_Dvm, TRUE, NULL, TRUE);
		return 0;
	}
#endif
	...
	if (IsAdjust(strMacroID))
	{
		...
		{
			if (m_oGrpPatameter.GetCount() > 0)
			{
				//仅下发非默认值的参数的部分
				nRet = m_oSttTestClient.Adjust_Adjust(strMacroID, &m_oGrpPatameter, TRUE, NULL, TRUE);
			}
			else
			{
...
BOOL CVmSttMacroTest::SetParameter(CTestMacro *pTestMacro, const CString & strParameter)
{
	...
	if(nLen > 6)
	{
		...
		if(strLeft == _T("<group"))
		{
			...
			BOOL bRet = m_oGrpPatameter.SetXml(bstrText, CDataMngrXmlRWKeys::g_pXmlKeys);
			CDataMngrXmlRWKeys::g_pXmlKeys->m_nIsGroupUseDvmData--;
			...
			if(bRet)
			{
				/*  //非默认值，则不能直接删除后克隆
				m_oFaultParas_Dvm.DeleteAll();
				m_oFaultParas_Dvm.AppendCloneEx(m_oGrpPatameter, TRUE);
				*/
				m_oFaultParas_Dvm.InitDataValues(&m_oGrpPatameter, TRUE);
				return TRUE;
			}
		}
	}
...
{	
	...
	CString strErrInfo;
	BOOL bSucc = m_oSttTestClient.ConnectSttServer(NULL, strErrInfo);
	...
	{
		if (strErrInfo.GetLength() > 0)
		{
			strErrInfo = _T("[autotest error]") + strErrInfo;
			LogDebugInfor(strErrInfo);
		}
...
long CVmSttMacroTest::Test_StartTest(const CString &strMacroID, CDataGroup *pParas,BOOL bDoEvents,CSttCmdData *pRetData,BOOL bSendCmdAsync)
{
	...
	{
		//仅下发非默认值参数的部分
		return m_oSttTestClient.Test_StartTest(strMacroID, &m_oGrpPatameter, bDoEvents, pRetData, bSendCmdAsync);
	}
...
void CVmSttMacroTest::OnTestState(const CString &strMacroID, CDataGroup *pParas)
{
	...
	while (pos != NULL)
	{
		...
		m_oReportBuffer.AppendEquation(pData->m_strID.GetString(), pData->m_strValue.GetString());
		m_oResults_Dvm.SetDataValue(pData->m_strID, pData->m_strValue, TRUE);
		...
		if (pTime != NULL)
		{
			...
			m_oResults_Dvm.SetDataValue(strID, pTime->m_strValue, TRUE);
		}
		...
		if (pRealTime != NULL)
		{
			...
			m_oResults_Dvm.SetDataValue(strID, pRealTime->m_strValue, TRUE);
		}
	}
}
...
long CVmSttMacroTest::OnAts_Stop(CDataGroup *pParas)
{
	...
	pEngine->m_pSttClientSocket->XGetLocalIP(strIP);
...
void CVmSttMacroTest::ResetParasByStateCount(CDataGroup *pParas, long nStateCount)
{
	...
	UINT nClassID = 0;
	CExBaseObject *pObj = NULL;
	BOOL bStateFrom_0 = false;  //参数ID，状态数索引号是从0开始的：state0、state1、……
	BOOL bFirstState = FALSE;
	...
	while (pos != NULL)
	{
		...
		long nStateIndex = CString_To_long(strID);

		if (bFirstState != TRUE)
		{
			bFirstState = TRUE;

			//判断第一个状态的参数ID，其索引号是否是从0开始；
			if (nStateIndex == 0)
			{
				bStateFrom_0 = TRUE;
			}
		...
		if (bStateFrom_0)
		{
			nStateNum = nStateIndex + 1;
		}
		else
		{
			nStateNum = nStateIndex;
		}
...
void CVmSttMacroTest::LogDebugInfor(const CString &strInfor)
{
	if (g_theNativeTestEngine == NULL)
	{
		return;
