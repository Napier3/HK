...
public:
	...
	//begin以下故障参数和结果参数，是测试开始时，从TestMacro.xml中获取的全部参数，先全部取值全部为默认值，后在SetParameter中会更新部分参数的实际设置值
	//同时，以下两个group，会将所有参数全部作为测试报告，回填到GetReport中
	CDataGroup m_oFaultParas_Dvm;  //故障参数+扩展故障参数+系统参数。
	CDataGroup m_oResults_Dvm;   //结果参数+扩展结果参数
	//end
	//SetParameter中传递的全部参数。不包含TestMacro.xml中定义的值为缺省值的参数
	//为节省流量，下发给TestServer的参数，可以仅包含非缺省值的部分参数。例如状态序列，可以仅下发勾选的状态的参数
	CDataGroup m_oGrpPatameter;
