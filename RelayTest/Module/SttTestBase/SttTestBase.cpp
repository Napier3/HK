...
void CSttTestBase::ReturnTestStateEvent_TestFinish(double dTime, double dRealTime, bool bWithEvent)
{
	...
	oRtdata.oTime.dTime = oTime.dTime;
	oRtdata.oTime.dRealTime = oTime.dRealTime;
	...
	for(int nIndex = 0; nIndex < g_nRtDataVoltCnt; nIndex++)
	{
		...
		oRtdata.m_oVolCh[nRtIndex].m_fMag = 0;
...
void CSttTestBase::CalStepValue_Gradient(tmt_GradientParas *pTmt_GradientParas,int nHarmIndex,long nGradientChCount)
{
    ...
    if(nStepCount > GRADIENT_MAX_COUNT)
    {
		...
		CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("GradientCnt(%d) >(%d), Warning！")
    ...
    if (nStepCount>GRADIENT_MAX_COUNT)
    {
        ...
        for(int i=0;i<=GRADIENT_MAX_COUNT;i++)
        {
			...
			GetFaultValue(pUIStepData,fCurValue);
...
void CSttTestBase::UpdateMonitorCur(STT_RTDATA_EVENT &oRtdata)
{
	long nChTmtPos = 0;
	float fTotalCurAmp = 0;
	...
	for(int i = 0;i < g_theDeviceSystemParameter->m_nModuleCount_I;i++)
	{
		...
		for(int nChan = 0;nChan < pModule->m_oModuleAttr.m_nChannelNum && nChan < STT_MODULE_CHANNEL_MAX;nChan++)
		{
			PSTT_MODULE_CH_DEF pChDef = &pModule->m_oModuleAttr.m_oChDefMap.m_oChDefs.m_oChDef[nChan];
			if(pChDef == NULL)
			{
				continue;
			}

			nChTmtPos = pModule->m_nChTmtPos[nChan];
			if(nChTmtPos >= 0)
			{
				for(int j = 0;j < oRtdata.m_nCurChCount && j < MAX_CURRENT_COUNT;j++)
				{
					if(nChTmtPos == oRtdata.m_oCurCh[j].m_nIndex)
					{
						fTotalCurAmp = oRtdata.m_oCurCh[j].m_fMag;
						if(g_nTmtParaMode == 0)
						{
							//界面一次值
							fTotalCurAmp = pChDef->m_fTotalCurAmp
									* g_theSystemConfig->GetCTRate(nChTmtPos/3);;
						}
						pChDef->m_fTotalCurAmp = fTotalCurAmp;
						break;
					}
				}
			}
				...
				if(nChTmtPos >= 0)
				{
					for(int j = 0;j < oRtdata.m_nCurChCount && j < MAX_CURRENT_COUNT;j++)
					{
						if(nChTmtPos == oRtdata.m_oCurCh[j].m_nIndex)
						{
							...
							{
								//界面一次值
								fTotalCurAmp = fTotalCurAmp * g_theSystemConfig->GetCTRate(nChTmtPos/3);;
							}
							pChDef->m_fTotalCurAmp = fTotalCurAmp;
							break;
						}
...
void CSttTestBase::ProGradientRtDatasEvent(STT_RTDATA_EVENT &oRtdata,int nStep,int nType)
{
    ...
    for(int nIndex=0;nIndex<g_nRtDataVoltCnt;nIndex++)
    {
        ...
        if(nType == 2)
        {
            if(oGradientParas.m_uiVOL[2][nIndex].Ramp[0].nIndex==0)
            {
                if(oGradientParas.m_nType==GRADIENT_AMP)
                {
					...
					fVal += oGradientParas.m_nStepDirection*oGradientParas.m_fStep*fPTRate*nStep;
			...
			if(oGradientParas.m_uiVOL[2][nIndex].Ramp[0].nIndex > 0)
            {
				if(oGradientParas.m_uiVOL[2][nIndex].Ramp[0].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
					nStepDirection = oGradientParas.m_nStepDirection2;
				}
				else
				{
					fStart = oGradientParas.m_fStart;
					fStop = oGradientParas.m_fStop;
					fStep = oGradientParas.m_fStep;
					nStepDirection = oGradientParas.m_nStepDirection;
				}
				...
				{
					fValBase+=nStepDirection*fStep*fPTRate*nStep;
					if(fStart > fStop)
					{
						if(fValBase < fStop)
							fValBase = fStop;
				...
				for(int i=2;i<MAX_HARM_COUNT;i++)
				{
					if(oGradientParas.m_nStepHarm==i)
					{
						...
						fVal+=nStepDirection*fStep*fPTRate*nStep;
						if(fStart > fStop)
						{
							if(fVal < fStop)
								fVal = fStop;
			...
			if(oGradientParas.m_uiVOL[2][nIndex].Ramp[1].nIndex==1)
			{
				if(oGradientParas.m_uiVOL[2][nIndex].Ramp[1].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
				...
				fVal+=nStepDirection*fStep*nStep;
				if(fStart > fStop)
				{
					if(fVal < fStop)
						fVal = fStop;
			...
			if(oGradientParas.m_uiVOL[2][nIndex].Ramp[2].nIndex==1)
			{
				if(oGradientParas.m_uiVOL[2][nIndex].Ramp[2].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
        ...
        else
        {
            ...
            else
            {
				...
				if(oGradientParas.m_uiVOL[2][nIndex].Ramp[0].nIndex == 0)
                {
					oRtdata.m_oVolCh[nRtIndex].m_fMag=oGradientParas.m_fStart;
                }
    ...
    for(int nIndex=0;nIndex<g_nRtDataCurrCnt;nIndex++)
    {
        ...
        if(nType == 2)
        {
			...
			if(oGradientParas.m_uiCUR[2][nIndex].Ramp[0].nIndex > 0)
            {
				if(oGradientParas.m_uiCUR[2][nIndex].Ramp[0].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
				...
				if(oGradientParas.m_nStepHarm==1)
				{
					fValBase+=nStepDirection*fStep*nStep;
					if(fStart > fStop)
					{
						if(fValBase < fStop)
							fValBase = fStop;
			...
			if(oGradientParas.m_uiCUR[2][nIndex].Ramp[1].nIndex==1)
			{
				if(oGradientParas.m_uiCUR[2][nIndex].Ramp[1].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
			...
			if(oGradientParas.m_uiCUR[2][nIndex].Ramp[2].nIndex==1)
			{
				if(oGradientParas.m_uiCUR[2][nIndex].Ramp[2].bSecondChan)
				{
					fStart = oGradientParas.m_fStart2;
					fStop = oGradientParas.m_fStop2;
					fStep = oGradientParas.m_fStep2;
        ...
        else
        {
            ...
            else
            {
				...
				if(oGradientParas.m_uiCUR[2][nIndex].Ramp[0].nIndex == 0)
                {
					oRtdata.m_oCurCh[nRtIndex].m_fMag = oGradientParas.m_fStart;
                }
...
char* stt_test_new_drvparas_buff()
{
    ...
    g_pszSttTestDrvParasBuff = (char*)malloc(40000000);   //tlw-12-28  88000000
...
{
    qDebug() <<"stt_test_new_base_manualtest_buff() in";
...
{
    qDebug() <<"*stt_test_new_base_harmtest_buff() in";
