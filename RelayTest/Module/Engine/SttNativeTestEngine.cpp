...
void CSttNativeTestEngine::OnSocketClose(CSttXcmdChInterface *pSocket)
{
...
//         }
    ...
    if(pSocket != m_pSttCloudClientSocket)
    {//本地自动测试连接断链
        CSttSocketDataBase_File *pSocketFile = (CSttSocketDataBase_File*)pSocket;
        m_pServerSocket->OnSttScktClose(pSocketFile);
    }
    else
    {//连接云端socket断链
        if (m_pSttCloudClientSocket != NULL)
        {
            CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("云端：%s Disonnect")
                                       , pSocket->XGetRemoteIP().GetString() );
                                       //, pSocket->m_strIPRemote.GetString());
...
BOOL CSttNativeTestEngine::SendBufferToChildren(CSttXcmdChInterface *pExceptSocket,BYTE *pBuf,long nLen)
{
    if(m_pSttCloudClientSocket != NULL)
    {
    ...
    if (m_pServerSocket != NULL)
    {
         CSttSocketDataBase *pXSocket = (CSttSocketDataBase*)pExceptSocket;
        m_pServerSocket->SendBufferToChildren(pXSocket,pBuf,nLen);
    }
    return 1;
}
...
long CSttNativeTestEngine::SendSysState(CSttSysState *pSysState)
{
    ...
    return m_pSttCloudClientSocket->XSendSysState(pSysState);
}

//收到测试命令，先检查是否有权限，无权限则返回失败
BOOL CSttNativeTestEngine::CheckBeforeProcessCmdTest(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, char *pszCmdID, char *pszTestor)
{
    ...
    if (!bRet)
    {
        ...
        pClientSocket->XRetFailureSysState(&oTestCmd);
    }

    return bRet;
}

BOOL CSttNativeTestEngine::CheckBeforeProcessCmdAts(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, char *pszCmdID, char *pszTestor)
{
    CSttSystemCmd oSysCmd;
    ...
    if (!bRet)
    {
        ...
        pClientSocket->XRetFailureSysState(&oAtsCmd);
    }
...
*/
long CSttNativeTestEngine::ProcessCmd_CloudTest(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    CSttSysState oSysState;
...
//本地Ats和远程登录
long CSttNativeTestEngine::Process_Cmd_System_Login(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    ...
    pClientSocket->XSendSysState(&oSysState);	//应答自身登录信息
...
long CSttNativeTestEngine::Process_Cmd_System_Logout(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    //更新用户链表
    DWORD nIDTester = 0;
...
void CSttNativeTestEngine::CheckHeartbeatOverTime()
{
    ...
    long nTimeGap = pFind->m_pRefSttSckt->XRecvHeartbeatCounterAdd(); //zhouhj 20210821 参考SttTestServerBase修改
...
BOOL CSttNativeTestEngine::OnTestMsgEx(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, long nCmdType, char *pszCmdID, char *pszTestor, char *pszRetCmdType, BOOL &bRet)
{
      监视所有命令报文
    ...
    {//单机测试命令，先判断是否具有ClientEngine权限
        ...
        pClientSocket->XSendBuffer(pSendBuf,nSendBufLen);
		delete pSendBuf;
		pSendBuf = NULL;
		nSendBufLen = 0;
...
long CSttNativeTestEngine::Process_SysState(CSttXcmdChInterface *pClientSocket, CSttSysState &oSysState)
{
...
long CSttNativeTestEngine::Process_SysState_InputData(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
    long nCmdExecState = oSysState.Get_ExecStatus();
...
long CSttNativeTestEngine::SendCmd(CSttCmdBase *pCmd,long nTimeOut,BOOL bCompress,BOOL bReset)
{
    ...
    return m_pSttCloudClientSocket->XSendCmdAsync(pCmd,nTimeOut,this,bCompress,bReset);
}
...
long CSttNativeTestEngine::SendCmdSync(CSttCmdBase *pCmd,long nTimeOut,CSttCmdData *pRetData,BOOL bDoEvents)
{
    ...
    return m_pSttCloudClientSocket->XSendCmdSync(pCmd,nTimeOut,pRetData,bDoEvents);
}
...
long CSttNativeTestEngine::Ats_InputData(CSttXcmdChInterface *pClientSocket,CDataGroup *pDatas,CExBaseList *pMsgs)
{
    ...
    oAtsCmd.Set_Xml_Pkg_With_Name(TRUE);     return data with name
...
CSttTestClientUser* CSttNativeTestEngine::User_Login(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    CSttTestClientUser *pUser = NULL;
...
long CSttNativeTestEngine::Process_Cmd_System_Request(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    //0:不可授权，1：已授权，2、可授权但未授权
...
long CSttNativeTestEngine::Process_Cmd_System_Authority(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    //0:不可授权，1：已授权，2、可授权但未授权
...
long CSttNativeTestEngine::Process_Cmd_Ats(CSttXcmdChInterface *pClientSocket
...
{//更新当前自动测试命令
    ...
    if (g_nSttLogServerDebugInfor)
    {
        CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, ">> rcv cmd [%s]............", pAtsCmd->m_strID.GetString());
    }
...
long CSttNativeTestEngine::Process_Cmd_Ats(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
// #ifdef _SttTestServer_For_Debug_
...
// #endif
	...
	else if (oAtsCmd.m_strID == STT_CMD_TYPE_ATS_UpdateReportHead)
	{
		return Process_Cmd_Ats_UpdateReportHead(pClientSocket, oAtsCmd);
	}
	else if (oAtsCmd.m_strID == STT_CMD_TYPE_ATS_UpdateCharacteristic)
	{
		return Process_Cmd_Ats_UpdateCharacteristic(pClientSocket, oAtsCmd);
	}
	else
    {
        return 0;
...
long CSttNativeTestEngine::Process_Cmd_Test(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, char *pszCmdID, char *pszTestor)
{
	char* pszXmlBuffer = new char[2 * nLen + 10];
	memset(pszXmlBuffer, 0, 2 * nLen + 10);
	CSttTestCmd *pTestCmd = new CSttTestCmd();
...
void CSttNativeTestEngine::Return_Ats_Success(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd, unsigned int nState, CDvmDataset *pDatas, CExBaseObject *pGuideBook)
{
    CSttSysState oSysState;
...
long CSttNativeTestEngine::Process_Cmd_Ats_CreateTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_StartTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_StopTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_CloseTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_TestItem(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_TestFrom(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_TestAllFailedItems(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_GetItemReport(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_GetReportFile(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_GetSystemState(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
<sys-state>
*/
long CSttNativeTestEngine::Process_Cmd_Ats_CreateDevice(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
/*
long CSttNativeTestEngine::Process_Cmd_Ats_ConnectDevice(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_ConfigDevice(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
...
<sys-state>
*/
long CSttNativeTestEngine::Process_Cmd_Ats_GetDataset(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
<sys-state>
*/
long CSttNativeTestEngine::Process_Cmd_Ats_SetDataset(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
...
*/
long CSttNativeTestEngine::Process_Cmd_Ats_RunProcedure(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
*/
long CSttNativeTestEngine::Process_Cmd_Ats_CloseDevice(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
*/
long CSttNativeTestEngine::Process_Cmd_Ats_ConfigEngine(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_FinishInputData(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_QueryAtsFile(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_SetItemState(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_GenerateTemplate(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_GenerateItems(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
    ...
    if (nRet == 0)
    {
		...
		else if (strGenerateItems == STT_CMD_PARA_generate_items_create_template)
		{
			//创建模板，pItems为测试前准备
			...
			//如果是create-template指令，应当返回全局数据和系统参数
			pGlobalDatas = new CDataGroup;
            pGlobalDatas->m_strName = g_sLangTxt_GlobalData;//_T("全局数据");
            ...
            pSysPara->m_strName = g_sLangTxt_SystemPara; //_T("系统参数");
		...
		else if (strGenerateItems == STT_CMD_PARA_generate_items_macro_add)
		{
			CSttParas *pParas = oSysState.GetSttParas();

			CMacroTest *pMacroTest = m_pSmartTest->GetSttMacroTest_AtsGen();

			if (pMacroTest != NULL)
			{
				CItems *pCItems = (CItems *)pMacroTest->GetAncestor(GBCLASSID_ITEMS);  //paras节点目前只能携带items
				CString strParentItemPath;
				pGenerateItems->GetDataValue(STT_CMD_PARA_ItemsPath, strParentItemPath);
				pParas->AddNewData(STT_CMD_PARA_ParentItemsPath, strParentItemPath);

#ifdef STT_CMD_PARAS_HAS_GUIDEBOOK
				CSttItems *pSttItems = new CSttItems();
				CSttCmdGuideBookTransTool oTool;
				oTool.Trans(pCItems, pSttItems);

				pRetParas->AddTail(pSttItems);
#endif
			}
		}
		else
		{
			if (m_pSmartTest->IsInternational())
			{
				pItems = m_pSmartTest->GetSttItems_AtsGenEx();  //�国际版，返回当前选中的，子节点为追加的项目
...
long CSttNativeTestEngine::Process_Cmd_Ats_GenerateMacroTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_SetItemPara(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Procss_Cmd_Ats_SetItemPara_SystemConfig(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
...
long  CSttNativeTestEngine::Process_Cmd_Ats_ManualTrigger(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_SaveTemplate(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_SaveTest(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_ExportWordRpt(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_OpenTemplate(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
*/
long CSttNativeTestEngine::Process_Cmd_Ats_OpenGbrptFile(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
*/
long CSttNativeTestEngine::Process_Cmd_Ats_AdjustMainWnd(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_ExportReport(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_QueryItem(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_UpdateItem(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_AdjustReport(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_ClearReportsRslts(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	 ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine::Process_Cmd_Ats_UpdateReportHead(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    /*
	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
    ...
    long nRet = m_pSmartTest->Ats_UpdateReportHead(&oAtsCmd, NULL);
...
long CSttNativeTestEngine::Process_Cmd_Ats_UpdateCharacteristic(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
/*	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
	...
	long nRet = m_pSmartTest->Ats_UpdateCharacteristic(&oAtsCmd, NULL);
...
void CSttNativeTestEngine::Return_NoDevice(CSttXcmdChInterface *pClientSocket, CSttSysState &oSysState)
{
    oSysState.Set_ExecStatus_Failure();
...
void CSttNativeTestEngine::Return_Ats_Event(CSttXcmdChInterface *pClientSocket, CSttSysState &oSysState)
{
    if (pClientSocket != NULL)
    {
...
long CSttNativeTestEngine::ValidateSmartTest(CSttXcmdChInterface *pClientSocket, CSttSysState &oSysState)
{
    if (m_pSmartTest != NULL)
    {
        return 1;
...
void CSttNativeTestEngine::Return_DeviceParameter_AfterLogin(CSttXcmdChInterface *pClientSocket, CSttSysState &oSysState)
{
    ...
    strVersion = _T("2024.06.08 - 16:33");
    ...
    long nRet = pClientSocket->XSendSysState(&oSysState);
	CSttCmdDefineXmlRWKeys::g_pXmlKeys->m_nXmlOnlyWrite_Id_Value++;
}
