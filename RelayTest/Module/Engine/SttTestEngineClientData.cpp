...
long g_nTimeOut_Meas = 3000; 

  生成模板的时候，带调试信息，例如：返回默认报告数据用于调试报告填写等
long g_nAtsGenTemplateForDebug = 0;
long g_nSttXcmdCh_Type = SttXcmdChType_SttClientSocket;
...
BOOL CSttTestEngineClientData::IsConnectServerSucc()
{
	...
	if (! m_pSttClientSocket->IsSttXcmdChValid())
	{
		return FALSE;
	}
...
BOOL CSttTestEngineClientData::ConnectServer(const CString &strIPServer, long nPort){return FALSE;}

void CSttTestEngineClientData::OnSocketClose(CSttXcmdChInterface *pSocket, BOOL bDeleteSocket)
{
	m_bTestStarted = FALSE;
...
void CSttTestEngineClientData::DisConnect()
{
	if (m_pSttClientSocket != NULL)
	{
		m_pSttClientSocket->CloseSttXcmdCh();
	}
}
...
long CSttTestEngineClientData::SendBuffer(BYTE *pBuf,long nLen)
{
    ...
    return m_pSttClientSocket->XSendBuffer(pBuf, nLen);
}
										   ...
										   ,BOOL bDoEvents,BOOL bCompress,BOOL bReset)
{
	...
	return m_pSttClientSocket->XSendCmdSync(pCmd,nTimeOut,pRetData,bDoEvents,bCompress,bReset);
}
											...
											, BOOL bCompress,BOOL bReset)
{
	...
	return m_pSttClientSocket->XSendCmdAsync(pCmd,nTimeOut, pSttCmdOverTimeMsgRcv,bCompress,bReset);
}
...
long CSttTestEngineClientData::SendCmdOnly(CSttCmdBase *pCmd, BOOL bCompress,BOOL bReset)
{
	...
	long nRet = m_pSttClientSocket->XSendCmd(pCmd,bCompress,bReset);
...
#endif
	m_oCurrUser.IP_User(m_pSttClientSocket->SttXmdChAddr());
	m_oCurrUser.Name_User(m_pSttClientSocket->SttXmdChAddr());
...
long CSttTestEngineClientData::Test_GetSystemState(CStringArray &astrTypes, BOOL bDoEvents,CSttCmdData *pRetData)
{
	...
	CSttSystemCmd oTestCmd;   由TestCmd改为SystemCmd
...
long CSttTestEngineClientData::Test_SendHeartbeat()
{
	...
	return m_pSttClientSocket->XSendBuffer(pSendBuf,STT_PKG_HEAD_LEN);
}
...
long CSttTestEngineClientData::Comtrade_SendDataStart(long nIndex,long nSendLen)
{
	...
	return m_pSttClientSocket->XSendBuffer(pSendBuf,STT_PKG_HEAD_LEN+nDataLen);
}
...
long CSttTestEngineClientData::Process_Cmd_Ats_InputData(CSttXcmdChInterface *pClientSocket, CSttAtsCmd &oAtsCmd)
{
	if (m_pSttClientSocket == NULL)
	{
		return 0;
													...
													, bool bDoEvents, CSttCmdData *pRetData,const CString &strDvmFile)
{
	...
	pParas->AddNewData(STT_CMD_PARA_HasDigital, nHasDigital);  
	pParas->AddNewData(STT_CMD_PARA_DvmFile, strDvmFile/*_T("SttIecRecordDetectDvm.xml")*/);  //20221011 zhouhj 暂时先直接赋值,后期考虑改进传参方式,或增加成员变量方式处理

	
	if (g_nAtsGenTemplateForDebug == 1)
	{
		pParas->AddNewData(STT_CMD_PARA_GenForDebug, g_nAtsGenTemplateForDebug);  
	}
...
long CSttTestEngineClientData::Ats_GenerateTemplate(CDataGroup *pGenParas, const CString &strRetMode
													, long nHasDigital, bool bDoEvents, CSttCmdData *pRetData,const CString &strDvmFile)
{
	if (m_pSttClientSocket == NULL)
	{
		return 0;
...
//		strPath += "Ats_GenerateItems.xml";
//		oAtsCmd.SaveXmlFile(strPath, CSttCmdDefineXmlRWKeys::g_pXmlKeys);
		return SendCmdSync(&oAtsCmd,g_nTimeOut_Ats,pRetData,bDoEvents);
	}
...
long CSttTestEngineClientData::Ats_CreateTemplate(const CString &strTestMacroUI_ID, const CString &strDeviceName, const CString &strDeviceID, const CString &strDvmFile,
						long nHasDigital,bool bDoEvents,CSttCmdData *pRetData)
{
	if (m_pSttClientSocket == NULL)
	{
		return 0;
	...
	CSttParas *pParas = oAtsCmd.GetSttParas();
	pParas->AddNewData(STT_CMD_PARA_HasDigital, nHasDigital);  
	pParas->AddNewData(STT_CMD_PARA_TestMacroUI_ID, strTestMacroUI_ID);  
	CDataGroup *pGenerateGroup = new CDataGroup;
	pGenerateGroup->m_strName = STT_CMD_PARA_generate_items;
	pGenerateGroup->m_strID = STT_CMD_PARA_generate_items;
	pGenerateGroup->m_strDataType = STT_CMD_PARA_generate_items;
	pGenerateGroup->m_strValue = STT_CMD_PARA_generate_items_create_template;
	pParas->AddNewChild(pGenerateGroup);
	CDataGroup *pDeviceGroup = new CDataGroup;
	pDeviceGroup->m_strID = XPARA_ID_device;
	pGenerateGroup->AddNewChild(pDeviceGroup);
	pDeviceGroup->AddNewData(STT_CMD_PARA_DeviceName,strDeviceName);
	pDeviceGroup->AddNewData(STT_CMD_PARA_DeviceID,strDeviceID);
	pDeviceGroup->AddNewData(STT_CMD_PARA_DvmFile, strDvmFile);
	return SendCmdSync(&oAtsCmd,g_nTimeOut_Ats,pRetData,bDoEvents);
}

long CSttTestEngineClientData::Ats_UpdateReportHead(CDataGroup *pReportHead, bool bDoEvents, CSttCmdData *pRetData)
{
	if (m_pSttClientSocket == NULL)
	{
		return 0;
	...
	CSttAtsCmd oAtsCmd;
	oAtsCmd.m_strID = STT_CMD_TYPE_ATS_UpdateReportHead;
	oAtsCmd.m_strTestor = STT_SOFT_ID_ATS;

	CSttParas *pParas = oAtsCmd.GetSttParas();
	pParas->AddNewChild((CExBaseObject*)pReportHead->Clone());
	return SendCmdSync(&oAtsCmd, g_nTimeOut_Ats, pRetData, bDoEvents);
}

long CSttTestEngineClientData::Ats_UpdateCharacteristic(CSttContents *pChar, const CString &strParentItemsPath, bool bDoEvents, CSttCmdData *pRetData)
{
	if (m_pSttClientSocket == NULL)
	{
		return 0;
	...
	CSttAtsCmd oAtsCmd;
	oAtsCmd.m_strID = STT_CMD_TYPE_ATS_UpdateCharacteristic;
	oAtsCmd.m_strTestor = STT_SOFT_ID_ATS;

	CSttParas *pParas = oAtsCmd.GetSttParas();
	pParas->AddNewChild((CExBaseObject*)pChar->Clone());
	pParas->AddNewData(STT_CMD_PARA_ParentItemsPath, strParentItemsPath);
	return SendCmdSync(&oAtsCmd, g_nTimeOut_Ats, pRetData, bDoEvents);
}
...
{
	long nCmdExecState = oSysState.Get_ExecStatus();
	//zhouhj 2024.10.9 在返回结果报文处理完成,再通知当前命令已执行完成,否则如添加单个测试项目时,对应添加命令已经执行完成,但实际测试功能对象还未添加到测试模板中
//	CSttCmdOverTimeTool::OnSocketReceive_FixedCmd(m_pSttClientSocket,oSysState,nCmdExecState);
...
{
	return m_pSttClientSocket->XSttDownloadFile(strSrcFileRealPath,strDestFilePath,0,pProgress);
}
...
{
	return m_pSttClientSocket->XSttSendFile(strSrcFilePath,strDestFileRealPath,0,pProgress);
}
...
long CSttTestEngineClientData::Process_SysState_System_License(CSttSysState &oSysState)
{
    long nCmdExecState = oSysState.Get_ExecStatus();
    if (m_pTestEventRcv != NULL)
    {
        m_pTestEventRcv->OnRecvLicense(oSysState);
    }
...
long CSttTestEngineClientData::Process_SysState_Exception(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_Report(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_Event(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_RtData(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_Packet(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_Update(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
long CSttTestEngineClientData::Process_SysState_Log(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
/*
	CSttParas *pParas = oSysState.GetSttParas();
	POS pos = pParas->GetHeadPosition();
...
long CSttTestEngineClientData::Process_SysState_Meas(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
    if (m_pTestEventRcv == NULL)
    {
        return 0;
...
long CSttTestEngineClientData::Process_SysState_Ats_Stop(CSttXcmdChInterface *pClientSocket,CSttSysState &oSysState)
{
	if (m_pTestEventRcv == NULL)
	{
		return 0;
...
void CSttTestEngineClientData::OnSendHeartbeatTimer_NotCheakAuthority()
{
	...
	{  使用命令通道接口对象，兼容Socket和自动测试库
		m_pSttClientSocket->XSendHeartbeatCmd();
		//		CLogPrint::LogString(XLOGLEVEL_TRACE, _T("发送心跳...."));
	}
}

void CSttTestEngineClientData::AddPkgDispatch(CSttPkgDispatchInterface *pPkgDispatch){};
void CSttTestEngineClientData::RemovePkgDispatch(CSttPkgDispatchInterface *pPkgDispatch){};
