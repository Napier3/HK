...
void OnRtData_SttGlobalRtDataMngr(CDataGroup *pRtDataGroup)
{
	...
	if (pRtDataMngr->m_nMode == STT_RT_DATA_MODE_SYN)
	{
		...
		if (pDataGroup != NULL)
		{
			stt_GetDataValueByID(pDataGroup, _T("Z"), pMacroChs->m_oZ_Fault.m_oRtDataCh.m_fMag);
			stt_GetDataValueByID(pDataGroup, _T("ZAng"), pMacroChs->m_oZ_Fault.m_oRtDataCh.m_fAng);
...
void CSttPowerTestEngineBase::SendIecDetect(long bEnable)
{
	CDataGroup oGroup;
...
void CSttPowerTestEngineBase::GenerateTestCmdFromParas(CSttTestCmd &oSttTestCmd,bool bSel){};
void CSttPowerTestEngineBase::UpdateTestCmdFromParas(CSttTestCmd &oSttTestCmd,bool bSel){};
void CSttPowerTestEngineBase::GenerateTestCmdFromIEC(CSttParas &oSttParas){};
void CSttPowerTestEngineBase::On_Process_SysState(CSttSocketDataBase *pClientSocket, CSttSysState &oSysState){};
void CSttPowerTestEngineBase::OnTestResults(CDataGroup *pResults){};
void CSttPowerTestEngineBase::OnTestInitialize(CEventResult *pEventInfo,CDataGroup *pParas){};
long CSttPowerTestEngineBase::OnUpdateSyncTime(CDataGroup *pSyncTime){ return 0;};//zhouhj 2022-6-9  lijunqing
void CSttPowerTestEngineBase::On_IecDetect(CDataGroup *pIecCb) {}
