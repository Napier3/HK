...
long CSttTestServerBase::Process_Cmd_Test(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, char *pszCmdID, char *pszTestor)
{
    ...
    {
        char *pString = (char*)(pBuf+STT_PKG_HEAD_LEN);
        qDebug() << pString;
...
{
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("CSttTestServerBase::InitSttServerEngine,  nTestPort: %d, nReplayPort: %d"), nTestPort, nReplayPort);
    qDebug()<<"CSttTestServerBase::InitSttServerEngine";
#ifndef _STT_NOT_IN_TEST_SERVER_
    qDebug()<<"_STT_NOT_IN_TEST_SERVER_";
    stt_test_new_drvparas_buff();
    stt_test_new_drvresult_buff();
    stt_create_test_event_cycle_array();
    stt_create_rtdatas_cycle_array();
#endif
    qDebug()<<"_STT_NOT_IN_TEST_SERVER_22";
    ...
    if(nReplayPort > 0)
    {
        qDebug()<<"_STT_NOT_IN_TEST_SERVER_33";
...
{
    qDebug()<<"CSttTestServerBase::ExitSttServerEngine()";
...
long CSttTestServerBase::Process_Cmd_Test_StartTest(CSttXmlSerializeTool &oSttXmlSerializeTool)
{
    ...
    {
        qDebug() << "pCurrTest->m_strID " << pCurrTest->m_strID;
