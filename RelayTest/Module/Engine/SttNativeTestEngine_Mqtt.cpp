...
void CSttNativeTestEngine_Mqtt::MqttMsgProcess(CSttMqttTopicParser &oSttMqttTopicParser)
{
	...
	if (pszAtsCmd == NULL)
	{
	 ...
	 动态访问模式， m_nObjectRefCount初始值为0，所以手动调用AddRef
...
//    strFile += _T("AtsCmd_Received.xml");
//    oAtsCmd.SaveXmlFile(strFile, CSttCmdDefineXmlRWKeys::g_pXmlKeys);
...
long CSttNativeTestEngine_Mqtt::Process_Cmd_Ats_ClearReportsRslts(void *pClientSocket, CSttAtsCmd &oAtsCmd)
{
    CSttSysState oSysState;
    oSysState.UpdateSysStateHead(&oAtsCmd);
     ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine_Mqtt::Process_Cmd_Ats_UpdateReportHead(void *pClientSocket, CSttAtsCmd &oAtsCmd)
{
/*	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
long CSttNativeTestEngine_Mqtt::Process_Cmd_Ats_UpdateCharacteristic(void *pClientSocket, CSttAtsCmd &oAtsCmd)
{
/*	CSttSysState oSysState;
	oSysState.UpdateSysStateHead(&oAtsCmd);
	ReturnExecReply(&oAtsCmd, STT_CMD_ExecStatus_ACK);
...
void CSttNativeTestEngine_Mqtt::Return_DeviceParameter_AfterLogin(void *pClientSocket, CSttSysState &oSysState)
{
	...
	if (pDevice != NULL)
	{
		...
		if (pDeviceAttrs != NULL)
		{
			...
			if (pAppVersion == NULL)
			{
                pDeviceAttrs->AddNewData(_T("SttTestMainCore版本"), _T("SttTestMainCore_Ver"), _T("AppVersion"), _T("2024.06.08 - 16:33"));
			}
			else
			{
                pAppVersion->m_strValue = _T("2024.06.08 - 16:33");
			}
		}
	}
