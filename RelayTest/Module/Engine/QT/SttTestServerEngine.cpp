...
CSttTestServerEngine* CSttTestServerEngine::Create(int fd, char *pszIP, int nTestPort,int nReplayPort)
{
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("CSttTestServerEngine* CSttTestServerEngine::%d"),m_nSttTestServerEngineRef);
    qDebug()<<"CSttTestServerEngine* CSttTestServerEngine::Create";
	if (m_nSttTestServerEngineRef == 1)
	{
         qDebug()<<"CSttTestServerEngine* 1111111";
...
{
    qDebug() << "void CSttTestServerEngine::Init(int fd)";
#ifdef _PSX_QT_LINUX_
	...
	m_bRunThread = TRUE;
	pthread_t nthreadid;
    qDebug() << ">>>>>>Create TestServerEngine_ProTimerThread1111";
    int nRet = pthread_create(&nthreadid, NULL, ProTimerThread, (LPVOID)this);//xuhm 20241211
    qDebug() << ">>>>>>Create TestServerEngine_ProTimerThread2222";
  if(nRet == 0)
	{
...
{
    qDebug() << ">>>>>>void *ProTimerThread(LPVOID pParam)";
