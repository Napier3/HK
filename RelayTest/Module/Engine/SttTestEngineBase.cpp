...
{
    qDebug() << "CSttTestEngineBase::OnTestMsg";
/*    char *pNewBuff = (char*)malloc(nLen+10);
    memcpy(pNewBuff, pBuf, nLen);
    QSttTestEngineMsgRcv::emit_OnTestMsg((void*)this, (void*)pClientSocket, pNewBuff, nLen);
    return  0;
*/
...
long CSttTestEngineBase::Call_OnTestMsg(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen)
{
    ASSERT(pClientSocket != NULL);
...
BOOL CSttTestEngineBase::OnTestMsgEx(CSttXcmdChInterface *pClientSocket, BYTE *pBuf, long nLen, long nCmdType, char *pszCmdID, char *pszTestor, char *pszRetCmdType, BOOL &bRet)
{
    ...
    qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
    qDebug() << "nCmdType : " << nCmdType;
	if (nCmdType == STT_CMD_TYPE_SYSTEM)
	{
        char *pString = (char*)(pBuf+STT_PKG_HEAD_LEN);   //--tlw-2025-02-10
        qDebug() << pString;                              //--tlw-2025-02-10
        bRet = Process_Cmd_System(pClientSocket,pBuf, nLen, pszCmdID, pszTestor);   //CSttTestServerBase::Process_Cmd_System
	}
	else if (nCmdType == STT_CMD_TYPE_TEST)
	{
        bRet = Process_Cmd_Test(pClientSocket,pBuf, nLen, pszCmdID, pszTestor);     //CSttTestServerBase::Process_Cmd_System
	}
	...
	{
        bRet = Process_Cmd_Adjust(pClientSocket,pBuf, nLen, pszCmdID, pszTestor);  //CSttTestServerBase::Process_Cmd_Adjust
	}
    ...
    else if (nCmdType == STT_CMD_TYPE_COMTRADE)   //comtrade replay
	{
        bRet = Process_Cmd_Comtrade(pClientSocket,pBuf, nLen, pszCmdID, pszTestor);     //CSttTestServerBase::Process_Cmd_System
	}
    qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
	return bRet;
}
