...
{//登录失败，则点对点返回，成功则广播
    ...
    {//录波登录
		if(g_nLogPrintIsZN)
		{
            //CLogPrint::LogFormatString(XLOGLEVEL_DEBUG,_T("录波通道建立:%d"),pClientSocket->m_hSocket);
		}
		else
		{
    ...
    if(pUser != NULL)
    {
		...
		oDevAttrsGroup.AddNewData(_T("TestMngr版本"),_T("MngrVer"),_T("MngrVersion"),oDeviceAttrs.m_strMngrVer);
		...
		oDevAttrsGroup.AddNewData(_T("主板ST口模式可设置"),_T("STModeSet"),_T("STModeSet"),strSTModeSet);
		CString strStateCount;
		strStateCount.AppendFormat(_T("%d"),oDeviceAttrs.m_nStateCount);
		oDevAttrsGroup.AddNewData(_T("状态序列容量(个)"),_T("StateCount"),_T("StateCount"),strStateCount);
...
long CSttTestEngineServerData::ProcessCmd_SetSysConfig(CSttXcmdChInterface *pClientSocket, CSttSystemCmd &oSysCmd)
{
    ...
    if (listGroup.GetCount() > 0)
    {
        ...
        if (nCount > 0)
        {
			stt_file_save_subips(astrIP, astrMask, astrGateWay, nMetric);//存文件
			stt_net_set_subips(astrIP, astrMask,2);//20220526 zhoulei '1'预留给DHCP功能了  stt_net_dhcp();
