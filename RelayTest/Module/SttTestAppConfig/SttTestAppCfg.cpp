...
long CSttTestAppCfg::Open(const CString &strFile)
{
	m_strFile.Empty();
    ...
    {
        if (strFile.Find('\\') >= 0 || strFile.Find('/') >= 0)
        {//为文件路径，兼容windows和linux
            strTemp = strFile;
        }
        else
        {
            strTemp = _P_GetConfigPath() + strFile;
        }
    }

    if (! IsFileExist(strTemp))
    {
        strTemp = GetOldCfgFile();
    }
...
BOOL CSttTestAppCfg::SttOpen(const CString &strFile)
{
	CString strTemp = strFile;
...
BOOL CSttTestAppCfg::stt_xml_serialize(CSttXmlSerializeBase *pXmlSierialize, long nIndex)
{
	...
	pSerialize->xml_serialize("testapp-sn","testapp-sn","","string", m_strTestAppSN);
...
//shaolei 20211008
BOOL CSttTestAppCfg::CopyOwn(CBaseObject* pDest)
{
	if(this == pDest)
	{
		return TRUE;
	...
	CSttTestAppCfg *p = (CSttTestAppCfg*)pDest;
	p->m_strMacroFileID = m_strMacroFileID;
	p->m_strType = m_strType;
	p->m_strLocalIP = m_strLocalIP;
	p->m_strTestAppIP = m_strTestAppIP;
	p->m_nTestServerPort = m_nTestServerPort;
	p->m_nNativeServerPort = m_nNativeServerPort;
	p->m_strMulticastIP = m_strMulticastIP;
	p->m_nMulticastPort = m_nMulticastPort;
	p->m_strCloudIP = m_strCloudIP;
	p->m_nCloudServerPort = m_nCloudServerPort;
	p->m_strCopName = m_strCopName;
	p->m_strUserName = m_strUserName;
	p->m_nMultiLocalPort = m_nMultiLocalPort;
	p->m_strTestAppSN = m_strTestAppSN;
...
BOOL CSttTestApps::stt_xml_serialize(CSttXmlSerializeBase *pXmlSierialize)
{
	CSttXmlSerializeBase *pSerialize =pXmlSierialize->xml_serialize("test-apps config", "test-apps", "test-apps", stt_ParaGroupKey());
	...
	if (stt_xml_serialize_is_read(pXmlSierialize))
	{
		...
		while (TRUE)
		{
			p = new CSttTestAppCfg();

			if (p->stt_xml_serialize(pSerialize, nIndex))
			{
				AddNewChild(p);
				nIndex++;
			}
	...
	else
	{
		...
		while (pos != NULL)
		{
			p = (CSttTestAppCfg *)GetNext(pos);
			p->stt_xml_serialize(pSerialize, nIndex);
			nIndex++;
		}
	}
...
long CSttTestApps::Open()
{
	...
	m_strFile += _T("TestAppsConfig.xml");
	...
	CSttXmlSerializeTool oTool;
	CSttXmlSerializeBase *pXmlSierialize = oTool.CreateXmlSerializeRead(m_strFile);
