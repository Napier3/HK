...
void CSttPic200dIoTest::TtmToDrv()
{
    ...
    drv_reg_ioctl(__FILE__, __LINE__, CSttDeviceBase::g_pSttDeviceBase->m_nCommfd,0x010b,nVal);
    nVal = 0xffffffff;
    for(int i =0 ;i<16;i++)
    {
        if(m_nIO2[i] == 0)
    ...
    drv_reg_ioctl(__FILE__, __LINE__, CSttDeviceBase::g_pSttDeviceBase->m_nCommfd,0x010d,nVal);
    nVal = 0xffffffff;
    for(int i =0 ;i<8;i++)
    {
        if(m_nIO3[i] == 0)
    ...
    drv_reg_ioctl(__FILE__, __LINE__, CSttDeviceBase::g_pSttDeviceBase->m_nCommfd,0x010f,nVal);
#endif
}
