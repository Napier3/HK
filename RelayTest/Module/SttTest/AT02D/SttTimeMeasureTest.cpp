...
void CSttTimeMeasureTest::TtmToDrv()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, nFd,0x301,0);
	drv_reg_wite(__FILE__, __LINE__, nFd,(char*)&para,sizeof(Drv_StatePara));
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	drv_reg_ioctl(__FILE__, __LINE__, nFd,0x301,1);
	drv_reg_wite(__FILE__, __LINE__, nFd,pBuf,nSize);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
#endif
}
