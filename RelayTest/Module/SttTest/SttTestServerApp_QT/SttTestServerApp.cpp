...
#include <QDebug>
 ...
 不同的产品，使用的方式是不同的，例如：AT02D使用分配内存方式
...
void CSttTestServerApp::InitSttTestServerBaseApp()
{
    ...
    CLogPrint::LogString(XLOGLEVEL_TRACE,"InitSttTestServerBaseApp");
	...
	{
        qDebug()<<"Common/Language/LanguageConfig.xml-----OK";
		char pszBuf[100];
        ...
        qDebug()<<"Common/Language/LanguageConfig.xml-----Failed";
    }
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("222stt_open_adjust_file result=%d"),bRet);
    ...
    CSttCmdOverTimeTool::Create();
     CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("2stt_open_adjust_file result=%d"),bRet);
#ifdef _PSX_QT_LINUX_
      CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("3stt_open_adjust_file result=%d"),bRet);
    int fd=CSttDeviceBase::g_pSttDeviceBase->InitDevice(bRet);      //-------init device hardwar--------//
     CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("4stt_open_adjust_file result=%d"),bRet);
#endif
     CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("44stt_open_adjust_file result=%d"),bRet);
    ...
    system(strCmd.toUtf8());
    system("sync");
 CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("5stt_open_adjust_file result=%d"),bRet);
    //解读SttNativeCfg.xml
    ...
    if(oNativeCfg.Open())
    {
		...
		g_theDeviceSystemParameter->m_oDeviceAttrs.m_nIndex_System =
				pNativeLiveUpdate->m_nIndex_System;
    }

 CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("6stt_open_adjust_file result=%d"),bRet);
   ...
   CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("7fd=%d"),fd);
    g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);//xuhm 20241210
    drv_reg_read(__FILE__, __LINE__, fd, buf_tes, 0x205);//xuhm 20241210
    oSysUtc=*(Drv_SysUtc *)buf_tes;
     CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("oSysUtc.tmFlag=%d,oSysUtc.tm_l=%d,oSysUtc.tm_h:%d"),oSysUtc.tmFlag,oSysUtc.tm_l,oSysUtc.tm_h);
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("sizeof(oSysUtc): %d"),sizeof(Drv_SysUtc));
      // drv_reg_read(__FILE__, __LINE__, fd, (char *)&oSysUtc, 0x205);
    g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
   CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("8stt_open_adjust_file result=%d"),bRet);
    g_dwSttAdjSysParaCurrVersion = oSysUtc.tm_l;//以系统启动时刻纳秒部分作版本标识
#endif
   CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("9stt_open_adjust_file result=%d"),bRet);
    CString strIP, strMask;
    stt_net_get_deviceip(strIP, strMask);
    QByteArray ba = strIP.toLatin1();
   CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("10stt_open_adjust_file result=%d"),bRet);
    CSttTestServerEngine::Create(fd,ba.data(),STT_PORT_TEST_SERVER,STT_PORT_COMTRADE_SERVER);  // test-server-port=19814  comtrade-server-port=19813

    CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("333stt_open_adjust_file result=%d"),bRet);
}
