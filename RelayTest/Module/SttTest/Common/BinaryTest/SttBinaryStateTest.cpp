...
void CSttBinaryStateTest::SaveLog()
{
	CString strXMLPath;
	strXMLPath = _P_GetDBPath();
	strXMLPath += ("TestLog/BinaryStateTest");
	strXMLPath += CString::number(m_nLogFileCnt);
	strXMLPath += (".txt");
	...
	stt_xml_serialize(&m_oResults, pMacroResults);
	oSttXmlSerializeTool.Stt_AppendFile(strXMLPath);
}
...
long CSttBinaryStateTest::OnTimer(BOOL &bFinish, long nTimerLongMs)
{
    ...
    for(int i = 0; i < nret / m_nDrvResItemLen; i++)
    {
        ...
        switch(m_oDrvResItem.nType)
        {
            ...
            {
                SetTestStarting(FALSE);
				if(m_oResults.m_nresultCount > 0)
				{
					SaveLog();
				}
                bTestFinished = TRUE;
...
void CSttBinaryStateTest::StartTest()
{
    ...
    if(!GetTestStarting())
    {
		...
		else
		{
			...
			strXMLPath = _P_GetDBPath();
			strXMLPath += ("TestLog");
			QDir oDir(strXMLPath);
			if(oDir.exists())
			{
				CString strCmd = "rm -rf ";
				strCmd += strXMLPath;
				strCmd += "/*";
				system(strCmd.GetString());
				system("sync");
			}
			else
			{
				CString strCmd = "mkdir ";
				strCmd += strXMLPath;
				system(strCmd.GetString());
				system("sync");
			}
