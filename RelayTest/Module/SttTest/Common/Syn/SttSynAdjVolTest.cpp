#include "stdafx.h"
#include "SttSynAdjVolTest.h"
#include "../SttSystemConfig.h"
#ifdef _PSX_QT_LINUX_
#include "../../../SttDevice/SttDeviceBase.h"
#endif

CString CSttSynAdjVolTest::m_strMacroName = "调压周期测试";
CString CSttSynAdjVolTest::m_strMacroID = STT_MACRO_ID_SynAdjVolTest;
CString CSttSynAdjVolTest::m_strFilePostFix = "saft";
CString CSttSynAdjVolTest::m_strMacroVer ="1.0";


CSttSynAdjVolTest::CSttSynAdjVolTest()
{
...
void CSttSynAdjVolTest::Init()
{
    CSttSynActVolTest::Init();
}

CSttSynAdjVolTest::~CSttSynAdjVolTest(void)
{

}

void CSttSynAdjVolTest::ReCalVoltCurr(STT_RTDATA_EVENT &oRTDATA)
{
#ifdef _PSX_QT_LINUX_
    int nCommfd = CSttDeviceBase::g_pSttDeviceBase->GetDriverFd();
	int nSize = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nSize = drv_reg_read(__FILE__, __LINE__, nCommfd,(char*)&m_oSyncStepRst,0x250);
...
long CSttSynAdjVolTest::CalReport(UINT nState)
{
  	m_oSynResults.m_fFreqBordValue = 0.0f;
 	m_oSynResults.m_nAct = 0;
    m_oSynResults.m_fUAdjWidth = 0;
    m_oSynResults.m_fUAdjTime = 0;
    ...
    for(int i = 0; i < 8;i++)
    {
        ...
        {
            m_oSynResults.m_fUAdjWidth = oSynResult.m_dTimeBinAct[i][1] -
                    oSynResult.m_dTimeBinAct[i][0];
            m_oSynResults.m_fUAdjTime = oSynResult.m_dTimeBinAct[i][2] -
                    oSynResult.m_dTimeBinAct[i][0];
...
void CSttSynAdjVolTest::Para_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
    stt_xml_serialize_AdjVolCycle(m_pTmtSynParas,pXmlSerialize);
    m_pTmtSynParas->m_nTestType = Stt_Syn_Test_Type_AdjVolCycle;
...
void CSttSynAdjVolTest::Report_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
    stt_xml_serialize_AdjVolCycle(&m_oSynResults,  pXmlSerialize);
}

void CSttSynAdjVolTest::TranslateToStateParas()
{
