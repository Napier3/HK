...
bool stt_open_adjust_file(const char *pszFileName)
{
    ...
    qDebug()<< "strXMLPath is : "<< strXMLPath;
	...
	if(pPara->m_oDeviceAttrs.m_nDCMeas)
	{
		...
		CSttXmlSerializeBase *pDCAdjXmlAttrs = pXmlSerialize->xml_serialize("", "DCAdjustParas", "DCAdjustParas", stt_ParaGroupKey());
			...
			if (pDCAdjXmlAttrs != NULL)
			{
                ...
                //qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
				CSttXmlSerializeBase *pIAdjXmlAttrs = pDCAdjXmlAttrs->xml_serialize("电流", "DCI", "DCAdj", stt_ParaGroupKey());
                //qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
				if (pIAdjXmlAttrs != NULL)
				{
                    //qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
					stt_xml_serialize(&pPara->m_oDCI,pIAdjXmlAttrs);
                    //qDebug() << __FILE__ << __FUNCTION__ << __LINE__;
				}
			}
		}
