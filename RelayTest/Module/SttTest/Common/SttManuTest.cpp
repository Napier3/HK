...
long CSttManuTest::OnTimer(BOOL &bFinish, long nTimerLongMs)
{
    ...
    if(GetTestStarting())
    {
		if(m_fOutPutTime >= 0.001 && IsTestTimeFinished(m_fOutPutTime))
        {
            if(!m_pManualTest->m_oManuParas.m_bAging)
            {
				...
				if(g_nLogPrintIsZN)
				{
					CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("[warning] 输出限时[%f] 时间到"),
											   m_fOutPutTime);
				}
				else
				{
					CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("[warning] OutPutTime[%f] Arrived"),
										 m_fOutPutTime);
				}
...
void CSttManuTest::GetBinStatus()
{
	...
	nret = drv_reg_read(__FILE__, __LINE__, nFd, &oBiStatus, 0x202);
...
void CSttManuTest::AfterSetParameter()
{
	...
	BOOL bRet = FALSE;
#ifdef _PSX_QT_LINUX_
	bRet = CSttDeviceBase::g_pSttDeviceBase->SetOutputTime(m_pManualTest->m_oManuParas.m_uiCUR, m_pManualTest->m_oManuParas.m_bDC,fOutPutTime);
#endif
	if((!m_pManualTest->m_oManuParas.m_bAuto) && bRet)
	{
		//大电流限时
		if(m_pManualTest->m_oManuParas.m_fOutPutTime < 0.0001f)
		{
			//用户未设定限时
		}
		else
		{
			if(fOutPutTime < 0.0001f)
			{
				//硬件不限时，按用户值执行
				fOutPutTime = m_pManualTest->m_oManuParas.m_fOutPutTime;
			}
			else
			{
				//取硬件限时与用户设定的最小值
				if(fOutPutTime > m_pManualTest->m_oManuParas.m_fOutPutTime)
				{
					fOutPutTime = m_pManualTest->m_oManuParas.m_fOutPutTime;
				}
			}
	...
	m_fOutPutTime = fOutPutTime;
	if(m_fOutPutTime > 0.9)
	{
		if(g_nLogPrintIsZN)
		{
			CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("输出限时[%f]秒"),m_fOutPutTime);
		}
		else
		{
			CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("OutPutTime[%f]s"),m_fOutPutTime);
		}
	}
}
    ...
    if(!GetTestStarting())
	{
        ...
        else
        {
            ...
            else if(m_pManualTest->m_oManuParas.m_nFuncType < 3)
            {
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x110, 1);//合并单元角差，开启秒同步
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
...
void CSttMUAccuracyTest::Para_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
	stt_xml_serialize(&m_pManualTest->m_oManuParas, pXmlSerialize);
...
void CSttManuTest::Report_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
	...
	for(int i = 0; i < MAX_BINARYIN_COUNT && i < g_nBinCount; i++)
    {
	...
	for(int i = 0; i < oSwitchModules.m_nModulesBiCount && i < MAX_ExBINARY_COUNT; i++)
    {
