...
void CSttPcapReplay::AfterSetParameter()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x35A, 0);
	drv_reg_wite(__FILE__, __LINE__, nFd, (char *)&m_oPcapParam, sizeof(DrvPcapParam));
#endif
}
...
#ifdef _PSX_QT_LINUX_
	int nFd = CSttDeviceBase::g_pSttDeviceBase->GetDriverFd();
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x35A, 1);
	memcpy(m_pPcapData->dat, pszSrcBuf, nDataLen * 4);
	m_pPcapData->pkglen = nDataLen;
	drv_reg_wite(__FILE__, __LINE__, nFd, (char *)m_pPcapData, sizeof(DrvPcapData));
#endif
//    oDebugFile.drv_reg_wite(__FILE__, __LINE__, (char *)pszSrcBuf, nDataLen * sizeof(float));
//    oDebugFile.flush();
//    system("sync");
//    if(m_bSendFinish)
//    {
...
BOOL CSttPcapReplay::EventProcess()
{
	...
	nret = drv_reg_read(__FILE__, __LINE__, nfd, pDrvResults, 0x203);
...
BOOL CSttPcapReplay::EventProcessEx()
{
	...
	long nRet = 0;
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nRet = drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x501, 0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	if(nRet == REPLAY_FLAG_WRITE_FINISH)
	{
		if(m_bSendFinish)
		{
			...
			g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
			drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x103, 0x12);
			g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			//开始回放
	...
	else if(nRet == REPLAY_FLAG_TEST_FINISH)
	{
		...
		if(m_nIsLoop)
		{
			if(m_nCurReplayLoopIndex++ < m_nLoopNum)
			{
				m_bInLooping = TRUE;
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x146, 0);
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
			}
			else
			{
				...
				g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
				drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x103, 0);
				g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
				bTestFinished = TRUE;
			}
		}
