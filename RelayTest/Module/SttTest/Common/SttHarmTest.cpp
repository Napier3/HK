...
void CSttHarmTest::AfterSetParameter()
{
	...
	BOOL bRet = FALSE;
#ifdef _PSX_QT_LINUX_
	bRet = CSttDeviceBase::g_pSttDeviceBase->SetOutputTime(m_pHarmTest->m_oHarmParas.m_uiCUR,FALSE,fOutPutTime); //大电流限时
#endif

	if((!m_pHarmTest->m_oHarmParas.m_bAuto) && bRet)
	{
		//大电流限时
		if(m_pHarmTest->m_oHarmParas.m_fOutPutTime < 0.001f)
		{
			//用户未设定限时
		}
		else
		{
			if(fOutPutTime < 0.001f)
			{
				//硬件不限时，按用户值执行
				fOutPutTime = m_pHarmTest->m_oHarmParas.m_fOutPutTime;
			}
			else
			{
				//取硬件限时与用户设定的最小值
				if(fOutPutTime > m_pHarmTest->m_oHarmParas.m_fOutPutTime)
				{
					fOutPutTime = m_pHarmTest->m_oHarmParas.m_fOutPutTime;
				}
			}
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_DEBUG, _T("电流限时输出%f秒"),m_fOutPutTime);
	}
}
...
void CSttMUHarmTest::Para_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
	stt_xml_serialize(&m_pHarmTest->m_oHarmParas, pXmlSerialize);
