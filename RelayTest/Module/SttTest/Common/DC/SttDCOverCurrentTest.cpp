...
{
	tmt_StateResult*  presultState = &m_pStateTest->m_oStateResults.m_resultState[1];
	if(presultState->m_nAct)
	{
		for(int i = 0; i < 8; i++)
		{
			m_oDCOverCurrentResults.m_fFirstTripTime[i] = presultState->m_frTimeBinAct[i][0];
		}
	}
