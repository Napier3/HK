...
void CSttBinMeasure::StartTest()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x126, 1);
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x7000407c, nCode);
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x315, 0);
	drv_reg_wite(__FILE__, __LINE__, nFd,(char*)&oBinMeas,sizeof(Drv_BinMeas));
	drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x103, 0x16);
...
long CSttBinMeasure::SetParameter(DWORD dwState)
{
	...
	if(m_nWorkMode == 0)
	{
		...
		Sleep(20);
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x131, 0);
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x126, 0);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	else
	{
		...
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x126, 1);
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x131, 0x03);//数字录波
		drv_reg_ioctl(__FILE__, __LINE__, nFd, 0x7000407c, nCode);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	}
	return 0;
