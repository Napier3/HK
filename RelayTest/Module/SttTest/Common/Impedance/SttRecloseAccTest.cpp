...
{ 
    ...
    m_fFreq = g_theSystemConfig->m_fFNom;
    m_oResults.init();
...
long CSttRecloseAccTest::CalReport(UINT nState)
{
	...
	for (int nIndex = nBeginIndex;nIndex<nEndIndex;nIndex++)
    {
		if (m_nDInputState[nIndex] == STT_IMPEDANCE_BIN_STATE_RECLOCE)
        {
			...
			if(m_pStateTest->m_oStateResults.m_resultState[2].m_nAct)
			{
        ...
        else
        {
			...
			if(m_pStateTest->m_oStateResults.m_resultState[3].m_nAct)
			{
...
void CSttRecloseAccTest::TranslateToStateParas()
{
    ...
    m_pStateTest->m_oStateParas.m_paraState[1].m_fTimeState = m_fTSetting[0]+m_fMarginTime+m_fActionTime;
...
//	m_oFaultCalculatTool.CalPreFaultComplex(oComplexUI[0],fStdLN_Vol,0.0f);//计算故障前三相电压
    ...
    m_pStateTest->m_oStateParas.m_paraState[2].m_uiVOL[3].Harm[1].fFreq = m_fVzFreq;
    ...
    for (int j=0;j<4;j++)
    {
        m_pStateTest->m_oStateParas.m_paraState[3].m_uiVOL[j].Harm[1].fFreq = m_fFreq;
    }
    for (int j=0;j<3;j++)
    {
        m_pStateTest->m_oStateParas.m_paraState[3].m_uiCUR[j].Harm[1].fFreq = m_fFreq;
    }
    m_pStateTest->m_oStateParas.m_paraState[3].m_uiVOL[3].Harm[1].fFreq = m_fVzFreq;
...
void CSttRecloseAccTest::StartTest()
{
	...
	drv_reg_ioctl(__FILE__, __LINE__, nfd, 0x149, 1);
