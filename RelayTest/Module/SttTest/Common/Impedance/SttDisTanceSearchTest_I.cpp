...
#endif
    double dTemp =m_oDistanceSearchResults.m_fImpAngle * pi / 180;
    m_oDistanceSearchResults.m_fImpR = m_oDistanceSearchResults.m_fImp * cos(dTemp);
    m_oDistanceSearchResults.m_fImpX = m_oDistanceSearchResults.m_fImp * sin(dTemp);
	return 0;
}
...
void CSttDisTanceSearchTest_I::Para_XmlSerialize(CSttXmlSerializeBase *pXmlSerialize)
{
	...
	{
        m_oParas.m_fTSetting+=m_oParas.m_fActionTime;
...
// 		m_oParas.m_fCurrScanVal,m_oParas.m_fCurrScanValAngle);
    ...
    //m_pImpBaseParas->m_pStateParas->m_paraState[2].m_fTimeState += m_oParas.m_fActionTime;
    //状态3：跳闸后,等待重合闸状态
