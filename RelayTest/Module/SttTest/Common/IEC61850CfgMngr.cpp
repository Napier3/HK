...
QString DecToBin(QString strDec)//10杩涘埗锟斤拷?2杩涘埗
{
...
void CIEC61850CfgMngr::Create92Packet(tmt_sv_92 &m_92Struct,QByteArray & m_ResultArray,QString m_SavePath, int m_pcapmode)
{
	...
	for (int m = 0; m < t_ASDUNum; m++)
	{
		...
		if(m_92Struct.m_bSmpRate)
		{
			...
			for (int i=1;i>=0;i--)
			{
				char byte = 0;
				byte = nLen>>i*8 & 0xFF;
				t_ASDU->m_SmpRate.push_back(byte); //锟斤拷锟侥筹拷锟斤拷
			}
		}
	...
	else if (m_pcapmode==1)//pcap锟侥硷拷锟斤拷式锟斤拷锟斤拷
	{
		QByteArray t_FHead;
		QByteArray t_PHead1;
		QByteArray t_PHead2;
...
void CIEC61850CfgMngr::CreateGooseDataSetList(QByteArray& oAr, ChanelInfo &chan,char timequality)
{
	...
	if(QString::compare(data_type,"FLOAT32")==0)
	{
		...
		ConverStruct t_s;//锟斤拷锟矫癸拷锟斤拷锟斤拷转
		t_s.f = value;
		oAr.push_back(0x87);
		oAr.push_back(0x05);
		oAr.push_back(0x08);
...
void CIEC61850CfgMngr::CreateGooseData(QByteArray& oAr, tmt_goose &t_GOOSE)
{
	...
	{
		t_iGooseDataPos = t_FLength + 3;//锟斤拷锟斤拷锟斤拷锟捷硷拷
	}
...
#ifdef _AT02D_
	   m_pIECConfig->item[0].module=1<<(4-1);//位置4
#else
...
#ifdef _AT02D_
	   stMaps->boMd[0].info[0][oChanelInfo.nBOIndex-1].stMap[index].module=1<<(4-1);//位置4
#else
...
#ifdef _AT02D_
	   stMaps->boMd[0].info[nFlag][nBoIndex-nFlag*32].stMap[index].module=1<<(4-1);//位置4
#else
