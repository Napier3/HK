		...
		else
		{
			...
			if(file.is_open())
			{
				...
				if(m_pChar != NULL)
				{
					...
					drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x30f,1<<(m_nModuleIndex - 1));
					drv_reg_write(__FILE__, __LINE__, m_nfd,m_pChar,nSize);
...
BOOL CSttUpdateFPGA::EventProcessEx()
{
	...
	g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
	nFlag = drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x502,0);
	g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
	...
	{
		g_oDrvMutex.lock(__FILE__,__FUNCTION__,__LINE__);
		m_nUpdateIndex = drv_reg_ioctl(__FILE__, __LINE__, m_nfd,0x505,0);
		g_oDrvMutex.unlock(__FILE__,__FUNCTION__,__LINE__);
		SendUpdateIndex();
	}
...
void CSttUpdateFPGA::SendUpdateIndex()
{
	...
	while(pos != NULL)
	{
		...
		if(pUser != NULL)
		{
			...
			{
                pUser->m_pRefSttSckt->XSendSysState(&oSysState);
			}
		}
	}
}
