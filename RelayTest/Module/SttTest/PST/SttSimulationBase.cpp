...
//                                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    ...
    for(int i = 0;i< 72000;i++)//按120秒进行初始化
    {
        ...
        for(int j = 0;j < PST_PORT2M_MAX_COUNT;j++)
        {
			...
			nNSecPort += 100000;//间隔100微秒，不然硬件反应不过来
            ...
            if(m_oReplayData2M.cnt == 20000)
            {
                ...
                drv_reg_ioctl(__FILE__, __LINE__, nCommfd,0x309,0);
                drv_reg_wite(__FILE__, __LINE__, nCommfd,(char*)&m_oReplayData2M,sizeof(DrvReplayData2M));
                m_oReplayData2M.cnt = 0;
            }
        }
    }
...
void CSttSimulationBase::Send2MDataToDrv(CPstRcdDataBuffers* p2MReplay,int nCommfd)
{
    ...
    while(TRUE)
    {
        ...
        if(dPreTime > 1)
        {
            ...
            if(dDiff < 0.00005)//间隔50微秒，不然硬件反应不过来
            {
                dTimeCurPoint = dPreTime + 0.00005;
...
void CSttSimulationBase::Replay2MDataLog(CPstRcdDataBuffers* p2MReplay)
{
    ...
    if(file.open(QIODevice::WriteOnly))
    {
        ...
        while(nDataLen > 0)
        {
            ...
            nDataLen -=nRdLen;
            file.drv_reg_wite(__FILE__, __LINE__, pDestBuf,nRdLen);
        }
...
void CSttSimulationBase::SaveRTDSDataDebug(CSttHdChRcdObject* pRtdsSampleRcdObject)
{
    ...
    if(file.open(QIODevice::WriteOnly))
    {
        while(TRUE)
        {
            ...
            if(nReadLen == nLen)
            {
                file.drv_reg_wite(__FILE__, __LINE__, pDestBuf,nLen);
            }
            else
                break;
        }
...
void CSttSimulationBase::SaveRTDSDataDebug(CPstRcdDataBuffer* pRtdsReplayData)
{
    ...
    if(file.open(QIODevice::WriteOnly))
    {
        while(TRUE)
        {
            ...
            if(nReadLen == 1024)
            {
                file.drv_reg_wite(__FILE__, __LINE__, pDestBuf,1024);
            }
            else
                break;
        }
