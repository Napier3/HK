...
QString DecToBin(QString strDec)//10进制?2进制
{
...
void CIEC61850CfgMngr::Create92Packet(tmt_sv_92 &m_92Struct,QByteArray & m_ResultArray,QString m_SavePath, int m_pcapmode)
{
	...
	for (int m = 0; m < t_ASDUNum; m++)
	{
		...
		if(m_92Struct.m_bSmpRate)
		{
			...
			for (int i=1;i>=0;i--)
			{
				char byte = 0;
				byte = nLen>>i*8 & 0xFF;
				t_ASDU->m_SmpRate.push_back(byte); //ij
			}
		}
	...
	else if (m_pcapmode==1)//pcapl'
	{
		...
		{
			t_DataFile.drv_reg_wite(__FILE__, __LINE__, t_FHead, 24);
			t_DataFile.drv_reg_wite(__FILE__, __LINE__, t_PHead1, 8);
			t_DataFile.drv_reg_wite(__FILE__, __LINE__, t_PHead2, 8);
			t_DataFile.drv_reg_wite(__FILE__, __LINE__, t_SaveDataArray);
		}
		t_DataFile.close();
	}
}
...
void CIEC61850CfgMngr::InitPara()
{
	...
	for(int index=0;index<ngoutcount;index++)
	{
		...
		for(int chanid=0;chanid<m_tmt_goose.m_ChanNum;chanid++)
		{
			...
			{
				if(QString::compare(m_strMap.left(4),"bout")==0)
				{
...
//				m_pIECConfig->item[nBoard].sv[m_pIECConfig->item[nBoard].smvCount].group;
	...
	for(int index=0;index<data_gse.count();index++)
	{
		ba.clear();
		tmt_goose &m_tmt_goose=data_gse[index];
...
void CIEC61850CfgMngr::InitBoMap()
{
	...
	for(int i=0;i<data_gse.count();i++)
	{
		tmt_goose otmt_goose=data_gse[i];
		for(int chan=0;chan<otmt_goose.vChanelInfo.count();chan++)
		{
			ChanelInfo oChanelInfo=otmt_goose.vChanelInfo[chan];
			if(oChanelInfo.nBOIndex>0&&oChanelInfo.nBOIndex<9)
			{
...
void CIEC61850CfgMngr::SetGooseValue(tmt_StateParas &oStateParas,int nIndex,drv_state *pDrvState)
{
	...
	pDrvState->GooseChangedItems.datModules[0].group=data_gse.count();

	for(int index=0;index<data_gse.count();index++)
	{
		qDebug()<<"index"<<index;
		tmt_goose otmt_goose=data_gse.at(index);
...
#ifdef DEBUG_MODE
					qDebug()<<"nCount "<<oChanelInfo.nLen;
					qDebug()<<"nPos "<<otmt_goose.nDataSetPos+oChanelInfo.nPos;
#endif
