...
int g_nUITableWidth = 0;	//20241018 huangliang 记录开始时U/I图表刻度的宽度
...
void QSttTestCntrFrameBase::slot_UpdateSysConfig()
{
	...
	if (g_oSttSystemConfig.GetSelSysPata() &&(stt_Frame_TestMacroUI() != NULL) && (m_pMacroEditView!=NULL))//判断当前是否加载了测试功能模块,并判断当前测试功能模块是否为网页
	{
		if (stt_Frame_TestMacroUI()->IsUIOriginal())
		{	
	...
	if (m_pMacroEditView != NULL)
	{
		if (m_pMacroEditView->IsInternationalMode())
		{
			...
			if (pSttMultiMacroParaEditViewBase->ItemStateChanged(pSttItem) && pTestMacroUI->HasCharLib())
			{
				...
				long nFaultType = -1;
				pSttMultiMacroParaEditViewBase->GetMacroIDAndFaultType(pSttItem, strMacroID_Tester, nFaultType);//该函数需要特性曲线模块重载获取对应的故障类型值
...
//		emit sig_UpdateHtmlManuTrigerBtn_Testing();
	...
	if (nClassID == STTGBXMLCLASSID_CSTTMACROCHARITEMS)
	{
		...
		if(pSearchLine)
		{
			...
			//20240805 zhangyq 添加系列后开始试验pSttChar为空闪退
			if (pSttChar == NULL)
			{
				return;
			}
...
//	stt_xml_serialize_write_ReplayTest();
	...
	QSoftKeyBoard::AttachObj(this);
	  ...
	  如果文件不存在，输出提示信息的时候窗口还没创建成功，程序崩溃
...
// 		, this, SLOT(slot_UpdateSearchResults(QString, QString, CSttSearchResults*)), Qt::QueuedConnection);//add wangtao 20241022 刷新特性图搜索点
...
void QSttTestCntrFrameBase::UpdateResultWidget(CSttReport *pReport,const CString &strItemName,const CString &strMacroID)
{
	...
	*/
	...
	{
		//add wangtao 20240902 处理故障递变测试结果
    ...
    //20240906 wanmj 振荡 通过ActDsec判断是否动作
...
void QSttTestCntrFrameBase::OpenDialog(const CString &strDialogID)
{
 ...
 //   CXMessageBox::information(NULL,"002","000000" );
	...
	}//2024-09-09 zhouyangyong新增用于开入量配置
...
//	CLogPrint::LogFormatString(XLOGLEVEL_TRACE,_T("OnReport_ReadDevice函数开始执行."));
	...
	//2024-10-21 wuxinyi 读取主板开入数量及开出数量
...
// 		if (strIDMacroUI == STT_ORG_MACRO_ReplayTest)//故障回放模块,需要更新测试资源 
 		...
 		if (strIDMacroUI == STT_ORG_MACRO_ReplayTest || bBinBoutNumChange)//2024-10-21 wuxinyi 新增开入开出数量变化时更新测试资源
		{
			emit sig_UpdateTestResource();//发送信号更新软件资源
		}
	}
...
void QSttTestCntrFrameBase::InitAllButtonStateByTestMacroUI(CSttMacroTestUI_TestMacroUI *pTestMacroUI)
{
	...
	//自动准同期国际版不使能按键特性曲线编辑 gongyiping 20240731
...
// 
// 
//     //CLogPrint::LogString(XLOGLEVEL_TRACE, ">> end GenerateTemplate >>");chenling 2024.8.13 注释
// 
// 	if (g_nLogDebugInfor == 1)
// 	{
	...
	BOOL bHasCharacter,long nRepeatTimes, long nRptTitle, long nTitleLevel)
{
	if (pParentItems == NULL)
	{
		...
		{
			CLogPrint::LogFormatString(XLOGLEVEL_INFOR,_T("国际版模板生成完成."));
            ...
            ClearAllReports_QT(); //ftt 2024.10.16 add 清空报告
		...
		{
			CLogPrint::LogFormatString(XLOGLEVEL_INFOR,_T("国际版模板生成失败."));
			return false;
		}
	}
    ...
    //AddUpdateReport_QT(pNewItems);//zhouhj 2024.10.16 在创建国际版测试项后,更新其报告
	...
	// #ifdef _PSX_QT_WINDOWS_  //zhouhj 2024.7.3 国际版暂时不增加此操作
...
{
	//2024-8-13 wuxinyi 正在测试过程中不需要再次下发
...
// #endif
    ...
    //CLogPrint::LogString(XLOGLEVEL_TRACE, ">> end GenerateTemplate >>");2024.8.13 注释
											...
											,CSttTestMacroCharParas *pSttTestMacroCharParas)
{
	...
	//2024-10-11 wuxinyi 补充打开模板时初始化右侧视图
	if (pTestMacroUI->HasCharLib())
	{
		g_theTestCntrFrame->InitCharLibWidget();
	}
...
#ifdef _PSX_QT_WINDOWS_
	InitWaveDrawWidget();//波形图
	InitSystemDiagramWidget();//系统图
	InitWiringDiagramWidget();//接线图
#endif
...
void QSttTestCntrFrameBase::GetCharacteristicsFile(CSttMacroTestUI_TestMacroUI *pTestMacroUI, CString &strFile)
{
	if (pTestMacroUI == NULL)
	{
	 ...
	 缺省、默认的特性曲线，保存到和测试参数相同的路径，文件名后面增加_Char
	CString strTitle;
...
// 		}
	...
	if ((strCurrUI_ID == STT_ORG_MACRO_DiffTest_I) && (strMacroID == STT_MACRO_ID_PsuDiffRateTest))//wangtao 20241028 修改id判断为现有宏定义，下同
	{
		if (pSearchResults->GetDataValue("Ir", dXc) && pSearchResults->GetDataValue("CurrSearchVal", dYc))
		{
			m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc);//当前测试点
...
// 					m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc, TESTLINE_ACT_STATE_INELIGIBLE);	//未动作，理论动作点
// 					emit sig_UpdateDrawCharacters();
// 				}
// 			}
// 			else
...
// 					dXc = CString_To_double(pCurrIr->m_strValue);
// 					dYc = CString_To_double(pCurrSearchVal->m_strValue);
// 					m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc, TESTLINE_ACT_STATE_ELIGIBLE);	//测试结果，实际动作点
// 					emit sig_UpdateDrawCharacters();
// 				}
// 			}
// 		}
...
// 				{
// 					dXc = CString_To_double(pRateHarmSet->m_strValue) * 100;//谐波制动系数定值转化为百分比
// 					dYc = CString_To_double(pIdSetting->m_strValue);
// 					m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc, TESTLINE_ACT_STATE_INELIGIBLE);	//未动作，理论动作点
// 					emit sig_UpdateDrawCharacters();
// 				}
...
// 				{
// 					dXc = CString_To_double(pCurrRHarm->m_strValue) * 100;//谐波制动系数定值转化为百分比
// 					dYc = CString_To_double(pCurrId->m_strValue);
// 					m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc, TESTLINE_ACT_STATE_ELIGIBLE);	//测试结果，实际动作点
// 					emit sig_UpdateDrawCharacters();
// 				}
...
// 			}
		...
		else
		{
			...
			{
				m_pCharacteristics->UpdateCurrTestPoint(dXc, dYc, TESTLINE_ACT_STATE_ELIGIBLE);	//未动作，理论动作点
				bUpdateCharactView = TRUE;
			}
...
// 			}
	...
	else if (strMacroID == STT_MACRO_ID_Distance_I || strMacroID == STT_MACRO_ID_ActionTime)
	{
		...
		{
			int nState;
			//20241025 luozibing 根据测试项状态设置测试点状态 除了不合格外，其它都显示合格
...
//	CString strCurrUI_ID = GetCurrentUI_ID();
//	if (strCurrUI_ID == STT_CMD_ATS_MACRO_ID_LowFreqTest || strCurrUI_ID == STT_ORG_MACRO_LowFreqTest_I)//wangtao20240730 增加低周减载国际版状态图处理
//	{
...
{
	if ((stt_Frame_TestMacroUI() != NULL)&&(m_pSttReportViewHtml!= NULL))//zhouhj 20220219 在第二次打开时,删除上一次选择的测试功能的报告
	{
...
void QSttTestCntrFrameBase::Ats_UpdateParameter()
{
	...
	m_pMacroEditView->GetDatas(&oRetParas);
	SaveUIParaFile(&oRetParas);	 记录和保存当前页面设置的值：设置为默认值
...
void QSttTestCntrFrameBase::SysStateReport_OnStateChanged(CEventResult *pEventInfo)
{
	...
	else if (stt_Frame_TestMacroUI()->IsUIWeb()&&HasManuTrigerBtn()&&(pEventInfo->m_nCurrStateIndex == 1))//zhouhj 20220403 在网页模块 进入第二个状态后,都需要将手动触发灰掉
	{
		EnableManualTriggerButton(false);
//		UpdateButtonStateByID(STT_CNTR_CMD_ManuTriger,false,true);
	}
...
void QSttTestCntrFrameBase::OnCmd_CharactEditView()
{
	...
	QSttMultiMacroParaEditViewBase *pSttMultiMacroParaEditViewBase = (QSttMultiMacroParaEditViewBase*)m_pMacroEditView->m_pMacroParaEditWnd;
	//zhouhj 2024.8.14 对于特性曲线相关模块，如特性曲线编辑界面用到测试项相关参数需要通过此函数获取当前功能测试项通用参数
	//差动模块一定要实现改功能，获取对应的参数
...
//	oCharEditMainDialog.m_strCharParasFile = strFile;
	...
	{//20240809 luozibing 特性曲线唯一参数改变，更新相关页面显示
		m_pCharLibWidget->InitCharacteristicsLib("", "");//zhouhj 2024.8.14国际版软件此处不需要更新
//		m_pCharLibWidget->GroupDatasToChar(m_pTestMacroUI_Paras);
//		m_pCharLibWidget->ShowCharacteristics(m_pCharacteristics);
//		m_pCharLibWidget->m_strCharParasFile = strFile;
		...
		oCharEditMainDialog.CharToGroupDatas(m_pTestMacroUI_Paras);//此接口与上面的对应从特性曲线编辑界面获取编辑结果
        //20240830 wanmj 清除m_pCharacteristics里的测试点/线 重新添加
...
//			m_pCharactWidgetForTest->InitCharacteristic(pTestMacroUI->m_strID);
		...
		//在调用UpdateParasByCharEdit接口时,更新到全部测试项,主要用于差动模块
		//20240809 luozibing 更新模块对应定值参数
...
// 	}
	...
	oDeviceModelDlg_Eng.m_nDlgModeType = _FIXED_VALUE_MODIFICATION_TYPE_;//定值修改模式
...
//             m_pCharLibWidget->ShowCharacteristics(m_pCharacteristics);
			...
			oDeviceModelDlg_Eng.CharToGroupDatas(m_pTestMacroUI_Paras);//此接口与上面的对应从特性曲线编辑界面获取编辑结果
			pSttMultiMacroParaEditViewBase->UpdateParasByCharEdit();
        }
		...
		if (oDeviceModelDlg_Eng.m_bHasChangedXrio || oDeviceModelDlg_Eng.m_bHasModifyDevModel)//设备数据模型被修改后,需要更新给自动测试
		{
			if (g_pTheSttTestApp->m_pTestCtrlCntr->Ats_ImportDvmFile(g_theGbSmartGenWzd->m_pDvmDevice) == STT_CMD_ExecStatus_SUCCESS)
			{
				...
				g_theGbSmartGenWzd->SaveSmartGenWzdFile();//2024-10-24 wuxinyi 应北京需求每次定值关联完，被测对象导入之后需支持全模块调用
			}
			else
			{
...
// 	}
	...
	if (pRootNode == NULL)
	{
        ...
        return TRUE;  //shaolei  2024-7-29, 改为TRUE
	}
...
//20240704 huangliang  测试弹出定值选择界面
// void QSttTestCntrFrameBase::OnCmd_SettingSelect(QWidget *parent)
// {
// 	CXDeviceModel *pXDeviceModel = &((CSttTestCtrlCntrNative*)(g_pTheSttTestApp->m_pTestCtrlCntr))->m_oXrioDeviceModel;
// 	pXDeviceModel->InitDvmDevice(g_theGbSmartGenWzd->m_pDvmDevice);
// 	QDeviceModelDlg_Eng oDeviceModelDlg_Eng(parent);//定制关联模式
//     oDeviceModelDlg_Eng.m_nDlgModeType = _FIXED_VALUE_CORRELATION_TYPE_;//定制关联模式
// 	oDeviceModelDlg_Eng.InitXrioDeviceModel(pXDeviceModel);
// #ifdef _USE_SoftKeyBoard_	
...
// 		m_pTestCtrlCntrBase->FillReport(&m_pTestCtrlCntrBase->m_oTestReportHead);//刷新报告头
		...
		m_pTestCtrlCntrBase->Ats_UpdateReportHead();//更新报告头
...
{
	//2024-10-16 wuxinyi 修改点击报告设置确认之后闪退问题
...
void QSttTestCntrFrameBase::UpdateStatusBar_BinBinaryBtns()//20240919 huangliang 更新底部开入开出状态灯
{
...
void QSttTestCntrFrameBase::UpdateStatusWight_BinBinaryBtns()	//20240912 huangliang 更新非模态对话框的开入开出信息
{
	if (m_pMacroEditView == NULL)
		return;
	CSttMacroParaEditViewOriginal* pOri = (CSttMacroParaEditViewOriginal*)m_pMacroEditView;
...
void QSttTestCntrFrameBase::UpdateStatusTable_BinBinaryBtns()	 //20240912 huangliang 更新信息图
{
	if (m_pInfoWidget == NULL)
		return;
...
//20240806 huangliang 适用于在模态对话框中使用结构体地址编辑定值关联
...
QString Stt_Global_GetBinBoutNewString(const QString &strValue, bool bLeft, int iAddLen)	//20241012 huangliang 依据最大宽度获取最新字符串
{
	int iMLen = g_nNewPlotWidth + iAddLen;	//左右偏移不一样导致
...
{
	//2024-10-22 zhouyangyong 先初始化表格
// 	QSttMultiMacroParaEditViewBase *pSttMultiMacroParaEditViewBase = (QSttMultiMacroParaEditViewBase*)m_pMacroEditView->m_pMacroParaEditWnd;
