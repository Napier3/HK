...
{
	////20240704 huangliang	切换到那个模块就需要更新地址
...
//	CreateTestResource();
	...
	CopyBinaryConfig();
	//stt_Frame_UpdateToolButtons();//QSttTestCntrFrameBase::OpenMacroTestUI_Test会再次重复刷新
...
//		m_pManualTest_UI->horizontalSpacer_3->changeSize(0,0);
		...
		for (int nIndex = 0; nIndex < m_pManualTest_UI->horizontalLayout_3->count(); nIndex++)
		{
			QWidget* widget = m_pManualTest_UI->horizontalLayout_3->itemAt(nIndex)->widget();
			if (widget) 
			{
				widget->setVisible(false); 
			}
		}		
		for (int nIndex = 0; nIndex < m_pManualTest_UI->horizontalLayout_2->count(); nIndex++)
		{
			QWidget* widget = m_pManualTest_UI->horizontalLayout_2->itemAt(nIndex)->widget();
			if (widget)
			{
				widget->setVisible(false);
			}
...
// 	}
	...
	if (m_oIV.m_pHarmUIParaWidget != NULL)
	{
		m_oIV.m_pHarmUIParaWidget->initUI(stt_GetSttTestResource());
		m_oIV.m_pHarmUIParaWidget->initData();
		m_oIV.m_pHarmUIParaWidget->setMaxMinAndEDVal();
	}

	m_oParas.SetData(stt_GetSttTestResource(), m_pManualParas, 0);
	UpdateHarmCheckComboxUI(TRUE);
...
//	m_oParas.m_pCheckBoxHarm = m_pManualTest_UI->m_checkBoxHarm;
...
#ifdef _PSX_QT_WINDOWS_
    //2024.8.13 chenling 2880*1800分辨率
    ...
    if (fWidth >= 2880)
    {
        m_pManualTest_UI->m_boxOutSetting->setFixedWidth(140);
    }
    else if (fWidth >= 2280)
    {
        m_pManualTest_UI->m_boxOutSetting->setFixedWidth(120);
    }
    else
    {
...
// 	m_pManualTest_UI->gridLayout_2->setContentsMargins(0, 0, 0, 0); 
	m_pManualTest_UI->horizontalLayout_6->addWidget(m_pWidget);
...
//    containerLayout->addWidget(m_pManualTest_UI->m_frmOtherPara, 0, 0, 2, 1); // 第1、2行
//    containerLayout->addWidget(m_pManualTest_UI->m_boxOutSetting, 0, 2); // 第2行
//    containerLayout->addItem(m_pManualTest_UI->gridLayout_2->itemAtPosition(2, 1), 0, 1); // 水平间隔4
//    containerLayout->addItem(m_pManualTest_UI->gridLayout_2->itemAtPosition(2, 3), 0, 3); // 水平间隔5
//    containerLayout->addWidget(m_pManualTest_UI->m_frmParas, 1, 0, 1, 4); // 第4行
//    pSplitter->addWidget(m_pManualTest_UI->m_tabWidget);
//    pSplitter->addWidget(containerWidget);
//    m_pManualTest_UI->gridLayout_2->addWidget(pSplitter);
}
...
void QSttMacroParaEditViewManual::SetDatas(CDataGroup *pDataset)
{
	...
	stt_Frame_InitVectorWidget(m_pManualParas->m_uiVOL,m_pManualParas->m_uiCUR);
 
	if (g_nLogDebugInfor == 1)	{		CLogPrint::LogString(XLOGLEVEL_TRACE, ">> begin QSttMacroParaEditViewManual::SetDatas -  >>");	}
	stt_Frame_InitPowerWidget(m_pManualParas->m_uiVOL,m_pManualParas->m_uiCUR);
...
{
	//20240703 huangliang 
	if (m_pMUAccuracyTestNum_Edit->inherits(STT_SETTING_LINEEDIT_ClassID/*"QSettingEdit"*/))
	{
		if(((QSettingLineEdit*)m_pMUAccuracyTestNum_Edit)->IsSetting())
		{
			return ;
		}		
	}
...
{
	//20240703 huangliang 
	if (m_pMUTestTime_Edit->inherits(STT_SETTING_LINEEDIT_ClassID/*"QSettingEdit"*/))
	{
		if(((QSettingLineEdit*)m_pMUTestTime_Edit)->IsSetting())
		{
			return ;
		}		
	}
