...
//	m_crAxisInstructX_FontColor = RGB( 0, 255, 0);
	...
	m_oLeftButtonLine.SetEndPoint(0, 0);
     ...
     m_nFaultType = -1;  //20240828 wanmj
	 m_bShowCurrCharactColor = FALSE;  //20241025 wanmj 是否更新选中特性曲线颜色
//    m_pDrawListRef = NULL;  //20241025 wanmj 当前特性曲线drawlist

//	 m_pDrawListNoShow = NULL;//20241010 luozibing 保存界面不显示曲线绘图对象
}
...
BOOL CPsuCharDrawView::IsShowCharacteristic(CCharacteristic *pCharTemp)
{
	if (m_nFaultType == -1)
		return TRUE;

	if ((pCharTemp->m_strID != CHAR_DISTANCE_FAULT_TYPE_LN) && (pCharTemp->m_strID != CHAR_DISTANCE_FAULT_TYPE_LL)
		&& (pCharTemp->m_strID != CHAR_DISTANCE_FAULT_TYPE_LLN) && (pCharTemp->m_strID != CHAR_DISTANCE_FAULT_TYPE_LLL))
	{
		return TRUE;
	}
	//20240830 wanmj 根据故障类型和id显示特性曲线
...
// 		strFaultType = "LL";
// 
// 	if (strFaultType != pCharTemp->m_strID)
// 		return;
	if (m_nFaultType<3)
	{
		strFaultType = CHAR_DISTANCE_FAULT_TYPE_LN;
	}
	else if (m_nFaultType<6)
	{
		strFaultType = CHAR_DISTANCE_FAULT_TYPE_LL;
	}
	else if (m_nFaultType < 9)
	{
		strFaultType = CHAR_DISTANCE_FAULT_TYPE_LLN;
	}
	else if (m_nFaultType == 9)
	{
		strFaultType = CHAR_DISTANCE_FAULT_TYPE_LLL;
	}
	else
	{
		strFaultType = CHAR_DISTANCE_FAULT_TYPE_LN;
	...
	return (strFaultType == pCharTemp->m_strID);
}
DWORD CPsuCharDrawView::XCreateElement(CExBaseObject *pData, CWnd *pParentWnd, BOOL bViewTestLines, BOOL bCalCharTemp)
{
	return CCharacterDrawView::XCreateElement(pData, pParentWnd, bViewTestLines, bCalCharTemp);
}
DWORD CPsuCharDrawView::XCreateElement(CExBaseObject *pData, CWnd *pParentWnd)
{
// 	if (m_pDrawListNoShow != NULL)
// 	{
// 		m_pDrawListNoShow->DeleteAll();
// 		m_pDrawListNoShow = NULL;
// 	}
	m_oHidingDrawList.DeleteAll();
	return CCharacterDrawView::XCreateElement(pData, pParentWnd);
}

// void CPsuCharDrawView::UpdateCurCharact(CXDrawList *pDrawList)
// {
// 	if (pDrawList == NULL || m_bUpdateCharactColor == FALSE)
//         return;
// 
// 	SetCurDrawList(pDrawList);
// }

void CPsuCharDrawView::InitCharDrawView(CCharacteristic *pCharTemp, CWnd *pParentWnd)
{
	if (!IsShowCharacteristic(pCharTemp))
	{
		//20241009 luozibing 只初始化相同故障类型的曲线时，添加系列计算交点时不同故障类型曲线没有绘图数据
		//不同故障类型曲线初始化但不显示
		CXDrawList *pDrawListNoShow = pCharTemp->InitCharDrawElement_ForDraw();
		m_oHidingDrawList.AddNewChild(pDrawListNoShow);
		return;
	}
...
void CPsuCharDrawView::InitDrawLineColor(const COLORREF &oColor)
{
    ...
    //20241025 wanmj 修改当前选中特性曲线颜色
	if ((m_pCharacteristic != NULL) && (m_bShowCurrCharactColor))
	{
		CXDrawList *pDrawList = (CXDrawList*)m_pCharacteristic->GetCurDrawList();

		if (pDrawList != NULL)
		{
			pDrawList->InitDrawLineColor(RGB(255, 0, 255));
		}
	}	
}
...
void CPsuCharDrawView::InitDrawLineColor(const COLORREF &oLinesColor,const COLORREF &oTestLineColor_sys,const COLORREF &oTestLineColor_User)
{
	...
	CCharTestLine *pCharTestLine = NULL;
	...
	while (pos_XDrawList != NULL)
	{
		...
		while (pos != NULL)
		{
			...
			{
				if (pXDrawBase->m_pXDrawDataRef->GetClassID() == CHARCLASSID_CCHARTESTLINE)
				{
					pCharTestLine = (CCharTestLine*)pXDrawBase->m_pXDrawDataRef;
...
void CPsuCharDrawView::CalLinesPoints(long nIndex, CXDrawDatas* pDatas, CString strType)
{
	...
	while (pos)
	{
		...
		{
			if(pObj->m_pXDrawDataRef->GetClassID() == CHARCLASSID_CCHARTESTLINE)
			{
			...
			xdraw_SolveEquation(pBase, &m_oLeftButtonLine, *pDatas,false);
		}
...
void CPsuCharDrawView::DrawAxisTestPoints(CDC *pDC,float fZoomRatio,CPoint ptOffset)
{
	CAxisInterface *pAxisInterface = NULL;
	m_pXDrawMngr_Axis->GetAxisInterface(&pAxisInterface);
	if(pAxisInterface)
...
void CPsuCharDrawView::Draw(CDC *pDC, double fZoomRatio,const CPoint &ptOffset)
{
	...
	DrawAxisTestPoints(pDC,fZoomRatio,ptOffset);
	//画鼠标竖线以及相交点
...
void CPsuCharDrawView::UpdateAxisType(const CString& strAxis,BOOL bLockXY)
{
	...
	if (m_pXDrawMngr_Axis->m_strAxis != strAxis)
	{
	...
	m_pXDrawMngr_Axis->SetAxisXYUnitLock(bLockXY);
