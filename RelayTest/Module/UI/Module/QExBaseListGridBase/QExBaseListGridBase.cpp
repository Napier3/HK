...
#define STT_EXBASEGRID_SPACE_BELOW_2280		30
#define STT_EXBASEGRID_SPACE_2280			20
#define STT_EXBASEGRID_SPACE_2880			10

#else
#define STT_EXBASEGRID_SPACE	10
#endif
...
void QExBaseListGridBase::Show_ItemString(const int& nRow, const int& nCol, const CString &strText)
{
	...
	SetItemText(pItem, strText);	//20240708 huangliang
	setItem(nRow,nCol,pItem);
}
...
void QExBaseListGridBase::Update_StaticString(CExBaseObject *pData, const int& nRow, const int& nCol, CString *pString)
{
	...
	pItem = NewGVItem(*pString);	//20240708 huangliang
...
QGV_ITEM* QExBaseListGridBase::AddItem(int nRow, int nCol, const CString &strText, BOOL &bHasSetItem, QColor oTextColor)
{
	bHasSetItem = FALSE;
	QGV_ITEM *pItem = GetCell(nRow,nCol);
	...
	if (pItem == NULL)
	{
		pItem = NewGVItem(strText, oTextColor);	//20240708 huangliang
...
void QExBaseListGridBase::EndEditCell_Hex (int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
	...
	{
		((QExBaseListGridBase*)pGrid)->SetModifiedFlag(pVCellData->pObj);	//20240926 huangliang 添加通知表格上一级刷新
		return ;		
	}
...
void  QExBaseListGridBase::Show_DataType_Index(CExBaseObject *pData, const int& nRow, const int& nCol, const CString &strDataType, CString *pstrIndex, GRID_CELL_EDIT_FUNC pFunc)
{
	...
	pItem->lParam = (LPARAM)pVCellData;
	pItem->nDataType = QT_GVET_COMBOBOX;
	pItem->nReserved = QT_GRID_COMBOBOX_StringType_DataTypeIndex;
	pItem->m_pEditFunc = pFunc;
	pVCellData->pExBaseList = pDataType;
	pVCellData->nVt = VCD_STRING;
...
void QExBaseListGridBase::EndEditCell_String (int nRow, int nCol, QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
    ...
    if (*pVCellData->pString != CString(pCell->text()) )
	{
...
int QExBaseListGridBase::GetRowIndex(QGV_ITEM *pItem)
{
	...
	for (int nRowIndex = 0;nRowIndex<nRowCount;nRowIndex++)
	{
		...
		{
			if (item(nRowIndex,nColIndex) == pItem)
			{
				return nRowIndex;
			}
		}
...
CString QExBaseListGridBase::GetHeaderTextFromCol(int nCol)
{
	if (nCol < 0 || nCol >= GetColCount()) return _T("");

	QTableWidgetItem *pItem = horizontalHeaderItem(nCol);
	if (pItem == NULL)
	{
		return _T("");
	}
...
//	resizeRowsToContents();
	...
	for (nCol = m_nAdjustBeginCol/*m_nAdjustBeginCol+1*/; nCol <= nCols; nCol++)
	{
...
BOOL QExBaseListGridBase::AutoSizeColumn(int nCol, BOOL bResetScrollBars)
{
	...
	QString strHeaderText = GetHeaderTextFromCol(nCol);
	//  	int nHeaderWidth = fontMetrics.width(strHeaderText);

	QRect headerRect = fontMetrics.boundingRect(strHeaderText);
	int nHeaderTextWidth = headerRect.width();
	...
	int nWidth = nHeaderTextWidth * dUIShowCoef;
#else
	int nWidth = nHeaderTextWidth ;
	double dUIShowCoef = 1.0f;
	...
	for (int nRow = 0; nRow < nNumRows; nRow++)
	{
		...
		nCurrSize *= dUIShowCoef;

		if (pItem->nDataType == QT_GVET_COMBOBOX)//下拉框
		{
#ifdef _PSX_QT_WINDOWS_
			nCurrSize += 10;
#else
	...
	float fWidth = rect.width() * dUIShowCoef;

	if (fWidth >= 2880)
	{
		setColumnWidth(nCol, nWidth + STT_EXBASEGRID_SPACE_2880);
	}
	else if (fWidth >= 2280)
	{
		setColumnWidth(nCol, nWidth + STT_EXBASEGRID_SPACE_2280);
	}
	else
	{
		setColumnWidth(nCol, nWidth + STT_EXBASEGRID_SPACE_BELOW_2280);
	}
#else
	setColumnWidth(nCol, nWidth + STT_EXBASEGRID_SPACE);
#endif // _PSX_QT_WINDOWS_
...
void QExBaseListGridBase::slot_CellChanged(int nrow,int ncol)
{
	...
	{
		if ((pitem->m_pEditFunc != NULL) && (itemDelegateForColumn(ncol) == NULL))//20240926 huangliang 变更后调用
		{
...
void QExBaseListGridBase::EndEditCell_DataType_Val (int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
	...
	else
	{
		...
		if (pVCellData->nVt == VCD_STRING)
		{
			if (pCell->nReserved == QT_GRID_COMBOBOX_StringType_DataTypeIndex)
			{
				long nIndex1 = CString_To_long(*pVCellData->pString);
				long nIndex2 = CString_To_long(pValue->m_strIndex);
			...
			{
				if (*(pVCellData->pString) != pValue->m_strID)
				{
		...
		{
			long nDataTypeIndex = pValue->GetIndex();
		...
		{
			double dDataTypeIndex = pValue->GetIndex();
		...
		{
			DWORD nDataTypeIndex = pValue->GetIndex();
