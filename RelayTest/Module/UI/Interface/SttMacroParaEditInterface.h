...
//	virtual void UpdateRtSyncTime(CDataGroup *pRtSycTime){}//20220920 zhouhj 更新对时、守时同步时间
    virtual void Debug();
    virtual BOOL IsHtmlView();
    virtual long GetMacroItemsXml(const CString &strMacroItemsID, char **ppszItemsXml);
    virtual UINT GetMacroEditViewType();

	 提高程序界面初始化打开的速度，将矢量图、状态图等的初始化放到线程
    ...
    virtual CExBaseList* GetCurrentMacroDatas() = 0;  //20240716 wanmj 单功能多点的测试功能获取当前测试项的数据
