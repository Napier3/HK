...
class QSttTestCntrFrameBase : public QMainWindow, public CLogBase, CSttTestCtrlCntrMsgInterface,
	public CSttWndAssistInterface   测试助手相关的接口
{
    Q_OBJECT
...
public:
	...
	virtual void OnAtsQueryItem(CExBaseObject *pItemPara);  
...
public:
	...
	void InitCharacteristics(CSttContents *pContents);  
	...
	virtual void ViewMacroEditWidget(CExBaseObject *pSelObj);
	virtual void SaveMacroParasToCurrWzd(CDataGroup *pCommCmd);  
...
public://界面
	...
	virtual void SetAddMacroEnable(bool bShow);     SetSelectEnable函数名非常不合理，做了修改
...
public:
	...
	virtual bool GenerateTemplate(CExBaseObject *pParentItems, const CString &strItemsName, const CString &strItemsID, long nRepeatTimes=1
		, long nRptTitle=0, long nTitleLevel=1);   修改为虚函数，Win版本和LINUX版本处理模式不一致
...
//	virtual void OnCmd_CharaView();
	...
	virtual void OnCmd_IecCapWriteFile(); 
	virtual void OnCmd_IecCapDebugHead(); 
	...
	virtual void InitIecCapTest(bool bActiveIecCap, bool bStartDetect,bool bShowMenuBtns = true);  //参数3为是否显示菜单等按钮,IEC探测时,不需要显示
