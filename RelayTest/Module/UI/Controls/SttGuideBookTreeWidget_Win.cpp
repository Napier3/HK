...
void QSttGuideBookTreeWidget_Win::OnItemSelChanged(CExBaseListTreeCtrl *pTreeCtrl, CExBaseObject *pSelObj)
{
	...
	BOOL bHasCharact = FALSE;

	if (pItems != NULL)
	{
		bHasCharact = (pItems->m_strCharacteristicID.GetLength() > 0);

		if (!bHasCharact)
		{
			bHasCharact = g_theTestCntrFrame->IsCharactTestMacroUI(pItems->m_strstxmlFile);
		}
	}

	if (bHasCharact/*pItems && pItems->m_strCharacteristicID.GetLength()*/)
	{
	...
	g_theTestCntrFrame->UpdateCharacterEditButtonState(false, false, true);
...
void QSttGuideBookTreeWidget_Win::slot_OnBtnDeleteCurr()
{
    ...
    //ftt 2024.10.22
    QString strItemsPath = pItems->GetIDPathEx(STTGBXMLCLASSID_CSTTDEVICE, FALSE);
...
void QSttGuideBookTreeWidget_Win::slot_OnBtnSaveCurr()
{
	...
	BOOL IsInternationalMode = g_theTestCntrFrame->CurrMacroTestUI_IsInternationalMode();
	...
	g_theTestCntrFrame->OnSaveCurr(m_pCurrSelRootNode,IsInternationalMode);   
...
{
	return g_theTestCntrFrame->SendSetItemState(pItemBase);
