...
void CSttCmdGuideBookTransTool::Trans(CMacroTest *pMacroTest, CSttMacroTest *pSttMacroTest)
{
    ...
    pSttMacroTest->m_strMacroID = pMacroTest->GetTestMacroID();  
	...
	Trans(pMacroTest->GetMacroTestPara(), pSttMacroTest->GetSttMacroTestPara(TRUE));
...
void CSttCmdGuideBookTransTool::Trans(CMacroTestPara *pMacroPara, CSttMacroTestPara *pSttMacroPara)
{
	if (pMacroPara == NULL || pSttMacroPara == NULL)
	{
		return;
	}

	pSttMacroPara->SetMacroID(pMacroPara->GetMacroID());
	pSttMacroPara->SetTestMode(pMacroPara->GetTestMode());

	CSttMacroTestParaDatas *pSttParaDatas = pSttMacroPara->GetSttMacroTestParaDatas(TRUE);
	CMacroTestParaDatas* pParaDatas = pMacroPara->GetFaultParaDatas();

	if (pSttParaDatas == NULL || pParaDatas == NULL)
	{
		return;
	}
	...
	while (pos != NULL)
	{
		CMacroTestParaData *pPara = (CMacroTestParaData *)pParaDatas->GetNext(pos);

		CSttMacroTestParaData *pNew = pSttParaDatas->AddData(pPara->m_strName, pPara->m_strID, pPara->m_strExpression);
	}
...
*/
		...
		{
			//找不到，考虑从结果判断表达式中查找。
			CXExprNode *pNode = (CXExprNode *)pItem->GetRsltExpr(FALSE);
