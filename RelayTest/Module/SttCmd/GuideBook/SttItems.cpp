...
BOOL CSttItems::DeleteChildItem(CExBaseObject *pChildItem)
{
	if (pChildItem == NULL)
	{
		return FALSE;
	...
	while (pos != NULL)		
	{
		...
		else if (pItem->GetClassID() == STTGBXMLCLASSID_CSTTITEMS)
		{
			if (((CSttItems*)pItem)->DeleteChildItem(pChildItem))
			{
				return TRUE;
			}
		}
...
void stt_gb_get_all_macro_items(const CString &strMacroID, CExBaseList *pMacroList,CSttItems* pRootNode)
{
	CExBaseObject *pCurrObj = NULL;
	POS pos = pRootNode->GetHeadPosition();

	while(pos)
	{
		pCurrObj = pRootNode->GetNext(pos);

		if (pCurrObj->GetClassID() == STTGBXMLCLASSID_CSTTITEMS)
		{
			stt_gb_get_all_macro_items(strMacroID,pMacroList,(CSttItems*)pCurrObj);
		}
