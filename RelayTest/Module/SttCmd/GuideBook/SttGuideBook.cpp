...
#endif
#include "../../UI/Config/MacroTestUI/SttTestCtrrlMacroTestUIMngrGlobal.h"
...
{		
		//2024-9-9 wuxinyi 修改DeleteAll，但没有清空地址导致的指针内容改变导致的中断问题
// 	if (m_pGlobalDatas != NULL)
// 	{
// 
...
//国际版软件模式,单功能模式,直接在后面追加,多功能模式,需要找到对应的文件夹进行追加
CSttItemBase* CSttGuideBook::InsertItems_InternationalMode(const CString &strParentPath, CSttItems *pItems, BOOL &bUpdateParent)
{
	if (pItems == NULL)
	{
		return NULL;
...
//	else
	{
// 		CSttItems *pFind = (CSttItems*)pParent->FindByID(pItems->m_strID);
// 		pFind->m_strstxmlFile = pItems->m_strstxmlFile;
// 
// 		if (pFind == NULL)
// 		{
// 			pParent->AddNewChild(pItems);
...
// 		else
		{
			...
			while(pos)
			{
				...
				{
					//�Զ����Է��ص��ǵ�ǰ��Ŀ¼�µ�ȫ��ģ��,�˴����˵��Ѿ���ӵĲ�����Ŀ
					if (pParent->FindByID(pCurrObj->m_strID) == NULL)
					{
						pItems->Remove(pCurrObj);
						pParent->AddNewChild(pCurrObj);
					}
				}
				else if (pCurrObj->GetClassID() == STTGBXMLCLASSID_CSTTITEMS)
				{
					pChildItems = (CSttItems*)pParent->FindByID(pCurrObj->m_strID);

					if (pChildItems == NULL)
					{
						pChildItems = new CSttItems;
						pChildItems->m_strType = GBITEMS_NODETYPE_NONE;
						pChildItems->m_strName = pCurrObj->m_strName;
						pChildItems->m_strID = pCurrObj->m_strID;
						pParent->AddNewChild(pChildItems);
					}
					...
					while(pos_child)
					{
						...
						{
							//�Զ����Է��ص��ǵ�ǰ��Ŀ¼�µ�ȫ��ģ��,�˴����˵��Ѿ���ӵĲ�����Ŀ
							if (pChildItems->FindByID(pCurrObj->m_strID)== NULL)
							{
								pCurrObjList->Remove(pCurrObj);
								pChildItems->AddNewChild(pCurrObj);
							}
// 							pCurrObjList->Remove(pCurrObj);
// 							pChildItems->AddNewChild(pCurrObj);
						}
						else if (pCurrObj->GetClassID() == STTGBXMLCLASSID_CSTTMACROCHARITEMS)
						{   
						//add wangtao 20240725 差动节点为ITEMS -> macro-char-items -> macrotest
							pCurrObjList = (CExBaseList*)pCurrObj;
							POS pos_child = pCurrObjList->GetHeadPosition();
				...
				{
					CSttMacroCharItems *pMacroCharItems = NULL;
					pMacroCharItems = (CSttMacroCharItems*)pParent->FindByID(pCurrObj->m_strID);

					if (pMacroCharItems == NULL)
					{
						pMacroCharItems = new CSttMacroCharItems;
						//pMacroCharItems->m_strType = GBITEMS_NODETYPE_NONE;
						pMacroCharItems->m_strName = pCurrObj->m_strName;
						pMacroCharItems->m_strID = pCurrObj->m_strID;
						pParent->AddNewChild(pMacroCharItems);
					}
					...
					while(pos_child)
					{
						...
						if (pCurrObj->GetClassID() == STTGBXMLCLASSID_CSTTMACROTEST)
						{
							if (pMacroCharItems->FindByID(pCurrObj->m_strID)== NULL)
							{
								pCurrObjList->Remove(pCurrObj);
								pMacroCharItems->AddNewChild(pCurrObj);
							}
						}
					}
				}
...
CSttItemBase* CSttGuideBook::UpdateBeforeTestItems_InternationalMode(CSttGuideBook *pSttGuideBook)
{
	CSttDevice *pDevice_Ex = pSttGuideBook->GetDevice();
	...
	CSttItemBase *pBeforeTestItems_Ex = (CSttItemBase*)pDevice_Ex->FindByID(STT_ITEMS_ID_BEFORETEST);

	if (pBeforeTestItems_Ex == NULL)
	{
		return NULL;
	}

	CSttItemBase *pBeforeTestItems_Inner = stt_gb_find_itembase(this, STT_ITEMS_ID_BEFORETEST);

	if (pBeforeTestItems_Inner == NULL)
	{
		return NULL;
	}

	pBeforeTestItems_Inner->DeleteAll();
	pBeforeTestItems_Inner->AppendEx(*pBeforeTestItems_Ex);
	pBeforeTestItems_Ex->RemoveAll();
	return pBeforeTestItems_Inner;
}
