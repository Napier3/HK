...
public:
	  函数增加格式，没有采用缺省值，编译的时候会导致语法错误，从强制修改，实现代码的统一
...
public:
	...
	long DoWait(CSttXcmdChInterface *pSocket,STT_CMD_INFO *pCmdInfo,long nTimeOut = -1,CSttCmdData *pRetData=NULL,BOOL bDoEvents=TRUE);


private:
	 暂时保留这个变量，后续去掉，采用统一的标准的接口
	CSttXcmdChInterface *m_pRefSocketDataBase; 

	 新的接口变量，后续使用这个接口
	void *m_pCmmOptrInterface;     由g改为void*

	//私有成员变量访问方法
public:
	 暂时保留这个变量，后续去掉，采用统一的标准的接口
	void SetRefSocketData(CSttXcmdChInterface *pSocket)  {   m_pRefSocketDataBase = pSocket; }
	CSttXcmdChInterface* GetRefSocketData()  {   return m_pRefSocketDataBase;    }
