...
public:
	...
	virtual long Ats_CreateTemplate(const CString &strTestMacroUI_ID, const CString &strDeviceName, const CString &strDeviceID, const CString &strDvmFile,
		long nHasDigital,bool bDoEvents=TRUE,CSttCmdData *pRetData = NULL);//
	virtual long Ats_UpdateCharacteristic(CSttContents *pChar, const CString &strParentItemsPath, bool bDoEvents = TRUE, CSttCmdData *pRetData = NULL);

	virtual long Ats_UpdateReportHead(CDataGroup *pReportHead, bool bDoEvents, CSttCmdData *pRetData = NULL);
...
public:
	virtual void Ats_IecDetect(long bEnable, long nIECType = 0);
