...
BOOL CSttAtsTestClient::ConnectAtsTestServer(const CString &strIP,long nPort,const CString &strSoftID)
{
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_TRACE, "正在连接自动测试服务......");//2024-7-19 wuxinyi 修改打印信息
	...
	if(bRet)
	{
        ...
        CLogPrint::LogFormatString(XLOGLEVEL_TRACE, "自动测试服务连接成功");
	...
	else
	{
        ...
        CLogPrint::LogFormatString(XLOGLEVEL_ERROR, "连接自动测试服务失败，请检查网络链路是否连通，IP地址及端口配置是否正确；然后点击网络连接按钮重新连接");
	}

#else
	CString strText,strText1,strText2;
    xlang_GetLangStrByFile(strText, "Native_ConnectingAtsTester");
    xlang_GetLangStrByFile(strText1, "Native_TesterConnectAtsSucc");
    xlang_GetLangStrByFile(strText2, "Native_TesterConnectAtsfail");
	if (g_nLogDebugInfor == 1) CLogPrint::LogFormatString(XLOGLEVEL_TRACE, strText.GetString());
	...
	if(bRet)
	{
		if (g_nLogDebugInfor == 1) CLogPrint::LogFormatString(XLOGLEVEL_TRACE, strText1.GetString());
#ifndef _PSX_OS_CENTOS_//zhouhj 2024.5.16 装在工控机内,此处不能设置AppIP
		g_oSttTestAppCfg.SetTestAppIP(strIP);
#endif
		g_oSttTestAppCfg.SetNativeServerPort(nPort);
...
long CSttAtsTestClient::Process_SysState_Log(CSttSysState &oSysState)
{
...
// 	return 0;
	...
	while (pos != NULL)
	{
		...
		if (pData->m_strMsg.Find("[autotest error]") != -1)
		{
			strLog = pData->m_strMsg;
			CLogPrint::LogString(XLOGLEVEL_ERROR, strLog);
		}
		else if (pData->m_strMsg.Find("[error]") != -1)
		{
			strLog = pData->m_strMsg;
			strLog.Replace("[error]", "[tester error]");
			CLogPrint::LogString(XLOGLEVEL_ERROR, strLog);
		}
		else if (g_nLogDebugInfor)
		{
			strLog = _T("*********************");
			strLog += pData->m_strMsg;
...
long CSttAtsTestClient::Ats_CreateTemplate(const CString &strTestMacroUI_ID, const CString &strDeviceName, const CString &strDeviceID, const CString &strDvmFile,
										   long nHasDigital,bool bDoEvents,CSttCmdData *pRetData)
{
	...
	return m_pXClientEngine->Ats_CreateTemplate(strTestMacroUI_ID, strDeviceName, strDeviceID, strDvmFile,nHasDigital, bDoEvents, pRetData);
}

long CSttAtsTestClient::Ats_UpdateCharacteristic(CSttContents *pChar, const CString &strParentItemsPath, bool bDoEvents, CSttCmdData *pRetData)
{
	if (!ConnectAtsTestServer())
	{
		return 0;
	...
	return m_pXClientEngine->Ats_UpdateCharacteristic(pChar, strParentItemsPath, bDoEvents, pRetData);
}

long CSttAtsTestClient::Ats_UpdateReportHead(CDataGroup *pReportHead, bool bDoEvents, CSttCmdData *pRetData)
{
	if (!ConnectAtsTestServer())
	{
		return 0;
	...
	return m_pXClientEngine->Ats_UpdateReportHead(pReportHead, bDoEvents, pRetData);
...
void CSttAtsTestClient::OnTestState(const CString &strMacroID, CDataGroup *pParas)
{
	...
	if (pParas->m_strID == SYS_STATE_XEVENT_EVENTID_BIN)
	{
		...
		stt_GetDataValueByID(pParas, STT_SYS_STATE_ID_CurrStateIndex, pEventInfo->m_nCurrStateIndex);	//chenling20240226获得状态索引
		stt_GetDataValueByID(pParas, STT_SYS_STATE_ID_CurrentStepIndex, pEventInfo->m_nCurrStepIndex);     //dingxy 20240520 获取状态序列递变步数
...
void AtsTestClient_OnRtData_GlobalRtDataMngr(CDataGroup *pRtDataGroup)
{
	...
	if (pRtDataMngr->m_nMode == STT_RT_DATA_MODE_SYN)
	{
		...
		if (pDataGroup != NULL)
		{
			...
			pMacroChs->m_oIfCh.m_oHisDataBuf.AppendBuffer(&pMacroChs->m_oIfCh.m_oRtDataCh, 1);
		}

		pDataGroup = (CDataGroup*)pRtDataGroup->FindByID("Zf");
		if (pDataGroup != NULL)
		{
			stt_GetDataValueByID(pDataGroup, _T("mag"), pMacroChs->m_oZ_Fault.m_oRtDataCh.m_fMag);
			stt_GetDataValueByID(pDataGroup, _T("ang"), pMacroChs->m_oZ_Fault.m_oRtDataCh.m_fAng);
			pMacroChs->m_oZ_Fault.m_oHisDataBuf.AppendBuffer(&pMacroChs->m_oZ_Fault.m_oRtDataCh, 1);
		}
...
void CSttAtsTestClient::Ats_IecDetect(long bEnable, long nIECType)
{
	if (!ConnectAtsTestServer())
	{
		return ;
	...
	if (g_oSttTestResourceMngr.m_oIecDatasMngr.GetCount() >0)
	{
		...
		if (pIecCfgDatasSmvIn)
		{
			nFT3_CrcType = pIecCfgDatasSmvIn->m_nFT3CRCType;
	...
	if (nFT3_CrcType == 1)
	{
		...
		if (nCount > 0)
		{
			for (int i = 0;i < nCount; i++)
			{
				...
				CString strPkgName;
				strPkgName.Format(_T("Ft3_PkgLength%d"),nFiberIndex);
...
BOOL CSttAtsTestClient::Ats_UartConfig(CDataGroup *pUartConfigParas)
{
	if (!ConnectAtsTestServer())
	{
		return FALSE;
...
void CSttAtsTestClient::Ats_BinRecord(CDataGroup *pBinRecordParas)
{
	if (!ConnectAtsTestServer())
	{
		return ;
	...
	m_pXClientEngine->Test_StartTest(_T("BinMeas"), pBinRecordParas);
}
