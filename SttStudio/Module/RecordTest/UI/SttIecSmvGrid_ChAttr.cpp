...
void CSttIecSmvGrid_ChAttr::InitGridTitle()
{
	...
	astrGridTitle[STT_SMVGRID_ChAttr_ColIndex_ChAngType] = /*_T("通道相别")*/g_sLangTxt_channelphase; //lcq
...
void CSttIecSmvGrid_ChAttr::ShowData(CExBaseObject *pData, int& nRow, BOOL bInsertRow)
{
	...
	if (pSmvCh == NULL)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("表格中当前通道(%s)数据未查找到.")*/g_sLangTxt_tablechannel.GetString(),pChData->m_strID.GetString()); //lcq
		return;
	}
...
void CSttIecSmvGrid_ChAttr::EndEditCell_Smv92ChSelChangged(int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE,/* _T("Row=%d  Col=%d  数据类型对象不存在")*/g_sLangTxt_DataTypeUnExist.GetString(), nRow, nCol); //lcq
		return;
	}
	...
	else
	{
		...
		{
			CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前数据类型对应的表格出错.")*/g_sLangTxt_faulttype.GetString()); //lcq
		}
	}
}
...
void CSttIecSmvGrid_ChAttr::EndEditCell_Smv92ChAngTypeChangged(int nRow, int nCol,QGV_ITEM *pCell, QExBaseListGridBase *pGrid)
{
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_TRACE, /*_T("Row=%d  Col=%d  数据对象不存在")*/g_sLangTxt_faulttype.GetString(), nRow, nCol); //lcq
		return;
	}
