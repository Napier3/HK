...
void CSttTestClientBase::DisConnectForClose(BOOL bDelete)
{
	...
	{
		m_pXClientEngine->m_pSttClientSocket->Close();
	}
...
BOOL CSttTestClientBase::ConnectSttServer(CSttTestAppConfigTool *pTool)
{
	...
	if (pTool != NULL)
	{
		...
		m_pXClientEngine = pTool->CreateTestEngine(&m_oTestAppCfg);
	}
	else
	{
...
long CSttTestClientBase::Remote_BindAndLogin(CSttTestRemoteRegister *pRegister,CSttTestClientUser *pUser)
{
	...
	//设置LocalIP
	pClientData->m_pSttClientSocket->m_strIPLocal = m_oTestAppCfg.GetLocalIP();
	...
	pClientData->m_oCurrUser.IP_User(pClientData->m_pSttClientSocket->m_strIPLocal);
