...
void stt_cmd_info::OnSendCmdStatusChanged(long nExecState,CSttSysState &oSysState)
{
	if (pSysState != NULL)
	{
		delete pSysState;
...
BOOL stt_cmd_info::CheckTimeOut(DWORD dwCurTime)
{
	...
	if (bRet)
	{
//		long nVal = ((CSttSocketDataBase_File *)pSocketRef)->m_oExpandRecvBuf.GetDataLength();
...
void CSttCmdOverTimeTool::Clear(CSttSocketDataBase_File *pSocket)
{
...
STT_CMD_INFO *CSttCmdOverTimeTool::AddSttFileCmd(CSttSocketDataBase_File *pSocket, DWORD nCmdType
...
STT_CMD_INFO *CSttCmdOverTimeTool::AddSttCmd(CSttSocketDataBase_File *pSocket, DWORD nCmdType
...
long CSttCmdOverTimeTool::OnSocketReceive_FileCmd(CSttSocketDataBase_File *pSocket, DWORD nCmdType
...
long CSttCmdOverTimeTool::OnSocketFailFinish_FileCmd(CSttSocketDataBase_File *pSocket, DWORD nCmdType
								, DWORD nCmdPara, DWORD dwCurrTime)
{
	STT_CMD_INFO *p = NULL;
	for (int i=0; i<=g_pSttCmdOverTimeTool->m_nMaxIndex; i++)
...
long CSttCmdOverTimeTool::OnSocketClosed(CSttSocketDataBase_File *pSocket)
{
...
#if !defined(_OA_MNGR_CLIENT) && !defined(_STT_FTP_SVR_)
long CSttCmdOverTimeTool::OnSocketReceive_FixedCmd(CSttSocketDataBase_File *pSocket, CSttSysState &oSysState,long nExecStatus)
{
	STT_CMD_INFO *p = NULL;
	char pszCmdID[64] = {0};
	...
	for (int i=0; i<=g_pSttCmdOverTimeTool->m_nMaxIndex; i++)
	{
		...
		{//命令一致
            ...
            int nTmpValue1 = oSysState.m_nIDTester;
            int nTmpValue2 = pSocket->GetIDTest();

            if ((nTmpValue1 != 0) && (nTmpValue1 == nTmpValue2))//Xuzt 64位和32位中使用DWORD存在问题，转换为int进行比较
            {
				p->OnSendCmdStatusChanged(nExecStatus,oSysState);
				return 1;
			}	
			else
			{
				if (g_nSttCmdOverTimeViewLog == 1)
				{
                    CLogPrint::LogFormatString(XLOGLEVEL_TRACE, _T("Error: (%s) oSysState.m_nIDTester(%ld) != SocketID(%ld)")
                        , pszCmdID,nTmpValue1, nTmpValue2);
				}
			}
		}
	}
