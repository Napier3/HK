...
{//脱机软件中远程连接
    ...
    //sun connect(m_pSocket,SIGNAL(error(QAbstractSocket::SocketError))),this,SLOT(OnError(QAbstractSocket::SocketError));
    //sun m_pSocket->connectToHost(QHostAddress(pszIP), nPort);
    //sun
    //sun m_strIPRemote = pszIP;
	m_nPortRemote = nPort;
