...
//2020-12-27  Eric  后续如果结构体增加数据，则递增版本号
...
//2021-5-2  Eric  因为外部需要访问，所以将模块属性定义部分放到  tmt_adjust_sys_para_module_def.h
...
public:
	...
	//2021-5-2  Eric  以下的部分，是否有必要单独作为一个对象
	//????????????????????????????????????????????????
...
typedef struct stt_Module
{
	...
	//2021-5-2  Eric  根据m_dwVersion做初始化处理
...
//	UINT  m_nGseInTotalSize;//goosein 最大组数
	...
	char  m_strSN[32];//装置序列号，字符串型 2020-10-14  Eric
