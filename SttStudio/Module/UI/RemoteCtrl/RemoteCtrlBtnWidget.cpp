...
// 	CDvmDevice* pDvmDevice = g_theTestCntrFrame->m_pEngineClientWidget->m_pDvmDevice;
	...
	while(pos)
	{
		...
		while(pos_dataset)
		{
            ...
            if (pCurrDataset->m_strID/*.Find(g_strPxDataSetIDKey[PX_DSID_dsDin]) >= 0*/ == _T("dsEna"))
			{
			...
			else if (pCurrDataset->m_strID/*.Find(g_strPxDataSetIDKey[PX_DSID_dsDin]) >= 0*/ == _T("dsDin"))
			{
