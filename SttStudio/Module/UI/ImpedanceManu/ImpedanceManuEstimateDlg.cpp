...
ImpedanceEstimateGrid::ImpedanceEstimateGrid(int rows, int columns, QWidget* parent) : QTableWidget(rows, columns, parent)
{
...
// 	strText = "频率动作值(Hz)";
	...
	setItem(3, 2, new QTableWidgetItem(QString::number(pParas->m_fImpValue_RelErr)));
	...
	setItem(3, 3, new QTableWidgetItem(QString::number(pParas->m_fImpValue_AbsErr)));
	...
	setItem(3, 4, new QTableWidgetItem(QString::number(pParas->m_fImpSet)));
	...
	if (pParas->m_bAuto)//自动递变方式时，开放动作值、返回系数、相位动作值
	{
		...
		{
			if (pParas->m_nCalMode== 1)		
			{
				if (pParas->m_nFirstMode== 3)
				{
					EnableRow(1);//电压
				}
				else
...
void ImpedanceManuEstimateDlg::slot_btnOK_Clicked()
{
    ...
    m_pParas->m_fImpValue_RelErr = m_pGrid->item(3, 2)->text().toFloat();
	...
	m_pParas->m_fImpValue_AbsErr = m_pGrid->item(3, 3)->text().toFloat();
	...
	m_pParas->m_nImpValue_ErrorLogic = ((QComboBox*)m_pGrid->cellWidget(3, 1))->currentIndex();
	...
	m_pParas->m_fImpSet = m_pGrid->item(3, 4)->text().toFloat();
