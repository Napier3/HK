...
QSttMacroParaEditViewPowerManu::QSttMacroParaEditViewPowerManu(QWidget *parent) 
{
	...
	g_theTestCntrFrame->InitPowerWidget(m_pManualParas->m_uiVOL,m_uiCUR);
...
void QSttMacroParaEditViewPowerManu::UpdateTestResource(BOOL bCreateChMaps)
{
	...
	m_oPV.m_pPUParaWidget->initUI(g_theTestCntrFrame->GetSttTestResource());
	m_oPV.m_pPUParaWidget->initData();
	m_oPV.m_pPUParaWidget->setMaxMinAndEDVal();

	m_pPowerManualWidget->SetData(g_theTestCntrFrame->GetSttTestResource(), m_pManualParas, 0);
...
void QSttMacroParaEditViewPowerManu::SetDatas(CDataGroup *pDataset)
{
    ...
    g_theTestCntrFrame->ClearInfoWidget();
	//g_theTestCntrFrame->InitPowerWidget(m_pManualParas->m_uiVOL,m_uiCUR);//chenling
