...
void QCharEditWidget_LineDiff::slot_btnUserClear(bool bState)
{
	...
	while (pos)
	{
		...
		{
			CCharElementTestLine* pTestLine = (CCharElementTestLine*)pCharLine->GetAt(i);
...
void QCharEditWidget_LineDiff::InitChar()
{
	...
	{
		//没有配置文件,则初始化
		g_theTestCntrFrame->m_pCharacteristics->InitEmptyCharacter(AXIS_TYPE_ID_CARTES);
...
{
	return (CCharacteristic*)g_theTestCntrFrame->m_pCharacteristics->GetHead();
}
...
// void QCharEditWidget_Diff::AddTestPoint(CExBaseList* pValue)
// {
// 	//取最新的动作点
// 	CDvmValues* pValues = (CDvmValues*)pValue;
// 	CDvmValue* pX = (CDvmValue*)pValues->FindByID("TripIr");
// 	CDvmValue* pY = (CDvmValue*)pValues->FindByID("TripId");
// 	if(pX && pY)
// 	{
// 		double dX = CString_To_double(pX->m_strValue);
// 		double dY = CString_To_double(pY->m_strValue);
// 		g_theTestCntrFrame->AddTestPoints(dX, dY);
// 	}
// }
...
CCharacteristic* QCharEditWidget_Diff::GetCurrentCharacter()
{
	...
	return (CCharacteristic*)g_theTestCntrFrame->m_pCharacteristics->GetTail();
}
