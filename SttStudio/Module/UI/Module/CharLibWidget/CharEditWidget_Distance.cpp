...
void QCharEditGrid_Character::InitDataTypes()
{
	...
	pDataType->AddNewValue(_T("I"), _T("I"), _T("0"));
	pDataType->AddNewValue(_T("II"), _T("II"), _T("1"));
	pDataType->AddNewValue(_T("III"), _T("III"), _T("2"));
	pDataType->AddNewValue(_T("IV"), _T("IV"), _T("3"));
	pDataType->AddNewValue(_T("V"), _T("V"), _T("4"));
...
void QCharEditWidget_Distance::UpdateCharacters(BOOL bCharChaged)
{
	...
	m_pGridCharacter->ShowDatas(g_theTestCntrFrame->m_pCharacteristics);
...
// void QCharEditWidget_DistanceSearch::slot_ZKClicked(int nRow, int nCol)
// {
// 	if (nRow < 0)
// 	{
// 		return;
...
// 
// 	QCharEditMainWidget* pMainWidget = (QCharEditMainWidget*)parentWidget();
// 	if (pMainWidget->m_nZKCurrentRow != nRow)
// 	{
// 		pMainWidget->m_nZKCurrentRow = nRow;
...
CCharacteristic* QCharEditWidget_DistSearchOne::GetCharacteristic()
{
	...
	CCharacteristic *pCharacteristic = (CCharacteristic*)g_theTestCntrFrame->m_pCharacteristics->FindByClassID(CHARCLASSID_CCHARACTERISTIC);
...
{
	CCharacteristic *pNew = (CCharacteristic*)pObj->Clone();
	g_theTestCntrFrame->m_pCharacteristics->DeleteAll();
	g_theTestCntrFrame->m_pCharacteristics->AddNewChild(pNew);
	UpdateZKFeature(/*pNew,*/ TRUE);
	emit sig_CharChanged(NULL,TRUE);
}
