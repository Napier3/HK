...
// 	errorTypeList<<QObject::tr("A相接地")<<QObject::tr("B相接地")<<QObject::tr("C相接地")<<QObject::tr("AB短路")<<QObject::tr("BC短路")\
// 		<<QObject::tr("CA短路")<<QObject::tr("AB相接地短路")<<QObject::tr("BC相接地短路")\
// 		<<QObject::tr("CA相接地短路")<<QObject::tr("ABC短路")<<QObject::tr("单相阻抗");
...
{
	QStringList List;
	List<</*QObject::tr(" 跳  A ")*/g_sLangTxt_Gradient_JumpA<</*QObject::tr(" 跳  B ")*/g_sLangTxt_Gradient_JumpB<</*QObject::tr(" 跳  C ")*/g_sLangTxt_Gradient_JumpC<<g_sLangTxt_State_Coincidence<<g_sLangTxt_State_RearAcceleration;//lcq
...
extern bool cacuIFMAX3000( float fstart,float fend,float fstep )
{
	...
	{
		//emit sig_AddLog(LEVEL_WARN,tr("递变点数超过3000点，请重新输入！"));
		return false;
	}
	else
	{
...
extern QString comparaSwitchStatus( int ch[],int ch2[],int num )
{
	...
	if (b[0] != a[0])
	{
		str = /*QObject::tr("跳A")*/g_sLangTxt_Gradient_Jjump + 'A'; //lcq
		return str;
	}

	if (b[1] != a[1])
	{
		str = /*QObject::tr("跳B")*/g_sLangTxt_Gradient_Jjump + 'B';//lcq
		return str;
	}

	if (b[2] != a[2])
	{
		str = /*QObject::tr("跳C")*/g_sLangTxt_Gradient_Jjump + 'C';//lcq
		return str;
	}

	if (b[3] != a[3])
	{
		str = /*QObject::tr("跳D")*/g_sLangTxt_Gradient_Jjump + 'D'; //lcq
		return str;
	}

	if (b[4] != a[4])
	{
		str = /*QObject::tr("跳E")*/g_sLangTxt_Gradient_Jjump + 'E'; //lcq
		return str;
	}

	if (b[5] != a[5])
	{
		str = /*QObject::tr("跳F")*/g_sLangTxt_Gradient_Jjump + 'F'; //lcq
		return str;
	}

	if (b[5] != a[5])
	{
		str = /*QObject::tr("跳G")*/g_sLangTxt_Gradient_Jjump + 'G'; //lcq
		return str;
	}

	if (b[6] != a[6])
	{
		str = /*QObject::tr("跳H")*/g_sLangTxt_Gradient_Jjump + 'H'; //lcq
		return str;
	}

	if (b[7] != a[7])
	{
		str = /*QObject::tr("跳I")*/g_sLangTxt_Gradient_Jjump + 'I'; //lcq
		return str;
	}

	if (b[8] != a[8])
	{
		str = /*QObject::tr("跳J")*/g_sLangTxt_Gradient_Jjump + 'J';//lcq
		return str;
	}

	if (b[9] != a[9])
	{
		str = /*QObject::tr("跳K")*/g_sLangTxt_Gradient_Jjump + 'K'; //lcq
		return str;
	}
...
// 	return;
	...
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前相位值越限出错(%f).")*/g_sLangTxt_Gradient_CurPhaseErr.GetString(),fAngle); //lcq
		fAngle = 0;
		return;
	}
...
;
	...
	if (ch[0] == 1)
	{
		tempStr[0] = /*QObject::tr("跳A")*/g_sLangTxt_Gradient_Jjump + 'A';
	}

	if (ch[1] == 1)
	{
		tempStr[1] = /*QObject::tr("跳B")*/g_sLangTxt_Gradient_Jjump + 'B';
	}

	if (ch[2] == 1)
	{
		tempStr[2] = /*QObject::tr("跳C")*/g_sLangTxt_Gradient_Jjump + 'C';
	}

	if (ch[3] == 1)
	{
		tempStr[3] = /*QObject::tr("跳D")*/g_sLangTxt_Gradient_Jjump + 'D';
	}

	if (ch[4] == 1)
	{
		tempStr[4] = /*QObject::tr("跳E")*/g_sLangTxt_Gradient_Jjump + 'E';
	}

	if (ch[5] == 1)
	{
		tempStr[5] = /*QObject::tr("跳F")*/g_sLangTxt_Gradient_Jjump + 'F';
	}

	if (ch[6] == 1)
	{
		tempStr[6] = /*QObject::tr("跳G")*/g_sLangTxt_Gradient_Jjump + 'G';
	}

	if (ch[7] == 1)
	{ 
		tempStr[7] = /*QObject::tr("跳H")*/g_sLangTxt_Gradient_Jjump + 'H';
	}
