...
SequenceEstimateGrid::SequenceEstimateGrid(int rows, int columns, QWidget* parent) : QTableWidget(rows, columns, parent)
{
...
void SequenceManualEstimateDlg::InitUI()
{
	...
	m_pGrid = new SequenceEstimateGrid(5, 5, this);
	m_pGrid->InitGrid();
...
void SequenceManualEstimateDlg::slot_btnOK_Clicked()
{
	...
	m_pParas->m_fAngleActVal_RelErr = m_pGrid->item(3, 2)->text().toFloat();
	m_pParas->m_fRetCoef_RelErr = m_pGrid->item(4, 2)->text().toFloat();
	...
	m_pParas->m_fAngleActVal_AbsErr = m_pGrid->item(3, 3)->text().toFloat();
	m_pParas->m_fRetCoef_AbsErr = m_pGrid->item(4, 3)->text().toFloat();
	...
	m_pParas->m_nAngleActVal_ErrorLogic = ((QComboBox*)m_pGrid->cellWidget(3, 1))->currentIndex();
	...
	m_pParas->m_fAngleSet = m_pGrid->item(3, 4)->text().toFloat();
	m_pParas->m_fRetCoefSet = m_pGrid->item(4, 4)->text().toFloat();
