...
QSttSelSclFileWidget *g_pSttSelSclFileWidget = nullptr;

QSttSelSclFileWidget::QSttSelSclFileWidget(QWidget *parent)
	: QWidget(parent)
{
    m_pBtns_HBoxLayout = nullptr;
    m_pMainVLayout = nullptr;
    m_pClearAll_Btn = nullptr;
    m_pDelCurr_Btn = nullptr;
    m_pAddScl_Btn = nullptr;
    m_pSclIedSelect_Btn= nullptr;
    m_pSttSelSclFileGrid = nullptr;
}
...
void QSttSelSclFileWidget::slot_AddScl_Clicked()
{
 ...
 #else
	CString strText = g_sLangTxt_Native_SCLPath.GetString();
 	QFileDialog dlg(this, strText);
 	dlg.setFileMode(QFileDialog::AnyFile);
 	//strText = _T("SCD文件(*.scd);;ICD文件(*.icd);;CID文件(*.cid)");
	strText =g_sLangTxt_Native_SCLFilter.GetString();
	...
	//strText = _T("文件路径");
	dlg.setLabelText(QFileDialog::LookIn, g_sLangTxt_FilePath.GetString());
	//strText = _T("文件名");
	dlg.setLabelText(QFileDialog::FileName,g_sLangTxt_FileName.GetString());
	//strText = _T("文件类型");
	dlg.setLabelText(QFileDialog::FileType,g_sLangTxt_Native_FileType.GetString());
 	dlg.setAcceptMode(QFileDialog::AcceptOpen);
	//strText = _T("打开");
	dlg.setLabelText(QFileDialog::Accept,g_sLangTxt_Open.GetString());
 
	//strText = _T("取消");
	dlg.setLabelText(QFileDialog::Reject,g_sLangTxt_Cancel.GetString());
    ...
    CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前系统剩余内存大小%lu B.")*/g_sLangTxt_RemainingMemorySize.GetString(), ufreeram);
     if (nSize * STT_IECCONFIG_FILE_SCALE >= ufreeram)
     {
        CString strTip = /*_T("提示")*/g_sLangTxt_Message.GetString();
        CString strText = /*_T("当前系统剩余内存不足")*/g_sLangTxt_InsufficientSystemMemory.GetString();
...
bool QSttSelSclFileWidget::AddScl_ReNameFile(CString strFilePath)
{
	...
	{
        CXSclFileObj *pXSclFileObjNodes=nullptr;
		...
		while(pos)
		{
			...
			{
				CString strMsg;
				strMsg =/*"当前存在同名SCD文件，是否继续？"*/g_sLangTxt_ExistenceSCDFiles.GetString(); 
...
// 	int result = CXMessageBox::information(this,g_sLangTxt_Message,g_sLangTxt_confirmfiles  , QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
	...
	QPushButton *yesButton = messageBox.addButton(/*tr("是")*/g_sLangTxt_State_Yes.GetString(), QMessageBox::YesRole);
	QPushButton *noButton = messageBox.addButton(/*tr("否")*/g_sLangTxt_State_No.GetString(), QMessageBox::NoRole);
...
// 	}
	...
	if (nPos<=0)
	{
		CLogPrint::LogFormatString(XLOGLEVEL_ERROR,/*_T("当前文件(%s)没有扩展名不是SCL文件.")*/g_sLangTxt_NoExistenceSCLFiles.GetString(),strSclFilePath.GetString());
		return "";
	}
	...
	CTickCount32 oWait; 
	CString strUIText; 
	strUIText = /*"SCL解析"*/g_sLangTxt_AnalysisSCLFiles.GetString();
	oScdFileRWDlg.setWindowTitle(strUIText);
	strUIText = /*"正在加载SCL文件..."*/g_sLangTxt_LoadSCLFiles.GetString();
...
//	CLogPrint::LogFormatString(XLOGLEVEL_RESULT,_T("Time1(%ld)."),oWait.GetTickCountLong());
	...
	strUIText = /*"SCL解析"*/g_sLangTxt_AnalysisSCLFiles.GetString();
	oScdFileRWDlg.setWindowTitle(strUIText);
	strUIText = /*"正在保存SCL配置..."*/g_sLangTxt_SaveSCLFiles.GetString();
...
// 	oSclFileRead.ReadSclFile(strSclFilePath,nullptr, pSclStation);
// 	CTickCount32 oWait;
// 
// 	while( oSclFileRead.IsInReadState() )
// 	{
...
// {
// 	unsigned char *pgbkString = nullptr,*putf8String = nullptr;
