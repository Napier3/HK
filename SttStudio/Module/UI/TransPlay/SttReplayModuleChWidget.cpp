...
void CSttReplayModuleChWidget::initUI()
{
	...
	{
		m_pGridLayout_RcdChs->addWidget(m_plbChanel, 0, 2, 1, 2,Qt::AlignHCenter);
		m_pGridLayout_RcdChs->addWidget(m_plbChanelA, 0, 4, 1, 4,Qt::AlignHCenter);
		m_pGridLayout_RcdChs->addWidget(m_plbChanelB, 0, 9, 1, 4,Qt::AlignHCenter);
	}
	else
	{
	...
	QScrollComboBox *pWeekChTypeBox = NULL;
	...
	while(pos != NULL)
	{
		...
		if(bWeekChChangeModule)
		{
			...
			m_WeekChSelTypeList.append(pWeekChTypeBox);

			if(nChIndex <= 4)
			{
				pWeekChTypeBox->setCurrentIndex(0);
			}
			else
			{
				pWeekChTypeBox->setCurrentIndex(1);
			}
		}
...
// 		m_pGridLayout_RcdChs->addWidget(pedtK, nChIndex, 12, 1, 2,Qt::AlignHCenter);
		...
		if(bWeekChChangeModule)
		{
			m_pGridLayout_RcdChs->addWidget(pWeekChTypeBox, nChIndex, 0, 1, 1,Qt::AlignHCenter);
			m_pGridLayout_RcdChs->addWidget(pChSel, nChIndex, 2, 1, 2);
			m_pGridLayout_RcdChs->addWidget(pAChSel, nChIndex, 4, 1, 4);
			m_pGridLayout_RcdChs->addWidget(plbOptrSub, nChIndex, 8, 1, 1,Qt::AlignHCenter);
			m_pGridLayout_RcdChs->addWidget(pBChSel, nChIndex, 9, 1, 4);
			m_pGridLayout_RcdChs->addWidget(plbOptrDiv, nChIndex, 13, 1, 1);
			m_pGridLayout_RcdChs->addWidget(pedtK, nChIndex, 14, 1, 2,Qt::AlignHCenter);
		}
		else
		{
			m_pGridLayout_RcdChs->addWidget(pChSel, nChIndex, 0, 1, 2);
