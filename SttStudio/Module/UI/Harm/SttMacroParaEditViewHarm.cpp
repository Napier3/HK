...
QSttMacroParaEditViewHarm* g_pHarmTest = nullptr;
...
{
    if (m_pHarmWidget != nullptr)
	{
		delete m_pHarmWidget;
        m_pHarmWidget = nullptr;
	}
}
// 
...
void QSttMacroParaEditViewHarm::UpdateHarmParas()
{
	...
	{
        m_pHarmWidget->m_pExBinParaWidget->setData(m_oTmtHarmTest.m_oHarmParas.m_oBinaryConfig.m_binInEx,nullptr);
	}

	if(m_pHarmWidget->ExistExBoutParaWidget())
	{
        m_pHarmWidget->m_pExBoutParaWidget->setData(nullptr,m_oTmtHarmTest.m_oHarmParas.m_oBinaryConfig.m_binOutEx);
	}
...
void QSttMacroParaEditViewHarm::OnViewTestStart()
{
	...
	{
        if (m_pHarmWidget->m_pGooseParaWidget != nullptr)
		{
        ...
        if (m_pHarmWidget->m_pFT3OutParaWidget != nullptr)
		{
...
void QSttMacroParaEditViewHarm::UpdateGoutTab_UI( CIecCfgGoutDatas* pCfgGoutDatas )
{
    ...
    if ((m_pHarmWidget->m_pGooseParaWidget == nullptr)&&(g_oSystemParas.m_nHasDigital)&&(g_oSttTestResourceMngr.m_nTotalFiberNum_LC> 0))//更新前无GOOSE界面,更新后需要GOOSE界面
	{
    ...
    else if ((m_pHarmWidget->m_pGooseParaWidget != nullptr)&&(!g_oSystemParas.m_nHasDigital))//更新前有GOOSE界面,更新后不需要
	{
    ...
    else if ((m_pHarmWidget->m_pGooseParaWidget != nullptr)&&(g_oSystemParas.m_nHasDigital))
	{
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_GooseDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_RepairDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_GoutMapChanged()), this, SLOT(slot_GoutMapChanged()));
    ...
    else if ((m_pHarmWidget->m_pGooseParaWidget != nullptr)&&(g_oSttTestResourceMngr.m_nTotalFiberNum_LC == 0))//更新前有GOOSE界面,更新后LC口为0不需要
	{
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_GooseDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_RepairDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pHarmWidget->m_pGooseParaWidget, SIGNAL(sig_GoutMapChanged()), this, SLOT(slot_GoutMapChanged()));
...
void QSttMacroParaEditViewHarm::UpdateFT3Tab_UI(CIecCfgDatasSMV* pIecCfgDatasSMV)
{
    ...
    if ((m_pHarmWidget->m_pFT3OutParaWidget == nullptr)&&(g_oSystemParas.IsDigitalOutputFT3())&&(g_oSttTestResourceMngr.m_nTotalFiberNum_STSend> 0) && (nFT3Block > 0))
	{
    ...
    else if ((m_pHarmWidget->m_pFT3OutParaWidget != nullptr)&&(!g_oSystemParas.IsDigitalOutputFT3()))
	{
...
{
    if ((m_pHarmWidget->m_pExBinParaWidget == nullptr)&&(g_oLocalSysPara.m_nCHBinInExNum > 0))
	{
    ...
    else if ((m_pHarmWidget->m_pExBinParaWidget != nullptr)&&(g_oLocalSysPara.m_nCHBinInExNum == 0))
	{
    ...
    if ((m_pHarmWidget->m_pExBoutParaWidget == nullptr)&&(g_oLocalSysPara.m_nCHBinOutExNum>0))
	{
    ...
    else if ((m_pHarmWidget->m_pExBoutParaWidget != nullptr)&&(g_oLocalSysPara.m_nCHBinOutExNum == 0))
	{
...
void QSttMacroParaEditViewHarm::slot_currentChangedTabWidget(int nCurrIndex)
{
	...
	BOOL bIsWaveCtrls = (m_pHarmWidget->m_pTabWidget->currentWidget() == m_pHarmWidget->m_pOscillogramGroupWidget );
	BOOL bVisibleParaCtrls = (m_pHarmWidget->m_pTabWidget->currentWidget() == m_pHarmWidget->m_pParaWidget );
	//判断页面大小
	BOOL bVisibleHeigthCtrls = ( g_theTestCntrFrame->m_pSttFrameConfig->m_nHeight <700 );
	//判断是否隐藏
