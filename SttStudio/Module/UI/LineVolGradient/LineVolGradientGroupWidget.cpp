...
void QLineVolGradientGroupWidget::InitLineVolGridData(tmt_LineVolGradientParasEx* pGradientParas)
{
	...
	while (pos)
	{
		...
		if(pCh->m_strName == "3U1-0")//20231101 zhouyy 将3U1_0通道常态故障态数据都初始化为0
		{
			...
			pData->m_pfAmp = &pGradientParas->m_uiVOL[0][0].Harm[1].fAmp;
			pData->m_pfAngle = &pGradientParas->m_uiVOL[0][0].Harm[1].fAngle;
			pData->m_pfFreq = &pGradientParas->m_uiVOL[0][0].Harm[1].fFreq;
			pData->m_strName = pCh->m_strName;
			m_oNormalVoltList.AddNewChild(pData);

			pData = new QGradientGroupData;
			pData->m_pfAmp = &pGradientParas->m_uiVOL[2][0].Harm[1].fAmp;
			pData->m_pfAngle = &pGradientParas->m_uiVOL[2][0].Harm[1].fAngle;
			pData->m_pfFreq = &pGradientParas->m_uiVOL[2][0].Harm[1].fFreq;
			pData->m_strName = pCh->m_strName;
