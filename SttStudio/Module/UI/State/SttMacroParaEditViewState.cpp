...
QSttMacroParaEditViewState* g_pStateTest = nullptr;
    ...
    :CSttMacroParaEditViewOriginal(parent)
{
	...
	m_bHorizontal = bHorizontal;
    m_pParaTab = nullptr;
    m_pCommonPara = nullptr;
    m_pSettingValue = nullptr;
    m_pEstimate = nullptr;
        m_pBinarySet=nullptr;
    m_pShortCalc = nullptr;
    m_pDiffCurrCalcBtn = nullptr;
    m_pGradientSet = nullptr;
    //m_pFreLabel = nullptr;
    //m_pFreEdit = nullptr;
    m_pDC = nullptr;
    m_pDCPlus = nullptr;
    m_pTao = nullptr;
    m_pLabelTao = nullptr;
    m_pStatePara = nullptr;
    m_pCopyStatePara = nullptr;
    m_pCopyGoutDatas = nullptr;
    m_pCopyFT3Datas = nullptr;
    m_pSVParaSet = nullptr;
        m_pTrigerWidget = nullptr;
	m_nCurStateIndex = 0;
...
{
    if (m_pCopyStatePara != nullptr)
	{
		delete m_pCopyStatePara;
        m_pCopyStatePara = nullptr;
	}

    if (m_pCopyGoutDatas != nullptr)
	{
		delete m_pCopyGoutDatas;
        m_pCopyGoutDatas = nullptr;
	}

    if (m_pCopyFT3Datas != nullptr)
	{
...
{
// 	if (m_pOriginalSttTestResource == nullptr)
// 	{
...
void QSttMacroParaEditViewState::InitGoosePubDataGroups()
{
    ...
    CIecCfgGoutDatas *pIecCfgGoutDatas = nullptr;
...
void QSttMacroParaEditViewState::InitFT3PubDataGroups()
{
    ...
    CIecCfgDatasSMV *pIecCfgDatasSMV = nullptr;
...
// 	}
    ...
    CSttXmlSerializeBase *pXmlChild = nullptr;
	...
	for (int nIndex=0;nIndex<m_oStateTest.m_oStateParas.m_nStateNumbers;nIndex++)
	{
        ...
        if ((pIecCfgGoutDatas != nullptr)&&(bHasGoosePub))
		{
			nType = 0x88b8;
        ...
        if ((pIecCfgDatasSMV != nullptr)&&(bHasGoosePub))
		{
			//sv 的网络标识不可编辑
// 			nType = 0x88ba;
// 			strNet1.Format("%04X",nType);
...
// 	}
	...
	{
        m_pParaTab->m_pExBinParaWidget->setData(m_pStatePara->m_binInEx,nullptr);
	}

	if(m_pParaTab->ExistExBoutParaWidget())
	{
        m_pParaTab->m_pExBoutParaWidget->setData(nullptr,m_pStatePara->m_binOutEx);
	}
...
{
    ASSERT(pStatePara != nullptr);
    tmt_channel* pChannel = nullptr;
...
{
    if ((m_pParaTab->m_pExBinParaWidget == nullptr)&&(g_oLocalSysPara.m_nCHBinInExNum>0))
	{
		m_pParaTab->AddExBinParaWidget();
        m_pParaTab->m_pExBinParaWidget->setData(m_oStateTest.m_oStateParas.m_paraState[m_nCurStateIndex].m_binInEx,nullptr);
		connect(m_pParaTab->m_pExBinParaWidget, SIGNAL(sig_ExSwitchChanged()), this, SLOT(slot_SwitchStateChanged()),Qt::UniqueConnection);
	}
    else if ((m_pParaTab->m_pExBinParaWidget != nullptr)&&(g_oLocalSysPara.m_nCHBinInExNum == 0))
	{
    ...
    if ((m_pParaTab->m_pExBoutParaWidget == nullptr)&&(g_oLocalSysPara.m_nCHBinOutExNum>0))
	{
		m_pParaTab->AddExBoutParaWidget();
        m_pParaTab->m_pExBoutParaWidget->setData(nullptr,m_oStateTest.m_oStateParas.m_paraState[m_nCurStateIndex].m_binOutEx);
		connect(m_pParaTab->m_pExBoutParaWidget, SIGNAL(sig_ExSwitchChanged()), this, SLOT(slot_SwitchStateChanged()),Qt::UniqueConnection);
	}
    else if ((m_pParaTab->m_pExBoutParaWidget != nullptr)&&(g_oLocalSysPara.m_nCHBinOutExNum == 0))
	{
...
void QSttMacroParaEditViewState::UpdateFT3Tab_UI(CIecCfgDatasSMV *pIecCfgDatasSMV)
{
    ...
    if ((m_pParaTab->m_pFT3OutParaWidget == nullptr)&&(g_oSystemParas.IsDigitalOutputFT3())&&(g_oSttTestResourceMngr.m_nTotalFiberNum_STSend> 0) && (nFT3Block > 0))
	{
    ...
    else if ((m_pParaTab->m_pFT3OutParaWidget != nullptr)&&(!g_oSystemParas.IsDigitalOutputFT3()))
	{
    ...
    else if ((m_pParaTab->m_pFT3OutParaWidget != nullptr)&&(g_oSystemParas.IsDigitalOutputFT3())&&(g_oSttTestResourceMngr.m_nTotalFiberNum_STSend> 0))
	{
		if(nFT3Block > 0)
	{
		disconnect(m_pParaTab->m_pFT3OutParaWidget, SIGNAL(sig_FT3DataChanged()), this, SLOT(slot_FT3DataChanged()));
...
{
    tmt_StatePara* pStatePara = nullptr;
	...
	for(int i=0;i<m_oStateTest.m_oStateParas.m_nStateNumbers;i++)
	{
        ...
        CIecCfgSmvDataBase *pIecCfgSmvData = nullptr;
		POS pos = pIecCfgDatasSMV->GetHeadPosition();
...
{
    if ((m_pParaTab->m_pGooseParaWidget == nullptr)&&(g_oSystemParas.m_nHasDigital)&&(g_oSttTestResourceMngr.m_nTotalFiberNum_LC> 0))//更新前无GOOSE界面,更新后需要GOOSE界面
	{
    ...
    else if ((m_pParaTab->m_pGooseParaWidget != nullptr)&&(!g_oSystemParas.m_nHasDigital))//更新前有GOOSE界面,更新后不需要
	{
    ...
    else if ((m_pParaTab->m_pGooseParaWidget != nullptr)&&(g_oSystemParas.m_nHasDigital))
	{
		disconnect(m_pParaTab->m_pGooseParaWidget, SIGNAL(sig_GooseDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pParaTab->m_pGooseParaWidget, SIGNAL(sig_RepairDataChanged()), this, SLOT(slot_GooseDataChanged()));
    ...
    else if ((m_pParaTab->m_pGooseParaWidget != nullptr)&&(g_oSttTestResourceMngr.m_nTotalFiberNum_LC == 0))//更新前有GOOSE界面,更新后LC口为0不需要
	{
		disconnect(m_pParaTab->m_pGooseParaWidget, SIGNAL(sig_GooseDataChanged()), this, SLOT(slot_GooseDataChanged()));
		disconnect(m_pParaTab->m_pGooseParaWidget, SIGNAL(sig_RepairDataChanged()), this, SLOT(slot_GooseDataChanged()));
...
// 	stt_state_paras_insert_before(&m_oStateTest.m_oStateParas, nullptr, m_nCurStateIndex);
...
// 	stt_state_paras_insert_after(&m_oStateTest.m_oStateParas, nullptr, m_nCurStateIndex);
...
//	CLogPrint::LogFormatString(XLOGLEVEL_ERROR,_T("[问题查找]OnViewTestStop_StateTest."));
    ...
    m_pBtnWidget->stopInit(m_pCopyStatePara != nullptr);
...
//	char *strReport = nullptr,*strNewReport = nullptr;
//	long nLen = 0;
