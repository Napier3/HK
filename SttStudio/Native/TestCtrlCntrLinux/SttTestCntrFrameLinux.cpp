...
BOOL QSttTestCntrFrameLinux::CreateEngineClientWidget()
{
    ...
    //sun }
    //sun
    //sun CString strDevMode;
    //sun //strDevMode = g_oSttSystemConfig.GetDevModel();
    //sun
    //sun CSttAdjDevice *pCurDevice = &g_oSttTestResourceMngr.m_oCurrDevice;
    //sun strDevMode = pCurDevice->m_strModel;
    //sun
    //sun QPpSttIotEngineClientWidgetMain *pMainWidget  = NULL;
    //sun
    //sun if (strDevMode == _T("PNS331") || strDevMode.Find(_T("PTU200L")) >= 0)
    //sun {
    //sun     //pMainWidget = new QSttCommCfgDeviceWidget();
    //sun }
    //sun else
    //sun {
    //sun 	pMainWidget = new QPpSttIotEngineClientWidgetMain();
    //sun }
    //sun
    //sun
//su//sun n	m_pEngineClientWidget = pMainWidget;
    //sun pMainWidget->setParent(this);
    //sun pMainWidget->hide();
    //sun pMainWidget->setGeometry(m_rcWorkArea);
    //sun
// 	//sun if (g_pTheSttTestApp->m_pDvmDevice != NULL)
// 	//sun {
// 	//sun 	m_pEngineClientWidget->ShowDvmDevice( g_pTheSttTestApp->m_pDvmDevice );
// 	//sun 	m_pEngineClientWidget->NewPpSttIotDeviceClient(&g_pTheSttTestApp->m_oCommConfig, g_pTheSttTestApp->m_pDvmDevice);
// 	//sun }
    //sun
    //sun connect(m_pEngineClientWidget,  SIGNAL(sig_OnConfigDevice(CDataGroup *)), this, SLOT(slot_OnConfigDevice(CDataGroup *)));
	return TRUE;
}
...
void QSttTestCntrFrameLinux::InitUI()
{
	...
	//项目列表视图
    //sun CSttFrame_GbTree *pTree = m_pSttFrameConfig->GetGbTree();
    //sun m_pSttGbTreeView = new QSttGuideBookTreeWidget(NULL, pTree, this);
    //sun m_pSttGbTreeView->hide();
	...
	InitRemoteCtrl();
	//初始化完成，再加载报告视图
    //sun m_pSttReportViewHtml = new QSttReportViewHtml(this);
    //sun m_pSttReportViewHtml->hide();
    //sun m_pSttReportViewHtml->OpenHtmlFile(STT_REPORT_HEAD_HTML_FILE);   //打开报告头文件
...
void QSttTestCntrFrameLinux::OnCmd_PpIotEngineClientDlg()
{
    ...
    //sun if (m_pEngineClientWidget == NULL)
    //sun {
    //sun 	CreateEngineClientWidget();
    //sun }

    //sun QSttCommCfgDeviceDlg oSttComDlg(m_pEngineClientWidget,this);
	//oSttComDlg.setParent(this);
	//=====================
//sun #ifdef _USE_SoftKeyBoard_
//sun 	QSoftKeyBoard::AttachObj(&oSttComDlg, Keyboard::NUMBER);
//sun #endif
//sun 	oSttComDlg.exec();
#ifdef _USE_SoftKeyBoard_
...
void QSttTestCntrFrameLinux::OnCmd_PpIotEngineClient()
{
    ...
    //sun SetActiveWidget(m_pEngineClientWidget, ActiveWidgetType_Other);
...
void QSttTestCntrFrameLinux::FillReport_ForState()
{
    ...
    //sun CSttReports oSttReports;
    //sun CString strFile;
    //sun strFile = stt_ui_GetDebugFile("Debug_TestFillReport.xml");
    //sun oSttReports.OpenXmlFile(strFile, CSttCmdDefineXmlRWKeys::g_pXmlKeys);
    //sun CSttReport *pReport = (CSttReport*)oSttReports.GetTail();
    //sun
    //sun if (pReport == NULL)
    //sun {
    //sun 	return;
    //sun }
    //sun
    //sun char *pszReport = NULL;
    //sun pReport->GetXml_ASCII(CSttCmdDefineXmlRWKeys::g_pXmlKeys, &pszReport, 0, _JSON_TYPE_);
    //sun //m_pSttReportViewHtml->FillReportData("Table", "StateTest$StateTest", "NORMAL", pszReport);
    //sun
    //sun delete pszReport;
}
