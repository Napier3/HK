...
void CDrawVariable::DrawElements(CDC* pDC, double fZoomRatio,const CPoint &ptOffset, DWORD dwDrawFlag)
{
	if (m_pDrawList != NULL)
	{
		m_pDrawList->LogicPointToDevicePoint(m_pRulerAxis);
		m_pDrawList->Draw(pDC, fZoomRatio, ptOffset, dwDrawFlag );
	}
}
