...
BOOL CRcdComtradeFile::InsertNormalCyclePoints(double dVolValue,double dCurrentValue,double dTime_Second)
{
	...
	long nInsertPoints = dTime_Second*m_pdSamRate[0];//插入的采样点数
...
void CRcdComtradeFile::UpdateSampleRaes(long nSamAmount,const double* pdSamRate,const long* pnDataPoints )
{
	...
	m_nSamAmount = nSamAmount;
	m_pnDataPoints = new long[nSamAmount];
	m_pdSamRate    = new float[nSamAmount];

	// 复制数据点和采样率
	for (int i = 0; i < nSamAmount; ++i) 
	{
		m_pnDataPoints[i] = pnDataPoints[i];
		m_pdSamRate[i] = pdSamRate[i];
	}
	...
	if(nBeginIndex == -1 || nEndIndex == -1 || nEndIndex < nBeginIndex)
{
	...
	double *pdSamRate = NULL;
	long *pnDataPoints = NULL;

	if(nBeginIndex != nEndIndex)//不在同一个区间
	{
		pdSamRate = new double(nNewSamAmount);
		pnDataPoints = new long(nNewSamAmount);

		if(nDeleteSegNum > 1)//跨采样段数
		{
			...
			for (int nIndex = 0; nIndex < m_nSamAmount; nIndex++)
			{
				...
				{
					pnDataPoints[nNewIndex] = m_pnDataPoints[nIndex];
					pdSamRate[nNewIndex] = m_pdSamRate[nIndex];
					nNewIndex++;
				}
				else if (nIndex == nBeginIndex)
				{
					pnDataPoints[nNewIndex] = m_pnDataPoints[nIndex] - nBeginDistance;
					pdSamRate[nNewIndex] = m_pdSamRate[nIndex];
					nNewIndex++;
				}
				else if (nIndex == nEndIndex)
				{
					pnDataPoints[nNewIndex] = m_pnDataPoints[nIndex] - nEndDistance;
					pdSamRate[nNewIndex] = m_pdSamRate[nIndex];
					nNewIndex++;
				}
			}
		}
		else if(nDeleteSegNum == 1)
		{
			m_pnDataPoints[nBeginIndex] -= nBeginDistance;
			m_pnDataPoints[nEndIndex] -= nEndDistance ;
		}

		UpdateSampleRaes(nNewSamAmount,pdSamRate,pnDataPoints);
	}
	else
	{
		//更新当前段采样点数
...
void CRcdComtradeFile::UpdateSamDateAfterInsert( int nBeginPos, int nEndPos, int nInsertPos, int nCycNum )
{
	...
	if(nBeginIndex != nEndIndex)//不在同一个区间
	{
		...
		for (int nIndex = 0; nIndex < m_nSamAmount; nIndex++)
		{
			if(nIndex >= nBeginIndex && nIndex <= nEndIndex)
			{
				...
				{
					pnInsertDataPoints[nNewIndex] = nBeginDistance;
		...
		//插入的后半部分
		int nLastIndex = nInsertIndex + (nEndIndex - nBeginIndex + 1) * nCycNum + 1;
		// 复制原数组的前半部分
		for (int nIndex = 0; nIndex <= nInsertIndex; ++nIndex) 
		{
			if(nIndex == nInsertIndex)
			{
				...
				//插入的后半部分
				pnNewDataPoints[nLastIndex] = m_pnDataPoints[nInsertIndex] - nInsertDistance;
				pdNewSamRate[nLastIndex] = m_pdSamRate[nIndex];
				nLastIndex++;
			}
			else
			{
		...
		// 复制原数组的后半部分
		for (int nIndex = nInsertIndex + 1; nIndex < m_nSamAmount; ++nIndex) 
		{
			pdNewSamRate[nLastIndex] = m_pdSamRate[nIndex];
			pnNewDataPoints[nLastIndex] = m_pnDataPoints[nIndex];
			nLastIndex++;
		}
	...
	{
		m_pnDataPoints[nBeginIndex] += (nEndDistance-nBeginDistance) * nCycNum;
	}
