...
BOOL CDvmMapTool::OpenDvmCmpFile(const CString &strFile)
{
	...
	if (m_oDvmDeviceMap.m_strName.GetLength() <= 5)
	{
		...
		dvm_PopOpenDvmDlg(strStandDvmFile, AfxGetMainWnd(), g_sLangTxt_SelStdDataFile/*_T("请选择标准设备数据模型文件")*/);
	}
	else
	{
...
{
	if (m_pPrgsInterface != NULL)
	{
		CString strMsg;
		strMsg = _T("Open: ") + GetPathFileNameFromFilePathName(strStandDvmFile);
		strMsg += _T("    ") + GetPathFileNameFromFilePathName(strDestDvmFile);
		m_pPrgsInterface->ShowMsg(strMsg);
	}
...
void CDvmMapTool::UpdateMapDataset_Map(CDvmDatasetMap *pDatasetMap)
{
    ...
    strPath.Format(_T("%s$%s"), pDatasetMap->m_strLdviceMap.GetString(), pDatasetMap->m_strMapID.GetString());
	...
	{
        CLogPrint::LogFormatString(XLOGLEVEL_ERROR, g_sLangTxt_DataSetNoOnly.GetString()/*_T("数据集【%s】不唯一")*/
                                   , strPath.GetString());
	}
...
void CDvmMapTool::CmpDvmDevice()
{
	...
	{
		//不精确的计算
		long nCount = m_pStand->GetDatasetCount() + m_pMap->GetDatasetCount();
		m_pPrgsInterface->SetMaxRange(nCount);
	}

	pos = m_oDvmDeviceMap.GetHeadPosition();
...
void CDvmMapTool::CmpDvmLdevice_Map_Dest(CDvmLDeviceMap *pLDviceMap, CDvmLogicDevice *pDestCmpLdevice, BOOL bFullCmp)
{
	...
	while (pos!= NULL)
	{
		...
		while (posDs != NULL)
		{
			...
			{
				CString strMsg;
				strMsg = pDvmDataset->m_strID + _T("  << == >>  ") + pStandDataset->m_strID;
				m_pPrgsInterface->ShowMsg(strMsg);
			}
			...
			else if (nCmpRate > nMaxCmpRate)
			{
 			...
 			else if (pStandDataset->m_strID.Find(_T("dsSetting")) >= 0 || pStandDataset->m_strID.Find(_T("dsRelayEna")) >= 0)
 			{
				...
				//20231025 huangliang 操作改变的是pAdd->m_pStand
				//需要同步pDvmDatasetMap->m_pStand数据
				CDvmData *pDataSrc = NULL;
				CDvmData *pDataUpdate = NULL;
				int iNumCount = pStandDataset->GetCount();
				for(long i=0; i<iNumCount; i++)
				{
					pDataSrc = (CDvmData *)((CDvmDataset *)pAdd->m_pStand)->GetAt(i);
					pDataUpdate = (CDvmData *)pStandDataset->GetAt(i);
					if(pDataSrc->m_dwReserved == 1)	//仅同步使用标识即可
					{
						pDataUpdate->m_dwReserved = 1;
					}
				}
 			}
			else
...
// 	}
	...
	if (pData != NULL)
	{
		...
		//20231102 huangliang 新增CDvmData和CDvmValue间匹配
		if(pData->GetCount() <= 0 && pStandData->GetCount() > 0)
		{			
			CmpDataMapValue(pDestDataset, pStandData, pDvmCmpObj);
		}
		else
		{
...
//20231102 huangliang
void CDvmMapTool::CmpDataMapValue(CDvmDataset *pDestDataset, CDvmData *pStand, CDvmDataMap *pDataMap)
{
	POS pos = pStand->GetHeadPosition();
	...
	while (pos != NULL)
	{
		...
		if(CmpDataMapValue(pDestDataset, pValue, pDataMap, bDelParent))
		{
			bDelParent = false;
		}
	}
}
bool CDvmMapTool::CmpDataMapValue(CDvmDataset *pDestDataset, CDvmValue *pStandValue, CDvmDataMap *pDataMap, bool bDelParent)
{
	CDvmData *pData = NULL;
	CDvmValueMap *pValueMap = NULL;

	//查找可以与pStandValue比较的pData
	pData = FindCmpDataMapValue(pDestDataset, pStandValue);
	...
	if (pData != NULL)
	{
		...
		if (pValueMap != NULL)
		{
			pValueMap->SetMap0(pData);
		}
		else
		{
			pValueMap = pDataMap->AddDvmMap(pStandValue, pData);
		}
		return true;
	}
	return false;
...
//20231102 huangliang 
CDvmData* CDvmMapTool::FindCmpDataMapValue(CExBaseList *pDest, CDvmValue *pStandValue)
{
	POS pos = pDest->GetHeadPosition();
	...
	{
		//遍历pStand的每一个Value，判断是否可以与pDestCmpValue进行比较
		pValue = (CDvmData *)pDest->GetNext(pos);
		...
		//20231107 huangliang 匹配度 后缀相同时 +1
		if (pValue->m_strID.Right(nLen) == strValueAttr)
		{
			nCmpRate += 1;
		}
