...
CXDes2::CXDes2()
{
	...
	InitializeKey("#power-sky-lijq##power-sky-lijq#", 0);
}
CXDes2::~CXDes2()
{
}

void CXDes2::InitializeKey(char* srcBytes,unsigned int keyN)
{
...
void CXDes2::CreateSubKey(char* sz_56key,unsigned int keyN)
{
	char szTmpL[28] = {0};
	char szTmpR[28] = {0};
	char szCi[28] = {0};
...
void CXDes2::EncryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void CXDes2::DecryptData(char* _srcBytes,unsigned int keyN)
{
	char szSrcBits[64] = {0};
	char sz_IP[64] = {0};
	char sz_Li[32] = {0};
...
void CXDes2::FunctionF(char* sz_Li,char* sz_Ri,unsigned int iKey,unsigned int keyN)
{
	char sz_48R[48] = {0};
	char sz_xor48[48] = {0};
	char sz_P32[32] = {0};
...
void CXDes2::InitialPermuteData(char* _src,char* _dst)
{
	//IP
	for(int i=0;i<64;i++)
	{
...
void CXDes2::ExpansionR(char* _src,char* _dst)
{
	for(int i=0;i<48;i++)
	{
...
void CXDes2::XOR(char* szParam1,char* szParam2, unsigned int uiParamLength, char* szReturnValueBuffer)
{
...
void CXDes2::CompressFuncS(char* _src48, char* _dst32)
{
	char bTemp[8][6]={0};
	char dstBits[4]={0};
	for(int i=0;i<8;i++)
...
void CXDes2::PermutationP(char* _src,char* _dst)
{
	for(int i=0;i<32;i++)
	{
...
void CXDes2::Bytes2Bits(char *srcBytes, char* dstBits, unsigned int sizeBits)
{
	for(unsigned int i=0; i < sizeBits; i++)
		dstBits[i] = ((srcBytes[i>>3]<<(i&7)) & 128)>>7;
}

void CXDes2::Bits2Bytes(char *dstBytes, char* srcBits, unsigned int sizeBits)
{
...
void CXDes2::Int2Bits(unsigned int _src, char* dstBits)
{
	for(unsigned int i=0; i < 4; i++)
		dstBits[i] = ((_src<<i) & 8)>>3;
}

void CXDes2::Bits2Hex(char *dstHex, char* srcBits, unsigned int sizeBits)
{
...
void CXDes2::Hex2Bits(char *srcHex, char* dstBits, unsigned int sizeBits)
{
...
char* CXDes2::GetCiphertextInBinary()
{
...
char* CXDes2::GetCiphertextInHex()
{
...
char* CXDes2::GetCiphertextInBytes()
{
	return szCiphertextInBytes;
}

char* CXDes2::GetPlaintext()
{
...
char* CXDes2::GetCiphertextAnyLength()
{
	return szFCiphertextAnyLength;
}

char* CXDes2::GetCiphertextAnyLength_Hex()
{
	return szFCiphertextAnyLengthHex;
}

char* CXDes2::GetPlaintextAnyLength()
{
	return szFPlaintextAnyLength;
}

void CXDes2::EncryptAnyLength(char* _srcBytes,unsigned int _bytesLength,unsigned int keyN)
{
	if(_bytesLength == 8)
	{
	...
	else if(_bytesLength > 8)
	{
		...
		_bytesLength= (iParts+1)<<3;
	}

	char *pSrc = szFCiphertextAnyLength;
	char *pDest = szFCiphertextAnyLengthHex;
	ValueToHex((BYTE*)szFCiphertextAnyLength, _bytesLength,szFCiphertextAnyLengthHex, FALSE, FALSE);
}

void CXDes2::DecryptAnyLengthHex(char* _srcBytes, int nLen, unsigned int keyN)
{
	static char _srcBytes_buffer[8192];
	ZeroMemory(_srcBytes_buffer, 8192);
	HexToValue((const char*)_srcBytes, (BYTE*)_srcBytes_buffer);
	DecryptAnyLength(_srcBytes_buffer, nLen, keyN);
}

void CXDes2::DecryptAnyLength(char* _srcBytes,unsigned int _bytesLength, unsigned int keyN)
{
	ZeroMemory(szFPlaintextAnyLength, 8192);
	...
	char *pTemp = szFPlaintextAnyLength;
}

CString CXDes2::Encrypt(const CString &strText)
{
	CString strDest;
	char *pText = NULL;
	long nLen = CString_to_char(strText, &pText);

	EncryptAnyLength(pText, strlen(pText), 0);
	strDest = GetCiphertextAnyLength_Hex();
	delete pText;

	return strDest;
}

CString CXDes2::Decrypt(const CString &strText)
{
	char *pText = NULL;
	...
	DecryptAnyLengthHex(pText,nLen, 0);
	strDestDest = GetPlaintextAnyLength();
...
long CXDes2::Encrypt(const CString &strText, char *pszBuffer)
{
	char *pText = NULL;
	long nLen = CString_to_char(strText, &pText);
	...
	sprintf(pszBuffer, "%d$", nLen);
	strcat(pszBuffer, szFCiphertextAnyLengthHex);
...
long CXDes2::Decrypt(const CString &strText, char *pszBuffer)
{
	char *pText = NULL;
	CString strLen;
	long nPos = strText.Find(_T("$"));
	long nLen = 8;
	CString strSrc;

	if (nPos > 0)
	{
		strLen = strText.Left(nPos);
		nLen = CString_To_long(strLen);
		strSrc = strText.Mid(nPos+1);
	}
	else
	{
		strSrc = strText;
	...
	CString_to_char(strSrc, &pText);
	CString strDestDest;

	DecryptAnyLengthHex(pText,nLen, 0);
	strcpy(pszBuffer, szFPlaintextAnyLength);
...
long CXDes2::Decrypt(char *strSrcText, CString &strDest)
{
	char *pText = NULL;
	char *pFind = strstr(strSrcText, "$");
	long nLen = 8;
	CString strSrc;
	...
	DecryptAnyLengthHex(pText,nLen, 0);
	strDest = szFPlaintextAnyLength;

	return strDest.GetLength();
}


long CXDes2::Decrypt(char *strText, char *pszBuffer)
{
	char *pText = NULL;
	char *pFind = strstr(strText, "$");
	long nLen = 8;
	CString strSrc;
...
CString xdes_Encrypt(const CString &strText)
{
	...
	CXDes2 oXdes;
	return oXdes.Encrypt(strText);
}


CString xdes_Decrypt(const CString &strText)
{
	if (strText.GetLength() == 0)
	{
		return strText;
	...
	CXDes2 oXdes;
	return oXdes.Decrypt(strText);
}


long xdes_Encrypt(const CString &strText, char *pszBuffer)
{
	CXDes2 oXdes;
	return oXdes.Encrypt(strText, pszBuffer);
}

long xdes_Decrypt(const CString &strText, char *pszBuffer)
{
	CXDes2 oXdes;
	return oXdes.Decrypt(strText, pszBuffer);
}

long xdes_Decrypt(char *strText, char *pszBuffer)
{
	CXDes2 oXdes;
	return oXdes.Decrypt(strText, pszBuffer);
}

long xdes_Decrypt(char *strSrcText, CString &strDest)
{
	CXDes2 oXdes;
	return oXdes.Decrypt(strSrcText, strDest);
}
