...
CXmlRWNodeBase* CJsonNode::FindLocalChildNode(CJSON *pNode)
{
    ...
    CJsonNode *pJsonNode = nullptr;
    CJsonNode *pFind = nullptr;
...
CXmlRWNodeBase* CJsonNode::GetChildNode(const CString &strNodeName)
{
    ...
    if (m_pJsonNode == nullptr)//20220601  zhouhj 此处根节点解析有问题,但为确保与之前功能使用兼容,暂时还原回之前方式
	{
...
{
    //清除json节点中数组子对象的string=nullptr，否则析构错误
...
long CJsonRWDoc::GetXml(char **ppszRetBuffer, long nOffset)
{
    if (m_pJsonDoc == nullptr)
	{
        *ppszRetBuffer = nullptr;
		return 0;
	}
