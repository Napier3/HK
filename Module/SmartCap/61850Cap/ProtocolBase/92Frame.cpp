...
*/
		...
		case 0x80:	
			{
				ReadTagValue(nLen,  Num_ASDU,  pBufferPos);
				pDatas->oData92.Num_ASDU = Num_ASDU; // add mym 2020-07-13

				//zhouhj 2023.10.25 增加用于处理,增加了安全标记可选项的报文
				BYTE bSecurityByte = *Ep_GetBufferCurr(pBufferPos);

				if (bSecurityByte == 0x81)
				{
					Ep_OffsetBufferCurr(pBufferPos, 1);
					bSecurityByte = *Ep_GetBufferCurr(pBufferPos);

					if (bSecurityByte == 0x81)
					{
						Ep_OffsetBufferCurr(pBufferPos, 2);
					}
					else if (bSecurityByte == 0x82)
					{
						Ep_OffsetBufferCurr(pBufferPos, 3);
					}
				}
				break;
			}
	...
	for(int i  =  0; i  <  Num_ASDU;  i++)
	{
		...
		while (TRUE)
		{
			...
			case 0x81:	
				{
					ReadTagValue(nLen, nValue, pBufferPos);
					break;
				}		
			case 0x82:
				{
			...
			if (byteTag == 0x87)
			{	
				nChCount  = nLen/8;//20220614  zhouhj  通道数等于通道字节长度除以8
...
//		nChCount /= 8;//20220614  zhouhj 已在前面除以8
		...
		for (long nChIndex=1; nChIndex<=nChCount; nChIndex++)
		{
			...
			pData->dwValue  = (int)Ep_BufferReadDWord2(pBufferPos);
			pData->dwValue2 = (int)Ep_BufferReadDWord2(pBufferPos);
			pData++;
		}
	}
