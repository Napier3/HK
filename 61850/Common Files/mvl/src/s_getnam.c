...
/* Now create the response data structure	*/
  ...
  switch (req_info->obj.mms_class)
    ...
    case MMS_CLASS_VAR:                		/* NamedVariables 	*/
      num_obj = u_gnl_ind_vars(req_info, ptr, &resp_info->more_follows, 
				MVL_MAX_GNL_NAMES);
    break;

    case MMS_CLASS_VARLIST:                  	/* NamedVariableLists 	*/
      num_obj = u_gnl_ind_nvls(req_info, ptr, &resp_info->more_follows, 
				MVL_MAX_GNL_NAMES);
    break;
    ...
    case MMS_CLASS_JOU:	      	/* Journals 		*/
      num_obj = u_gnl_ind_jous(req_info, ptr, &resp_info->more_follows, 
				MVL_MAX_GNL_NAMES);
    break;
...
/* First get the number of objects and pointer to the object table	*/
  ...
  if (req_info->objscope == VMD_SPEC)
    ...
    {
    domCtrl = mvl_find_dom (req_info->dname);
    if (domCtrl)
      {
...
/* First get the number of objects and pointer to the object table	*/
  ...
  switch (req_info->objscope)
    ...
    case DOM_SPEC:
      dom = mvl_find_dom (req_info->dname);
      if (dom)
        {
        num_obj = dom->num_jou;
        jou = dom->jou_tbl;
