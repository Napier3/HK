...
/************************************************************************/
/* This global variable is currently used only for support for AA scope	*/
/* objects.								*/

MVL_NET_INFO *_mvl_curr_net_info;

/************************************************************************/

ST_VOID *_mvl_curr_usr_ind_ctrl;

/************************************************************************/
/* These functions are used internally by mvl_comm_serve ()		*/

#ifdef _MVL_TYPE_CTRL_use_one_device_
static ST_VOID mvl_msg_rcvd (MVL_COMM_EVENT *event, MVL_TYPE_CTRL *mvl_type_ctrl);
#else
...
{
	MVL_COMM_EVENT *pEvent;
	ST_BOOLEAN activityFlag;
	pEvent = NULL;

	if (_mvl_comm_serve_active == SD_TRUE)
	{
		//mvl_log_err ("Error: mvl_comm_serv re-entered");
		return (SD_FALSE);
	}
	...
	/* to deal with								*/
	activityFlag = _mvl_net_service (nSelPipe, &pEvent);
	...
	if (pEvent)
	{
		...
		case MMS_MSG_RCVD :
#ifdef _MVL_TYPE_CTRL_use_one_device_
			mvl_msg_rcvd (pEvent, g_mvl_type_ctrl);
#else
			mvl_msg_rcvd (pEvent);
#endif
			break;
		...
		default :
			_mvl_free_comm_event (pEvent);	/* All done with this event	*/
			break;
		}
	}
...
#endif
{
	...
	if (rslt->err_code == NO_DECODE_ERR)
	{
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
			mvl_conf_rcvd (event, mvl_type_ctrl);
#else
			mvl_conf_rcvd (event);
#endif
			break;
		...
		case MMSCNCLRESP :
			...
			req_pend = _mvl_find_req_ctrl (event->net_info, MMSOP_CONCLUDE, -1);
			if (!req_pend)
			{
		...
		case MMSCANRESP	:
			mvl_log_err ("Unexpected MMS PDU type received: Cancel Response");
			_mvl_send_reject (event);
			break;

		case MMSCANERR :
			mvl_log_err ("Unexpected MMS PDU type received: Cancel Error");
			_mvl_send_reject (event);
			break;

		case MMSINITREQ	:
			mvl_log_err ("Unexpected MMS PDU type received: Initiate Request");
			_mvl_send_reject (event);
			break;

		case MMSINITRESP :
			mvl_log_err ("Unexpected MMS PDU type received: Initiate Response");
			_mvl_send_reject (event);
			break;

		case MMSINITERR	:
			mvl_log_err ("Unexpected MMS PDU type received: Initiate Error");
			_mvl_send_reject (event);
			break;

		case UNKNOWNTYPE :
			mvl_log_err ("Unexpected MMS PDU type received: Unknown");
			_mvl_send_reject (event);
			break;
...
ST_VOID mvl_clr_ind_que (MVL_NET_INFO *net_info)
{
	...
	while(pend_ind!=NULL)
	{
		next_ind = list_get_next(net_info->pend_ind,pend_ind);
...
static ST_VOID mvl_error_rcvd (MVL_COMM_EVENT *event)
{
	...
	if (!req)
	{
		mvl_log_err ("Error Received : mvl_find_req_ctrl failed");
		_mvl_free_comm_event (event);	/* All done with this event	*/
		return;
	}
	req->event = event;
...
static ST_VOID mvl_reject_rcvd (MVL_COMM_EVENT *event)
{
	...
	{
		req = _mvl_find_req_ctrl (event->net_info, -1, info->invoke);
		if (!req)
		{
			mvl_log_err ("Reject Received : mvl_find_req_ctrl failed");
			_mvl_free_comm_event (event);	/* All done with this event	*/
			return;
		}
		req->event = event; 
	...
	{
		mvl_log_err ("Reject Received : no invoke");
		_mvl_free_comm_event (event);	/* All done with this event	*/
	}
}
...
ST_VOID _mvl_send_reject (MVL_COMM_EVENT *event)
{
	...
	{
		rc = MVL_ERR_UNKNOWN_PDU_TYPE;
		mvl_log_err ("Sending Reject : Unable to decode PDU type, can't create reject");
	}
	...
	while (req->done == SD_FALSE)    /* wait forever until done		*/
	{
		if (u_mvl_check_timeout)
		{
			if ((*u_mvl_check_timeout)())
			{
				...
				mvl_log_err ("Wait Request Done : u_mvl_check_timeout timed out"); 
				break;
			}
		}
		mvl_comm_serve (nSelPipe);        	/* service communications		*/
	}    
	return (req->result);
}
...
MVL_REQ_PEND *_mvl_get_req_ctrl (MVL_NET_INFO *net_info, ST_INT16 op)
{
	...
	if (net_info == NULL)
	{
		mvl_log_err ("_mvl_get_req_ctrl error: connection invalid");
		return (NULL);
	}
	/*  req = (MVL_REQ_PEND *) M_CALLOC (MSMEM_REQ_CTRL, 1, sizeof (MVL_REQ_PEND));*/
	S_LOCK_COMMON_RESOURCES ();
	req = &req_pend_table[net_info->nACSI];
...
ST_VOID mvl_free_req_ctrl (MVL_REQ_PEND *pend_req)
{
	...
	{
		S_LOCK_COMMON_RESOURCES ();
		pend_req->canReturn = SD_FALSE;
		pend_req->done = SD_FALSE;
		...
		if (pend_req->event)
			...
			{	/* Only do this if it was allocated (see confirm function)	*/
				for (nIndex = 0;nIndex<pend_req->u.fdir.resp_info->num_dir_ent;nIndex++)
				{
					mms_str_quit(&pend_req->u.fdir.resp_info->dir_ent[nIndex].filename);
				}
