...
#ifdef DEBUG_SISCO
/* SD_CONST static ST_CHAR *SD_CONST thisFileName = __FILE__;  uncomment when needed */
#endif
...
ST_RET mvl_get_runtime (ST_INT type_id, 
						RUNTIME_TYPE **rt_out,
						ST_INT *num_rt_out, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
ST_RET mvl_get_runtime (ST_INT type_id, 
						RUNTIME_TYPE **rt_out,
						ST_INT *num_rt_out)
...
#endif
{
	...
	if (type_id >= mvl_num_types - _mvlu_num_dyn_types &&	/* dynamic type	*/
		type_id < mvl_num_types &&
		type_ctrl->num_rt > 0)
	{
		/* Cast "rt" to remove SD_CONST.	*/
...
/************************************************************************/
#ifdef _MVL_TYPE_CTRL_use_one_device_
MVL_TYPE_CTRL *mvl_type_ctrl_find (ST_INT TypeId, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
...
/************************************************************************/
#ifdef _MVL_TYPE_CTRL_use_one_device_
MVL_TYPE_CTRL *mvl_type_ctrl_find_dyn (ST_INT TypeId, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
...
#endif
{
	...
	if (TypeId < mvl_num_types - _mvlu_num_dyn_types)
		type_ctrl = NULL;				/* NOT dynamic. Return error*/
	else
	{
#ifdef _MVL_TYPE_CTRL_use_one_device_
		type_ctrl = mvl_type_ctrl_find (TypeId, mvl_type_ctrl);	/* COULD BE dynamic, find it*/
#else
...
/************************************************************************/
#ifdef _MVL_TYPE_CTRL_use_one_device_
ST_CHAR *mvl_typeid_to_typename (ST_INT type_id, MVL_TYPE_CTRL *mvl_type_ctrl)		
#else
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
ST_INT mvl_typename_to_typeid (ST_CHAR *type_id_string, MVL_TYPE_CTRL *mvl_type_ctrl)	 
#else
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
ST_RET mvlu_add_rt_type (RUNTIME_TYPE *rt, ST_INT numRt, ST_INT *typeIdOut, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
...
#endif
{
	...
	mvlu_type_id_base = mvl_num_types - _mvlu_num_dyn_types;
...
/* will be available for future use by mvlu_add_ty_type			*/

#ifdef _MVL_TYPE_CTRL_use_one_device_
ST_VOID mvlu_free_rt_type (ST_INT typeId, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
ST_VOID mvlu_free_rt_type (ST_INT typeId)
#endif
{
	if (typeId >= mvl_num_types - _mvlu_num_dyn_types)
		mvl_type_ctrl[typeId].num_rt = 0;
}
