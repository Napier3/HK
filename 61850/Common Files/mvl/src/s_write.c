...
static ST_VOID put_asn1_data (VARIABLE_LIST *vl,
...
/* Looks OK, prepare to process each variable in the list		*/
  ...
  for (i = 0; i < num_var; ++i, ++vl_ptr, ++va_ptr, ++vaCtrl)
    put_asn1_data (vl_ptr, va_ptr->data, va_ptr->len, vaCtrl);
...
MVL_TYPE_CTRL *type_ctrl = NULL;
  ...
  /* See if we need to restore the previous data */
    if (vaCtrl->va && (wr_rslt->resp_tag == WR_RSLT_FAILURE))
      {
#ifdef _MVL_TYPE_CTRL_use_one_device_
		  type_ctrl = mvl_type_ctrl_find (vaCtrl->va->type_id, type_ctrl);
#else
...
static ST_VOID put_asn1_data (VARIABLE_LIST *vl,
                             ST_UCHAR *asn1_buffer, ST_INT asn1_len,
			     MVLAS_WR_VA_CTRL *vaCtrl, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
static ST_VOID put_asn1_data (VARIABLE_LIST *vl,
							  ST_UCHAR *asn1_buffer, ST_INT asn1_len,
							  MVLAS_WR_VA_CTRL *vaCtrl)
...
/* There are more ways to fail than to succeed! */
  ...
  if (alt_access_pres)  /* no alternate access support now	*/
    ...
    mvl_log_err ("Put ASN1 Data : Alternate Access not supported");
    return;
#else
  /* now create the AA, given the ASN.1 spec				*/
    if (ms_asn1_to_aa (vl->alt_access.data, vl->alt_access.len, 
           	       &alt_acc))
      {
      mvl_log_err ("Write Indication : ASN.1 -> AA failure");
      return;
      }
#endif
    }
  ...
  switch (vl->var_spec.var_spec_tag)
    {
    case VA_SPEC_NAMED :
      va = _mvl_objname_to_va (MMSOP_WRITE, &vl->var_spec.vs.name, 
	    		       &vaCtrl->va_scope,
	      		       alt_access_pres,
          		       &alt_acc,
          		       &alt_access_done);
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
      type_ctrl = mvl_type_ctrl_find (va->type_id, mvl_type_ctrl);	/* find type	*/
#else
      type_ctrl = mvl_type_ctrl_find (va->type_id);	/* find type	*/
#endif
      va_data_size = type_ctrl->data_size;
      vaCtrl->oldData = M_MALLOC (MSMEM_GEN, va_data_size);
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
      rc = mvl_get_runtime (va->type_id, &rt, &num_rt, mvl_type_ctrl);
#else
  ...
  /* the data pointer initialized.					*/
    ...
    default :
      mvl_log_err ("Put ASN1 Data : Invalid Variable Specification Tag");
      return;
    break;
    }
...
/* local data format							*/
  ...
  if (alt_access_pres == SD_FALSE || aa_mode == MVL_USE_NO_AA)
    rc = ms_asn1_to_local (rt, num_rt, asn1_buffer, asn1_len, temp_data_buf,NULL);
  else
    {
  ...
  if (rc != SD_SUCCESS)
    {
    MVL_LOG_NERR1 ("Put ASN1 Data : ms_asn1_to_local failed, rc = %x", rc);
    M_FREE (MSMEM_GEN, vaCtrl->oldData);
  ...
  if (va->proc && va->proc->pre_write_aa)
    ...
    if (rc != SD_SUCCESS)
      {
      MVL_LOG_NERR1 ("Put ASN1 Data : call to Write pre-processor failed, rc = %x", rc);
      M_FREE (MSMEM_GEN, vaCtrl->oldData);
      vaCtrl->va = NULL;
      }
    }
...
/* a chance to look at it. If rc == SD_SUCCESS, we are OK.		*/
  ...
  /* Call the postprocessing function for this variable, if it exists	*/
    if (va->proc && va->proc->post_write_aa)
      (*va->proc->post_write_aa)(va, alt_access_pres, &alt_acc, _mvl_curr_net_info);
    }
  ...
  if (vaCtrl->resp_tag == WR_RSLT_SUCCESS)
