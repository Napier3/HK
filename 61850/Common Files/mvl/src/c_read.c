...
ST_RET mvl_read_variables (ST_INT  nSelPipe, MVL_NET_INFO  *net_info,
								  ...
								  MVL_REQ_PEND *req_pend)
{
	...
	if (read_info->va_spec.var_acc_tag == VAR_ACC_VARLIST)
	{
		...
		for (i = 0; i < num_data; ++i, ++vl)
		{
			if (parse_info[i].alt_acc_pres == SD_TRUE)
			{
				...
				if (rc != SD_SUCCESS)
				{
					mvl_log_err ("Error creating ASN.1 encoded Alternate Access");
					return (SD_FAILURE);
				}
			}
		}
	...
	/* Log the request */
	MVL_LOG_RESP (MMSOP_READ, read_info);
	return (rc);
}

/************************************************************************/
#ifdef _MVL_TYPE_CTRL_use_one_device_
ST_VOID _mvl_process_read_conf (MVL_REQ_PEND *req, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
...
#endif
{
	...
	{
		mvl_log_err ("Read Confirmation : num var != num result");
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
			if (mvl_get_runtime (parse_table->type_id,&rt, &rt_len, mvl_type_ctrl) == SD_SUCCESS)
#else
