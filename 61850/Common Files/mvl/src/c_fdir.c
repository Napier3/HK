...
ST_RET mvl_fdir (ST_INT  nSelPipe, 
...
ST_CHAR fdir_req_info[FDIR_REQ_INFO_SIZE];
  ...
  if ((ca_filename)&&(strlen(ca_filename)>0))//zhouhj20190521 保证在文件名称为空的情况下，不需要继续之前的
    {
  ...
  if (resp_info->num_dir_ent)
    ...
    for (j = 0;  j < resp_info->num_dir_ent;  j++, mvl_dir_ent++)
      ...
      mvl_dir_ent->mtime	   = dir_ent->mtime;
	   mms_str_init(&mvl_dir_ent->filename);
//	  mms_str_init2(&mvl_dir_ent->filename,((FILE_NAME *)(dir_ent + 1))->fn_len);
	  mms_str_assign2(&mvl_dir_ent->filename,((FILE_NAME *)(dir_ent + 1))->fname,((FILE_NAME *)(dir_ent + 1))->fn_len);

//       if (_mvl_concat_filename (mms_string(&mvl_dir_ent->filename), dir_ent->num_of_fname, 
//                       (FILE_NAME *)(dir_ent + 1), strlen(mms_string(&mvl_dir_ent->filename))))
//         {
