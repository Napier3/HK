...
/* Internal functions */
static ST_VOID ma_wait_req_done (ST_INT  nSelPipe, ST_RET *retOut);
#if (MMS_INIT_EN & RESP_EN)
static MVL_NET_INFO *ma_get_called_conn_ctrl (NET_BIND_CTRL *);
#endif
#if (MMS_INIT_EN & REQ_EN)
static MVL_NET_INFO *ma_get_calling_conn_ctrl (ST_INT);
#endif
static ST_VOID copp_unbind_sync (ST_INT  nSelPipe, ST_LONG copp_bind_id);
static ST_RET ma_bind (ST_INT  nSelPipe, NET_BIND_CTRL *bc, ST_BOOLEAN activate);

static MVL_COMM_EVENT *mvl_get_com_event (ST_INT);
...
ST_BOOLEAN _mvl_net_service (ST_INT  nSelPipe, MVL_COMM_EVENT **eventOut)
{
	...
	*eventOut = NULL;
	...
	activityFlag = copp_event (nSelPipe);
...
ST_INT mvl_start_acse (ST_INT  nSelPipe, MVL_CFG_INFO *cfgInfo)
{
	...
	*mvl_cfg_info = *cfgInfo;
	...
	for (i = 0; i < mvl_cfg_info->num_calling; ++i){
		mvl_calling_conn_ctrl[i].nACSI = i;
		//		memset(mvl_calling_conn_ctrl[i].EntryID, 0, (MAX_RPTID_NUM+1)*(MAX_DSNAME+1));
	}
#endif
	...
	for (i = 0; i < mvl_cfg_info->num_called; ++i)
		mvl_called_conn_ctrl[i].index = i;
#endif
	...
	for (i = 0; i < mvl_cfg_info->num_bind_ctrl; ++i)
	{	
		ret = ma_bind (nSelPipe, &mvl_cfg_info->bind_ctrl[i], SD_TRUE);
		if (ret != SD_SUCCESS)
			return (ret);
	}
...
ST_INT mvl_end_acse (ST_INT  nSelPipe)
{
	NET_BIND_CTRL *bc;
	ST_INT i;
	...
	for (i = 0; i < mvl_cfg_info->num_bind_ctrl; ++i)
	{
		...
		if (bc->copp_bind_id_tcp)
			copp_unbind_sync (nSelPipe, bc->copp_bind_id_tcp);
		if (bc->copp_bind_id_tp4)
			copp_unbind_sync (nSelPipe, bc->copp_bind_id_tp4);
#else	/* !LEAN_T	*/
...
ST_INT mvl_initiate_req (ST_INT  nSelPipe,
						 ST_CHAR *remAr,
	INIT_INFO *req_info, 
	INIT_INFO *resp_info,
	MVL_NET_INFO **net_info_out)
{
	...
	rc = mvl_wait_req_done (nSelPipe, req);
	if (rc != SD_SUCCESS)
	{
		mvl_log_err ("ACSE: Initiate failed");
	}
	...
	S_SEC_ENCRYPT_CTRL *encrypt_info)
{
	...
	*req_out = NULL;			/* make sure it's initialized	*/
	...
	net_info_new = ma_get_calling_conn_ctrl (remDe->idx);
	//  cc->index = remDe->idx;

	req = _mvl_get_req_ctrl (net_info_new, MMSOP_INITIATE);
	if (!req){
		net_info_new->in_use = SD_FALSE;
	...
	*req_out = req;
	...
	if (rc == SD_SUCCESS){
		...
		memcpy (&net_info_new->locl_init_info, req_info, sizeof (INIT_INFO));
		...
		else    locDe = net_info_new->bind_ctrl->de;
		...
		else      aarq_apdu.auth_info.auth_pres = SD_FALSE;
		...
		if (remDe->pres_addr.tp_type == TP_TYPE_TCP)
			acse_conn_id = a_associate_req (net_info_new->bind_ctrl->copp_bind_id_tcp, (ST_LONG) net_info_new, &aarq_apdu, remDe->idx);
		else      acse_conn_id = a_associate_req (net_info_new->bind_ctrl->copp_bind_id_tp4, (ST_LONG) net_info_new, &aarq_apdu, remDe->idx);
	...
	if (rc == SD_SUCCESS)
	{
		*net_info_out = net_info_new;
		net_info_new->acse_conn_id = acse_conn_id;
	}
	else
	{
		net_info_new->in_use = SD_FALSE;
...
ST_RET mvl_add_bind_ctrl (ST_INT  nSelPipe, NET_BIND_CTRL *bc)
{
	NET_BIND_CTRL *mvlBc;
	ST_BOOLEAN activate;
	ST_INT i;
	...
	{
		mvl_log_err ("Error: no availble bind control");
		return (SD_FAILURE);
	}
	...
	*mvlBc = *bc;
	...
	rc = ma_bind (nSelPipe, mvlBc, activate);
	return (rc);
}
	...
	AARQ_APDU *info)
{
	...
	if (cc)
	{
		...
		if (rslt.err_code == NO_DECODE_ERR && rslt.type == MMSINITREQ)
		{
			...
			if (ret != SD_SUCCESS)
			{
				mvl_log_err ("ACSE: User does not want connection");
		...
		{
			mvl_log_err ("ACSE: Initiate indication PDU parse error");
			free_dec_info = SD_FALSE;
			cc->in_use = SD_FALSE;
		}
	}
	...
	AARE_APDU *info)
{
	...
	if (info->result == SD_SUCCESS) 
	{
		...
		if (rslt.err_code == NO_DECODE_ERR)
		{
			...
			case MMSINITERR	:
				result = SD_FAILURE;
				cc->in_use = SD_FALSE;
				mvl_log_err ("ACSE: initiate confirm error"); 
		...
		{
			mvl_log_err ("ACSE: associate confirm PDU Decode Error");
			result = SD_FAILURE;
			info->result = SD_FAILURE;	/* Cause disconnect		*/
			cc->in_use = SD_FALSE;
			++mvl_mms_statistics.callingConnFail;
	...
	{
		mvl_log_err ("ACSE: associate confirm result != SD_SUCCESS");
		cc->in_use = SD_FALSE;
		++mvl_mms_statistics.callingConnFail;
	}
...
ST_VOID u_a_data_ind (ST_LONG user_conn_id, ST_UINT data_len, ST_UCHAR *data)
{
	...
	commEvent = mvl_get_com_event (ni->nACSI);
	if (!commEvent)
	{
...
/************************************************************************/
#if (MMS_INIT_EN & REQ_EN)
static MVL_NET_INFO *ma_get_calling_conn_ctrl (ST_INT n)
{
	MVL_NET_INFO *ni;

	if(n >= mvl_cfg_info->calling_used)
		return NULL;	
	ni = &mvl_calling_conn_ctrl[n];
	ni->in_use = SD_TRUE;
	ni->conn_active = SD_FALSE;
	return (ni);
}
