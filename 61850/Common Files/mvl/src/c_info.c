...
ST_ULONG mvl_info_data_to_local (MVL_COMM_EVENT *event,
				ST_INT num_va,
				MVL_VAR_ASSOC **info_va, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
ST_ULONG mvl_info_data_to_local (MVL_COMM_EVENT *event,
								 ST_INT num_va,
...
ST_ULONG  asn1ptr;
  ...
  /* Call the preprocessing function for this variable, if it exists	*/
    ...
    /* and store in local buffer                           */
#ifdef _MVL_TYPE_CTRL_use_one_device_
      if (mvl_get_runtime (va->type_id, &rt, &rt_len, mvl_type_ctrl) == SD_SUCCESS)
#else
      ...
      {
      mvl_log_err ("InfoRpt Ind : Access Result == SD_FAILURE");
      va->result = SD_FAILURE;
      }
  ...
  if (va_spec->var_acc_tag == VAR_ACC_NAMEDLIST)
    ...
    for (j = 0; j < info_ptr->num_of_acc_result; ++j, ++vl)
      ...
      if (vl->var_spec.var_spec_tag == VA_SPEC_NAMED)
        ...
        {
        mvl_log_err ("InfoRpt Ind : VA Spec not named");
        M_FREE (MSMEM_GEN, info_va);
        _mvl_free_comm_event (event);	/* All done with this event	*/
        return;
        }
