...
MVL_REQ_PEND *req_out;
  ...
  if (strlen (destfilename) <= MAX_FILE_NAME)
    ...
    {
    mvl_log_err ("ERROR: mvla_fget: destination filename save failed ");
    return (SD_FAILURE);
    }
  ...
  if (strlen (srcfilename) <= MAX_FILE_NAME)
    ...
    {
    mvl_log_err ("ERROR: mvla_fget: source filename save failed ");
    return (SD_FAILURE);
    }
  ...
  if (_mvl_tmpnam (fget_req_info->tempfilename) != SD_SUCCESS  ||
    ...
    if (fget_req_info->fp )
      ...
      mvl_log_err ("ERROR: mvla_fget: temp file creation failed ");
      }
    return(SD_FAILURE);
    }
  else
  ...
  if (ret == SD_SUCCESS)  /* read succeeded */
    ...
    resp_info = req->u.fread.resp_info;
     ...
     if ( fwrite (resp_info->filedata, sizeof(ST_CHAR), resp_info->fd_len, fget_req_info->fp) 
      ...
      {	 /* write failed */
      mvl_log_err ("ERROR: u_fread_done: fwrite failed ");
      ret = SD_FAILURE;
