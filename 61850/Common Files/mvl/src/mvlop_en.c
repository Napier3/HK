...
MVL_IND_PEND *indCtrl;
  ...
  if (indCtrl == NULL)
    {
    mvl_log_err ("Could not allocate indication control, rejecting");
    _mvl_send_reject (event);
    return;
    }
...
/************************************************************************/
 ...
 ST_VOID mvl_conf_rcvd (MVL_COMM_EVENT *event, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
		_mvl_process_read_conf (req_pend, mvl_type_ctrl);
#else
...
/* See if user has path they want to use */
  if (mvl_tmpfile_path != NULL)
    {
    pathLen = strlen (mvl_tmpfile_path);
    if (pathLen > MVL_MAX_TMPFILE_PATH)
      {
      mvl_log_err ("Error: temp file path too long, using working dir");
      }
    else
      {
      strcpy (dest, mvl_tmpfile_path);
      p = dest + pathLen;
      }
    }
