...
/* Now find the named variable list */
  ...
  nvlist = u_mvl_get_nvl (MMSOP_GET_VLIST, &req_info->vl_name, 
                         _mvl_curr_net_info);
#else	/* default	*/
  nvlist = mvl_find_nvl (&req_info->vl_name);
#endif	/* default	*/
  ...
  if (nvlist)
    ...
    /* For backwards compatibility only ... */
      if (nvlist->va_scope != NULL)
        ...
        {
        mvl_log_err ("Warning: Info Rpt nvl->va_scope is NULL, using VMD scope");
        va_scope = &vmdVaScope;
...
#if defined(MVL_UCA) || defined(USE_MANUFACTURED_OBJS)
    u_mvl_free_nvl (MMSOP_GET_VLIST, nvlist, _mvl_curr_net_info);
    ...
    {  
    mvl_log_err ("Get Var List Attributes Indication : could not find the list");
