...
#include "suicacse.h"
#include "acse2usr.h"
#include "..\mvl\usr\client\mms_string.h"
...
ST_BOOLEAN mvl_comm_serve (ST_INT  nSelPipe);
...
typedef struct mvl_net_info
 ...
 ST_INT index;			/* NET_INFO table index for this elmnt	*/
  ...
  ST_LONG acse_conn_id;		/* ACSE's connection ID, internal	*/
  ...
  S_SEC_LOC_AR *loc_ar_sec;
  S_SEC_REM_AR *rem_ar_sec;

//  ST_VOID *user_info[MAX_RPTID_NUM+1];		/* MVL user can use this for 'whatever'	*/
  ST_INT  nACSI;
//  ST_CHAR dsName[MAX_RPTID_NUM+1][MAX_DSNAME+1];
//  ST_CHAR rptID[MAX_RPTID_NUM+1][MAX_DSNAME+1];
//  ST_CHAR EntryID[MAX_RPTID_NUM+1][MAX_DSNAME+1];
  } MVL_NET_INFO;
...
#define MVL_MAX_TMPFILE_PATH  100
...
ST_RET _mvl_tmpnam (ST_CHAR *dest);
...
{
	DBL_LNK l;			 /* For linked list			*/
	ST_BOOLEAN	reconnect;	/*线程据此连接*/
	ST_BOOLEAN	canReturn;  /*线程据此返回消息*/
	ST_UINT32 invoke_id;             
	time_t request_time;
...
ST_RET mvl_wait_req_done (ST_INT  nSelPipe, MVL_REQ_PEND *req);
...
/* Identify */
ST_RET mvl_identify (ST_INT  nSelPipe, MVL_NET_INFO *net_info, MVL_REQ_PEND **req_out);
ST_RET mvla_identify (MVL_NET_INFO  *net_info, MVL_REQ_PEND **req_out);

/* Status */
ST_RET mvl_status (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, STATUS_REQ_INFO *req_info,
...
/* GetNameList */
ST_RET mvl_getnam (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, NAMELIST_REQ_INFO *req_info,
		   MVL_REQ_PEND **req_out);
...
/* Read */
ST_RET mvl_read_variables (ST_INT  nSelPipe, MVL_NET_INFO *net_info, READ_REQ_INFO *read_info,
                          ST_INT num_data, 
			  MVL_READ_RESP_PARSE_INFO *parse_info,
                          MVL_REQ_PEND **req_out);
...
/* Write */
ST_RET mvl_write_variables (ST_INT  nSelPipe, MVL_NET_INFO *net_info, WRITE_REQ_INFO *write_info,
...
/* GetVariableAccessAttributes */
ST_RET mvl_getvar (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, GETVAR_REQ_INFO *req_info, 
		   MVL_REQ_PEND **req_out);
ST_RET mvla_getvar (MVL_NET_INFO  *net_info, GETVAR_REQ_INFO *req_info,
		    MVL_REQ_PEND **req_out);

/* DefineNamedVariableList */
ST_RET mvl_defvlist (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, DEFVLIST_REQ_INFO *req_info,
		     MVL_REQ_PEND **req_out);
ST_RET mvla_defvlist (MVL_NET_INFO  *net_info, DEFVLIST_REQ_INFO *req_info,
		      MVL_REQ_PEND **req_out);

/* GetNamedVariableListAttributes */
ST_RET mvl_getvlist (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, GETVLIST_REQ_INFO *req_info,
		     MVL_REQ_PEND **req_out);
ST_RET mvla_getvlist (MVL_NET_INFO  *net_info, GETVLIST_REQ_INFO *req_info,
		      MVL_REQ_PEND **req_out);

/* DeleteNamedVariableList */
ST_RET mvl_delvlist (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, DELVLIST_REQ_INFO *req_info,
		     MVL_REQ_PEND **req_out);
ST_RET mvla_delvlist (MVL_NET_INFO  *net_info, DELVLIST_REQ_INFO *req_info,
		      MVL_REQ_PEND **req_out);

/* GetDomainAttributes */
ST_RET mvl_getdom (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, GETDOM_REQ_INFO *req_info,
		   MVL_REQ_PEND **req_out);
ST_RET mvla_getdom (MVL_NET_INFO  *net_info, GETDOM_REQ_INFO *req_info,
		    MVL_REQ_PEND **req_out);

/* File Open */
ST_RET mvl_fopen (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, ST_CHAR *filename,
		  ST_UINT32 init_pos, MVL_REQ_PEND **req_out);
ST_RET mvla_fopen (MVL_NET_INFO  *net_info, ST_CHAR *filename,
		   ST_UINT32 init_pos, MVL_REQ_PEND **req_out);

/* File Read */
ST_RET mvl_fread (ST_INT  nSelPipe, MVL_NET_INFO *net_info, FREAD_REQ_INFO *req_info,
		  MVL_REQ_PEND **req_out);
ST_RET mvla_fread (MVL_NET_INFO *net_info, FREAD_REQ_INFO *req_info,
		   MVL_REQ_PEND **req_out);

/* File Close */
ST_RET mvl_fclose (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, FCLOSE_REQ_INFO *req_info,
		   MVL_REQ_PEND **req_out);
ST_RET mvla_fclose (MVL_NET_INFO  *net_info, FCLOSE_REQ_INFO *req_info,
		    MVL_REQ_PEND **req_out);

/* File Directory */
ST_RET mvl_fdir (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, ST_CHAR *filename,
		 ST_CHAR *ca_filename, MVL_REQ_PEND **req_out);
ST_RET mvla_fdir (MVL_NET_INFO  *net_info, ST_CHAR *filename,
		  ST_CHAR *ca_filename,	MVL_REQ_PEND **req_out);

/* File Delete */
ST_RET mvl_fdelete (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, ST_CHAR *filename,
		    MVL_REQ_PEND **req_out);
ST_RET mvla_fdelete (MVL_NET_INFO  *net_info, ST_CHAR *filename,
		     MVL_REQ_PEND **req_out);

/* File Rename */
ST_RET mvl_frename (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, ST_CHAR *curfilename,
		    ST_CHAR *newfilename, MVL_REQ_PEND **req_out);
ST_RET mvla_frename (MVL_NET_INFO  *net_info, ST_CHAR *curfilename,
		    ST_CHAR *newfilename, MVL_REQ_PEND **req_out);

/* Obtainfile */
ST_RET mvl_obtfile (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, ST_CHAR *srcfilename,
...
/* InitializeJournal */
ST_RET mvl_jinit (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, JINIT_REQ_INFO *req_info,
		  MVL_REQ_PEND **req_out);
ST_RET mvla_jinit (MVL_NET_INFO  *net_info, JINIT_REQ_INFO *req_info,
		   MVL_REQ_PEND **req_out);

/* ReadJournal */
ST_RET mvl_jread (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, JREAD_REQ_INFO *req_info,
		  MVL_REQ_PEND **req_out);
...
/* JournalStatus */
ST_RET mvl_jstat (ST_INT  nSelPipe, MVL_NET_INFO  *net_info, JSTAT_REQ_INFO *req_info,
		  MVL_REQ_PEND **req_out);
...
/* Conclude */
ST_RET mvl_concl (ST_INT  nSelPipe, MVL_NET_INFO *net_info, MVL_REQ_PEND **req_out);
...
/* information report 							*/
ST_RET u_mvl_info_rpt_ind (MVL_COMM_EVENT *event);
...
ST_ULONG mvl_info_data_to_local (MVL_COMM_EVENT *event,
				ST_INT num_va,
...
ST_BOOLEAN _mvl_net_service (ST_INT  nSelPipe, MVL_COMM_EVENT **eventOut);
