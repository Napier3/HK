...
#define MAX_TSEL_LEN	4	/* GOSIP Ver2 recommends len of 2	*/ 
...
ST_BOOLEAN tp4_event (ST_INT  nSelPipe);
...
ST_INT tp4_ds_start (ST_VOID);
ST_INT tp4_ds_finish (ST_VOID);
ST_INT tp4_ds_retrieve (char *name, DS_INFO *ds_info);
ST_INT tp4_ds_first_local (DS_INFO *ds_info);
ST_INT tp4_ds_next_local (DS_INFO *ds_info);
ST_INT tp4_ds_first_remote (DS_INFO *ds_info);
ST_INT tp4_ds_next_remote (DS_INFO *ds_info);

/* Function for MOSI implementation:	*/
ST_RET tp4_session_timer (ST_LONG tp_conn_id, ST_BOOLEAN start);

#if defined(_WINDOWS)
ST_INT odbc_ds_start ();
ST_INT odbc_ds_finish ();
ST_INT odbc_ds_retrieve (char *name, DS_INFO *ds_info);
ST_INT odbc_ds_first_local (DS_INFO *ds_info);
ST_INT odbc_ds_next_local (DS_INFO *ds_info);
ST_INT odbc_ds_first_remote (DS_INFO *ds_info);
ST_INT odbc_ds_next_remote (DS_INFO *ds_info);
#endif

ST_RET tp0_initialize (ST_UINT max_conns, ST_UINT max_spdu_len);
ST_RET tp0_terminate (ST_VOID);

ST_BOOLEAN tp0_event (ST_INT  nSelPipe);
...
/* Global Variables							*/
/************************************************************************/
/* CLTP decode function pointer.	*/
extern ST_VOID (*cltp_decode_nsdu_fun) (N_UNITDATA *n_unitdata);
...
#define tp4_event(a)		tp0_event(a)
