...
extern "C" {
#endif
/* Uncomment this define to allow dynamic type creation.		*/
...
/* MMS-LITE uses a slightly different form of RUNTIME_TYPE, which is    */
/* ROM-able and a bit more compact. Foundry uses the standard form.	*/
#if defined(MMS_LITE) && (!defined(USE_RT_TYPE_2))
#define USE_RT_TYPE_3	/* Default for MMS Lite				*/
#endif
...
struct runtime_type
  ...
  ST_RTINT   el_size;	/* number of bytes occupied by this element	*/
  ...
  union
    ...
    struct		 /* primitive element				*/
      {
      ST_RTINT el_len;	 /* precision, in bytes except bitstr, time	*/
    ...
    struct		 /* Array (top or bottom)			*/
      {
      ST_RTINT num_elmnts;  /* # array elements (index limit)		*/
      ST_RTINT num_rt_blks; /* # runtime blks to get to strt/end elmnt	*/
#if !defined(USE_RT_TYPE_2) && !defined(USE_RT_TYPE_3)
      ST_BOOLEAN packd;	 /* Indicates if packed or not.			*/
#endif
...
ST_RET ms_asn1_to_local (SD_CONST RUNTIME_TYPE *rt, ST_INT num_rt,
			ST_UCHAR *asn1, ST_INT asn1_len, ST_CHAR *dest, ST_ULONG *);
