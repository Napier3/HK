...
#include "asn1defs.h"
#include "mloguser.h"
#include "../mvl/usr/client/sharemem.h"
...
static ST_VOID err_get_cac (ASN1_DEC_CTXT *aCtx)
  ...
  g_nCurErrorClass = err_info_ptr->eclass;
  g_nCurErrorCode = err_info_ptr->code;
...
ASN1_ENC_CTXT *aCtx = &localEncCtx;
  ...
  if (!(ret = _ms_check_ind_actv (indptr)))
    ...
    if (indptr->llp & (LLC30_AO_LLP | LLC30_AL_LLP))
      {
      indptr->add_addr_info->i.LLC.send_type = CONFIRMED_RESP_PDU;
      indptr->add_addr_info->i.LLC.req_ptr = (ST_CHAR *)indptr;
      indptr->add_addr_info->i.LLC.use_rem_addr = SD_TRUE;
      }
