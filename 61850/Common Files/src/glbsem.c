...
ST_MUTEX_SEM gs_sock_mutex;
ST_MUTEX_SEM gs_soe_mutex;
ST_MUTEX_SEM gs_rlysoe_mutex;
ST_MUTEX_SEM gs_ping_mutex;
...
/* Initialize the MUTEX semaphores. Both must succeed, so OR the returns.*/
  ...
  rc |= gs_mutex_create (&gs_sock_mutex);
  rc |= gs_mutex_create (&gs_ping_mutex);

  rc |= gs_named_mutex_create(&gs_soe_mutex, SOEFIFO_LOCK);
  rc |= gs_named_mutex_create(&gs_rlysoe_mutex, RELAYSOEFIFO_LOCK);
