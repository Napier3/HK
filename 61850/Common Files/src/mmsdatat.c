...
extern ST_BOOLEAN m_use_long_ints;
  ...
  /* Variable length items can have a data length of 0 */
    if (*data_dest_len)
      ...
      *data_dest = chk_calloc (1, (*rt)->offset_to_last);
      ...
      rc = ms_asn1_to_local (*rt, *t_len, asn1_data, asn1_data_len, *data_dest, NULL);
      if (rc)
        {
        chk_free (*data_dest);
        if (!rt_in)		/* RT table was chk_calloc'd			*/
...
ASN1_DEC_CTXT *aCtx = &localDecCtx;
  ...
  if (*tptr != NULL)
    ...
    {
    rt_base = chk_calloc (m_rt_type_limit, sizeof (RUNTIME_TYPE));
...
ST_INT st_size;
  ...
  switch (tag)		/* Calc storage size based on element fundamentals */
    ...
    case RT_GENERAL_TIME:		/* Generalized time */
      st_size = sizeof (ST_INT32);	/* Storage is always a ST_INT32 */
      el_len = 4;
    break;
...
/* If the user want's to use all ST_INT32 integers, so be it		*/
  if (m_use_long_ints && el_len < 4)
    el_len = 4;
