...
ST_RET sxd_get_runtime (ST_INT type_id, 
  ...
  if (type_id >= 0 && type_id < mvl_num_types && 
    ...
    *rt_out = (RUNTIME_TYPE *) mvl_type_ctrl[type_id].rt;
...
RUNTIME_TYPE *rt_tmp;
  ...
  if (ec->sxdXmlStyle == SXD_XML_STYLE_A)
    ...
    {
    sx_start_element (usr, elName, 0, NULL);
    }
...
#endif
  return (_sxdEncElement (usr, rt, _i1DtStr, buf));
  }
...
ST_CHAR buf[50];
  ...
  sprintf (buf, "%d", intVal);
#endif
  return (_sxdEncElement (usr, rt, _i2DtStr, buf));
  }
...
#if defined(_WIN32) || defined(__QNX__)
  _ltoa (long_val, buf, 10);
#else
  sprintf (buf, "%ld", long_val);
#endif
  return (_sxdEncElement (usr, rt, _i4DtStr, buf));
  }
...
ST_CHAR buf[80];
  ...
  sprintf (buf, "%lld", i64_val);
#elif (defined(__alpha) && !defined(__VMS)) 
  sprintf (buf, "%ld", i64_val);
#else
...
#if defined(_WIN32) || defined(__QNX__)
  _itoa ((int) uintVal, buf, 10);
#else
  sprintf (buf, "%u", uintVal);
#endif
  return (_sxdEncElement (usr, rt, _uI1DtStr, buf));
  }
...
#endif
  return (_sxdEncElement (usr, rt, _uI2DtStr, buf));
  }
...
#if defined(_WIN32) || defined(__QNX__)
  _ultoa (ulong_val, buf, 10);
#else
  sprintf (buf, "%lu", ulong_val);
#endif
  return (_sxdEncElement (usr, rt, _uI4DtStr, buf));
  }
...
#elif defined(_AIX) || defined(__hpux) || defined (linux) || defined(sun) || defined(__LYNX)
  sprintf (buf, "%llu", u64_val);
#elif (defined(__alpha) && !defined(__VMS)) 
  sprintf (buf, "%lu", u64_val);
#else
  #error Missing INT64 code for this platform.
#endif
...
ST_CHAR buf[100];
  ...
  sprintf (buf,"%.10f", d);
  return (_sxdEncElement (usr, rt, _r4DtStr, buf));
  }
...
ST_CHAR buf[100];
  ...
  d =  *data_ptr;
  sprintf (buf,"%.16f", d);
  return (_sxdEncElement (usr, rt, _r8DtStr, buf));
  }
...
ST_RET rc;
  ...
  buf = M_MALLOC (NULL, (3 * dataLen) + 1);
  hex_to_ascii_str (buf, data_ptr, dataLen, SD_TRUE);
  rc = _sxdEncElement (usr, rt, _binHexDtStr, buf);
  M_FREE (NULL, buf);
  return (rc);
  }
...
ST_CHAR *valStr;
  ...
  return (_sxdEncElement (usr, rt, _boolDtStr, valStr));
  }
...
ST_RET rc;
  ...
  rc = _sxdEncElement (usr, rt, _bitStringDtStr, buf);
  M_FREE (NULL, buf);
  return (rc);
  }
...
static ST_RET _sxdEncVis   (ST_VOID *usr, ST_CHAR  *data_ptr, RUNTIME_TYPE *rt)
  {
  return (_sxdEncElement (usr, rt, _visStringDtStr, data_ptr));
  }
...
ST_RET rc;
  ...
  return (_sxdEncElement (usr, rt, _dateTimeDtStr, buf));
  }
...
ST_CHAR timeText[_SXD_TIME_BUF_LEN];
  ...
  return (_sxdEncElement (usr, rt, _dateTimeDtStr, timeText));
  }
...
ST_RET _sxd_wr_data (SX_ENC_CTRL *sxEncCtrl, ST_INT typeId,
		   ...
		   ST_BOOLEAN encodeDt, ST_BOOLEAN *elPres, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
ST_RET _sxd_wr_data (SX_ENC_CTRL *sxEncCtrl, ST_INT typeId,
					 ST_CHAR *elName, ST_VOID *data, ST_INT sxdXmlStyle,
...
#ifdef _MVL_TYPE_CTRL_use_one_device_
  rc = sxd_get_runtime (typeId, &rt, &numRt, mvl_type_ctrl);
#else
...
ST_RET rc;
  ...
  rc = sxd_process_arb_data (data, rt, numRt, sxEncCtrl, 
...
#elif defined(_AIX) || defined(__hpux) || defined (linux) || defined(sun) || defined(__LYNX)
  if (sscanf (dc->valBuf, "%lld", &i64Val) != 1)
    return (SD_FAILURE);
#elif (defined(__alpha) && !defined(__VMS)) 
  if (sscanf (dc->valBuf, "%ld", &i64Val) != 1)
    return (SD_FAILURE);
#else
  #error Missing INT64 code for this platform.
#endif
...
#if defined(_WIN32)
  SXLOG_CDEC1 ("Integer64: %I64d", i64Val);
#elif defined(_AIX) || defined(__hpux) || defined (linux) || defined(sun) || defined(__LYNX)
  SXLOG_CDEC1 ("Integer64: %lld", i64Val);
#elif (defined(__alpha) && !defined(__VMS)) 
  SXLOG_CDEC1 ("Integer64: %ld", i64Val);
#else
  #error Missing INT64 code for this platform.
#endif
...
#if defined(_WIN32)
  if (sscanf (dc->valBuf, "%I64u", &intVal) != 1)
    return (SD_FAILURE);
#elif defined(_AIX) || defined(__hpux) || defined (linux) || defined(sun) || defined(__LYNX)
  if (sscanf (dc->valBuf, "%llu", &intVal) != 1)
    return (SD_FAILURE);
#elif (defined(__alpha) && !defined(__VMS)) 
  if (sscanf (dc->valBuf, "%lu", &intVal) != 1)
    return (SD_FAILURE);
...
#if defined(_WIN32)
  SXLOG_CDEC1 ("Unsigned64: %I64u", intVal);
#elif defined(_AIX) || defined(__hpux) || defined (linux) || defined(sun) || defined(__LYNX)
  SXLOG_CDEC1 ("Unsigned64: %llu", intVal);
#elif (defined(__alpha) && !defined(__VMS)) 
  SXLOG_CDEC1 ("Unsigned64: %lu", intVal);
#else
  #error Missing INT64 code for this platform.
#endif
...
ST_RET _sxd_decode_data_el (ST_CHAR *elName, 
			   ...
			   ST_BOOLEAN *elPres, ST_INT sxdXmlStyle, ST_BOOLEAN checkDt, MVL_TYPE_CTRL *mvl_type_ctrl)
#else
ST_RET _sxd_decode_data_el (ST_CHAR *elName, 
		  ST_CHAR *xml, ST_INT xmlLen, ST_INT *xmlUsed, 
		  ST_VOID *vdp, ST_INT typeId, 
