...
/* --------  ---  ------   -------------------------------------------	*/
/* 01/15/07  EJV    06     Chg S_LOCK_UTIL_RESOURCES to S_LOCK_UTIL_RESOURCES*/
/* 01/30/06  GLB    05     Integrated porting changes for VMS           */
/* 06/06/03  JRB    04     Use S_LOCK_UTIL_RESOURCES (new util mutex).	*/
...
#ifdef DEBUG_SISCO
  newSc->maxNumUsed = 0;
  newSc->usedSize = (ST_UINT16 *) malloc (sizeof (ST_UINT16) * numBuf);
#endif
  for (i = 0; i < numBuf; ++i)
...
/* Find the smemCtrl that this buffer belongs to */
  ...
  bufIndex = ((ST_CHAR *) ptr - sc->firstBuf)/sc->bufSize;
  ...
  for (ContextIndex = 0; ContextIndex < M_SMEM_MAX_CONTEXT; ContextIndex++)
    ...
    if (sc != NULL)	/* it was found	*/
      ...
      if ((ptr - sc->firstBuf) % sc->bufSize)
        ...
        {
        bufIndex = (ptr - sc->firstBuf) / sc->bufSize;
