...
/* 01/19/01  EJV     14    Moved strcmpi,_stricmp,strnicmp to str_util.c	*/
/* 11/07/00  JRB     13    Del QNX from ifdef. It supports _stricmp, etc.*/
...
/*			     _stricmp, strcmpi, strnicmp functions	*/
/* 04/03/98  RKR     04    added _stricmp, strcmpi, strnicmp		*/
...
struct cfg_kw_el *ret = NULL;
  ...
  for (i = 0; i < MAX_NUM_KEYWORDS; ++i)
    ...
    if (tbl_ptr[i].extract == NULL)		/* End of table         */
      break;
    else if (!_strcmpi (name, tbl_ptr[i].name))
      {			      			/*  we have a match	*/
...
/* keyword table and execute the selected function			*/
    ...
    if ((cfgStringLen = strlen (cfg_string_buf)) > 0)
      {				/* Find the keyword                     */
      if ((cfgStringLen == keywordLen) && !_strcmpi (keyword, cfg_string_buf))
        break;
