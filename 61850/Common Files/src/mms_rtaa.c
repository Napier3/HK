...
#ifdef DEBUG_SISCO
SD_CONST static ST_CHAR *sp[MAX_INDENT] =
  {
  "  ",
  "    ",
  "      ",
...
SD_CONST static ST_CHAR *sp2[MAX_INDENT] =
  {
  "        ",
  "      ",
  "    ",
...
/* Allocate the output array for max size                               */
  rtaa = (RT_AA_CTRL *) chk_calloc (m_max_rt_aa_ctrl, sizeof (RT_AA_CTRL));
  *rt_aa_out = rtaa;
  *num_aa_out = 0;
...
/* set the RTAA RT pointer and offset as each element is encountered.	*/
  ...
  while (rt < rt_end)
    ...
    if (*num_aa_out >= m_max_rt_aa_ctrl)
      {
      MLOG_ERR0 ("RTAA : too many elements");
      chk_free (*rt_aa_out);
      S_UNLOCK_COMMON_RESOURCES ();
      return (MVE_RTAA_SIZE);
      }
