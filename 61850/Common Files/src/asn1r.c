...
static ST_CHAR cr[] =
"(c) COPYRIGHT SYSTEMS INTEGRATION SPECIALISTS COMPANY INC., 1986 - 2002.  All Rights Reserved.";
...
static ST_VOID _do_decode_asn1 (ASN1_DEC_CTXT *ac, ST_UCHAR *ptr, ST_INT len)
{
	...
	done_ptr = ac->asn1r_field_ptr + len;  /* calculate the 'done' address     */
	...
	while (ac->asn1r_field_ptr < done_ptr && !ac->asn1r_decode_done)
	{
		...
		asn1r_head_decode (ac);		/* decodes the data element header	*/

		log_len = ac->asn1r_field_ptr - ac->asn1r_field_start; 
		if (ac->asn1r_indef_flag != SD_TRUE)
		...
		/* If this is not a indefinite length constructor, verify length is OK	*/
		if (((ac->asn1r_constr_elmnt && !ac->asn1r_indef_flag) || !ac->asn1r_constr_elmnt) &&
			ac->asn1r_field_ptr + ac->asn1r_elmnt_len > done_ptr)
		{
			ALOG_NERR0 ("ASN.1 decode: invalid element length");
			asn1r_set_dec_err (ac, ASN1E_INVALID_LENGTH);	/* call error function		*/
		...
		else
		{
			...
			{					/* (INDEF = 0xFF)	*/
				...
				ac->_asn1_byte_count[ac->asn1r_msg_level] = ASN1_MAX_FIELD;
				ac->asn1r_indef_flag  = SD_FALSE; 		/* clear flag		*/
			}
	...
	if (ac->asn1r_pdu_dec_err == NO_DECODE_ERR && 
		(ac->asn1r_field_ptr != done_ptr || ac->asn1r_msg_level != -1))
	{  					/* Check for indef cstr done	*/
		...
		if (ac->asn1r_field_ptr != done_ptr)
		{
	...
	if ((c &= 0x1F) == 0x1F)		/* if ID extender -		*/
	{
		if ((c = *(ac->asn1r_field_ptr++)) & 0x80) /* if next byte is not last -	*/
		{
			...
			{				/* ID octets, reject		*/
				...
				asn1r_set_dec_err(ac, ASN1E_ID_TOO_BIG);	/* call error function		*/
				return;
			}
	...
	if ((c = *(ac->asn1r_field_ptr++)) & 0x80)	/* check for long,indef forms	*/
	{
		if (c &= 0x7F)			/* if long form 		*/
		{
			...
			case 3 :	 		/* three byte length		*/
				...
				if (i != 0)
				{
					...
					asn1r_set_dec_err(ac, ASN1E_INVALID_LENGTH);	/* other lengths not supported	*/
				}
#endif
			...
			default :
				...
				asn1r_set_dec_err(ac, ASN1E_INVALID_LENGTH); /* others not supported	*/
				break;
			}
		}
		else				/* indefinite length		*/
		{
			...
			else
			{
				...
				asn1r_set_dec_err(ac, ASN1E_INVALID_LENGTH);	/* Bad PDU structure		*/
			}
		}
	}
	else					/* short form length		*/
		ac->asn1r_elmnt_len = c;			/* write length 		*/
...
ST_RET asn1_convert_utc_to_btod (MMS_UTC_TIME *utc, MMS_BTOD *btod)
{
	...
	btod->day = (utc->secs - tJan84) / SECONDS_PER_DAY;	  /* num of days since 1/1/1984		*/
	btod->ms = ((utc->secs - tJan84) % SECONDS_PER_DAY) * 1000; /* num milliseconds since midnight	*/
	/* NOTE: use 0x01000000 (2**24) in fraction computations.	*/
