...
/* --------  ---  ------   -------------------------------------------	*/
/* 10/17/06  MDE     85    Added support for unsigned hex ints (32 bit)	*/
/* 09/05/06  MDE     84    Don't check mandatory w/undefined handler 	*/
/* 08/04/06  RWM     83    Reverted log from SXLOG_NERR1 to SXLOG_CDEC1 */
/* 07/26/06  JRB     82    Add USE_EXPAT option.			*/
/* 07/26/06  JRB     81    Fix required element check (!SX_ELF_OPT).	*/
/*			   Fix u_sx_el_end ERR log msg in sxEndElement.	*/
/* 07/19/06  RWM     80    Changed log from SXLOG_CDEC1 to SXLOG_NERR1  */
/* 05/22/06  DSF     79    Added a log					*/
/* 04/13/06  EJV     78    fopen mode 'rt' is not ANSI, changed to 'r'.	*/
/* 04/07/06  MDE     77    Open files in text mode			*/
/* 01/30/06  GLB     76    Integrated porting changes for VMS           */
/* 02/13/06  DSF     75    Migrate to VS.NET 2005			*/
/* 02/02/06  EJV     74    Added sx_get_int32,      sx_get_uint32,	*/
/*			         sx_get_int32_attr, sx_get_uint32_attr,	*/
/*			   sx_get_ulong, sx_get_ulong_attr: changed	*/
/*			     out_ptr from *ST_UNIT32 to *ST_ULONG.	*/
/*			   sx_get_int16, sx_get_int16_attr: chk rc	*/
/*			     before setting *out_ptr.			*/
/*			   sx_get_value: check sscanf ret for 1.	*/
/* 11/02/05  EJV     73    Add sx_debug_sel_cfg to save SX masks while	*/
/*			    logging configuration is parsed.		*/
/*			   sxEndElement: rem redundant check.		*/
...
{
	{"SX_LOG_ERR",	SX_LOG_ERR,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Error"},
	{"SX_LOG_NERR",	SX_LOG_NERR,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Notice"},
	{"SX_LOG_FLOW",	SX_LOG_FLOW,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Flow"},
	{"SX_LOG_DEC",	SX_LOG_DEC,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Decode"},
	{"SX_LOG_ENC",	SX_LOG_ENC,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Encode"},
	{"SX_LOG_DEBUG",	SX_LOG_DEBUG,	&sx_debug_sel_cfg, _LOGCFG_DATATYPE_UINT_MASK, "Debug"},
};
...
ST_VOID UTF8ToMultiByte(ST_CHAR *srcStr);
					   ...
					   ST_RET (*u_sx_el_end_fun) (SX_DEC_CTRL *sxDecCtrl, ST_CHAR *tag))
{
	...
	result = fstat (fileno(fp), &statbuf);
	if (result != 0)
		return (SD_FAILURE);
...
ST_VOID sxStartElement (SX_DEC_CTRL *sxDecCtrl) 
{
	...
	if (item != NULL)
	{
		...
		{
			//sxDecCtrl->errCode = SX_DUPLICATE_NOT_ALLOWED;  hejian del on 2011-4-27
			SXLOG_NERR1 ("Duplicate of element '%s' not allowed",  tag);
			//return;       hejian del on 2011-4-27
		}
		++item->numOcc;
		if (item->numOcc > 1)
		{
...
ST_RET sx_get_string (SX_DEC_CTRL *sxDecCtrl, ST_CHAR *dest, ST_INT *lenOut)
{
	...
	if (*lenOut != 0)
	{
		if (strLen > *lenOut)
		{
			...
			//hejian alter on 2010-7-23, 去掉长度限制
			//sxDecCtrl->errCode = SD_FAILURE;
			//return (SD_FAILURE);
		}
	}
...
ST_RET sx_get_attr (SX_DEC_CTRL *sxDecCtrl, ST_CHAR *value, ST_CHAR *name)
{
	...
	for (i = 0; i < sxDecCtrl->sxDecElInfo.attrCount; ++i)
	{
		if (strcmp (sxDecCtrl->sxDecElInfo.attr[i].name, name) == 0)
		{
			UTF8ToMultiByte(sxDecCtrl->sxDecElInfo.attr[i].value);
			...
			while (*xml != '>' && *xml != '/') /* we could have attributes! */
			{
				...
				if (*xml != '>' && *xml != '/')
				{
					...
					*attribNameDest = 0;		/* terminate the attrib name */
					...
					///////hejian alter on 2010-3-26 del *(xml) != '\''/////////////
