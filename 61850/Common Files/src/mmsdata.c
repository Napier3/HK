...
ST_RET ms_asn1_to_local (SD_CONST RUNTIME_TYPE *rt_head, ST_INT rt_num, 
						 ST_UCHAR *asn1ptr, ST_INT asn1len, ST_CHAR *dptr, ST_ULONG *cptr)
{
	ST_RET ret;
...
static ST_VOID bs_cstr_done (ASN1_DEC_CTXT *aCtx)
{
	...
	{		/* need to move down to allow for size ST_INT16		*/
		from_ptr = decInfo->datptr+abs_len-1;
...
static ST_VOID get_bitstr_prim    (ASN1_DEC_CTXT *aCtx)
{
	...
	if (decInfo->rt_ptr->u.p.el_len >= 0)
	{
		if (asn1r_get_bitstr (aCtx,(ST_UCHAR *)decInfo->datptr, abs_count) ||
			aCtx->asn1r_bitcount != (ST_INT16) abs_count)
		{
...
static ST_VOID get_int_prim    (ASN1_DEC_CTXT *aCtx)
{
	...
	default :
		...
		asn1r_set_dec_err (aCtx,INTERNAL);	/* no other lengths are valid*/
	}
...
static ST_VOID get_octstr_prim (ASN1_DEC_CTXT *aCtx)
{
	...
	if (aCtx->asn1r_elmnt_len > abs_len)
	{
		MLOG_NERR0 ("Octet string too long");
		asn1r_set_dec_err (aCtx,BAD_DATA);
	}
	...
	if (decInfo->rt_ptr->u.p.el_len >= 0)
	{
		if (asn1r_get_octstr (aCtx,(ST_UCHAR *) decInfo->datptr) || aCtx->asn1r_octetcount != abs_len)
		{
			MLOG_NERR0 ("Octet string wrong length");
			asn1r_set_dec_err (aCtx,BAD_DATA);
		}
	}
	else		/* variable length octet string				*/
	{
		if (asn1r_get_octstr (aCtx,(ST_UCHAR *)decInfo->datptr + sizeof (ST_INT16)))
			asn1r_set_dec_err (aCtx,BAD_DATA);
		else
		{
			*((ST_INT16 *)decInfo->datptr) = (ST_INT16) aCtx->asn1r_octetcount;
...
/*   If data is encoded by itself, ms_local_to_asn1_easy is more	*/
						 ...
						 ST_CHAR *dptr)
{
	...
	{
		if ((rt_ptr->el_tag >= RT_BOOL) && (rt_ptr->el_tag <= RT_MAX_PRIM_TAG))
		{
...
#ifdef TIME_DATA_SUPPORT
	case RT_GENERAL_TIME :
		asn1r_wr_time (aCtx,*((ST_INT32 *)datptr));
		break;
#endif
...
static ST_VOID _log_data_type (SD_CONST RUNTIME_TYPE *rt_head, ST_INT rt_num)
{
	...
	{		/* conversion failed. Just log what we can.	*/
		MLOG_CALWAYS2 ("Local Runtime Type Array ptr = 0x%X, Num Entries = %d",
			rt_head, rt_num);
	}
}
