...
/************************************************************************/
#ifdef DEBUG_SISCO
static ST_VOID *_nd_m_malloc (SMEM_CONTEXT *smem_ctx, ST_UINT size);
static ST_VOID *_nd_m_calloc (SMEM_CONTEXT *smem_ctx, ST_UINT num, ST_UINT size);
static ST_VOID *_nd_m_realloc (SMEM_CONTEXT *smem_ctx, ST_VOID *ptr, ST_UINT size);
...
#ifdef SMEM_ENABLE//zhouhj 20200206  在定义SMEM_ENABLE此后使用
...
/* log allocation info, if enabled					*/
    MEMLOG_MALLOC2 ("m_malloc : size = %05u, ptr = %08lx"
...
/* log allocation info, if enabled					*/
    MEMLOG_CALLOC3 ("m_calloc : num = %05u, size = %05u Ptr = %08lx",num,size,new_mem_node+1);
    if (calling_line != -1)
      {
...
/* If new size is less than old size, just don't use the system realloc	*/
    ...
    if (m_no_realloc_smaller &&
        (size <= (ST_UINT) old_mem_node->chk_alloc_mem_size))
      {
...
/* log allocation info, if enabled					*/
    MEMLOG_REALLOC3 ("m_realloc : ptr = %08lx, size = %05u, ptr = %08lx",
...
/* log allocation info, if enabled					*/
    MEMLOG_FREE1 ("m_free   : ptr = %08lx",del_mem_ptr);
...
static ST_VOID *_nd_m_realloc (SMEM_CONTEXT *smem_ctx, ST_VOID *ptr, ST_UINT size)
#else
ST_VOID *nd_m_realloc (SMEM_CONTEXT *smem_ctx, ST_VOID *ptr, ST_UINT size)
#endif
  {		      
...
// ST_VOID dyn_mem_ptr_status ()//zhouhj 20200206 在mem_chks.c文件中已定义此函数
//   {
// #ifdef DEBUG_SISCO
//   CHK_DYN_MEM_NODE *chk_mem_node;
//   ST_UINT32 count;
...
//       {
//       MEMLOG_ERR2 ("List size (%d) vs. count (%d) mismatch", 
// 				count, chk_curr_mem_count);
//       }
// 
...
//       }
//  
//     MEMLOG_CDUMP1 ("Total number of allocated pointers:   %ld",
//   		  chk_curr_mem_count);
//     MEMLOG_CDUMP1 ("Total amount of memory allocated:     %ld bytes ",
//   		  chk_curr_mem_size);
//     MEMLOG_CDUMP1 ("Maximum number of pointers allocated: %ld",
//   		  chk_max_dyn_mem_ptrs);
//     MEMLOG_CDUMP1 ("Maximum amount of memory allocated:   %ld bytes",
...
ST_VOID dyn_mem_ptr_statistics_smem (ST_BOOLEAN log_to_screen)
  {
#ifdef DEBUG_SISCO
  if (m_mem_debug_saved)
    {
    ...
    if ( log_to_screen == SD_TRUE )
      ...
      printf ("\nTotal number of allocated pointers:   %ld",
             chk_curr_mem_count);
      printf ("\nTotal amount of memory allocated:     %ld bytes ",
             chk_curr_mem_size);
      printf ("\nMaximum number of pointers allocated: %ld",
             chk_max_dyn_mem_ptrs);
...
ST_UINT32 j;
  ...
  if (m_mem_debug_saved)
    ...
    if (j >= chk_max_dyn_mem_ptrs)
      ...
      MEMLOG_CERR2 ("Count = %d,  Should be = %d.",
          chk_curr_mem_count, j + 1);
      if (mem_chk_err)
        (*mem_chk_err)();
      }
    S_UNLOCK_UTIL_RESOURCES (); 
...
//lijq 2015-1-17
ST_VOID free_m_smem_ctxt()
{
	ST_INT i = 0;
	SMEM_POOL_CTRL *memlist = NULL;
	SMEM_POOL_CTRL *fordel = NULL;

	for (i=0; i<M_SMEM_MAX_CONTEXT; i++)
	{
		memlist = m_smem_ctxt[i].smemPoolCtrlList;

		while (memlist != NULL)
		{
			if (memlist->firstBuf != NULL)
			{
				free(memlist->firstBuf);
			}

			if (memlist->availIndexStack != NULL)
			{
				free(memlist->availIndexStack);
			}

#ifdef DEBUG_SISCO
			if (memlist->usedSize != NULL)
			{
				free(memlist->usedSize);
			}
#endif
