...
*/
		con->cosp_cn_ptr = spdu_ptr;
        con->cosp_vtca   = SD_FALSE;      /* transport conn initiator	*/
        }
      else
        {
        ret = COSP_ERR_TP4_RET;
        COSP_LOG_ERR1 ("COSP-ERROR: S-CONNECT.req acse_conn_id=0x%08X: T-CONNECT.req failed.",
          con);
        }
...
/*  case COSP_SI_GIVE_TOKEN:	!!! GIVE-TOKEN is concatinated with DATA*/
    ...
    default:
      ...
      COSP_LOG_ERR2 ("COSP-ERROR: T-DATA.ind acse_conn_id=0x%08X: Not supported SPDU code=%d received.",
        con, con->cosp_state);
