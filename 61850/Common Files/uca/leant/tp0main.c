...
ST_UINT tp4_debug_sel;	/* Only define here if not defined in TP4	*/
#ifdef DEBUG_SISCO
...
ST_UINT j;
  ...
  if ((errcode = sockets_init (max_conns)) != SD_SUCCESS)
    return (errcode);
...
ST_RET tp0_data (ST_LONG tp_conn_id, ST_INT eot, ST_UINT spdu_len, ST_CHAR *spdu_ptr)
  {
	  ST_RET ret;
	  TP0_CONN *tp_conn = NULL;
	  ST_UINT spdu_offset;
	  TPDU_DT tpdu_dt_tx;		/* ptr to TPDU of interest.	*/

	  if (tp0_conn_arr == NULL)
	  {
			 return (SD_SUCCESS);
	  }

	  tp_conn = &tp0_conn_arr [tp_conn_id-MIN_TP0_CONN_ID];
  ...
  if (ret != SD_SUCCESS)
    {
    TP_LOG_ERR1 ("TP-ERROR: failed to send SPDU on tp_conn_id=%ld (possible queue full condition)",
      tp_conn_id);
...
TPDU_DR tpdu_dr;
ST_LONG conn_id = tp0_conn_id - MIN_TP0_CONN_ID;

  if ((ST_ULONG) conn_id >= tp0_cfg.max_num_conns)
    return (TP4E_BADCONN);
...
ST_UINT16 data_len;
  ...
  if (data_len > 0)
    ...
    ret = tcp_send(tpdu_cc->sock_info, g_tpkt_enc_buf, data_len);
    }
  return (ret);
  }
...
ST_UINT16 data_len;
  ...
  if (data_len > 0)
    ...
    g_tpkt_enc_buf [2] = (ST_UCHAR) (data_len >> 8);	/* high byte	*/
    g_tpkt_enc_buf [3] = (ST_UCHAR) (data_len & 0xff);	/* low byte	*/
    ret = tcp_send(tpdu_dr->sock_info, g_tpkt_enc_buf, data_len);
    }
  return (ret);
  }
