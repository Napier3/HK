	...
	while (posLine!=NULL)
	{
		...
		if (pXScdDrawCtrl==(CXScdDrawIedThumbnail*)this)
		{
			if (pXScdDrawLine_Vert->m_ptBegin.x>pXScdDrawLine_Vert->m_ptEnd.x)
		 {
			 ...
			 pXScdDrawLine_Vert->pt_MainIedTurn.x=pXScdDrawLine_Vert->m_ptBegin.x-nHeight1_Left;
			  pXScdDrawLine_Vert->pt_MainIedTurn.y=pXScdDrawLine_Vert->m_ptBegin.y+nHeight1_Left;
			 nHeight1_Left-=4;
		 }
			else
		 {
			 pXScdDrawLine_Vert->pt_MainIedTurn.x=pXScdDrawLine_Vert->m_ptBegin.x+nHeight1_Right;
			   pXScdDrawLine_Vert->pt_MainIedTurn.y=pXScdDrawLine_Vert->m_ptBegin.y+nHeight1_Right;
			 nHeight1_Right-=4;
		 }
		...
		else
		{
			if (pXScdDrawLine_Vert->m_ptBegin.x<pXScdDrawLine_Vert->m_ptEnd.x)
		 {
			 pXScdDrawLine_Vert->pt_MainIedTurn.x=pXScdDrawLine_Vert->m_ptEnd.x-nHeight2_Left;
			 pXScdDrawLine_Vert->pt_MainIedTurn.y=pXScdDrawLine_Vert->m_ptEnd.y+nHeight2_Left;

			 nHeight2_Left-=4;
		 }
			else
		 {
			  pXScdDrawLine_Vert->pt_MainIedTurn.x=pXScdDrawLine_Vert->m_ptEnd.x+nHeight2_Right;
			 pXScdDrawLine_Vert->pt_MainIedTurn.y=pXScdDrawLine_Vert->m_ptEnd.y+nHeight2_Right;
			 nHeight2_Right-=4;
		 }	
